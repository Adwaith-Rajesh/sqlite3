...
Upsert *sqlite3UpsertDup(sqlite3 *db, Upsert *p) {
    if (p == 0)
        return 0;
    return sqlite3UpsertNew(db, sqlite3ExprListDup(db, p->pUpsertTarget, 0),
...
int sqlite3UpsertAnalyzeTarget(
    ...
    */
    ...
    for (; pUpsert && pUpsert->pUpsertTarget;
        ...
        if (HasRowid(pTab) && pTarget->nExpr == 1 &&
...
Upsert *sqlite3UpsertOfIndex(Upsert *pUpsert, Index *pIdx) {
    while (pUpsert && pUpsert->pUpsertTarget != 0 &&
           pUpsert->pUpsertIdx != pIdx) {
...
void sqlite3UpsertDoUpdate(
    ...
    if (pIdx && iCur != iDataCur) {
        if (HasRowid(pTab)) {
            ...
            for (i = 0; i < nPk; i++) {
                ...
                VdbeComment(
                    (v, "%s.%s", pIdx->zName, pTab->aCol[pPk->aiColumn[i]].zCnName));
            }
    ...
    ** So we have to make a copy before passing it down into sqlite3Update() */
    ...
    sqlite3Update(pParse, pSrc, sqlite3ExprListDup(db, pUpsert->pUpsertSet, 0),
                  sqlite3ExprDup(db, pUpsert->pUpsertWhere, 0), OE_Abort, 0, 0,
                  pUpsert);
