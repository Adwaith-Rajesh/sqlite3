...
static ssize_t ts_pwrite64(int fd, const void *aBuf, size_t nBuf,
                           sqlite3_uint64 off);
...
#define orig_pread64 \
    ((ssize_t(*)(int, void *, size_t, sqlite3_uint64))aSyscall[10].xOrig)
#define orig_write ((ssize_t(*)(int, const void *, size_t))aSyscall[11].xOrig)
#define orig_pwrite \
    ((ssize_t(*)(int, const void *, size_t, off_t))aSyscall[12].xOrig)
#define orig_pwrite64 \
    ((ssize_t(*)(int, const void *, size_t, sqlite3_uint64))aSyscall[13].xOrig)
#define orig_fchmod ((int (*)(int, mode_t))aSyscall[14].xOrig)
#define orig_fallocate ((int (*)(int, off_t, off_t))aSyscall[15].xOrig)
#define orig_mmap \
    ((void *(*)(void *, size_t, int, int, int, off_t))aSyscall[16].xOrig)
#define orig_mremap \
    ((void *(*)(void *, size_t, size_t, int, ...))aSyscall[17].xOrig)
...
static int tsErrno(const char *zFunc) {
    ...
    for (i = 0; aSyscall[i].zName; i++) {
        if (strlen(aSyscall[i].zName) != nFunc)
            continue;
        if (memcmp(aSyscall[i].zName, zFunc, nFunc))
            continue;
...
static ssize_t ts_write(int fd, const void *aBuf, size_t nBuf) {
    if (tsIsFailErrno("write")) {
        if (tsErrno("write") == EINTR)
            orig_write(fd, aBuf, nBuf / 2);
        return -1;
    }
...
static void *ts_mmap(void *pAddr, size_t nByte, int prot, int flags, int fd,
                     off_t iOff) {
...
static int SQLITE_TCLAPI test_syscall_install(void *clientData,
    ...
    for (i = 0; i < nElem; i++) {
        int iCall;
        int rc = Tcl_GetIndexFromObjStruct(interp, apElem[i], aSyscall,
                                           sizeof(aSyscall[0]), "system-call", 0,
                                           &iCall);
        if (rc)
            return rc;
...
static int SQLITE_TCLAPI test_syscall_uninstall(void *clientData,
                                                Tcl_Interp *interp, int objc,
                                                Tcl_Obj *CONST objv[]) {
    sqlite3_vfs *pVfs;
    int i;
...
static int SQLITE_TCLAPI test_syscall_reset(void *clientData,
                                            Tcl_Interp *interp, int objc,
                                            Tcl_Obj *CONST objv[]) {
    sqlite3_vfs *pVfs;
    int i;
...
static int SQLITE_TCLAPI test_syscall_exists(void *clientData,
                                             Tcl_Interp *interp, int objc,
                                             Tcl_Obj *CONST objv[]) {
    sqlite3_vfs *pVfs;
...
static int SQLITE_TCLAPI test_syscall_fault(void *clientData,
                                            Tcl_Interp *interp, int objc,
                                            Tcl_Obj *CONST objv[]) {
    int nCount = 0;
    int bPersist = 0;
...
static int SQLITE_TCLAPI test_syscall_errno(void *clientData,
                                            Tcl_Interp *interp, int objc,
                                            Tcl_Obj *CONST objv[]) {
    int iCall;
    int iErrno;
    ...
    struct Errno {
        ...
        {"EACCES", EACCES}, {"EINTR", EINTR}, {"EIO", EIO}, {"EOVERFLOW", EOVERFLOW}, {"ENOMEM", ENOMEM}, {"EAGAIN", EAGAIN}, {"ETIMEDOUT", ETIMEDOUT}, {"EBUSY", EBUSY}, {"EPERM", EPERM}, {"EDEADLK", EDEADLK}, {"ENOLCK", ENOLCK}, {0, 0}};
    ...
    rc = Tcl_GetIndexFromObjStruct(interp, objv[2], aSyscall, sizeof(aSyscall[0]),
                                   "system-call", 0, &iCall);
    if (rc != TCL_OK)
        return rc;
    rc = Tcl_GetIndexFromObjStruct(interp, objv[3], aErrno, sizeof(aErrno[0]),
                                   "errno", 0, &iErrno);
    if (rc != TCL_OK)
        return rc;
...
static int SQLITE_TCLAPI test_syscall_list(void *clientData, Tcl_Interp *interp,
    ...
    for (zSys = pVfs->xNextSystemCall(pVfs, 0); zSys != 0;
...
static int SQLITE_TCLAPI test_syscall_defaultvfs(void *clientData,
                                                 Tcl_Interp *interp, int objc,
                                                 Tcl_Obj *CONST objv[]) {
    sqlite3_vfs *pVfs;
...
static int ts_getpagesize(void) { return gSyscall.pgsz; }

static int SQLITE_TCLAPI test_syscall_pagesize(void *clientData,
                                               Tcl_Interp *interp, int objc,
                                               Tcl_Obj *CONST objv[]) {
    ...
    if (pgsz < 0) {
        ...
        gSyscall.pgsz = pgsz;
        pVfs->xSetSystemCall(pVfs, "getpagesize",
...
static int SQLITE_TCLAPI test_syscall(void *clientData, Tcl_Interp *interp,
                                      int objc, Tcl_Obj *CONST objv[]) {
    struct SyscallCmd {
        const char *zName;
        Tcl_ObjCmdProc *xCmd;
    } aCmd[] = {{"fault", test_syscall_fault},
    ...
    if (pVfs->iVersion < 3 || pVfs->xSetSystemCall == 0) {
        ...
        rc = Tcl_GetIndexFromObjStruct(interp, objv[1], aCmd, sizeof(aCmd[0]),
                                       "sub-command", 0, &iCmd);
    }
    if (rc != TCL_OK)
        return rc;
...
#else
int SqlitetestSyscall_Init(Tcl_Interp *interp) { return TCL_OK; }
#endif
