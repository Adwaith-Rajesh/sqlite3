...
static int tabIsReadOnly(Parse *pParse, Table *pTab) {
    ...
    if ((pTab->tabFlags & (TF_Readonly | TF_Shadow)) == 0)
        return 0;
    db = pParse->db;
...
sqlite3LimitWhere(Parse *pParse,      /* The parser context */
    ...
    ** and the SELECT subtree. */
    ...
    pSelect = sqlite3SelectNew(pParse, pEList, pSelectSrc, pWhere, 0, 0, pOrderBy,
                               0, pLimit);
...
#define isView 0
    ...
    if (sqlite3TreeTrace & 0x10000) {
        ...
        sqlite3TreeViewDelete(pParse->pWith, pTabList, pWhere, pOrderBy, pLimit,
                              pTrigger);
    }
#endif
    ...
    if (!isView) {
        pWhere =
            sqlite3LimitWhere(pParse, pTabList, pWhere, pOrderBy, pLimit, "DELETE");
        pOrderBy = 0;
        pLimit = 0;
    }
#endif
    ...
    */
...
#endif /* SQLITE_OMIT_TRUNCATE_OPTIMIZATION */
    {
        ...
        */
        pWInfo =
            sqlite3WhereBegin(pParse, pTabList, pWhere, 0, 0, 0, wcf, iTabCur + 1);
        if (pWInfo == 0)
            goto delete_from_cleanup;
        ...
        if (pPk) {
            for (i = 0; i < nPk; i++) {
                ...
                sqlite3ExprCodeGetColumnOfTable(v, pTab, iTabCur, pPk->aiColumn[i],
                                                iPk + i);
            }
            iKey = iPk;
        } else {
        ...
        */
...
#endif
        {
            ...
            sqlite3GenerateRowDelete(pParse, pTab, pTrigger, iDataCur, iIdxCur, iKey,
                                     nKey, count, OE_Default, eOnePass,
                                     aiCurOnePass[1]);
        }
...
void sqlite3GenerateRowDelete(
    ...
    assert(v);
    VdbeModuleComment(
        (v, "BEGIN: GenRowDel(%d,%d,%d,%d)", iDataCur, iIdxCur, iPk, (int)nPk));
    ...
    ** use for the old.* references in the triggers.  */
    if (sqlite3FkRequired(pParse, pTab, 0, 0) || pTrigger) {
        ...
        ** avoid copying the contents of the rowid register.  */
        mask = sqlite3TriggerColmask(pParse, pTrigger, 0, 0,
                                     TRIGGER_BEFORE | TRIGGER_AFTER, pTab, onconf);
        ...
        ** used by any BEFORE and AFTER triggers that exist.  */
        ...
        sqlite3CodeRowTrigger(pParse, pTrigger, TK_DELETE, 0, TRIGGER_BEFORE, pTab,
                              iOld, onconf, iLabel);
    ...
    */
    if (!IsView(pTab)) {
        ...
        if (pParse->nested == 0 ||
            0 == sqlite3_stricmp(pTab->zName, "sqlite_stat1")) {
    ...
    ** to the row just deleted. */
    ...
    if (pTrigger) {
        sqlite3CodeRowTrigger(pParse, pTrigger, TK_DELETE, 0, TRIGGER_AFTER, pTab,
                              iOld, onconf, iLabel);
    }
