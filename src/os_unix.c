...
#endif
#define osPread64 \
    ((ssize_t(*)(int, void *, size_t, off64_t))aSyscall[10].pCurrent)
...
#endif
#define osPwrite64 \
    ((ssize_t(*)(int, const void *, size_t, off64_t))aSyscall[13].pCurrent)
...
#endif
#define osReadlink \
    ((ssize_t(*)(const char *, char *, size_t))aSyscall[26].pCurrent)
...
static int robust_open(const char *z, int f, mode_t m) {
    ...
    while (1) {
        ...
        osClose(fd);
        sqlite3_log(SQLITE_WARNING, "attempt to open \"%s\" as file descriptor %d",
                    z, fd);
        fd = -1;
        if (osOpen("/dev/null", O_RDONLY, m) < 0)
            break;
    }
    if (fd >= 0) {
        if (m != 0) {
            struct stat statbuf;
            if (osFstat(fd, &statbuf) == 0 && statbuf.st_size == 0 &&
...
static int lockTrace(int fd, int op, struct flock *p) {
    ...
    sqlite3DebugPrintf("fcntl %d %d %s %s %d %d %d %d\n", threadid, fd, zOpName,
                       zType, (int)p->l_start, (int)p->l_len, (int)p->l_pid, s);
    if (s == (-1) && op == F_SETLK &&
        ...
        sqlite3DebugPrintf("fcntl-failure-reason: %s %d %d %d\n", zType,
                           (int)l2.l_start, (int)l2.l_len, (int)l2.l_pid);
    }
    errno = savedErrno;
    return s;
}
...
unixLogErrorAtLine(int errcode,       /* SQLite error code */
    ...
    */
    ...
    if (zPath == 0)
        zPath = "";
    sqlite3_log(errcode, "os_unix.c:%d: (%d) %s(%s) - %s", iLine, iErrno, zFunc,
                zPath, zErr);
...
static void robust_close(unixFile *pFile, int h, int lineno) {
    if (osClose(h)) {
        unixLogErrorAtLine(SQLITE_IOERR_CLOSE, "close", pFile ? pFile->zPath : 0,
                           lineno);
    }
}
...
afpSetLock(const char *path,          /* Name of the file to be locked or unlocked */
    ...
    if (err == -1) {
        ...
        rc = SQLITE_BUSY;
#else
        rc = sqliteErrorFromPosixError(tErrno, setLockFlag ? SQLITE_IOERR_LOCK
                                                           : SQLITE_IOERR_UNLOCK);
...
static int afpLock(sqlite3_file *id, int eFileLock) {
    ...
    */
    if (eFileLock == SHARED_LOCK) {
        ...
        lrc1 = afpSetLock(context->dbPath, pFile, SHARED_FIRST + pInode->sharedByte,
                          1, 1);
        ...
        if (!failed && eFileLock == EXCLUSIVE_LOCK) {
            ...
            */
            if (!(failed = afpSetLock(context->dbPath, pFile,
                                      SHARED_FIRST + pInode->sharedByte, 1, 0))) {
                int failed2 = SQLITE_OK;
                /* now attempt to get the exclusive lock range */
                failed =
                    afpSetLock(context->dbPath, pFile, SHARED_FIRST, SHARED_SIZE, 1);
                if (failed &&
                    (failed2 = afpSetLock(context->dbPath, pFile,
...
static int afpUnlock(sqlite3_file *id, int eFileLock) {
    ...
    if (pFile->eFileLock > SHARED_LOCK) {
        ...
        ** cache.  The use of a stale cache can lead to database corruption.
        */
        assert(pFile->inNormalWrite == 0 || pFile->dbUpdate == 0 ||
...
*/
static int seekAndRead(unixFile *id, sqlite3_int64 offset, void *pBuf,
                       int cnt) {
    int got;
    int prior = 0;
...
** wrong.
*/
static int unixRead(sqlite3_file *id, void *pBuf, int amt,
...
*/
static int unixWrite(sqlite3_file *id, const void *pBuf, int amt,
                     sqlite3_int64 offset) {
    unixFile *pFile = (unixFile *)id;
    int wrote = 0;
    assert(id);
...
*/
static int
unixShmSystemLock(unixFile *pFile, /* Open connection to the WAL file */
    ...
    if (pShmNode == 0) {
        ...
        */
        ...
        if (pInode->bProcessLock == 0) {
            ...
            if (pShmNode->hShm < 0) {
                pShmNode->hShm =
                    robust_open(zShm, O_RDONLY | O_NOFOLLOW, (sStat.st_mode & 0777));
                if (pShmNode->hShm < 0) {
...
static int unixShmMap(sqlite3_file *fd,  /* Handle open on database file */
    ...
    if (pShmNode->nRegion < nReqRegion) {
            ...
            if (sStat.st_size < nByte) {
                ...
                */
                    ...
                    for (iPg = (sStat.st_size / pgsz); iPg < (nByte / pgsz); iPg++) {
                        int x = 0;
                        if (seekAndWriteFd(pShmNode->hShm, iPg * pgsz + pgsz - 1, "", 1,
                                           &x) != 1) {
        ...
        while (pShmNode->nRegion < nReqRegion) {
            ...
            if (pShmNode->hShm >= 0) {
                pMem = osMmap(
                    0, nMap, pShmNode->isReadonly ? PROT_READ : PROT_READ | PROT_WRITE,
            ...
            for (i = 0; i < nShmPerMap; i++) {
                pShmNode->apRegion[pShmNode->nRegion + i] =
                    &((char *)pMem)[szRegion * i];
            }
...
*/
static int
unixShmLock(sqlite3_file *fd, /* Database file holding the shared memory */
...
*/
static void
unixShmBarrier(sqlite3_file *fd /* Database file holding the shared memory */
) {
    UNUSED_PARAMETER(fd);
...
#define IOMETHODS(FINDER, METHOD, VERSION, CLOSE, LOCK, UNLOCK, CKLOCK,         \
    ...
    static const sqlite3_io_methods *(*const FINDER)(                           \
        const char *, unixFile *p) = FINDER##Impl;
...
autolockIoFinderImpl(const char *filePath, /* name of the database file */
    ...
    } aMap[] = {{"hfs", &posixIoMethods}, {"ufs", &posixIoMethods}, {"afpfs", &afpIoMethods}, {"smbfs", &afpIoMethods}, {"webdav", &nolockIoMethods}, {0, 0}};
    int i;
...
static const sqlite3_io_methods *(*const autolockIoFinder)(
    const char *, unixFile *) = autolockIoFinderImpl;
...
static const sqlite3_io_methods *(*const vxworksIoFinder)(
    const char *, unixFile *) = vxworksIoFinderImpl;
...
#endif
    if (sqlite3_uri_boolean(((ctrlFlags & UNIXFILE_URI) ? zFilename : 0), "psow",
...
*/
static const char *azTempDirs[] = {0, 0, "/var/tmp", "/usr/tmp", "/tmp", "."};
    ...
    */
    ...
    if (zDir == 0) {
        ...
        do {
            ...
            assert(nBuf > 2);
            zBuf[nBuf - 2] = 0;
            sqlite3_snprintf(nBuf, zBuf, "%s/" SQLITE_TEMP_FILE_PREFIX "%llx%c", zDir,
                             r, 0);
...
unixOpen(sqlite3_vfs *pVfs,   /* The VFS for which this is the xOpen method */
    ...
    ** 'conch file' locking functions later on.  */
    ...
    if (fd < 0) {
        ...
        */
        if (openMode &&
            (flags & (SQLITE_OPEN_WAL | SQLITE_OPEN_MAIN_JOURNAL)) != 0) {
...
unixAccess(sqlite3_vfs *NotUsed, /* The VFS containing this xAccess method */
    ...
    ** two of them are actually used */
    ...
    if (flags == SQLITE_ACCESS_EXISTS) {
        struct stat buf;
        *pResOut =
            0 == osStat(zPath, &buf) && (!S_ISREG(buf.st_mode) || buf.st_size > 0);
    } else {
...
static void (*unixDlSym(sqlite3_vfs *NotUsed, void *p,
...
static int unixCurrentTimeInt64(sqlite3_vfs *NotUsed, sqlite3_int64 *piNow) {
    ...
    *piNow =
        unixEpoch + 1000 * (sqlite3_int64)sNow.tv_sec + sNow.tv_nsec / 1000000;
#else
    struct timeval sNow;
...
** used.
*/
static int unixGetLastError(sqlite3_vfs *NotUsed, int NotUsed2,
                            char *NotUsed3) {
    UNUSED_PARAMETER(NotUsed);
...
static int proxyCreateLockPath(const char *lockPath) {
    ...
    OSTRACE(
        ("CREATELOCKPATH  proxy lock path=%s pid=%d\n", lockPath, osGetpid(0)));
    return 0;
}
        ...
        if (rc == SQLITE_BUSY) {
            ...
            if (0 == proxyBreakConchLock(pFile, myHostID)) {
                ...
                if (lockType == EXCLUSIVE_LOCK) {
                    rc =
                        conchFile->pMethod->xLock((sqlite3_file *)conchFile, SHARED_LOCK);
                }
                if (!rc) {
                    rc = conchFile->pMethod->xLock((sqlite3_file *)conchFile, lockType);
                }
        ...
        */
        do { /* in case we need to try again for an :auto: named lock file */
            ...
            if (!createConch && !forceNewLockPath) {
                hostIdMatch =
                    !memcmp(&readBuf[PROXY_HEADERLEN], myHostID, PROXY_HOSTIDLEN);
            ...
            */
            ...
            if (rc == SQLITE_OK) {
                ...
                if (rc == SQLITE_OK && createConch) {
                    ...
                    if (err == 0) {
                        mode_t cmode = buf.st_mode & (S_IRUSR | S_IWUSR | S_IRGRP |
