...
int sqlite3_config(int op, ...) {
    ...
    */
    if (sqlite3GlobalConfig.isInit) {
        static const u64 mAnytimeConfigOption =
            0 | MASKBIT64(SQLITE_CONFIG_LOG) |
    ...
    switch (op) {
        ...
        */
        ...
        case SQLITE_CONFIG_PCACHE_HDRSZ: {
            ...
            *va_arg(ap, int *) = sqlite3HeaderSizeBtree() + sqlite3HeaderSizePcache() +
...
** SQLITE_ENABLE_MEMSYS5 and returns SQLITE_ERROR if invoked otherwise. */
        ...
        */
        ...
        case SQLITE_CONFIG_ROWID_IN_VIEW: {
            ...
            if (1 == *pVal)
                sqlite3GlobalConfig.mNoVisibleRowid = 0;
...
*/
static int rtrimCollFunc(void *pUser, int nKey1, const void *pKey1, int nKey2,
                         const void *pKey2) {
...
*/
static int nocaseCollatingFunc(void *NotUsed, int nKey1, const void *pKey1,
...
void sqlite3RollbackAll(sqlite3 *db, int tripCode) {
    ...
    ** corruption reports in some cases.  */
    sqlite3BtreeEnterAll(db);
    schemaChange =
        (db->mDbFlags & DBFLAG_SchemaChange) != 0 && db->init.busy == 0;
    ...
    for (i = 0; i < 2 && zName == 0; i++, rc &= 0xff) {
        switch (rc) {
            ...
            case SQLITE_READONLY_CANTINIT:
                zName = "SQLITE_READONLY_CANTINIT";
                break;
            ...
            case SQLITE_READONLY_DIRECTORY:
                zName = "SQLITE_READONLY_DIRECTORY";
                break;
...
int sqlite3CreateFunc(
    sqlite3 *db, const char *zFunctionName, int nArg, int enc, void *pUserData,
    ...
    if (zFunctionName == 0              /* Must have a valid name */
        ...
        || ((xValue == 0) !=
            (xInverse == 0)) /* Both or neither of xValue, xInverse */
    ...
    extraFlags =
        enc & (SQLITE_DETERMINISTIC | SQLITE_DIRECTONLY | SQLITE_SUBTYPE |
    ...
    */
    ...
    if (p && (p->funcFlags & SQLITE_FUNC_ENCMASK) == (u32)enc &&
        p->nArg == nArg) {
        if (db->nVdbeActive) {
            sqlite3ErrorWithMsg(
                db, SQLITE_BUSY,
...
*/
static int
createFunctionApi(sqlite3 *db, const char *zFunc, int nArg, int enc, void *p,
                  void (*xSFunc)(sqlite3_context *, int, sqlite3_value **),
                  void (*xStep)(sqlite3_context *, int, sqlite3_value **),
    ...
    rc = sqlite3CreateFunc(db, zFunc, nArg, enc, p, xSFunc, xStep, xFinal, xValue,
                           xInverse, pArg);
    if (pArg && pArg->nRef == 0) {
...
int sqlite3_create_function(
    sqlite3 *db, const char *zFunc, int nArg, int enc, void *p,
    void (*xSFunc)(sqlite3_context *, int, sqlite3_value **),
    void (*xStep)(sqlite3_context *, int, sqlite3_value **),
    void (*xFinal)(sqlite3_context *)) {
    return createFunctionApi(db, zFunc, nArg, enc, p, xSFunc, xStep, xFinal, 0, 0,
                             0);
}
...
int sqlite3_create_window_function(
    ...
    void (*xInverse)(sqlite3_context *, int, sqlite3_value **),
    void (*xDestroy)(void *)) {
    return createFunctionApi(db, zFunc, nArg, enc, p, 0, xStep, xFinal, xValue,
                             xInverse, xDestroy);
}

#ifndef SQLITE_OMIT_UTF16
int sqlite3_create_function16(
    sqlite3 *db, const void *zFunctionName, int nArg, int eTextRep, void *p,
    void (*xSFunc)(sqlite3_context *, int, sqlite3_value **),
    void (*xStep)(sqlite3_context *, int, sqlite3_value **),
    void (*xFinal)(sqlite3_context *)) {
    int rc;
...
#ifdef SQLITE_ENABLE_API_ARMOR
    if (!sqlite3SafetyCheckOk(db) || zFunctionName == 0)
        return SQLITE_MISUSE_BKPT;
#endif
    sqlite3_mutex_enter(db->mutex);
    assert(!db->mallocFailed);
    zFunc8 = sqlite3Utf16to8(db, zFunctionName, -1, SQLITE_UTF16NATIVE);
    rc = sqlite3CreateFunc(db, zFunc8, nArg, eTextRep, p, xSFunc, xStep, xFinal,
                           0, 0, 0);
...
int sqlite3_overload_function(sqlite3 *db, const char *zName, int nArg) {
    ...
    if (zCopy == 0)
        return SQLITE_NOMEM;
    return sqlite3_create_function_v2(db, zName, nArg, SQLITE_UTF8, zCopy,
                                      sqlite3InvalidFunction, 0, 0, sqlite3_free);
}
 ...
 */
int sqlite3_trace_v2(sqlite3 *db,     /* Trace this connection */
                     unsigned mTrace, /* Mask of events to be traced */
                     int (*xTrace)(unsigned, void *, void *,
                                   void *), /* Callback to invoke */
                     void *pArg             /* Context */
) {
#ifdef SQLITE_ENABLE_API_ARMOR
    if (!sqlite3SafetyCheckOk(db)) {
...
void *sqlite3_profile(sqlite3 *db,
    ...
    if (db->xProfile)
        db->mTrace |= SQLITE_TRACE_XPROFILE;
    sqlite3_mutex_leave(db->mutex);
    return pOld;
}
#endif /* SQLITE_OMIT_DEPRECATED */
...
** inserted or deleted using this database connection.
*/
void *sqlite3_preupdate_hook(sqlite3 *db,      /* Attach the hook to this database */
...
** wal_autocheckpoint()).
*/
int sqlite3WalDefaultHook(void *pClientData, /* Argument */
...
*/
void *sqlite3_wal_hook(sqlite3 *db, /* Attach the hook to this db handle */
...
** or TRUNCATE.
*/
int sqlite3Checkpoint(sqlite3 *db, int iDb, int eMode, int *pnLog,
                      int *pnCkpt) {
...
const void *sqlite3_errmsg16(sqlite3 *db) {
    static const u16 outOfMem[] = {'o', 'u', 't', ' ', 'o', 'f', ' ',
                                   'm', 'e', 'm', 'o', 'r', 'y', 0};
    static const u16 misuse[] = {'b', 'a', 'd', ' ', 'p', 'a', 'r', 'a', 'm',
                                 'e', 't', 'e', 'r', ' ', 'o', 'r', ' ', 'o',
                                 't', 'h', 'e', 'r', ' ', 'A', 'P', 'I', ' ',
                                 'm', 'i', 's', 'u', 's', 'e', 0};
...
int sqlite3_system_errno(sqlite3 *db) { return db ? db->iSysErrno : 0; }
...
*/
static int createCollation(sqlite3 *db, const char *zName, u8 enc, void *pCtx,
    ...
    */
    ...
    if (pColl && pColl->xCmp) {
        if (db->nVdbeActive) {
            sqlite3ErrorWithMsg(db, SQLITE_BUSY,
                                "unable to delete/modify collation sequence due to "
                                "active statements");
            return SQLITE_BUSY;
        }
        sqlite3ExpirePreparedStatements(db, 0);
...
int sqlite3_limit(sqlite3 *db, int limitId, int newLimit) {
    ...
    */
    ...
    assert(aHardLimit[SQLITE_LIMIT_LIKE_PATTERN_LENGTH] ==
           SQLITE_MAX_LIKE_PATTERN_LENGTH);
    assert(aHardLimit[SQLITE_LIMIT_VARIABLE_NUMBER] ==
        ...
        ** method that there may be extra parameters following the file-name.  */
        ...
        if (zUri[5] == '/' && zUri[6] == '/') {
            ...
            if (iIn != 7 && (iIn != 16 || memcmp("localhost", &zUri[7], 9))) {
                *pzErrMsg =
                    sqlite3_mprintf("invalid uri authority: %.*s", iIn - 7, &zUri[7]);
                rc = SQLITE_ERROR;
                goto parse_uri_out;
            }
        }
            ...
            if (nOpt == 3 && memcmp("vfs", zOpt, 3) == 0) {
                ...
                if (aMode) {
                    ...
                    if ((mode & ~SQLITE_OPEN_MEMORY) > limit) {
                        *pzErrMsg =
                            sqlite3_mprintf("%s mode not allowed: %s", zModeType, zVal);
                        rc = SQLITE_PERM;
                        goto parse_uri_out;
                    }
...
#endif
    db->flags |= SQLITE_ShortColNames | SQLITE_EnableTrigger | SQLITE_EnableView |
                 SQLITE_CacheSpill
...
*/
int sqlite3_create_collation(sqlite3 *db, const char *zName, int enc,
                             void *pCtx,
                             int (*xCompare)(void *, int, const void *, int,
                                             const void *)) {
...
** Register a new collation sequence with the database handle db.
*/
int sqlite3_create_collation_v2(sqlite3 *db, const char *zName, int enc,
                                void *pCtx,
                                int (*xCompare)(void *, int, const void *, int,
                                                const void *),
                                void (*xDel)(void *)) {
...
** Register a new collation sequence with the database handle db.
*/
int sqlite3_create_collation16(sqlite3 *db, const void *zName, int enc,
                               void *pCtx,
                               int (*xCompare)(void *, int, const void *, int,
                                               const void *)) {
    int rc = SQLITE_OK;
...
*/
int sqlite3_global_recover(void) { return SQLITE_OK; }
#endif
...
int sqlite3ReportError(int iErr, int lineno, const char *zType) {
    sqlite3_log(iErr, "%s at line %d of [%.10s]", zType, lineno,
                20 + sqlite3_sourceid());
    return iErr;
}
    ...
    switch (op) {
        ...
        */
        case SQLITE_TESTCTRL_VDBE_COVERAGE: {
            ...
            typedef void (*branch_callback)(void *, unsigned int, unsigned char,
                                            unsigned char);
        ...
        */
        case SQLITE_TESTCTRL_USELONGDOUBLE: {
            int b = va_arg(ap, int);
            if (b >= 2)
                b = hasHighPrecisionDouble(b);
            if (b >= 0)
                sqlite3Config.bUseLongDouble = b > 0;
...
*/
const char *sqlite3_create_filename(const char *zDatabase, const char *zJournal,
...
*/
int sqlite3_snapshot_get(sqlite3 *db, const char *zDb,
...
*/
int sqlite3_snapshot_open(sqlite3 *db, const char *zDb,
