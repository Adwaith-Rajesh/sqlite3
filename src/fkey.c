...
static void fkLookupParent(
    ...
    sqlite3VdbeVerifyAbortable(v, (!pFKey->isDeferred &&
    ...
    ** search for a matching row in the parent table.  */
    ...
    for (i = 0; i < pFKey->nCol; i++) {
        int iReg =
            sqlite3TableColumnToStorage(pFKey->pFrom, aiCol[i]) + regData + 1;
        ...
        if (pIdx == 0) {
            ...
            ** will have INTEGER affinity applied to it, which may not be correct.  */
            sqlite3VdbeAddOp2(v, OP_SCopy,
                              sqlite3TableColumnToStorage(pFKey->pFrom, aiCol[0]) +
                                  1 + regData,
                              regTemp);
            ...
            ** increment the constraint-counter.  */
            ...
            for (i = 0; i < nCol; i++) {
                sqlite3VdbeAddOp2(v, OP_Copy,
                                  sqlite3TableColumnToStorage(pFKey->pFrom, aiCol[i]) +
                                      1 + regData,
                                  regTemp + i);
            }
            ...
            */
            if (pTab == pFKey->pFrom && nIncr == 1) {
                ...
                for (i = 0; i < nCol; i++) {
                    ...
                    iParent +=
                        sqlite3TableColumnToStorage(pIdx->pTable, pIdx->aiColumn[i]);
    ...
    if (!pFKey->isDeferred && !(pParse->db->flags & SQLITE_DeferFKs) &&
        ...
        ** generated for will not open a statement transaction.  */
        assert(nIncr == 1);
        sqlite3HaltConstraint(pParse, SQLITE_CONSTRAINT_FOREIGNKEY, OE_Abort, 0,
                              P4_STATIC, P5_ConstraintFK);
    } else {
        ...
        if (sqlite3FkReferences(pTab) == 0) {
            ...
            ** when this statement is run.  */
            ...
            for (p = pTab->u.tab.pFKey; p; p = p->pNextFrom) {
                if (p->isDeferred || (db->flags & SQLITE_DeferFKs))
                    break;
            }
            if (!p)
                return;
...
*/
static int fkParentIsModified(Table *pTab, FKey *p, int *aChange,
                              int bChngRowid) {
    int i;
    for (i = 0; i < p->nCol; i++) {
...
*/
void sqlite3FkCheck(Parse *pParse, /* Parse context */
            ...
            if (pTo == 0) {
                ...
                */
                ...
                for (i = 0; i < pFKey->nCol; i++) {
                    ...
                    iReg =
                        sqlite3TableColumnToStorage(pFKey->pFrom, iFromCol) + regOld + 1;
...
*/
u32 sqlite3FkOldmask(Parse *pParse, /* Parse context */
...
*/
int sqlite3FkRequired(Parse *pParse, /* Parse context */
                      Table *pTab,   /* Table being modified */
...
fkActionTrigger(Parse *pParse,     /* Parse context */
    ...
    if (action != OE_None && !pTrigger) {
        ...
        for (i = 0; i < pFKey->nCol; i++) {
            ...
            assert(pIdx == 0 || pIdx->aiColumn[i] >= 0);
            sqlite3TokenInit(
                &tToCol, pTab->aCol[pIdx ? pIdx->aiColumn[i] : pTab->iPKey].zCnName);
            ...
            if (pChanges) {
                pEq = sqlite3PExpr(
                    ...
                    sqlite3PExpr(pParse, TK_DOT, sqlite3ExprAlloc(db, TK_ID, &tOld, 0),
                                 sqlite3ExprAlloc(db, TK_ID, &tToCol, 0)),
                    sqlite3PExpr(pParse, TK_DOT, sqlite3ExprAlloc(db, TK_ID, &tNew, 0),
                                 sqlite3ExprAlloc(db, TK_ID, &tToCol, 0)));
        ...
        if (action == OE_Restrict) {
            ...
            pSelect =
                sqlite3SelectNew(pParse, sqlite3ExprListAppend(pParse, 0, pRaise),
                                 pSrc, pWhere, 0, 0, 0, 0, 0);
            pWhere = 0;
        }
        ...
        pTrigger = (Trigger *)sqlite3DbMallocZero(
            db, sizeof(Trigger) +         /* struct Trigger */
        ...
        switch (action) {
            ...
            case OE_Cascade:
                ...
                /* no break */ deliberate_fall_through default : pStep->op = TK_UPDATE;
        }
