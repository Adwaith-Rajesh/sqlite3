...
static void wrMutexFree(sqlite3_mutex *p) { wrapped.mutex.xMutexFree(p); }
static void wrMutexEnter(sqlite3_mutex *p) { wrapped.mutex.xMutexEnter(p); }
static int wrMutexTry(sqlite3_mutex *p) { return wrapped.mutex.xMutexTry(p); }
static void wrMutexLeave(sqlite3_mutex *p) { wrapped.mutex.xMutexLeave(p); }
static int wrMutexHeld(sqlite3_mutex *p) { return wrapped.mutex.xMutexHeld(p); }
...
static sqlite3_pcache_page *wrPCacheFetch(sqlite3_pcache *p, unsigned a,
                                          int b) {
...
static void wrPCacheRekey(sqlite3_pcache *p, sqlite3_pcache_page *a, unsigned b,
                          unsigned c) {
...
static void wrPCacheDestroy(sqlite3_pcache *p) { wrapped.pcache.xDestroy(p); }

static void installInitWrappers(void) {
    sqlite3_mutex_methods mutexmethods = {
        wrMutexInit, wrMutexEnd, wrMutexAlloc, wrMutexFree, wrMutexEnter,
        wrMutexTry, wrMutexLeave, wrMutexHeld, wrMutexNotheld};
    sqlite3_pcache_methods2 pcachemethods = {1,
                                             0,
                                             wrPCacheInit,
                                             wrPCacheShutdown,
                                             wrPCacheCreate,
    ...
    sqlite3_mem_methods memmethods = {
        wrMemMalloc, wrMemFree, wrMemRealloc, wrMemSize,
        wrMemRoundup, wrMemInit, wrMemShutdown, 0};
...
int Sqlitetest_init_Init(Tcl_Interp *interp) {
    ...
    } aObjCmd[] = {{"init_wrapper_install", init_wrapper_install},
