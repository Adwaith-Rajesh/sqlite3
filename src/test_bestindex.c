...
*/
static int tclConnect(sqlite3 *db, void *pAux, int argc,
...
static int tclFilter(sqlite3_vtab_cursor *pVtabCursor, int idxNum,
    ...
    for (ii = 0; ii < argc; ii++) {
        ...
        if (zVal == 0) {
            sqlite3_value *pMem;
            pVal = Tcl_NewObj();
            for (rc = sqlite3_vtab_in_first(argv[ii], &pMem); rc == SQLITE_OK && pMem;
...
static int tclColumn(sqlite3_vtab_cursor *pVtabCursor, sqlite3_context *ctx,
                     int i) {
    tcl_cursor *pCsr = (tcl_cursor *)pVtabCursor;
...
static void testBestIndexObjConstraints(Tcl_Interp *interp,
...
static void testBestIndexObjOrderby(Tcl_Interp *interp,
                                    sqlite3_index_info *pIdxInfo) {
    int ii;
    Tcl_Obj *pRes = Tcl_NewObj();
    Tcl_IncrRefCount(pRes);
...
static int SQLITE_TCLAPI testBestIndexObj(
    ...
    const char *azSub[] = {"constraints", /* 0 */
                           "orderby",     /* 1 */
                           "mask",        /* 2 */
                           "distinct",    /* 3 */
                           "in",          /* 4 */
    ...
    switch (ii) {
        case 0:
            assert(sqlite3_stricmp(azSub[ii], "constraints") == 0);
            testBestIndexObjConstraints(interp, pIdxInfo);
            break;

        case 1:
            assert(sqlite3_stricmp(azSub[ii], "orderby") == 0);
            testBestIndexObjOrderby(interp, pIdxInfo);
            break;

        case 2:
            assert(sqlite3_stricmp(azSub[ii], "mask") == 0);
            Tcl_SetObjResult(interp, Tcl_NewWideIntObj(pIdxInfo->colUsed));
            break;

        case 3:
            assert(sqlite3_stricmp(azSub[ii], "distinct") == 0);
            {
        ...
        case 4:
            assert(sqlite3_stricmp(azSub[ii], "in") == 0);
            {
                int iCons;
                int bHandle;
                ...
                Tcl_SetObjResult(
                    interp, Tcl_NewIntObj(sqlite3_vtab_in(pIdxInfo, iCons, bHandle)));
                break;
            }

        case 5:
            assert(sqlite3_stricmp(azSub[ii], "rhs_value") == 0);
            {
                int iCons = 0;
                int rc;
...
static void tclFunction(sqlite3_context *pCtx, int nArg,
                        sqlite3_value **apArg) {
    ...
    for (ii = 0; ii < nArg; ii++) {
        ...
        Tcl_ListObjAppendElement(
            interp, pScript, (zArg ? Tcl_NewStringObj(zArg, -1) : Tcl_NewObj()));
    }
...
static int tclFindFunction(sqlite3_vtab *tab, int nArg, const char *zName,
