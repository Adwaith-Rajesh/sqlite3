...
** the NULL terminator.
*/
static int SQLITE_TCLAPI binarize(void *clientData, Tcl_Interp *interp,
...
** actually made.
*/
static int SQLITE_TCLAPI test_value_overhead(void *clientData,
    ...
    if (objc != 3) {
        Tcl_AppendResult(interp, "wrong # args: should be \"",
                         ...
                         " <repeat-count> <do-calls>", 0);
        return TCL_ERROR;
    }

    if (Tcl_GetIntFromObj(interp, objv[1], &repeat_count))
        return TCL_ERROR;
    if (Tcl_GetIntFromObj(interp, objv[2], &do_calls))
        return TCL_ERROR;
...
**
*/
static int SQLITE_TCLAPI test_translate(void *clientData, Tcl_Interp *interp,
                                        int objc, Tcl_Obj *CONST objv[]) {
    u8 enc_from;
    u8 enc_to;
    sqlite3_value *pVal;
...
static int SQLITE_TCLAPI test_translate_selftest(void *clientData,
                                                 Tcl_Interp *interp, int objc,
                                                 Tcl_Obj *CONST objv[]) {
