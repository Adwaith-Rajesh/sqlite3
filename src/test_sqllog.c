    ...
    ** copy and break out of the loop.  */
    while (feof(fd) == 0) {
        ...
        if (fgets(zLine, sizeof(zLine), fd)) {
            ...
            n = strlen(z);
            while (n > 0 && sqllog_isspace(z[n - 1]))
                n--;
...
static int
sqllogFindAttached(sqlite3 *db,         /* Database connection */
    ...
    if (p->fd == 0) {
        ...
        ** environment variable $ENVIRONMENT_VARIABLE1_NAME.  */
        if (sqllogglobal.zPrefix[0] == 0) {
            ...
            if (zVar == 0 || strlen(zVar) + 10 >= (sizeof(sqllogglobal.zPrefix)))
                return;
            sqlite3_snprintf(sizeof(sqllogglobal.zPrefix), sqllogglobal.zPrefix,
                             "%s/sqllog_%05d", zVar, getProcessId());
            sqlite3_snprintf(sizeof(sqllogglobal.zIdx), sqllogglobal.zIdx, "%s.idx",
        ...
        if (rc == SQLITE_OK) {
            ...
            if ((SQLLOG_NAMESZ - nFile) < 8) {
                sqlite3_log(SQLITE_IOERR,
                            "sqllogTraceDb(): database name too long (%d bytes)",
                            nFile);
                bRet = 0;
            } else {
