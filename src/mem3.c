...
static void memsys3Leave(void) { sqlite3_mutex_leave(mem3.mutex); }
...
static void memsys3FreeUnsafe(void *pOld) {
    ...
    if (mem3.iKeyBlk) {
        while ((mem3.aPool[mem3.iKeyBlk - 1].u.hdr.size4x & 2) == 0) {
            ...
            x = mem3.aPool[mem3.iKeyBlk - 1].u.hdr.size4x & 2;
            mem3.aPool[mem3.iKeyBlk - 1].u.hdr.size4x = mem3.szKeyBlk * 4 | x;
            mem3.aPool[mem3.iKeyBlk + mem3.szKeyBlk - 1].u.hdr.prevSize =
                mem3.szKeyBlk;
        }
        x = mem3.aPool[mem3.iKeyBlk - 1].u.hdr.size4x & 2;
        while ((mem3.aPool[mem3.iKeyBlk + mem3.szKeyBlk - 1].u.hdr.size4x & 1) ==
               0) {
            memsys3Unlink(mem3.iKeyBlk + mem3.szKeyBlk);
            mem3.szKeyBlk +=
                mem3.aPool[mem3.iKeyBlk + mem3.szKeyBlk - 1].u.hdr.size4x / 4;
            mem3.aPool[mem3.iKeyBlk - 1].u.hdr.size4x = mem3.szKeyBlk * 4 | x;
            mem3.aPool[mem3.iKeyBlk + mem3.szKeyBlk - 1].u.hdr.prevSize =
                mem3.szKeyBlk;
        }
...
void sqlite3Memsys3Dump(const char *zFilename) {
    ...
    for (i = 1; i <= mem3.nPool; i += size / 4) {
        ...
        if ((size & 1) == 0 &&
            mem3.aPool[i + size / 4 - 1].u.hdr.prevSize != size / 4) {
...
const sqlite3_mem_methods *sqlite3MemGetMemsys3(void) {
    static const sqlite3_mem_methods mempoolMethods = {
        memsys3Malloc, memsys3Free, memsys3Realloc, memsys3Size,
        memsys3Roundup, memsys3Init, memsys3Shutdown, 0};
