        ...
        if ((pStat = sqlite3FindTable(db, zTab, pDb->zDbSName)) == 0) {
            if (i < nToOpen) {
                ...
                ** because the OpenWrite opcode below will be needing it. */
                sqlite3NestedParse(pParse, "CREATE TABLE %Q.%s(%s)", pDb->zDbSName,
                                   zTab, aTable[i].zCols);
...
static void statInit(sqlite3_context *context, int argc, sqlite3_value **argv) {
    ...
    int mxSample =
        OptimizationEnabled(db, SQLITE_Stat4) ? SQLITE_STAT4_SAMPLES : 0;
#endif
    ...
    n = sizeof(*p) + sizeof(tRowcnt) * nColUp /* StatAccum.anEq */
    ...
    if (mxSample) {
        ...
        p->iPrn =
            0x689e962d * (u32)nCol ^ 0xd0944565 * (u32)sqlite3_value_int(argv[2]);
...
static const FuncDef statInitFuncdef = {4,           /* nArg */
...
*/
static int sampleIsBetterPost(StatAccum *pAccum, StatSample *pNew,
                              StatSample *pOld) {
...
*/
static int sampleIsBetter(StatAccum *pAccum, StatSample *pNew,
                          StatSample *pOld) {
...
static void samplePushPrevious(StatAccum *p, int iChng) {
    ...
    ** p->nMaxEqZero or greater set to zero. */
    for (i = p->nSample - 1; i >= 0; i--) {
        int j;
        for (j = p->nMaxEqZero; j < p->nCol; j++)
            assert(p->a[i].anEq[j] > 0);
    }
...
static const FuncDef statPushFuncdef = {2 + IsStat4, /* nArg */
                                        SQLITE_UTF8, /* funcFlags */
                                        0,           /* pUserData */
                                        0,           /* pNext */
                                        statPush,    /* xSFunc */
...
static const FuncDef statGetFuncdef = {1 + IsStat4, /* nArg */
                                       SQLITE_UTF8, /* funcFlags */
                                       0,           /* pUserData */
                                       0,           /* pNext */
                                       statGet,     /* xSFunc */
...
static void analyzeOneTable(
    ...
    if (db->xPreUpdateCallback) {
        ...
        pStat1->nCol = 3;
        pStat1->iPKey = -1;
        sqlite3VdbeAddOp4(pParse->pVdbe, OP_Noop, 0, 0, 0, (char *)pStat1,
                          P4_DYNAMIC);
    }
#endif
        ...
        if (nColTest > 0) {
            ...
            */
            ...
            for (i = 0; i < nColTest; i++) {
                ...
                aGotoChng[i] = sqlite3VdbeAddOp4(v, OP_Ne, regTemp, 0, regPrev + i,
                                                 pColl, P4_COLLSEQ);
...
*/
static Index *findIndexOrPrimaryKey(sqlite3 *db, const char *zName,
                                    const char *zDb) {
...
*/
static int loadStatTbl(sqlite3 *db,       /* Database handle */
    ...
    while (sqlite3_step(pStmt) == SQLITE_ROW) {
        ...
        ** the sqlite_stat4 table. */
        ...
        decodeIntArray((char *)sqlite3_column_text(pStmt, 1), nCol, pSample->anEq,
                       0, 0);
        decodeIntArray((char *)sqlite3_column_text(pStmt, 2), nCol, pSample->anLt,
                       0, 0);
        decodeIntArray((char *)sqlite3_column_text(pStmt, 3), nCol, pSample->anDLt,
                       0, 0);
...
int sqlite3AnalysisLoad(sqlite3 *db, int iDb) {
    ...
    if ((pStat1 = sqlite3FindTable(db, "sqlite_stat1", sInfo.zDatabase)) &&
        ...
        zSql = sqlite3MPrintf(db, "SELECT tbl,idx,stat FROM %Q.sqlite_stat1",
                              sInfo.zDatabase);
        if (zSql == 0) {
