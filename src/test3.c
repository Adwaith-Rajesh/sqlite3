...
static int SQLITE_TCLAPI btree_open(
    ...
    if (Tcl_GetInt(interp, argv[2], &nCache))
        return TCL_ERROR;
    nRefSqlite3++;
    if (nRefSqlite3 == 1) {
...
static int SQLITE_TCLAPI btree_pager_stats(
    ...
    */
    ...
    for (i = 0; i < 11; i++) {
        static char *zName[] = {"ref", "page", "max", "size", "state", "err",
...
static int SQLITE_TCLAPI btree_cursor(
    ...
    pBt = sqlite3TestTextToPtr(argv[1]);
    if (Tcl_GetInt(interp, argv[2], &iTable))
        return TCL_ERROR;
    if (Tcl_GetBoolean(interp, argv[3], &wrFlag))
        return TCL_ERROR;
    if (wrFlag)
        wrFlag = BTREE_WRCSR;
...
static int SQLITE_TCLAPI btree_varint_test(
    ...
    if (Tcl_GetInt(interp, argv[1], (int *)&start))
        return TCL_ERROR;
    if (Tcl_GetInt(interp, argv[2], (int *)&mult))
        return TCL_ERROR;
    if (Tcl_GetInt(interp, argv[3], (int *)&count))
        return TCL_ERROR;
    if (Tcl_GetInt(interp, argv[4], (int *)&incr))
        return TCL_ERROR;
    in = start;
    in *= mult;
    for (i = 0; i < (int)count; i++) {
        ...
        if ((in & 0xffffffff) == in) {
            ...
            if (in != out) {
                sqlite3_snprintf(
                    sizeof(zErr), zErr,
                    "Wrote 0x%016llx and got back 0x%016llx from GetVarint32", in, out);
                Tcl_AppendResult(interp, zErr, 0);
                return TCL_ERROR;
            }
        }
...
** Set the size of the cache used by btree $ID.
*/
static int SQLITE_TCLAPI btree_insert(ClientData clientData, Tcl_Interp *interp,
    ...
    memset(&x, 0, sizeof(x));
    if (objc == 4) {
        if (Tcl_GetIntFromObj(interp, objv[2], &rc))
            return TCL_ERROR;
        x.nKey = rc;
