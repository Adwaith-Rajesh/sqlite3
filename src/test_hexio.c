...
*/
static int SQLITE_TCLAPI hexio_read(void *clientData, Tcl_Interp *interp,
    ...
    if (Tcl_GetIntFromObj(interp, objv[2], &offset))
        return TCL_ERROR;
    if (Tcl_GetIntFromObj(interp, objv[3], &amt))
        return TCL_ERROR;
...
*/
static int SQLITE_TCLAPI hexio_write(void *clientData, Tcl_Interp *interp,
                                     int objc, Tcl_Obj *CONST objv[]) {
    int offset;
...
** hexadecimal digits.
*/
static int SQLITE_TCLAPI hexio_get_int(void *clientData, Tcl_Interp *interp,
                                       int objc, Tcl_Obj *CONST objv[]) {
    int val;
    int nIn, nOut;
    const unsigned char *zIn;
...
*/
static int SQLITE_TCLAPI hexio_render_int16(void *clientData,
    ...
    if (Tcl_GetIntFromObj(interp, objv[1], &val))
        return TCL_ERROR;
    aNum[0] = val >> 8;
    aNum[1] = val;
...
*/
static int SQLITE_TCLAPI hexio_render_int32(void *clientData,
                                            Tcl_Interp *interp, int objc,
                                            Tcl_Obj *CONST objv[]) {
    int val;
    unsigned char aNum[10];
...
*/
static int SQLITE_TCLAPI utf8_to_utf8(void *clientData, Tcl_Interp *interp,
                                      int objc, Tcl_Obj *CONST objv[]) {
#ifdef SQLITE_DEBUG
    int n;
    int nOut;
...
*/
static int SQLITE_TCLAPI read_fts3varint(void *clientData, Tcl_Interp *interp,
                                         int objc, Tcl_Obj *CONST objv[]) {
    int nBlob;
    unsigned char *zBlob;
    sqlite3_int64 iVal;
...
*/
static int SQLITE_TCLAPI make_fts3record(void *clientData, Tcl_Interp *interp,
                                         int objc, Tcl_Obj *CONST objv[]) {
    Tcl_Obj **aArg = 0;
    int nArg = 0;
