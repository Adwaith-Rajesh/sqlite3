...
static int dbpageConnect(sqlite3 *db, void *pAux, int argc,
    ...
    rc = sqlite3_declare_vtab(
        db, "CREATE TABLE x(pgno INTEGER PRIMARY KEY, data BLOB, schema HIDDEN)");
    if (rc == SQLITE_OK) {
...
static int dbpageColumn(sqlite3_vtab_cursor *pCursor, sqlite3_context *ctx,
                        int i) {
    DbpageCursor *pCsr = (DbpageCursor *)pCursor;
    int rc = SQLITE_OK;
    switch (i) {
...
static int dbpageUpdate(sqlite3_vtab *pVtab, int argc, sqlite3_value **argv,
                        sqlite_int64 *pRowid) {
    ...
    if (NEVER(pgno < 1) || NEVER(pBt == 0) ||
        NEVER(pgno > sqlite3BtreeLastPage(pBt))) {
    ...
    if (rc == SQLITE_OK) {
        ...
        if (pData && (rc = sqlite3PagerWrite(pDbPage)) == SQLITE_OK) {
