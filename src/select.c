...
sqlite3SelectNew(Parse *pParse,      /* Parsing context */
    ...
    pNew->nSelectRow = 0;
    if (pSrc == 0)
        pSrc = sqlite3DbMallocZero(pParse->db, sizeof(*pSrc));
    pNew->pSrc = pSrc;
    pNew->pWhere = pWhere;
...
int sqlite3JoinType(Parse *pParse, Token *pA, Token *pB, Token *pC) {
    ...
    for (i = 0; i < 3 && apAll[i]; i++) {
        ...
        testcase(j == 0 || j == 1 || j == 2 || j == 3 || j == 4 || j == 5 ||
                 j == 6);
...
static int tableAndColumnIndex(
    ...
    for (i = iStart; i <= iEnd; i++) {
        ...
        if (iCol >= 0 && (bIgnoreHidden == 0 ||
                          IsHiddenColumn(&pSrc->a[i].pTab->aCol[iCol]) == 0)) {
            if (piTab) {
...
static void unsetJoinExpr(Expr *p, int iTable, int nullable) {
    while (p) {
        if (iTable < 0 ||
            (ExprHasProperty(p, EP_OuterON) && p->w.iJoin == iTable)) {
...
static int sqlite3ProcessJoin(Parse *pParse, Select *p) {
    ...
    for (i = 0; i < pSrc->nSrc - 1; i++, pRight++, pLeft++) {
        ...
        if (NEVER(pLeft->pTab == 0 || pRightTab == 0))
            continue;
        ...
        */
        if (pRight->fg.jointype & JT_NATURAL) {
            ...
            for (j = 0; j < pRightTab->nCol; j++) {
                ...
                if (IsHiddenColumn(&pRightTab->aCol[j]))
                    continue;
                ...
                if (tableAndColumnIndex(pSrc, 0, i, zName, 0, 0, 1)) {
                    ...
                    if (pUsing) {
                        ...
                        pUsing->a[pUsing->nId - 1].zName =
                            sqlite3DbStrDup(pParse->db, zName);
                    }
                }
            }
            if (pUsing) {
            ...
            for (j = 0; j < pList->nId; j++) {
                ...
                if ((pSrc->a[0].fg.jointype & JT_LTORJ) != 0) {
                    ...
                    */
                    ...
                    while (tableAndColumnIndex(pSrc, iLeft + 1, i, zName, &iLeft,
                                               &iLeftCol, pRight->fg.isSynthUsing) != 0) {
...
*/
static void
innerLoopLoadRow(Parse *pParse,     /* Statement under construction */
                 Select *pSelect,   /* The query being coded */
                 RowLoadInfo *pInfo /* Info needed to complete the row load */
) {
    sqlite3ExprCodeExprList(pParse, pSelect->pEList, pInfo->regResult, 0,
                            pInfo->ecelFlags);
#ifdef SQLITE_ENABLE_SORTER_REFERENCES
    if (pInfo->pExtra) {
...
*/
static int makeSorterRecord(Parse *pParse, SortCtx *pSort, Select *pSelect,
                            int regBase, int nBase) {
    ...
    */
    ...
    if (nOBSat > 0) {
        ...
        pOp->p4.pKeyInfo = sqlite3KeyInfoFromExprList(
            pParse, pSort->pOrderBy, nOBSat, pKI->nAllField - pKI->nKeyField - 1);
    ...
    if (iLimit) {
        ...
        */
        ...
        iSkip = sqlite3VdbeAddOp4Int(v, OP_IdxLE, iCsr, 0, regBase + nOBSat,
                                     nExpr - nOBSat);
        VdbeCoverage(v);
    ...
    sqlite3VdbeAddOp4Int(v, op, pSort->iECursor, regRecord, regBase + nOBSat,
                         nBase - nOBSat);
    if (iSkip) {
        sqlite3VdbeChangeP2(v, iSkip,
                            pSort->labelOBLopt ? pSort->labelOBLopt
...
static void fixDistinctOpenEph(
    ...
    if (pParse->nErr == 0 && (eTnctType == WHERE_DISTINCT_UNIQUE ||
...
*/
static void
selectExprDefer(Parse *pParse,     /* Leave any error here */
    ...
    for (i = 0; i < pEList->nExpr; i++) {
        ...
        if (pItem->u.x.iOrderByCol == 0) {
            ...
            if (pExpr->op == TK_COLUMN && pExpr->iColumn >= 0 &&
                ALWAYS(ExprUseYTab(pExpr)) && (pTab = pExpr->y.pTab) != 0 &&
                IsOrdinaryTable(pTab) &&
                (pTab->aCol[pExpr->iColumn].colFlags & COLFLAG_SORTERREF) != 0) {
                int j;
                for (j = 0; j < nDefer; j++) {
                    if (pSort->aDefer[j].iCsr == pExpr->iTable)
                        break;
                }
                if (j == nDefer) {
        ...
        */
        ...
        if (pSort && hasDistinct == 0 && eDest != SRT_EphemTab &&
            ...
            ** from the sorter by the optimizations in this branch */
            ...
            assert(eDest == SRT_Set || eDest == SRT_Mem || eDest == SRT_Coroutine ||
                   eDest == SRT_Output || eDest == SRT_Upfrom);
        }
        ...
        if (pExtra)
            nResultCol += pExtra->nExpr;
#endif
        if (p->iLimit && (ecelFlags & SQLITE_ECEL_OMITREF) != 0 && nPrefixReg > 0) {
            assert(pSort != 0);
            ...
            ** to process OPFLAG_NOCHANGE values without an assert() failing. */
            ...
            if (eDest == SRT_DistFifo) {
                ...
                ** output table as well.  */
                ...
                VdbeCoverage(v);
                sqlite3VdbeAddOp4Int(v, OP_IdxInsert, iParm + 1, r1, regResult,
                                     nResultCol);
                assert(pSort == 0);
            }
#endif
        ...
        case SRT_Upfrom: {
            if (pSort) {
                ...
                ** aggregates do.  Don't record that empty row in the output table. */
                ...
                sqlite3VdbeAddOp3(v, OP_MakeRecord, regResult + (i2 < 0),
                                  nResultCol - (i2 < 0), r1);
                if (i2 < 0) {
        ...
        */
        ...
        case SRT_Queue: {
            ...
            if (eDest == SRT_DistQueue) {
                ...
                ** added to the queue. */
                addrTest = sqlite3VdbeAddOp4Int(v, OP_Found, iParm + 1, 0, regResult,
                                                nResultCol);
                VdbeCoverage(v);
            }
            ...
            for (i = 0; i < nKey; i++) {
                sqlite3VdbeAddOp2(v, OP_SCopy, regResult + pSO->a[i].u.x.iOrderByCol - 1,
                                  r2 + i);
            }
...
generateSortTail(Parse *pParse,    /* Parsing context */
    ...
    int addrContinue =
        sqlite3VdbeMakeLabel(pParse); /* Jump here for next cycle */
    ...
    ExplainQueryPlan2(addrExplain, (pParse, 0, "USE TEMP B-TREE FOR %sORDER BY",
                                    pSort->nOBSat > 0 ? "RIGHT PART OF " : ""));
    sqlite3VdbeScanStatusRange(v, addrExplain, pSort->addrPush,
                               pSort->addrPushEnd);
    ...
    if (pSort->nDefer) {
        ...
        for (i = 0; i < pSort->nDefer; i++) {
            ...
            if (HasRowid(pTab)) {
                ...
                sqlite3VdbeAddOp3(v, OP_SeekRowid, iCsr, sqlite3VdbeCurrentAddr(v) + 1,
                                  regKey);
            } else {
                int k;
                int iJmp;
    ...
    switch (eDest) {
        ...
        case SRT_Upfrom: {
            int i2 = pDest->iSDParm2;
            int r1 = sqlite3GetTempReg(pParse);
            sqlite3VdbeAddOp3(v, OP_MakeRecord, regRow + (i2 < 0), nColumn - (i2 < 0),
                              r1);
            if (i2 < 0) {
...
static const char *columnTypeImpl(NameContext *pNC,
                                  ...
                                  Expr *pExpr
#else
                                  Expr *pExpr, const char **pzOrigDb,
        ...
        case TK_COLUMN: {
            ...
            */
            ...
            if (pS) {
                ...
                */
                if (iCol < pS->pEList->nExpr && (!ViewCanHaveRowid || iCol >= 0)) {
...
void sqlite3GenerateColumnNames(
    ...
    for (i = 0; i < pEList->nExpr; i++) {
        ...
        if (pEList->a[i].zEName && pEList->a[i].fg.eEName == ENAME_NAME) {
            ...
            z = z == 0 ? sqlite3MPrintf(db, "column%d", i + 1)
    ...
    *paCol = aCol;
    ...
    for (i = 0, pCol = aCol; i < nCol && !pParse->nErr; i++, pCol++) {
        ...
        if ((zName = pX->zEName) != 0 && pX->fg.eEName == ENAME_NAME) {
            ...
            if (pColExpr->op == TK_COLUMN && ALWAYS(ExprUseYTab(pColExpr)) &&
...
void sqlite3SubqueryColumnTypes(
    ...
    if (db->mallocFailed || IN_RENAME_OBJECT)
        return;
    while (pSelect->pPrior)
        pSelect = pSelect->pPrior;
    a = pSelect->pEList->a;
    ...
    for (i = 0, pCol = pTab->aCol; i < pTab->nCol; i++, pCol++) {
        ...
        if (pCol->affinity >= SQLITE_AFF_TEXT && pSelect->pNext) {
            ...
            } else if (pCol->affinity >= SQLITE_AFF_NUMERIC && (m & 0x02) != 0) {
                pCol->affinity = SQLITE_AFF_BLOB;
            }
...
** freed.
*/
static KeyInfo *multiSelectOrderByKeyInfo(Parse *pParse, Select *p,
                                          int nExtra) {
...
generateWithRecursiveQuery(Parse *pParse,    /* Parsing context */
    ...
    */
    for (pFirstRec = p; ALWAYS(pFirstRec != 0); pFirstRec = pFirstRec->pPrior) {
        ...
        pFirstRec->op = TK_ALL;
        if ((pFirstRec->pPrior->selFlags & SF_Recursive) == 0)
            break;
    }
...
multiSelectValues(Parse *pParse,    /* Parsing context */
    ...
    ExplainQueryPlan(
        (pParse, 0, "SCAN %d CONSTANT ROW%s", nRow, nRow == 1 ? "" : "S"));
    while (p) {
...
static int hasAnchor(Select *p) {
    while (p && (p->selFlags & SF_Recursive) != 0) {
        p = p->pPrior;
    }
    return p != 0;
}
...
multi_select_end:
    ...
    if (pDelete) {
        sqlite3ParserAddCleanup(
            pParse, (void (*)(sqlite3 *, void *))sqlite3SelectDelete, pDelete);
    }
    return rc;
}
#endif /* SQLITE_OMIT_COMPOUND_SELECT */
...
static int generateOutputSubroutine(
    ...
    if (regPrev) {
        ...
        VdbeCoverage(v);
        addr2 =
            sqlite3VdbeAddOp4(v, OP_Compare, pIn->iSdst, regPrev + 1, pIn->nSdst,
    ...
    switch (pDest->eDest) {
        ...
        case SRT_Set: {
            ...
            r1 = sqlite3GetTempReg(pParse);
            sqlite3VdbeAddOp4(v, OP_MakeRecord, pIn->iSdst, pIn->nSdst, r1,
                              pDest->zAffSdst, pIn->nSdst);
            sqlite3VdbeAddOp4Int(v, OP_IdxInsert, pDest->iSDParm, r1, pIn->iSdst,
                                 pIn->nSdst);
            sqlite3ReleaseTempReg(pParse, r1);
            break;
        }
...
multiSelectOrderBy(Parse *pParse,    /* Parsing context */
    ...
    */
    ...
    if ((op == TK_ALL || op == TK_UNION) &&
        ...
        for (pSplit = p; pSplit->pPrior != 0 && pSplit->op == op;
                ...
                ** just as it did when it was a column of a view or sub-query. */
                {
                    ...
                    if (pNat != pColl ||
                        (pExpr->op != TK_COLUMN && pExpr->op != TK_COLLATE)) {
                        pExpr = sqlite3ExprAddCollateString(
                            pSubst->pParse, pExpr, (pColl ? pColl->zName : "BINARY"));
                    }
                }
...
static void
substSelect(SubstContext *pSubst, /* Description of the substitution */
...
static void
recomputeColumnsUsed(Select *pSelect,  /* The complete SELECT statement */
                     SrcItem *pSrcItem /* Which FROM clause item to recompute */
) {
    Walker w;
    if (NEVER(pSrcItem->pTab == 0))
        return;
    memset(&w, 0, sizeof(w));
...
*/
static void
srclistRenumberCursors(Parse *pParse, /* Parse context */
...
**     initialized to zero.
*/
static void renumberCursors(Parse *pParse, /* Parse context */
    ...
    */
    if (pSub->pPrior) {
        ...
        for (pSub1 = pSub; pSub1; pSub1 = pSub1->pPrior) {
            ...
            testcase((pSub1->selFlags & (SF_Distinct | SF_Aggregate)) ==
                     SF_Aggregate);
            assert(pSub->pSrc != 0);
        ...
        if (pSrc->nSrc > 1) {
            if (pParse->nSelect > 500)
                return 0;
            if (OptimizationDisabled(db, SQLITE_FlttnUnionAll))
                return 0;
...
*/
static void
constInsert(WhereConst *pConst, /* The WhereConst into which we are inserting */
    ...
    if (ExprHasProperty(pColumn, EP_FixedCol))
        return;
...
** is SQLITE_AFF_BLOB.
*/
static int propagateConstantExprRewriteOne(WhereConst *pConst, Expr *pExpr,
                                           int bIgnoreAffBlob) {
    int i;
    ...
    for (i = 0; i < pConst->nConst; i++) {
        ...
        pExpr->pLeft =
            sqlite3ExprDup(pConst->pParse->db, pConst->apExpr[i * 2 + 1], 0);
...
static int propagateConstantExprRewrite(Walker *pWalker, Expr *pExpr) {
    ...
    if (pConst->bHasAffBlob) {
        if ((pExpr->op >= TK_EQ && pExpr->op <= TK_GE) || pExpr->op == TK_IS) {
...
*/
static int
propagateConstants(Parse *pParse, /* The parsing context */
    ...
    do {
        ...
        x.apExpr = 0;
        x.bHasAffBlob = 0;
        if (ALWAYS(p->pSrc != 0) && p->pSrc->nSrc > 0 &&
...
static int pushDownWhereTerms(
    ...
    if (pSubq->pPrior) {
        ...
        for (pSel = pSubq; pSel; pSel = pSel->pPrior) {
            u8 op = pSel->op;
            assert(op == TK_ALL || op == TK_SELECT || op == TK_UNION ||
                   op == TK_INTERSECT || op == TK_EXCEPT);
...
#ifdef SQLITE_ALLOW_ROWID_IN_VIEW
    if (ViewCanHaveRowid &&
        (pWhere->op == TK_ISNULL || pWhere->op == TK_NOTNULL)) {
        Expr *pLeft = pWhere->pLeft;
        if (ALWAYS(pLeft) && pLeft->op == TK_COLUMN && pLeft->iColumn < 0) {
...
static Table *isSimpleCount(Select *p, AggInfo *pAggInfo) {
    ...
    if (p->pWhere || p->pEList->nExpr != 1 || p->pSrc->nSrc != 1 ||
        p->pSrc->a[0].pSelect || pAggInfo->nFunc != 1 || p->pHaving) {
        return 0;
    }
    ...
    if (ExprHasProperty(pExpr, EP_Distinct | EP_WinFunc))
        return 0;
...
int sqlite3IndexedByLookup(Parse *pParse, SrcItem *pFrom) {
    ...
    for (pIdx = pTab->pIndex; pIdx && sqlite3StrICmp(pIdx->zName, zIndexedBy);
         pIdx = pIdx->pNext);
    if (!pIdx) {
...
With *sqlite3WithPush(Parse *pParse, With *pWith, u8 bFree) {
    if (pWith) {
        if (bFree) {
            pWith = (With *)sqlite3ParserAddCleanup(
                pParse, (void (*)(sqlite3 *, void *))sqlite3WithDelete, pWith);
            if (pWith == 0)
                return 0;
        }
        if (pParse->nErr == 0) {
...
*/
static int
resolveFromTermToCte(Parse *pParse,   /* The parsing context */
        ...
        ** In this case, proceed.  */
        ...
        while (bMayRecursive && pRecTerm->op == pSel->op) {
            ...
            for (i = 0; i < pSrc->nSrc; i++) {
                ...
                if (pItem->zDatabase == 0 && pItem->zName != 0 &&
            ...
            if ((pRecTerm->selFlags & SF_Recursive) == 0)
                break;
    ...
    for (i = 0, pFrom = pTabList->a; i < pTabList->nSrc; i++, pFrom++) {
        ...
        if (pFrom->zName == 0) {
            ...
            if (!IsOrdinaryTable(pTab)) {
                ...
                else if (ALWAYS(IsVirtual(pTab)) && pFrom->fg.fromDDL &&
            ...
            if (pE->op != TK_ASTERISK &&
                ...
                ** expanded. */
                ...
                for (i = 0, pFrom = pTabList->a; i < pTabList->nSrc; i++, pFrom++) {
                    ...
                    if (i + 1 < pTabList->nSrc && pFrom[1].fg.isUsing &&
                    ...
                    nAdd = pTab->nCol;
                    if (VisibleRowid(pTab) && (selFlags & SF_NestedFrom) != 0)
                        nAdd++;
                    for (j = 0; j < nAdd; j++) {
                        const char *zName;
                        ...
                        if (j == pTab->nCol) {
                            ...
                            */
                            ...
                            if ((pTab->aCol[j].colFlags & COLFLAG_NOEXPAND) != 0 &&
                                zTName == 0 && (selFlags & (SF_NestedFrom)) == 0) {
                                continue;
                            }
                        }
                        assert(zName);
                        ...
                        if ((selFlags & SF_NestedFrom) != 0 && !IN_RENAME_OBJECT) {
                            if (pNestedFrom &&
                                (!ViewCanHaveRowid || j < pNestedFrom->nExpr)) {
                            ...
                            if ((pFrom->fg.isUsing &&
                                ...
                                (j < pTab->nCol &&
                                 (pTab->aCol[j].colFlags & COLFLAG_NOEXPAND))) {
                                pX->fg.bNoExpand = 1;
                            }
                        } else if (longNames) {
...
*/
void sqlite3SelectPrep(Parse *pParse,        /* The parser context */
...
static void printAggInfo(AggInfo *pAggInfo) {
    ...
    for (ii = 0; ii < pAggInfo->nFunc; ii++) {
        sqlite3DebugPrintf("agg-func[%d]: iMem=%d\n", ii,
                           pAggInfo->iFirstReg + pAggInfo->nColumn + ii);
...
static int aggregateIdxEprRefToColCallback(Walker *pWalker, Expr *pExpr) {
    ...
    if (pExpr->op == TK_AGG_FUNCTION)
        return WRC_Continue;
    if (pExpr->op == TK_IF_NULL_ROW)
        return WRC_Continue;
    pAggInfo = pExpr->pAggInfo;
    if (NEVER(pExpr->iAgg >= pAggInfo->nColumn))
        return WRC_Continue;
    assert(pExpr->iAgg >= 0);
...
static void resetAccumulator(Parse *pParse, AggInfo *pAggInfo) {
    ...
    for (pFunc = pAggInfo->aFunc, i = 0; i < pAggInfo->nFunc; i++, pFunc++) {
        ...
        if (pFunc->iOBTab >= 0) {
            ...
            sqlite3VdbeAddOp4(v, OP_OpenEphemeral, pFunc->iOBTab,
                              pOBList->nExpr + nExtra, 0, (char *)pKeyInfo,
                              P4_KEYINFO);
            ExplainQueryPlan(
                (pParse, 0, "USE TEMP B-TREE FOR %s(ORDER BY)", pFunc->pFunc->zName));
        }
    }
}
    ...
    for (i = 0, pF = pAggInfo->aFunc; i < pAggInfo->nFunc; i++, pF++) {
        ...
        if (ExprHasProperty(pF->pFExpr, EP_WinFunc)) {
            ...
            if (pAggInfo->nAccumulator &&
                (pF->pFunc->funcFlags & SQLITE_FUNC_NEEDCOLL) && regAcc) {
        ...
        if (pF->iDistinct >= 0 && pList) {
            ...
            pF->iDistinct = codeDistinct(pParse, eDistinctType, pF->iDistinct,
                                         addrNext, pList, regDistinct);
        }
        if (pF->iOBTab >= 0) {
            ...
            if (pF->pFunc->funcFlags & SQLITE_FUNC_NEEDCOLL) {
                ...
                if (regHit == 0 && pAggInfo->nAccumulator)
                    regHit = ++pParse->nMem;
                sqlite3VdbeAddOp4(v, OP_CollSeq, regHit, 0, 0, (char *)pColl,
                                  P4_COLLSEQ);
            }
            sqlite3VdbeAddOp3(v, OP_AggStep, 0, regAgg, AggInfoFuncReg(pAggInfo, i));
            sqlite3VdbeAppendP4(v, pF->pFunc, P4_FUNCDEF);
...
explainSimpleCount(Parse *pParse, /* Parse context */
    ...
    if (pParse->explain == 2) {
        ...
        sqlite3VdbeExplain(pParse, 0, "SCAN %s%s%s", pTab->zName,
    ...
    if (pExpr->op != TK_AND) {
        ...
        ** statements AggInfo structure.  */
        if (sqlite3ExprIsConstantOrGroupBy(pWalker->pParse, pExpr, pS->pGroupBy) &&
            ExprAlwaysFalse(pExpr) == 0 && pExpr->pAggInfo == 0) {
...
static SrcItem *isSelfJoinView(
    ...
    if (pThis->pSelect->selFlags & SF_PushDown)
        return 0;
    while (iFirst < iEnd) {
        Select *pS1;
...
static int countOfViewOptimization(Parse *pParse, Select *p) {
    ...
    if ((p->selFlags & SF_Aggregate) == 0)
        return 0; /* This is an aggregate */
    if (p->pEList->nExpr != 1)
        return 0; /* Single result column */
    if (p->pWhere)
        return 0;
    if (p->pHaving)
        return 0;
    ...
    if (p->pSrc->nSrc != 1)
        return 0; /* One table in FROM  */
    if (ExprHasProperty(pExpr, EP_WinFunc))
        return 0; /* Not a window function */
    pSub = p->pSrc->a[0].pSelect;
    if (pSub == 0)
        return 0; /* The FROM is a subquery */
    if (pSub->pPrior == 0)
        return 0; /* Must be a compound */
    if (pSub->selFlags & SF_CopyCte)
        return 0; /* Not a CTE */
    do {
...
static int sameSrcAlias(SrcItem *p0, SrcList *pSrc) {
    ...
    for (i = 0; i < pSrc->nSrc; i++) {
        ...
        if (p1->pSelect && (p1->pSelect->selFlags & SF_NestedFrom) != 0 &&
...
static int fromClauseTermCanBeCoroutine(
    ...
    if (pItem->fg.isCte) {
        ...
        if (pCteUse->eM10d == M10d_Yes)
            return 0; /* (2a) */
        if (pCteUse->nUse >= 2 && pCteUse->eM10d != M10d_No)
            return 0; /* (2b) */
    }
    if (pTabList->a[0].fg.jointype & JT_LTORJ)
        return 0; /* (3)  */
    if (OptimizationDisabled(pParse->db, SQLITE_Coroutines))
        return 0; /* (4)  */
    ...
    if (i == 0) {
        if (pTabList->nSrc == 1)
            return 1; /* (1a) */
        if (pTabList->a[1].fg.jointype & JT_CROSS)
            return 1; /* (1b) */
        if (selFlags & SF_UpdateFrom)
            return 0; /* (1c-iii) */
        return 1;
    }
    if (selFlags & SF_UpdateFrom)
    ...
    while (1 /*exit-by-break*/) {
        if (pItem->fg.jointype & (JT_OUTER | JT_CROSS))
            return 0; /* (1c-ii) */
        if (i == 0)
            break;
        i--;
...
int sqlite3Select(Parse *pParse,    /* The parser context */
    ...
    if (sqlite3TreeTrace & 0x10000) {
        if ((sqlite3TreeTrace & 0x10001) == 0x10000) {
            sqlite3TreeViewLine(0, "In sqlite3Select() at %s:%d", __FILE__, __LINE__);
        }
        sqlite3ShowSelect(p);
    }
#endif
        ...
        if ((pItem->fg.jointype & (JT_LEFT | JT_LTORJ)) != 0 &&
            ...
            if (pItem->fg.jointype & JT_LTORJ) {
                ...
                for (j = pTabList->nSrc - 1; j >= 0; j--) {
                    ...
                    if (pTabList->a[j].fg.jointype & JT_RIGHT)
                        break;
                }
            }
        }
    ...
    */
    if (p->pPrior) {
        ...
        if ((sqlite3TreeTrace & 0x400) != 0 &&
            ExplainQueryPlanParent(pParse) == 0) {
            sqlite3TreeViewSelect(0, p, 0);
        }
#endif
        if (p->pNext == 0)
    ...
    ** sqlite3WhereBegin().
    */
    if (p->pWhere != 0 && p->pWhere->op == TK_AND &&
    ...
    */
    if (sSort.pOrderBy) {
        KeyInfo *pKeyInfo;
        pKeyInfo =
            sqlite3KeyInfoFromExprList(pParse, sSort.pOrderBy, 0, pEList->nExpr);
        sSort.iECursor = pParse->nTab++;
        sSort.addrSortIndex =
            sqlite3VdbeAddOp4(v, OP_OpenEphemeral, sSort.iECursor,
                              sSort.pOrderBy->nExpr + 1 + pEList->nExpr, 0,
                              (char *)pKeyInfo, P4_KEYINFO);
    } else {
    ...
    if (pDest->eDest == SRT_EphemTab) {
        ...
        if (p->selFlags & SF_NestedFrom) {
            ...
            int ii;
            for (ii = pEList->nExpr - 1; ii > 0 && pEList->a[ii].fg.bUsed == 0;
                 ii--) {
    ...
    if (p->selFlags & SF_Distinct) {
        ...
        sDistinct.addrTnct = sqlite3VdbeAddOp4(
            v, OP_OpenEphemeral, sDistinct.tabTnct, 0, 0,
    ...
    if (!isAgg && pGroupBy == 0) {
        ...
        */
        ...
        if (pAggInfo) {
            sqlite3ParserAddCleanup(pParse, (void (*)(sqlite3 *, void *))agginfoFree,
                                    pAggInfo);
            testcase(pParse->earlyCleanup);
        }
        if (db->mallocFailed) {
        ...
        */
        if (pGroupBy) {
            ...
            */
            ...
            pKeyInfo =
                sqlite3KeyInfoFromExprList(pParse, pGroupBy, 0, pAggInfo->nColumn);
            addrSortingIdx = sqlite3VdbeAddOp4(v, OP_SorterOpen, pAggInfo->sortingIdx,
            ...
            */
            ...
            pWInfo = sqlite3WhereBegin(
                pParse, pTabList, pWhere, pGroupBy, pDistinct, p,
                (sDistinct.isTnct == 2 ? WHERE_DISTINCTBY : WHERE_GROUPBY) |
                    (orderByGrp ? WHERE_SORTBYGROUP : 0) | distFlag,
                0);
            if (pWInfo == 0) {
            ...
            if (sqlite3WhereIsOrdered(pWInfo) == pGroupBy->nExpr) {
                ...
                */
...
#endif
                ExplainQueryPlan2(
                    addrExp, (pParse, 0, "USE TEMP B-TREE FOR %s",
                              (sDistinct.isTnct && (p->selFlags & SF_Distinct) == 0)
                                  ? "DISTINCT"
                                  : "GROUP BY"));
                ...
                sqlite3VdbeScanStatusRange(v, addrExp, sqlite3VdbeCurrentAddr(v) - 2,
                                           -1);
            ...
            */
            ...
            if (groupBySort) {
                sqlite3VdbeAddOp3(v, OP_SorterData, pAggInfo->sortingIdx, sortOut,
                                  sortPTab);
            }
            ...
            */
             ...
             */
            if (groupBySort) {
                sqlite3VdbeAddOp2(v, OP_SorterNext, pAggInfo->sortingIdx,
                                  addrTopOfLoop);
                VdbeCoverage(v);
            } else {
                TREETRACE(0x2, pParse, p, ("WhereEnd\n"));
                ...
                */
                ...
                if (!p->pSrc->a[0].fg.notIndexed) {
                    for (pIdx = pTab->pIndex; pIdx; pIdx = pIdx->pNext) {
                        if (pIdx->bUnordered == 0 && pIdx->szIdxRow < pTab->szTabRow &&
