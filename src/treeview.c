...
static void sqlite3TreeViewPush(TreeView **pp, u8 moreToFollow) {
    ...
    if (p->iLevel < (int)sizeof(p->bLine))
...
*/
static void sqlite3TreeViewItem(TreeView *p, const char *zLabel,
                                u8 moreFollows) {
...
*/
void sqlite3TreeViewColumnList(TreeView *pView, const Column *aCol, int nCol,
                               u8 moreToFollow) {
    int i;
    ...
    for (i = 0; i < nCol; i++) {
        ...
        if (flg & COLFLAG_PRIMKEY)
            printf(" PRIMARY KEY");
        if (flg & COLFLAG_HIDDEN)
            printf(" HIDDEN");
#ifdef COLFLAG_NOEXPAND
        if (flg & COLFLAG_NOEXPAND)
            printf(" NO-EXPAND");
#endif
        if (flg)
...
void sqlite3TreeViewSrcList(TreeView *pView, const SrcList *pSrc) {
    ...
    for (i = 0; i < pSrc->nSrc; i++) {
        ...
        if (pItem->fg.isTabFunc)
            sqlite3_str_appendf(&x, " isTabFunc");
        if (pItem->fg.isCorrelated)
            sqlite3_str_appendf(&x, " isCorrelated");
        if (pItem->fg.isMaterialized)
            sqlite3_str_appendf(&x, " isMaterialized");
        if (pItem->fg.viaCoroutine)
            sqlite3_str_appendf(&x, " viaCoroutine");
        if (pItem->fg.notCte)
            sqlite3_str_appendf(&x, " notCte");
        if (pItem->fg.isNestedFrom)
            sqlite3_str_appendf(&x, " isNestedFrom");
    ...
    do {
        if (p->selFlags & SF_WhereBegin) {
            ...
            sqlite3TreeViewLine(pView,
                                ...
                                p->selId, p, p->selFlags, (int)p->nSelectRow);
        }
        if (cnt++)
            sqlite3TreeViewPop(&pView);
        if (p->pPrior) {
...
void sqlite3TreeViewWindow(TreeView *pView, const Window *pWin, u8 more) {
    ...
    if (pWin->pPartition) {
        sqlite3TreeViewExprList(pView, pWin->pPartition, nElement > 0,
                                "PARTITION-BY");
    }
    if (pWin->pOrderBy) {
        sqlite3TreeViewExprList(pView, pWin->pOrderBy, (--nElement) > 0,
                                "ORDER-BY");
    }
    if (pWin->eFrmType != 0 && pWin->eFrmType != TK_FILTER) {
        char zBuf[30];
...
void sqlite3TreeViewWinFunc(TreeView *pView, const Window *pWin, u8 more) {
    if (pWin == 0)
        return;
    sqlite3TreeViewPush(&pView, more);
    sqlite3TreeViewLine(pView, "WINFUNC %s(%d)", pWin->pWFunc->zName,
                        pWin->pWFunc->nArg);
...
void sqlite3TreeViewExpr(TreeView *pView, const Expr *pExpr, u8 moreToFollow) {
    ...
    if (pExpr->flags || pExpr->affExpr || pExpr->vvaFlags || pExpr->pAggInfo) {
        ...
        sqlite3_str_appendf(&x, " fg.af=%x.%c", pExpr->flags,
                            pExpr->affExpr ? pExpr->affExpr : 'n');
    ...
    switch (pExpr->op) {
        case TK_AGG_COLUMN: {
            sqlite3TreeViewLine(pView, "AGG{%d:%d}%s", pExpr->iTable, pExpr->iColumn,
                                zFlgs);
            break;
        }
        case TK_COLUMN: {
            if (pExpr->iTable < 0) {
                ...
                sqlite3TreeViewLine(pView, "COLUMN(%d)%s%s", pExpr->iColumn, zFlgs, zOp2);
            } else {
                assert(ExprUseYTab(pExpr));
                sqlite3TreeViewLine(pView, "{%d:%d} pTab=%p%s", pExpr->iTable,
                                    pExpr->iColumn, pExpr->y.pTab, zFlgs);
            }
        ...
        case TK_VARIABLE: {
            assert(!ExprHasProperty(pExpr, EP_IntValue));
            sqlite3TreeViewLine(pView, "VARIABLE(%s,%d)", pExpr->u.zToken,
                                pExpr->iColumn);
            break;
        }
        case TK_REGISTER: {
        ...
        case TK_TRUTH: {
            int x;
            const char *azOp[] = {"IS-FALSE", "IS-TRUE", "IS-NOT-FALSE", "IS-NOT-TRUE"};
            assert(pExpr->op2 == TK_IS || pExpr->op2 == TK_ISNOT);
            assert(pExpr->pRight);
            assert(sqlite3ExprSkipCollateAndLikely(pExpr->pRight)->op == TK_TRUEFALSE);
        ...
        case TK_FUNCTION: {
            ...
            if (pExpr->op == TK_AGG_FUNCTION) {
                ...
                zOp2 = zBuf;
                if (pExpr->op2 == NC_IsCheck)
                    zOp2 = "NC_IsCheck";
                if (pExpr->op2 == NC_IdxExpr)
                    zOp2 = "NC_IdxExpr";
                if (pExpr->op2 == NC_PartIdx)
                    zOp2 = "NC_PartIdx";
                if (pExpr->op2 == NC_GenCol)
                    zOp2 = "NC_GenCol";
                sqlite3TreeViewLine(pView, "FUNCTION %Q%s op2=%s", pExpr->u.zToken, zFlgs,
                                    zOp2);
            } else {
        ...
        case TK_IN: {
            ...
            if (ExprHasProperty(pExpr, EP_Subrtn)) {
                sqlite3_str_appendf(pStr, " subrtn(%d,%d)", pExpr->y.sub.regReturn,
                                    pExpr->y.sub.iAddr);
            }
        ...
        */
        ...
        case TK_TRIGGER: {
            ...
            */
            sqlite3TreeViewLine(pView, "%s(%d)", pExpr->iTable ? "NEW" : "OLD",
                                pExpr->iColumn);
            break;
        }
        case TK_CASE: {
...
#endif
        case TK_MATCH: {
            sqlite3TreeViewLine(pView, "MATCH {%d:%d}%s", pExpr->iTable, pExpr->iColumn,
                                zFlgs);
        ...
        case TK_SELECT_COLUMN: {
            sqlite3TreeViewLine(pView, "SELECT-COLUMN %d of [0..%d]%s", pExpr->iColumn,
                                pExpr->iTable - 1,
...
*/
void sqlite3TreeViewBareExprList(TreeView *pView, const ExprList *pList,
                                 const char *zLabel) {
                ...
                if (zName) {
                    switch (pList->a[i].fg.eEName) {
                        ...
                        case ENAME_TAB:
                            ...
                            if (pList->a[i].fg.bUsed)
                                fprintf(stdout, "(used) ");
                            if (pList->a[i].fg.bUsingTerm)
                                fprintf(stdout, "(USING-term) ");
                            if (pList->a[i].fg.bNoExpand)
                                fprintf(stdout, "(NoExpand) ");
                            break;
                        case ENAME_SPAN:
...
void sqlite3TreeViewExprList(TreeView *pView, const ExprList *pList,
...
*/
void sqlite3TreeViewBareIdList(TreeView *pView, const IdList *pList,
                               const char *zLabel) {
    if (zLabel == 0 || zLabel[0] == 0)
        zLabel = "LIST";
    if (pList == 0) {
...
void sqlite3TreeViewIdList(TreeView *pView, const IdList *pList,
                           u8 moreToFollow, const char *zLabel) {
    sqlite3TreeViewPush(&pView, moreToFollow);
...
*/
void sqlite3TreeViewUpsert(TreeView *pView, const Upsert *pUpsert,
                           u8 moreToFollow) {
    if (pUpsert == 0)
        return;
    sqlite3TreeViewPush(&pView, moreToFollow);
...
*/
void sqlite3TreeViewDelete(const With *pWith, const SrcList *pTabList,
                           const Expr *pWhere, const ExprList *pOrderBy,
                           const Expr *pLimit, const Trigger *pTrigger) {
    int n = 0;
    TreeView *pView = 0;
    sqlite3TreeViewPush(&pView, 0);
...
*/
void sqlite3TreeViewInsert(const With *pWith, const SrcList *pTabList,
                           const IdList *pColumnList, const Select *pSelect,
                           const ExprList *pExprList, int onError,
                           const Upsert *pUpsert, const Trigger *pTrigger) {
    TreeView *pView = 0;
    int n = 0;
    ...
    switch (onError) {
        case OE_Replace:
            ...
            break;
        case OE_Ignore:
            zLabel = "INSERT OR IGNORE";
            break;
        case OE_Rollback:
            zLabel = "INSERT OR ROLLBACK";
            break;
        case OE_Abort:
            zLabel = "INSERT OR ABORT";
            break;
        case OE_Fail:
            zLabel = "INSERT OR FAIL";
            break;
    }
    sqlite3TreeViewPush(&pView, 0);
...
*/
void sqlite3TreeViewUpdate(const With *pWith, const SrcList *pTabList,
                           const ExprList *pChanges, const Expr *pWhere,
                           int onError, const ExprList *pOrderBy,
                           const Expr *pLimit, const Upsert *pUpsert,
    ...
    switch (onError) {
        case OE_Replace:
            zLabel = "UPDATE OR REPLACE";
            break;
        case OE_Ignore:
            zLabel = "UPDATE OR IGNORE";
            break;
        case OE_Rollback:
            zLabel = "UPDATE OR ROLLBACK";
            break;
        case OE_Abort:
            zLabel = "UPDATE OR ABORT";
            break;
        case OE_Fail:
            zLabel = "UPDATE OR FAIL";
            break;
    }
    sqlite3TreeViewPush(&pView, 0);
    sqlite3TreeViewLine(pView, zLabel);
...
*/
void sqlite3TreeViewTriggerStep(TreeView *pView, const TriggerStep *pStep,
                                u8 moreToFollow, u8 showFullList) {
    int cnt = 0;
    if (pStep == 0)
        return;
...
*/
void sqlite3TreeViewTrigger(TreeView *pView, const Trigger *pTrigger,
                            u8 moreToFollow, u8 showFullList) {
    int cnt = 0;
    if (pTrigger == 0)
        return;
...
void sqlite3ShowExprList(const ExprList *p) {
    sqlite3TreeViewExprList(0, p, 0, 0);
}
...
void sqlite3ShowTrigger(const Trigger *p) {
    sqlite3TreeViewTrigger(0, p, 0, 0);
}
void sqlite3ShowTriggerList(const Trigger *p) {
    sqlite3TreeViewTrigger(0, p, 0, 1);
}
#endif
#ifndef SQLITE_OMIT_WINDOWFUNC
