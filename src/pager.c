...
static int assert_pager_state(Pager *p) {
    ...
    /* State must be valid. */
    assert(p->eState == PAGER_OPEN || p->eState == PAGER_READER ||
           p->eState == PAGER_WRITER_LOCKED ||
           p->eState == PAGER_WRITER_CACHEMOD ||
           p->eState == PAGER_WRITER_DBMOD ||
           p->eState == PAGER_WRITER_FINISHED || p->eState == PAGER_ERROR);
...
char *print_pager_state(Pager *p) {
    ...
    sqlite3_snprintf(1024, zRet,
                     ...
                     : p->journalMode == PAGER_JOURNALMODE_WAL      ? "wal"
                                                                    : "?error?",
                     (int)p->tempFile, (int)p->memDb, (int)p->useJournal,
                     p->journalOff, p->journalHdr, (int)p->dbSize,
                     (int)p->dbOrigSize, (int)p->dbFileSize);
...
static int pagerLockDb(Pager *pPager, int eLock) {
    ...
    assert(eLock == SHARED_LOCK || eLock == RESERVED_LOCK ||
...
static int readSuperJournal(sqlite3_file *pJrnl, char *zSuper, u32 nSuper) {
    ...
    if (SQLITE_OK != (rc = sqlite3OsFileSize(pJrnl, &szJ)) || szJ < 16 ||
        SQLITE_OK != (rc = read32bits(pJrnl, szJ - 16, &len)) || len >= nSuper ||
        len > szJ - 16 || len == 0 ||
...
static int writeJournalHdr(Pager *pPager) {
    ...
    u32 nHeader =
        (u32)pPager->pageSize; /* Size of buffer pointed to by zHeader */
    ...
    */
    for (nWrite = 0; rc == SQLITE_OK && nWrite < JOURNAL_HDR_SZ(pPager);
         nWrite += nHeader) {
...
*/
static int
readJournalHdr(Pager *pPager, /* Pager object */
               int isHot,
...
static int writeSuperJournal(Pager *pPager, const char *zSuper) {
    ...
    if (!zSuper || pPager->journalMode == PAGER_JOURNALMODE_MEMORY ||
        !isOpen(pPager->jfd)) {
        return SQLITE_OK;
    }
    pPager->setSuper = 1;
...
static void pager_unlock(Pager *pPager) {
    assert(pPager->eState == PAGER_READER || pPager->eState == PAGER_OPEN ||
    ...
    if (pagerUseWal(pPager)) {
        ...
        int iDc =
            isOpen(pPager->fd) ? sqlite3OsDeviceCharacteristics(pPager->fd) : 0;
...
static int pager_end_transaction(Pager *pPager, int hasSuper, int bCommit) {
    ...
    */
    ...
    releaseAllSavepoints(pPager);
    assert(
        isOpen(pPager->jfd) || pPager->pInJournal == 0 ||
...
static int pager_playback_one_page(
    ...
    */
    ...
    PAGERTRACE(("PLAYBACK %d page %d hash(%08x) %s\n", PAGERID(pPager), pgno,
                pager_datahash(pPager->pageSize, (u8 *)aData),
...
static void setSectorSize(Pager *pPager) {
    ...
    if (pPager->tempFile || (sqlite3OsDeviceCharacteristics(pPager->fd) &
...
static int readDbPage(PgHdr *pPg) {
    ...
    if (iFrame) {
        rc =
            sqlite3WalReadFrame(pPager->pWal, iFrame, pPager->pageSize, pPg->pData);
    } else
#endif
    {
...
*/
static int pagerWalFrames(Pager *pPager,  /* Pager object */
    ...
    rc = sqlite3WalFrames(pPager->pWal, pPager->pageSize, pList, nTruncate,
                          isCommit, pPager->walSyncFlags);
...
static int pagerOpenWalIfPresent(Pager *pPager) {
    ...
    if (!pPager->tempFile) {
        ...
        rc = sqlite3OsAccess(pPager->pVfs, pPager->zWal, SQLITE_ACCESS_EXISTS,
                             &isWal);
        if (rc == SQLITE_OK) {
            if (isWal) {
...
static int pagerPlaybackSavepoint(Pager *pPager, PagerSavepoint *pSavepoint) {
    ...
    */
    while (rc == SQLITE_OK && pPager->journalOff < szJ) {
        ...
        */
        ...
        for (ii = 0; rc == SQLITE_OK && ii < nJRec && pPager->journalOff < szJ;
             ii++) {
            rc = pager_playback_one_page(pPager, &pPager->journalOff, pDone, 1, 1);
        }
        assert(rc != SQLITE_DONE);
    ...
    */
    if (pSavepoint) {
        ...
        for (ii = pSavepoint->iSubRec; rc == SQLITE_OK && ii < pPager->nSubRec;
             ii++) {
...
** and FULL=3.
*/
void sqlite3PagerSetFlags(Pager *pPager,   /* The pager to set safety level for */
...
*/
static int
pagerOpentemp(Pager *pPager,       /* The pager object */
...
int sqlite3PagerSetPagesize(Pager *pPager, u32 *pPageSize, int nReserve) {
    ...
    */
    ...
    if ((pPager->memDb == 0 || pPager->dbSize == 0) &&
        sqlite3PcacheRefCount(pPager->pPCache) == 0 && pageSize &&
...
void enable_simulated_io_errors(void) { sqlite3_io_error_pending = saved_cnt; }
#else
...
static void assertTruncateConstraintCb(PgHdr *pPg) {
    ...
    if (pPg->pgno > pPager->dbSize) { /* if (a) is false */
        ...
        for (i = 0; i < pPg->pPager->nSavepoint; i++) {
            ...
            assert(p->nOrig < pgno ||
                   sqlite3BitvecTestNotNull(p->pInSavepoint, pgno));
        }
    }
}
...
*/
static int pagerAcquireMapPage(Pager *pPager, /* Pager object */
...
static void pagerReleaseMapPage(PgHdr *pPg) {
    ...
    sqlite3OsUnfetch(pPager->fd, (i64)(pPg->pgno - 1) * pPager->pageSize,
                     pPg->pData);
}
...
*/
Pgno sqlite3PagerPagenumber(DbPage *pPg) { return pPg->pgno; }
#endif
...
static int subjournalPage(PgHdr *pPg) {
    ...
    if (pPager->journalMode != PAGER_JOURNALMODE_OFF) {
        ...
        assert(pagerUseWal(pPager) || pageInJournal(pPager, pPg) ||
...
int sqlite3PagerOpen(
    ...
    */
    ...
    pPtr =
        (u8 *)sqlite3MallocZero(ROUND8(sizeof(*pPager)) + /* Pager structure */
    ...
    memcpy(pPtr, &pPager, SQLITE_PTRSIZE);
    pPtr += SQLITE_PTRSIZE;
        ...
        if (rc == SQLITE_OK) {
            ...
            if (!readOnly) {
...
#ifdef SQLITE_ENABLE_ATOMIC_WRITE
                {
                    ...
                    for (ii = szPageDflt; ii <= SQLITE_MAX_DEFAULT_PAGE_SIZE;
                         ii = ii * 2) {
    ...
    */
    ...
    if (rc == SQLITE_OK) {
        ...
        rc = sqlite3PcacheOpen(szPageDflt, nExtra, !memDb, !memDb ? pagerStress : 0,
                               (void *)pPager, pPager->pPCache);
    }
    ...
    sqlite3PagerSetFlags(pPager,
                         (SQLITE_DEFAULT_SYNCHRONOUS + 1) | PAGER_CACHESPILL);
    /* pPager->pFirst = 0; */
...
int sqlite3PagerSharedLock(Pager *pPager) {
    ...
    ** exclusive access mode.  */
    ...
    if (pPager->tempFile == 0 && pPager->eState == PAGER_OPEN &&
        rc == SQLITE_OK) {
...
static int getPageMMap(Pager *pPager,   /* The pager open on the database file */
                       Pgno pgno,       /* Page number to fetch */
                       DbPage **ppPage, /* Write a pointer to the page here */
                       int flags        /* PAGER_GET_XXX flags */
) {
    ...
    ** temporary or in-memory database.  */
    const int bMmapOk = (pgno > 1 && (pPager->eState == PAGER_READER ||
    ...
    ** common case where pgno is large. */
    ...
    if (bMmapOk && iFrame == 0) {
        void *pData = 0;
        rc = sqlite3OsFetch(pPager->fd, (i64)(pgno - 1) * pPager->pageSize,
...
static int getPageError(Pager *pPager,   /* The pager open on the database file */
                        Pgno pgno,       /* Page number to fetch */
                        DbPage **ppPage, /* Write a pointer to the page here */
                        int flags        /* PAGER_GET_XXX flags */
) {
 ...
 */
int sqlite3PagerGet(Pager *pPager,   /* The pager open on the database file */
                    Pgno pgno,       /* Page number to fetch */
                    DbPage **ppPage, /* Write a pointer to the page here */
                    int flags        /* PAGER_GET_XXX flags */
) {
...
static SQLITE_NOINLINE int pagerAddPageToRollbackJournal(PgHdr *pPg) {
    ...
    */
    ...
    PAGERTRACE(("JOURNAL %d page %d needSync=%d hash(%08x)\n", PAGERID(pPager),
                pPg->pgno, ((pPg->flags & PGHDR_NEED_SYNC) ? 1 : 0),
                pager_pagehash(pPg)));
...
int sqlite3PagerCommitPhaseOne(
    ...
    assert(pPager->eState == PAGER_WRITER_LOCKED ||
           pPager->eState == PAGER_WRITER_CACHEMOD ||
           pPager->eState == PAGER_WRITER_DBMOD || pPager->eState == PAGER_ERROR);
    assert(assert_pager_state(pPager));
        ...
        ** backup in progress needs to be restarted.  */
        ...
        if (pagerUseWal(pPager)) {
            ...
            */
            ...
            sqlite3_file *fd = pPager->fd;
            int bBatch =
                zSuper == 0 /* An SQLITE_IOCAP_BATCH_ATOMIC commit */
                && (sqlite3OsDeviceCharacteristics(fd) & SQLITE_IOCAP_BATCH_ATOMIC) &&
                !pPager->noSync && sqlite3JournalIsInMemory(pPager->jfd);
#else
#define bBatch 0
#endif
                ...
                if (rc == SQLITE_OK) {
                    ...
                    if (rc == SQLITE_OK && pPager->dbSize > pPager->dbFileSize) {
                        ...
                        rc = sqlite3OsWrite(pPager->fd, pTmp, szPage,
                                            ((i64)pPager->dbSize * pPager->pageSize) -
                                                szPage);
                    }
                    if (rc == SQLITE_OK) {
...
int sqlite3PagerMemUsed(Pager *pPager) {
    ...
    return perPageSize * sqlite3PcachePagecount(pPager->pPCache) +
           sqlite3MallocSize(pPager) + pPager->pageSize;
}
...
static SQLITE_NOINLINE int pagerOpenSavepoint(Pager *pPager, int nSavepoint) {
    ...
    */
    aNew = (PagerSavepoint *)sqlite3Realloc(pPager->aSavepoint,
    ...
    if (rc == SQLITE_OK && iSavepoint < pPager->nSavepoint) {
        ...
        */
            ...
            PagerSavepoint *pSavepoint =
                (nNew == 0) ? 0 : &pPager->aSavepoint[nNew - 1];
...
int sqlite3PagerMovepage(Pager *pPager, DbPage *pPg, Pgno pgno, int isCommit) {
    ...
    */
    ...
    PAGERTRACE(("MOVE %d page %d (needSync=%d) moves to %d\n", PAGERID(pPager),
                pPg->pgno, (pPg->flags & PGHDR_NEED_SYNC) ? 1 : 0, pgno));
...
int sqlite3PagerCheckpoint(
    ...
    if (pPager->pWal) {
        rc = sqlite3WalCheckpoint(
            ...
            pPager->pBusyHandlerArg, pPager->walSyncFlags, pPager->pageSize,
            (u8 *)pPager->pTmpSpace, pnLog, pnCkpt);
    }
    return rc;
}
...
int sqlite3PagerWalSupported(Pager *pPager) {
    ...
    return pPager->exclusiveMode ||
           (pMethods->iVersion >= 2 && pMethods->xShmMap);
}
...
*/
int sqlite3PagerOpenWal(Pager *pPager, /* Pager object */
