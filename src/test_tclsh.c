...
/* Forward declaration */
static int SQLITE_TCLAPI load_testfixture_extensions(ClientData cd,
                                                     Tcl_Interp *interp,
                                                     int objc,
                                                     Tcl_Obj *CONST objv[]);
...
const char *sqlite3TestInit(Tcl_Interp *interp) {
    ...
    ** be sure to generate core files if we crash */
    ...
    Tcl_CreateObjCommand(interp, "load_testfixture_extensions",
                         load_testfixture_extensions, 0, 0);
    return 0;
}
