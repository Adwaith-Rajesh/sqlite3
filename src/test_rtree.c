...
*/
static void circle_del(void *p) { sqlite3_free(p); }
...
static int circle_geom(sqlite3_rtree_geometry *p, int nCoord,
    ...
    ** and return SQLITE_OK. */
    for (i = 0; i < 2; i++) {
        if (xmin <= pCircle->aBox[i].xmin && xmax >= pCircle->aBox[i].xmax &&
            ymin <= pCircle->aBox[i].ymin && ymax >= pCircle->aBox[i].ymax) {
            *pRes = 1;
            return SQLITE_OK;
        }
    }
...
static void cube_context_free(void *p) { sqlite3_free(p); }
...
*/
static int cube_geom(sqlite3_rtree_geometry *p, int nCoord,
                     sqlite3_rtree_dbl *aCoord, int *piRes) {
    Cube *pCube = (Cube *)p->pUser;

    assert(p->pContext == (void *)&gHere);

    if (pCube == 0) {
        if (p->nParam != 6 || nCoord != 6 || p->aParam[3] <= 0.0 ||
            p->aParam[4] <= 0.0 || p->aParam[5] <= 0.0) {
            return SQLITE_ERROR;
        }
    ...
    assert(nCoord == 6);
    *piRes = 0;
    if (aCoord[0] <= (pCube->x + pCube->width) && aCoord[1] >= pCube->x &&
        aCoord[2] <= (pCube->y + pCube->height) && aCoord[3] >= pCube->y &&
        aCoord[4] <= (pCube->z + pCube->depth) && aCoord[5] >= pCube->z) {
        *piRes = 1;
    }
...
static int SQLITE_TCLAPI register_cube_geom(void *clientData,
...
static int SQLITE_TCLAPI register_circle_geom(void *clientData,
                                              Tcl_Interp *interp, int objc,
                                              Tcl_Obj *CONST objv[]) {
#ifndef SQLITE_ENABLE_RTREE
    UNUSED_PARAMETER(clientData);
    ...
    if (rc == SQLITE_OK) {
        rc = sqlite3_rtree_query_callback(db, "Qcircle", circle_query_func, 0, 0);
    }
    if (rc == SQLITE_OK) {
        rc = sqlite3_rtree_query_callback(db, "breadthfirstsearch", bfs_query_func,
                                          0, 0);
    }
    Tcl_SetResult(interp, (char *)sqlite3ErrName(rc), TCL_STATIC);
#endif
...
int Sqlitetestrtree_Init(Tcl_Interp *interp) {
    ...
    Tcl_CreateObjCommand(interp, "register_circle_geom", register_circle_geom, 0,
                         0);
    return TCL_OK;
}
