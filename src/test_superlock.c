...
*/
static int
superlockShmLock(sqlite3_file *fd,    /* Database file handle */
...
*/
static int
superlockWalLock(sqlite3 *db,         /* Database handle open on WAL database */
                 SuperlockBusy *pBusy /* Busy handler wrapper object */
) {
    int rc;               /* Return code */
...
int sqlite3demo_superlock(
    ...
    rc = sqlite3_open_v2(zPath, &pLock->db,
...
static int SQLITE_TCLAPI superunlock_cmd(ClientData cd, Tcl_Interp *interp,
...
*/
static int SQLITE_TCLAPI superlock_cmd(ClientData cd, Tcl_Interp *interp,
                                       int objc, Tcl_Obj *CONST objv[]) {
    ...
    Tcl_CreateObjCommand(interp, Tcl_GetString(objv[1]), superunlock_cmd, pLock,
                         superunlock_del);
