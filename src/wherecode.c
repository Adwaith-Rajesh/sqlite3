...
*/
static Expr *
removeUnindexableInClauseTerms(Parse *pParse,    /* The parsing context */
            ...
            for (i = iEq; i < pLoop->nLTerm; i++) {
                if (pLoop->aLTerm[i]->pExpr == pX) {
                    ...
                    if (pOrigLhs) {
                        ...
                        pLhs =
                            sqlite3ExprListAppend(pParse, pLhs, pOrigLhs->a[iField].pExpr);
...
static int codeEqualityTerm(
    ...
    if (pX->op == TK_EQ || pX->op == TK_IS) {
        ...
        if ((pLoop->wsFlags & WHERE_VIRTUALTABLE) == 0 &&
            pLoop->u.btree.pIndex != 0 && pLoop->u.btree.pIndex->aSortOrder[iEq]) {
            testcase(iEq == 0);
            testcase(bRev);
            bRev = !bRev;
        }
        ...
        if (!ExprUseXSelect(pX) || pX->x.pSelect->pEList->nExpr == 1) {
            ...
            if (pExpr->iTable == 0 || !ExprHasProperty(pExpr, EP_Subrtn)) {
                ...
                if (!db->mallocFailed) {
                    ...
                    eType =
                        sqlite3FindInIndex(pParse, pX, IN_INDEX_LOOP, 0, aiMap, &iTab);
                    pExpr->iTable = iTab;
                }
                ...
                aiMap =
                    (int *)sqlite3DbMallocZero(pParse->db, sizeof(int) * MAX(nEq, n));
        ...
        pLevel->u.in.nIn += nEq;
        pLevel->u.in.aInLoop =
            sqlite3WhereRealloc(pTerm->pWC->pWInfo, pLevel->u.in.aInLoop,
        ...
        if (pIn) {
            ...
            testcase(iEq > 0 && (pLoop->wsFlags & WHERE_IN_SEEKSCAN) == 0 &&
...
static int codeCursorHintCheckExpr(Walker *pWalker, Expr *pExpr) {
    ...
    assert(pHint->pIdx != 0);
    if (pExpr->op == TK_COLUMN && pExpr->iTable == pHint->iTabCur &&
...
static int codeCursorHintIsOrFunction(Walker *pWalker, Expr *pExpr) {
    if (pExpr->op == TK_IS || pExpr->op == TK_ISNULL || pExpr->op == TK_ISNOT ||
...
** the table cursor.
*/
static void codeDeferredSeek(WhereInfo *pWInfo, /* Where clause context */
    ...
    if ((pWInfo->wctrlFlags & (WHERE_OR_SUBCLAUSE | WHERE_RIGHT_JOIN)) &&
        ...
        u32 *ai =
            (u32 *)sqlite3DbMallocZero(pParse->db, sizeof(u32) * (pTab->nCol + 1));
        if (ai) {
            ai[0] = pTab->nCol;
...
** coded.
*/
static void whereApplyPartialIndexConstraints(Expr *pTruth, int iTabCur,
                                              WhereClause *pWC) {
    int i;
    WhereTerm *pTerm;
    ...
    while (++iLevel < pWInfo->nLevel) {
        ...
        **  vvvvv--'    pLevel->regFilter if this were true. */
        ...
        if (pLoop->wsFlags & WHERE_IPK) {
            ...
            sqlite3VdbeAddOp4Int(pParse->pVdbe, OP_Filter, pLevel->regFilter, addrNxt,
                                 regRowid, 1);
            VdbeCoverage(pParse->pVdbe);
        } else {
            u16 nEq = pLoop->u.btree.nEq;
...
Bitmask sqlite3WhereCodeOneLoopStart(
    ...
    VdbeModuleComment(
        (v, "Begin WHERE-loop%d: %s", iLevel, pTabItem->pTab->zName));
        ...
        ** the u.vtab.idxStr.  NULL it out to prevent a use-after-free */
        ...
        for (j = 0; j < nConstraint; j++) {
            ...
            if ((pTerm->eOperator & WO_IN) != 0 &&
                (SMASKBIT32(j) & pLoop->u.vtab.mHandleIn) == 0 && !db->mallocFailed) {
                ...
                */
                ...
                if (!db->mallocFailed) {
                    ...
                    if (pRight) {
                        ...
                        sqlite3ExprIfFalse(pParse, pCompare, pLevel->addrCont,
                                           SQLITE_JUMPIFNULL);
                    }
                    pCompare->pLeft = 0;
                }
        ...
        if ((pLoop->wsFlags & WHERE_IPK) != 0 &&
            ...
            */
            ...
            if (pStart) {
                ...
                */
                const u8 aMoveOp[] = {/* TK_GT */ OP_SeekGT,
                                      /* TK_LE */ OP_SeekLE,
                                      /* TK_LT */ OP_SeekLT,
                                      /* TK_GE */ OP_SeekGE};
            ...
            */
            ...
            regBase =
                codeAllEqualityTerms(pParse, pLevel, bRev, nExtraReg, &zStartAff);
            ...
            if (pLoop->nSkip > 0 && nConstraint == pLoop->nSkip) {
                ...
                ** so no further seeking is needed */
                ...
                if (regBignull) {
                    ...
                    VdbeCoverageIf(v, op == OP_SeekLE);
                    testcase(op == OP_SeekLE);
                    assert(op == OP_Rewind || op == OP_Last || op == OP_SeekGE ||
                           op == OP_SeekLE);
                }
            }
            ...
            */
            ...
            omitTable =
                (pLoop->wsFlags & WHERE_IDX_ONLY) != 0 &&
            ...
            if (omitTable) {
                ...
                sqlite3VdbeAddOp4Int(v, OP_NotFound, iCur, addrCont, iRowidReg,
                                     pPk->nKeyCol);
                VdbeCoverage(v);
            }
            ...
            */
            if (pWInfo->nLevel > 1) {
                ...
                pOrTab = sqlite3DbMallocRawNN(db, sizeof(*pOrTab) +
            ...
            */
            if (pWC->nTerm > 1) {
                ...
                for (iTerm = 0; iTerm < pWC->nTerm; iTerm++) {
                    ...
                    if ((pWC->a[iTerm].wtFlags &
                         (TERM_VIRTUAL | TERM_CODED | TERM_SLICE)) != 0) {
                        continue;
                    }
                ...
                if (pOrTerm->leftCursor == iCur || (pOrTerm->eOperator & WO_AND) != 0) {
                    ...
                    testcase((pTabItem[0].fg.jointype & JT_LEFT) != 0 &&
                             !ExprHasProperty(pOrExpr,
                                              EP_OuterON)); /* See TH3 vtab25.400 and
                    ...
                    if (pSubWInfo) {
                        WhereLoop *pSubLoop;
                        int addrExplain =
                            sqlite3WhereExplainOneScan(pParse, pOrTab, &pSubWInfo->a[0], 0);
                        ...
                        */
                        if ((pWInfo->wctrlFlags & WHERE_DUPLICATES_OK) == 0) {
                            ...
                            if (HasRowid(pTab)) {
                                ...
                                ** insert the key into the temp table and proceed with
                                *processing
                                ** the row.
                                **
                                ...
                                ** be tested for.  */
                                if (iSet) {
                                    jmp1 =
                                        sqlite3VdbeAddOp4Int(v, OP_Found, regRowset, 0, r, nPk);
                                    VdbeCoverage(v);
                                }
                                if (iSet >= 0) {
                                    sqlite3VdbeAddOp3(v, OP_MakeRecord, r, nPk, regRowid);
                                    sqlite3VdbeAddOp4Int(v, OP_IdxInsert, regRowset, regRowid, r,
                                                         nPk);
                                    if (iSet)
        ...
        for (pTerm = pWC->a, j = pWC->nTerm; j > 0; j--, pTerm++) {
            ...
            if ((pTerm->wtFlags & TERM_LIKECOND) != 0) {
                ...
                ** that compares BLOBs. */
                ...
                if (x > 0) {
                    skipLikeAddr =
                        sqlite3VdbeAddOp1(v, (x & 1) ? OP_IfNot : OP_If, (int)(x >> 1));
            ...
            if (sqlite3WhereTrace) {
                VdbeNoopComment((v, "WhereTerm[%d] (%p) priority=%d", pWC->nTerm - j,
                                 pTerm, iLoop));
            }
    ...
    */
    for (pTerm = pWC->a, j = pWC->nBase; j > 0; j--, pTerm++) {
        ...
        if (pAlt->wtFlags & (TERM_CODED))
            continue;
        if ((pAlt->eOperator & WO_IN) && ExprUseXSelect(pAlt->pExpr) &&
...
** for a RIGHT JOIN.
*/
SQLITE_NOINLINE void sqlite3WhereRightJoinLoop(WhereInfo *pWInfo, int iLevel,
                                               WhereLevel *pLevel) {
    Parse *pParse = pWInfo->pParse;
    Vdbe *v = pParse->pVdbe;
    WhereRightJoin *pRJ = pLevel->pRJ;
