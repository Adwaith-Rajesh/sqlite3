...
*/
static int
vdbeSorterCompareInt(SortSubtask *pTask,           /* Subtask context (for pKeyInfo) */
                     int *pbKey2Cached,            /* True if pTask->pUnpacked is pKey2 */
                     const void *pKey1, int nKey1, /* Left side of comparison */
                     const void *pKey2, int nKey2  /* Right side of comparison */
) {
...
static void vdbeSorterRewindDebug(const char *zEvent) {
    ...
    if (ALWAYS(pVfs))
        sqlite3OsCurrentTimeInt64(pVfs, &t);
    fprintf(stderr, "%lld:X %s\n", t, zEvent);
}
static void vdbeSorterPopulateDebug(SortSubtask *pTask, const char *zEvent) {
    i64 t;
    int iTask = (pTask - pTask->pSorter->aTask);
    sqlite3OsCurrentTimeInt64(pTask->pSorter->db->pVfs, &t);
    fprintf(stderr, "%lld:bg%d %s\n", t, iTask, zEvent);
}
static void vdbeSorterBlockDebug(SortSubtask *pTask, int bBlocked,
                                 const char *zEvent) {
    if (bBlocked) {
        i64 t;
        sqlite3OsCurrentTimeInt64(pTask->pSorter->db->pVfs, &t);
...
vdbeSorterOpenTempFile(sqlite3 *db, /* Database handle doing sort */
    ...
    rc = sqlite3OsOpenMalloc(db->pVfs, 0, ppFd,
                             SQLITE_OPEN_TEMP_JOURNAL | SQLITE_OPEN_READWRITE |
...
*/
static void vdbePmaWriterInit(sqlite3_file *pFd, /* File handle to write to */
...
static void vdbePmaWriteBlob(PmaWriter *p, u8 *pData, int nData) {
    ...
    while (nRem > 0 && p->eFWErr == 0) {
        ...
        if (p->iBufEnd == p->nBuffer) {
            p->eFWErr = sqlite3OsWrite(p->pFd, &p->aBuffer[p->iBufStart],
...
static int vdbePmaWriterFinish(PmaWriter *p, i64 *piEof) {
    ...
    if (p->eFWErr == 0 && ALWAYS(p->aBuffer) && p->iBufEnd > p->iBufStart) {
        p->eFWErr =
            sqlite3OsWrite(p->pFd, &p->aBuffer[p->iBufStart],
                           p->iBufEnd - p->iBufStart, p->iWriteOff + p->iBufStart);
    }
        ...
        ** advanced (iPrev) and the one next to it in the array.  */
        ...
        for (i = (pMerger->nTree + iPrev) / 2; i > 0; i = i / 2) {
            ...
            if (pReadr1->pFd == 0) {
                ...
                iRes = pTask->xCompare(pTask, &bCached, pReadr1->aKey, pReadr1->nKey,
                                       pReadr2->aKey, pReadr2->nKey);
            }
...
*/
int sqlite3VdbeSorterWrite(const VdbeCursor *pCsr, /* Sorter cursor */
    ...
    */
    ...
    if (pSorter->mxPmaSize) {
        if (pSorter->list.aMemory) {
            bFlush =
                pSorter->iMemory && (pSorter->iMemory + nReq) > pSorter->mxPmaSize;
        } else {
    ...
    if (pSorter->list.aMemory) {
        ...
        if (pSorter->list.pList) {
            pNew->u.iNext =
                (int)((u8 *)(pSorter->list.pList) - pSorter->list.aMemory);
        }
    } else {
...
static int vdbeIncrMergerNew(
    ...
    IncrMerger *pIncr = *ppOut =
        (IncrMerger *)(sqlite3FaultSim(100) ? 0
                                            : sqlite3MallocZero(sizeof(*pIncr)));
    if (pIncr) {
        pIncr->pMerger = pMerger;
        pIncr->pTask = pTask;
        pIncr->mxSz =
            MAX(pTask->pSorter->mxKeysize + 9, pTask->pSorter->mxPmaSize / 2);
...
static void vdbeMergeEngineCompare(
    ...
    if (p1->pFd == 0) {
        ...
        res = pTask->xCompare(pTask, &bCached, p1->aKey, p1->nKey, p2->aKey,
                              p2->nKey);
        if (res <= 0) {
            iRes = i1;
        } else {
