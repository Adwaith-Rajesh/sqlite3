...
*/
static int fsdirFilter(sqlite3_vtab_cursor *pVtabCursor, int idxNum,
...
**   argv[...] -> other module argument fields.
*/
static int fstreeConnect(sqlite3 *db, void *pAux, int argc,
                         const char *const *argv, sqlite3_vtab **ppVtab,
                         char **pzErr) {
    FstreeVtab *pTab;
...
static int fstreeNext(sqlite3_vtab_cursor *cur) {
    ...
    if (rc != SQLITE_ROW) {
        ...
        rc = SQLITE_OK;
        pCsr->fd =
            open((const char *)sqlite3_column_text(pCsr->pStmt, 0), O_RDONLY);
    }
...
** xFilter method implementation.
*/
static int fstreeFilter(sqlite3_vtab_cursor *pVtabCursor, int idxNum,
                        const char *idxStr, int argc, sqlite3_value **argv) {
    ...
    if (idxNum) {
        ...
        if (sqlite3_strnicmp(zQuery, zPrefix, nPrefix) == 0) {
            ...
            for (i = nPrefix; zQuery[i]; i++) {
                if (zQuery[i] == aWild[0] || zQuery[i] == aWild[1])
                    break;
                if (zQuery[i] == '/')
                    nDir = i;
            }
...
**   argv[...] -> other module argument fields.
*/
static int fsConnect(sqlite3 *db, void *pAux, int argc, const char *const *argv,
...
static int fsFilter(sqlite3_vtab_cursor *pVtabCursor, int idxNum,
                    const char *idxStr, int argc, sqlite3_value **argv) {
    int rc;
    ...
    if (idxNum == 1) {
        char *zStmt =
            sqlite3_mprintf("SELECT * FROM %Q.%Q WHERE rowid=?", p->zDb, p->zTbl);
        if (!zStmt)
            return SQLITE_NOMEM;
...
static sqlite3_module fsModule = {
    0, /* iVersion */
    fsConnect, fsConnect, fsBestIndex, fsDisconnect, fsDisconnect,
