...
void sqlite3VdbeAddDblquoteStr(sqlite3 *db, Vdbe *p, const char *z) {
    if (p) {
        ...
        DblquoteStr *pStr =
            sqlite3DbMallocRawNN(db, sizeof(*pStr) + n + 1 - sizeof(pStr->z));
        if (pStr) {
...
addOp4IntSlow(Vdbe *p, /* Add the opcode to this VM */
...
int sqlite3VdbeAddOp4Int(Vdbe *p, /* Add the opcode to this VM */
                         int op,  /* The new opcode */
                         int p1,  /* The P1 operand */
                         int p2,  /* The P2 operand */
                         int p3,  /* The P3 operand */
...
void sqlite3VdbeMultiLoad(Vdbe *p, int iDest, const char *zTypes, ...) {
    ...
    for (i = 0; (c = zTypes[i]) != 0; i++) {
        if (c == 's') {
            ...
            sqlite3VdbeAddOp4(p, z == 0 ? OP_Null : OP_String8, 0, iDest + i, 0, z,
                              0);
        } else if (c == 'i') {
...
int sqlite3VdbeAddFunctionCall(
    ...
    addr = sqlite3VdbeAddOp4(v, eCallCtx ? OP_PureFunc : OP_Function, p1, p2, p3,
                             (char *)pCtx, P4_FUNCCTX);
...
** P4_REAL type.
*/
int sqlite3VdbeAddOp4Dup8(Vdbe *p,       /* Add the opcode to this VM */
                          int op,        /* The new opcode */
                          int p1,        /* The P1 operand */
                          int p2,        /* The P2 operand */
                          int p3,        /* The P3 operand */
...
int sqlite3VdbeExplain(Parse *pParse, u8 bPush, const char *zFmt, ...) {
    ...
    ** But omit them (for performance) during production builds */
...
#endif
    {
        ...
        v = pParse->pVdbe;
        iThis = v->nOp;
        addr = sqlite3VdbeAddOp4(v, OP_Explain, iThis, pParse->addrExplain, 0, zMsg,
                                 P4_DYNAMIC);
        sqlite3ExplainBreakpoint(bPush ? "PUSH" : "",
                                 sqlite3VdbeGetLastOp(v)->p4.z);
        if (bPush) {
...
static SQLITE_NOINLINE void resizeResolveLabel(Parse *p, Vdbe *v, int j) {
    ...
    p->aLabel =
        sqlite3DbReallocOrFree(p->db, p->aLabel, nNewSize * sizeof(p->aLabel[0]));
    if (p->aLabel == 0) {
        p->nLabelAlloc = 0;
    } else {
#ifdef SQLITE_DEBUG
        int i;
        for (i = p->nLabelAlloc; i < nNewSize; i++)
            p->aLabel[i] = -1;
#endif
...
*/
void sqlite3VdbeRunOnlyOnce(Vdbe *p) { sqlite3VdbeAddOp2(p, OP_Expire, 1, 1); }
...
int sqlite3VdbeAssertMayAbort(Vdbe *v, int mayAbort) {
    ...
    while ((pOp = opIterNext(&sIter)) != 0) {
        ...
        if (opcode == OP_Destroy || opcode == OP_VUpdate || opcode == OP_VRename ||
            opcode == OP_VDestroy || opcode == OP_VCreate ||
            opcode == OP_ParseSchema || opcode == OP_Function ||
            opcode == OP_PureFunc ||
        ...
        if (mayAbort) {
            ...
            ** require one. This is not so bad - it is an inefficiency, not a bug. */
            if (opcode == OP_CreateBtree && pOp->p3 == BTREE_BLOBKEY)
                hasCreateIndex = 1;
            if (opcode == OP_Clear)
                hasCreateIndex = 1;
        }
...
void sqlite3VdbeIncrWriteCounter(Vdbe *p, VdbeCursor *pC) {
    if (pC == 0 || (pC->eCurType != CURTYPE_SORTER &&
                    pC->eCurType != CURTYPE_PSEUDO && !pC->isEphemeral)) {
        p->nWrite++;
    }
}
#endif
        ...
        if ((sqlite3OpcodeProperty[pOp->opcode] & OPFLG_JUMP) != 0) {
            ...
            if (iDest < iFirst || iDest > iLast) {
                ...
                for (; j < v->nOp; j++) {
                    ...
                    if (pX->opcode == OP_Noop)
                        continue;
                    if (pX->opcode == OP_Explain)
                        continue;
                    if (pErr == 0) {
...
void sqlite3VdbeVerifyAbortable(Vdbe *p, int onError) {
    if (onError == OE_Abort)
        sqlite3VdbeAddOp0(p, OP_Abortable);
}
#endif
...
*/
void sqlite3VdbeScanStatusRange(Vdbe *p, int addrExplain, int addrStart,
                                int addrEnd) {
    if (IS_STMT_SCANSTATUS(p->db)) {
        ScanStatus *pScan = 0;
        int ii;
        for (ii = p->nScan - 1; ii >= 0; ii--) {
            pScan = &p->aScan[ii];
            if (pScan->addrExplain == addrExplain)
                break;
            pScan = 0;
        }
        if (pScan) {
            if (addrEnd < 0)
                addrEnd = sqlite3VdbeCurrentAddr(p) - 1;
...
** addrExplain.
*/
void sqlite3VdbeScanStatusCounters(Vdbe *p, int addrExplain, int addrLoop,
                                   int addrVisit) {
    if (IS_STMT_SCANSTATUS(p->db)) {
        ScanStatus *pScan = 0;
        int ii;
...
void sqlite3VdbeJumpHereOrPopInst(Vdbe *p, int addr) {
    if (addr == p->nOp - 1) {
        assert(p->aOp[addr].opcode == OP_Once || p->aOp[addr].opcode == OP_If ||
...
void sqlite3VdbeReleaseRegisters(
    ...
    if (N == 0 || OptimizationDisabled(pParse->db, SQLITE_ReleaseReg))
        return;
    assert(pParse->pVdbe);
    assert(iFirst >= 1);
    ...
    if (N > 0) {
        ...
        if (bUndefine)
            sqlite3VdbeChangeP5(pParse->pVdbe, 1);
    }
}
#endif /* SQLITE_DEBUG */
...
*/
static void SQLITE_NOINLINE vdbeChangeP4Full(Vdbe *p, Op *pOp, const char *zP4,
                                             int n) {
    if (pOp->p4type) {
        freeP4(p->db, pOp->p4type, pOp->p4.p);
        pOp->p4type = 0;
...
*/
char *
sqlite3VdbeDisplayComment(sqlite3 *db,    /* Optional - Oom error reporting only */
        ...
        case P4_KEYINFO: {
            ...
            for (j = 0; j < pKeyInfo->nKeyField; j++) {
                ...
                sqlite3_str_appendf(
                    ...
                    (pKeyInfo->aSortFlags[j] & KEYINFO_ORDER_BIGNULL) ? "N." : "", zColl);
            }
...
void sqlite3VdbePrintOp(FILE *pOut, int pc, VdbeOp *pOp) {
    ...
    ** information from the vdbe.c source text */
    fprintf(pOut, zFormat1, pc, sqlite3OpcodeName(pOp->opcode), pOp->p1, pOp->p2,
            pOp->p3, zP4 ? zP4 : "", pOp->p5, zCom ? zCom : "");
    fflush(pOut);
    sqlite3_free(zP4);
    sqlite3_free(zCom);
        ...
        do {
            ...
            */
            ...
            if (p->flags & (MEM_Agg | MEM_Dyn)) {
                testcase((p->flags & MEM_Dyn) != 0 &&
                         p->xDel == sqlite3VdbeFrameMemDel);
...
int sqlite3VdbeFrameIsValid(VdbeFrame *pFrame) {
    if (pFrame->iFrameMagic != SQLITE_FRAME_MAGIC)
        return 0;
    return 1;
}
#endif
        ...
        */
        ...
        if (eMode == 2) {
            ...
            if (pOp->opcode == OP_OpenRead)
                break;
            if (pOp->opcode == OP_OpenWrite && (pOp->p5 & OPFLAG_P2ISREG) == 0)
                break;
...
void sqlite3VdbeRewind(Vdbe *p) {
    ...
    assert(p != 0);
    assert(p->eVdbeState == VDBE_INIT_STATE ||
           p->eVdbeState == VDBE_READY_STATE || p->eVdbeState == VDBE_HALT_STATE);
    ...
    for (i = 0; rc == SQLITE_OK && i < db->nDb; i++) {
        ...
        if (sqlite3BtreeTxnState(pBt) == SQLITE_TXN_WRITE) {
            ...
            ** journal modes use a super-journal and which do not */
            static const u8 aMJNeeded[] = {/* DELETE   */ 1,
                                           /* PERSIST   */ 1,
                                           /* OFF       */ 0,
                                           /* TRUNCATE  */ 1,
                                           /* MEMORY    */ 0,
        ...
        */
        for (i = 0; i < db->nDb; i++) {
            ...
            if (sqlite3BtreeTxnState(pBt) == SQLITE_TXN_WRITE) {
                ...
                assert(zFile[0] != 0);
                rc = sqlite3OsWrite(pSuperJrnl, zFile, sqlite3Strlen30(zFile) + 1,
                                    offset);
...
int sqlite3VdbeHalt(Vdbe *p) {
    ...
    ** SQL statement does not read or write a database file.  */
    if (p->bIsReader) {
        ...
        ** above has occurred.
        */
        if (!sqlite3VtabInSync(db) && db->autoCommit &&
...
static void vdbeInvokeSqllog(Vdbe *v) {
    if (sqlite3GlobalConfig.xSqllog && v->rc == SQLITE_OK && v->zSql &&
        ...
        if (zExpanded) {
            sqlite3GlobalConfig.xSqllog(sqlite3GlobalConfig.pSqllogArg, v->db,
                                        zExpanded, 1);
...
#ifdef VDBE_PROFILE
    {
        ...
        if (out) {
            ...
            for (i = 0; i < p->nOp; i++) {
                ...
                sqlite3_snprintf(sizeof(zHdr), zHdr, "%6u %12llu %8llu ", cnt, cycles,
                                 cnt > 0 ? cycles / cnt : 0);
                fprintf(out, "%s", zHdr);
...
void sqlite3VdbeDeleteAuxData(sqlite3 *db, AuxData **pp, int iOp, int mask) {
    while (*pp) {
        AuxData *pAux = *pp;
        if ((iOp < 0) ||
            (pAux->iAuxOp == iOp && pAux->iAuxArg >= 0 &&
...
** is avoided.
*/
static void serialGet(const unsigned char *buf, /* Buffer to deserialize from */
...
static int serialGet7(const unsigned char *buf, /* Buffer to deserialize from */
                      Mem *pMem                 /* Memory cell to write value into */
) {
    u64 x = FOUR_BYTE_UINT(buf);
    u32 y = FOUR_BYTE_UINT(buf + 4);
...
*/
UnpackedRecord *
sqlite3VdbeAllocUnpackedRecord(KeyInfo *pKeyInfo /* Description of the record */
) {
    UnpackedRecord *p; /* Unpacked record to return */
    int nByte;         /* Number of bytes required for *p */
    nByte =
        ROUND8P(sizeof(UnpackedRecord)) + sizeof(Mem) * (pKeyInfo->nKeyField + 1);
...
*/
static int vdbeRecordCompareDebug(int nKey1, const void *pKey1, /* Left key */
    ...
    do {
        ...
        */
        if (d1 + (u64)serial_type1 + 2 > (u64)nKey1 &&
            ...
            if (serial_type1 >= 1 && serial_type1 <= 7 &&
...
*/
static int
vdbeCompareMemString(const Mem *pMem1, const Mem *pMem2, const CollSeq *pColl,
...
*/
int sqlite3MemCompare(const Mem *pMem1, const Mem *pMem2,
                      const CollSeq *pColl) {
    int f1, f2;
    int combined_flags;
...
int sqlite3VdbeRecordCompareWithSkip(
    ...
    ** that this routine begins comparing at the second field. */
    ...
    VVA_ONLY(mem1.szMalloc = 0;) /* Only needed by assert() statements */
    assert(pPKey2->pKeyInfo->nAllField >= pPKey2->nField || CORRUPT_DB);
    ...
    while (1 /*exit-by-break*/) {
        ...
        /* RHS is null */
            ...
            serial_type = aKey1[idx1];
            if (serial_type == 0 || serial_type == 10 ||
...
int sqlite3VdbeRecordCompare(int nKey1, const void *pKey1, /* Left key */
                             UnpackedRecord *pPKey2        /* Right key */
) {
...
*/
static int vdbeRecordCompareInt(int nKey1, const void *pKey1, /* Left key */
                                UnpackedRecord *pPKey2        /* Right key */
) {
...
*/
static int vdbeRecordCompareString(int nKey1, const void *pKey1, /* Left key */
                                   UnpackedRecord *pPKey2        /* Right key */
) {
...
vrcs_restart:
    ...
    assert(vdbeRecordCompareDebug(nKey1, pKey1, pPKey2, res) || CORRUPT_DB ||
           pPKey2->pKeyInfo->db->mallocFailed);
    return res;
}
