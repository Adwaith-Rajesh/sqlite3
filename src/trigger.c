...
Trigger *sqlite3TriggerList(Parse *pParse, Table *pTab) {
    ...
    while (p) {
        ...
        if (pTrig->pTabSchema == pTab->pSchema && pTrig->table &&
        ...
        if (!db->init.busy) {
            ...
            ** then raise an error - do not allow the trigger to be created. */
            if (sqlite3ReadOnlyShadowTables(db)) {
                ...
                for (pStep = pTrig->step_list; pStep; pStep = pStep->pNext) {
                    if (pStep->zTarget != 0 &&
                        ...
                        sqlite3ErrorMsg(
                            pParse, "trigger \"%s\" may not write to shadow table \"%s\"",
            ...
            sqlite3VdbeAddParseSchemaOp(
                v, iDb, sqlite3MPrintf(db, "type='trigger' AND name='%q'", zName), 0);
        }
...
*/
TriggerStep *
sqlite3TriggerSelectStep(sqlite3 *db,        /* Database connection */
...
*/
static TriggerStep *
triggerStepAllocate(Parse *pParse,      /* Parser context */
...
TriggerStep *sqlite3TriggerInsertStep(
    ...
    pTriggerStep =
        triggerStepAllocate(pParse, TK_INSERT, pTableName, zStart, zEnd);
    if (pTriggerStep) {
        if (IN_RENAME_OBJECT) {
            pTriggerStep->pSelect = pSelect;
            pSelect = 0;
...
TriggerStep *sqlite3TriggerUpdateStep(
    ...
    pTriggerStep =
        triggerStepAllocate(pParse, TK_UPDATE, pTableName, zStart, zEnd);
    if (pTriggerStep) {
        if (IN_RENAME_OBJECT) {
...
TriggerStep *sqlite3TriggerDeleteStep(
    ...
    pTriggerStep =
        triggerStepAllocate(pParse, TK_DELETE, pTableName, zStart, zEnd);
    if (pTriggerStep) {
        if (IN_RENAME_OBJECT) {
            pTriggerStep->pWhere = pWhere;
            pWhere = 0;
...
static int tempTriggersExist(sqlite3 *db) {
    if (NEVER(db->aDb[1].pSchema == 0))
        return 0;
    if (sqliteHashFirst(&db->aDb[1].pSchema->trigHash) == 0)
        return 0;
    return 1;
}
    ...
    if (pList != 0) {
        ...
        if ((pParse->db->flags & SQLITE_EnableTrigger) == 0 &&
            ...
            ** includes only TEMP triggers */
            ...
            while (ALWAYS(p->pNext) && p->pNext != pTab->pTrigger)
                p = p->pNext;
            p->pNext = 0;
            p = pList;
        }
    ...
    if (pSrc) {
        ...
        if (pStep->pFrom) {
            ...
            if (pDup && pDup->nSrc > 1 && !IN_RENAME_OBJECT) {
                ...
                Token as;
                pSubquery =
                    sqlite3SelectNew(pParse, 0, pDup, 0, 0, 0, 0, SF_NestedFrom, 0);
                as.n = 0;
                as.z = 0;
...
** form "table.*".
*/
static int isAsteriskTerm(Parse *pParse, /* Parsing context */
                          Expr *pTerm    /* A term in the RETURNING clause */
) {
    assert(pTerm != 0);
    if (pTerm->op == TK_ASTERISK)
        return 1;
    if (pTerm->op != TK_DOT)
        return 0;
    assert(pTerm->pRight != 0);
    assert(pTerm->pLeft != 0);
    if (pTerm->pRight->op != TK_ASTERISK)
        return 0;
...
*/
static ExprList *
sqlite3ExpandReturning(Parse *pParse,   /* Parsing context */
    ...
    for (i = 0; i < pList->nExpr; i++) {
        ...
        if (isAsteriskTerm(pParse, pOldExpr)) {
            ...
            for (jj = 0; jj < pTab->nCol; jj++) {
                Expr *pNewExpr;
                if (IsHiddenColumn(pTab->aCol + jj))
                    continue;
...
static void codeReturningTrigger(
    ...
    if (pParse->nErr == 0) {
        ...
        if (sqlite3ResolveExprListNames(&sNC, pNew) == SQLITE_OK &&
            ...
            sqlite3VdbeAddOp3(v, OP_Insert, pReturning->iRetCur, reg + i,
                              reg + i + 1);
        }
    }
        ...
        */
        ...
        if (pStep->zSpan) {
            sqlite3VdbeAddOp4(v, OP_Trace, 0x7fffffff, 1, 0,
                              sqlite3MPrintf(db, "-- %s", pStep->zSpan), P4_DYNAMIC);
        }
#endif

        switch (pStep->op) {
            case TK_UPDATE: {
                sqlite3Update(pParse, sqlite3TriggerStepSrc(pParse, pStep),
                              sqlite3ExprListDup(db, pStep->pExprList, 0),
                              sqlite3ExprDup(db, pStep->pWhere, 0), pParse->eOrconf, 0, 0,
                              0);
                sqlite3VdbeAddOp0(v, OP_ResetCount);
                break;
            }
            case TK_INSERT: {
                sqlite3Insert(pParse, sqlite3TriggerStepSrc(pParse, pStep),
            ...
            case TK_DELETE: {
                sqlite3DeleteFrom(pParse, sqlite3TriggerStepSrc(pParse, pStep),
...
codeRowTrigger(Parse *pParse,     /* Current parse context */
    ...
    ** trigger sub-program.  */
    ...
    if (v) {
        VdbeComment((v, "Start: %s.%s (%s %s%s%s ON %s)", pTrigger->zName,
                     onErrorText(orconf),
                     (pTrigger->tr_tm == TRIGGER_BEFORE ? "BEFORE" : "AFTER"),
                     (pTrigger->op == TK_UPDATE ? "UPDATE" : ""),
                     (pTrigger->op == TK_INSERT ? "INSERT" : ""),
                     (pTrigger->op == TK_DELETE ? "DELETE" : ""), pTab->zName));
#ifndef SQLITE_OMIT_TRACE
        if (pTrigger->zName) {
            sqlite3VdbeChangeP4(v, -1,
                                sqlite3MPrintf(db, "-- TRIGGER %s", pTrigger->zName),
                                P4_DYNAMIC);
        }
#endif
...
void sqlite3CodeRowTriggerDirect(
    ...
    ** is a pointer to the sub-vdbe containing the trigger program.  */
    if (pPrg) {
        int bRecursive =
            (p->zName && 0 == (pParse->db->flags & SQLITE_RecTriggers));
...
void sqlite3CodeRowTrigger(
    ...
    for (p = pTrigger; p; p = p->pNext) {
        ...
        */
        if ((p->op == op ||
             (p->bReturning && p->op == TK_INSERT && op == TK_UPDATE)) &&
