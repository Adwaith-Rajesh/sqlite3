...
struct quotaGroup {
    ...
    void (
        *xCallback)(                        /* Callback invoked when going over quota */
...
*/
static quotaFile *
quotaFindFile(quotaGroup *pGroup, /* Group in which to look for the file */
    ...
    if (pFile == 0 && createFlag) {
        ...
        if (pFile) {
            ...
            if (pGroup->pFiles)
                pGroup->pFiles->ppPrev = &pFile->pNext;
...
*/
static int quotaOpen(sqlite3_vfs *pVfs,   /* The quota VFS */
...
*/
static int quotaDelete(sqlite3_vfs *pVfs, /* The quota VFS */
...
** further processing.
*/
static int quotaRead(sqlite3_file *pConn, void *pBuf, int iAmt,
                     sqlite3_int64 iOfst) {
    sqlite3_file *pSubOpen = quotaSubOpen(pConn);
...
** original VFS.
*/
static int quotaWrite(sqlite3_file *pConn, const void *pBuf, int iAmt,
                      sqlite3_int64 iOfst) {
    quotaConn *p = (quotaConn *)pConn;
    sqlite3_file *pSubOpen = quotaSubOpen(pConn);
 ...
 */
static int quotaShmMap(sqlite3_file *pConn, /* Handle open on database file */
 ...
 */
static int
quotaShmLock(sqlite3_file *pConn, /* Database file holding the shared memory */
...
int sqlite3_quota_shutdown(void) {
    ...
    for (pGroup = gQuota.pGroup; pGroup; pGroup = pGroup->pNext) {
        if (quotaGroupOpenFileCount(pGroup) > 0)
            return SQLITE_MISUSE;
    }
    while (gQuota.pGroup) {
...
int sqlite3_quota_set(
    ...
    if (pGroup == 0) {
        ...
        if (gQuota.pGroup)
            gQuota.pGroup->ppPrev = &pGroup->pNext;
...
*/
size_t sqlite3_quota_fread(void *pBuf,   /* Store the content here */
...
*/
size_t
sqlite3_quota_fwrite(const void *pBuf, /* Take content to write from here */
                     size_t size,      /* Size of each element */
...
*/
void sqlite3_quota_rewind(quota_FILE *p) { rewind(p->f); }
...
*/
int sqlite3_quota_ferror(quota_FILE *p) { return ferror(p->f); }
...
*/
static void
tclQuotaCallback(const char *zFilename,  /* Name of file whose size increases */
                 sqlite3_int64 *piLimit, /* IN/OUT: The current limit */
...
*/
static int SQLITE_TCLAPI test_quota_initialize(void *clientData,
    ...
    if (Tcl_GetBooleanFromObj(interp, objv[2], &makeDefault))
        return TCL_ERROR;
    if (zName[0] == '\0')
        zName = 0;
...
*/
static int SQLITE_TCLAPI test_quota_shutdown(void *clientData,
                                             Tcl_Interp *interp, int objc,
                                             Tcl_Obj *CONST objv[]) {
...
*/
static int SQLITE_TCLAPI test_quota_set(void *clientData, Tcl_Interp *interp,
    ...
    if (Tcl_GetWideIntFromObj(interp, objv[2], &iLimit))
        return TCL_ERROR;
    pScript = objv[3];
...
*/
static int SQLITE_TCLAPI test_quota_file(void *clientData, Tcl_Interp *interp,
                                         int objc, Tcl_Obj *CONST objv[]) {
...
*/
static int SQLITE_TCLAPI test_quota_dump(void *clientData, Tcl_Interp *interp,
                                         int objc, Tcl_Obj *CONST objv[]) {
    Tcl_Obj *pResult;
    Tcl_Obj *pGroupTerm;
    Tcl_Obj *pFileTerm;
    ...
    for (pGroup = gQuota.pGroup; pGroup; pGroup = pGroup->pNext) {
        ...
        for (pFile = pGroup->pFiles; pFile; pFile = pFile->pNext) {
            ...
            Tcl_ListObjAppendElement(interp, pFileTerm, Tcl_NewStringObj(zTemp, -1));
...
*/
static int SQLITE_TCLAPI test_quota_fopen(void *clientData, Tcl_Interp *interp,
                                          int objc, Tcl_Obj *CONST objv[]) {
    const char *zFilename; /* File pattern to configure */
...
*/
static int SQLITE_TCLAPI test_quota_fread(void *clientData, Tcl_Interp *interp,
                                          int objc, Tcl_Obj *CONST objv[]) {
    quota_FILE *p;
    char *zBuf;
    int sz;
    ...
    if (Tcl_GetIntFromObj(interp, objv[2], &sz))
        return TCL_ERROR;
    if (Tcl_GetIntFromObj(interp, objv[3], &nElem))
        return TCL_ERROR;
...
*/
static int SQLITE_TCLAPI test_quota_fwrite(void *clientData, Tcl_Interp *interp,
                                           int objc, Tcl_Obj *CONST objv[]) {
    quota_FILE *p;
    char *zBuf;
    int sz;
...
*/
static int SQLITE_TCLAPI test_quota_fclose(void *clientData, Tcl_Interp *interp,
                                           int objc, Tcl_Obj *CONST objv[]) {
    quota_FILE *p;
    int rc;
...
*/
static int SQLITE_TCLAPI test_quota_fflush(void *clientData, Tcl_Interp *interp,
                                           int objc, Tcl_Obj *CONST objv[]) {
    quota_FILE *p;
    int rc;
    int doSync = 0;
    ...
    p = sqlite3TestTextToPtr(Tcl_GetString(objv[1]));
    if (objc == 3) {
        if (Tcl_GetBooleanFromObj(interp, objv[2], &doSync))
            return TCL_ERROR;
    }
...
*/
static int SQLITE_TCLAPI test_quota_fseek(void *clientData, Tcl_Interp *interp,
                                          int objc, Tcl_Obj *CONST objv[]) {
    quota_FILE *p;
    int ofst;
    const char *zWhence;
    ...
    p = sqlite3TestTextToPtr(Tcl_GetString(objv[1]));
    if (Tcl_GetIntFromObj(interp, objv[2], &ofst))
        return TCL_ERROR;
    ...
    if (strcmp(zWhence, "SEEK_SET") == 0) {
        ...
        Tcl_AppendResult(interp, "WHENCE should be SEEK_SET, SEEK_CUR, or SEEK_END",
                         (char *)0);
        return TCL_ERROR;
    }
...
*/
static int SQLITE_TCLAPI test_quota_rewind(void *clientData, Tcl_Interp *interp,
                                           int objc, Tcl_Obj *CONST objv[]) {
    quota_FILE *p;
    if (objc != 2) {
        Tcl_WrongNumArgs(interp, 1, objv, "HANDLE");
...
*/
static int SQLITE_TCLAPI test_quota_ftell(void *clientData, Tcl_Interp *interp,
                                          int objc, Tcl_Obj *CONST objv[]) {
    quota_FILE *p;
    sqlite3_int64 x;
    if (objc != 2) {
...
*/
static int SQLITE_TCLAPI test_quota_ftruncate(void *clientData,
                                              Tcl_Interp *interp, int objc,
                                              Tcl_Obj *CONST objv[]) {
    quota_FILE *p;
    sqlite3_int64 x;
    ...
    p = sqlite3TestTextToPtr(Tcl_GetString(objv[1]));
    if (Tcl_GetWideIntFromObj(interp, objv[2], &w))
        return TCL_ERROR;
    x = (sqlite3_int64)w;
...
*/
static int SQLITE_TCLAPI test_quota_file_size(void *clientData,
                                              Tcl_Interp *interp, int objc,
                                              Tcl_Obj *CONST objv[]) {
    quota_FILE *p;
    sqlite3_int64 x;
...
*/
static int SQLITE_TCLAPI test_quota_file_truesize(void *clientData,
                                                  Tcl_Interp *interp, int objc,
                                                  Tcl_Obj *CONST objv[]) {
    quota_FILE *p;
    sqlite3_int64 x;
...
*/
static int SQLITE_TCLAPI test_quota_file_mtime(void *clientData,
                                               Tcl_Interp *interp, int objc,
                                               Tcl_Obj *CONST objv[]) {
    quota_FILE *p;
    time_t t;
...
*/
static int SQLITE_TCLAPI test_quota_remove(void *clientData, Tcl_Interp *interp,
                                           int objc, Tcl_Obj *CONST objv[]) {
    const char *zFilename; /* File pattern to configure */
    int rc;
    if (objc != 2) {
...
*/
static int SQLITE_TCLAPI test_quota_glob(void *clientData, Tcl_Interp *interp,
                                         int objc, Tcl_Obj *CONST objv[]) {
...
** the file.
*/
static int SQLITE_TCLAPI test_quota_file_available(void *clientData,
                                                   Tcl_Interp *interp, int objc,
                                                   Tcl_Obj *CONST objv[]) {
    quota_FILE *p;
    sqlite3_int64 x;
...
*/
static int SQLITE_TCLAPI test_quota_ferror(void *clientData, Tcl_Interp *interp,
                                           int objc, Tcl_Obj *CONST objv[]) {
    quota_FILE *p;
    int x;
    if (objc != 2) {
