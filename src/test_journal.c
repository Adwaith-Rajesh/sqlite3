...
*/
static int jtRead(sqlite3_file *pFile, void *zBuf, int iAmt,
                  sqlite_int64 iOfst) {
    jt_file *p = (jt_file *)pFile;
...
*/
static int
decodeJournalHdr(const unsigned char *zBuf, /* Input: 28 byte journal header */
    ...
    if (memcmp(aMagic, zBuf, 8))
        return SQLITE_ERROR;
    if (pnRec)
        *pnRec = decodeUint32(&zBuf[8]);
    if (pnPage)
        *pnPage = decodeUint32(&zBuf[16]);
    if (pnSector)
        *pnSector = decodeUint32(&zBuf[20]);
    if (pnPagesize)
        *pnPagesize = decodeUint32(&zBuf[24]);
    return SQLITE_OK;
}
...
static int jtWrite(sqlite3_file *pFile, const void *zBuf, int iAmt,
    ...
    if (p->flags & SQLITE_OPEN_MAIN_DB && p->pWritable) {
        if (iAmt < (int)p->nPagesize && p->nPagesize % iAmt == 0 &&
...
*/
static int jtOpen(sqlite3_vfs *pVfs, const char *zName, sqlite3_file *pFile,
...
*/
static int jtAccess(sqlite3_vfs *pVfs, const char *zPath, int flags,
                    int *pResOut) {
...
*/
static int jtFullPathname(sqlite3_vfs *pVfs, const char *zPath, int nOut,
                          char *zOut) {
...
*/
void jt_unregister(void) { sqlite3_vfs_unregister(&jt_vfs); }
