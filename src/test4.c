...
static int parse_thread_id(Tcl_Interp *interp, const char *zArg) {
    if (zArg == 0 || zArg[0] == 0 || zArg[1] != 0 ||
        !isupper((unsigned char)zArg[0])) {
...
static int SQLITE_TCLAPI tcl_thread_argv(
    ...
    if (argc != 3) {
        Tcl_AppendResult(interp, "wrong # args: should be \"", argv[0], " ID N", 0);
        return TCL_ERROR;
    }
    i = parse_thread_id(interp, argv[1]);
    if (i < 0)
...
static int SQLITE_TCLAPI tcl_thread_compile(
    ...
    int i;
    if (argc != 3) {
        Tcl_AppendResult(interp, "wrong # args: should be \"", argv[0], " ID SQL",
                         0);
        return TCL_ERROR;
    }
    i = parse_thread_id(interp, argv[1]);
...
static int SQLITE_TCLAPI tcl_thread_step(
    ...
    int i;
    if (argc != 2) {
        Tcl_AppendResult(interp, "wrong # args: should be \"", argv[0], " IDL", 0);
        return TCL_ERROR;
    }
    i = parse_thread_id(interp, argv[1]);
    if (i < 0)
...
static int SQLITE_TCLAPI tcl_thread_swap(
    ...
    sqlite3 *temp;
    if (argc != 3) {
        Tcl_AppendResult(interp, "wrong # args: should be \"", argv[0], " ID1 ID2",
                         0);
        return TCL_ERROR;
    }
    i = parse_thread_id(interp, argv[1]);
...
static int SQLITE_TCLAPI tcl_thread_db_put(
    ...
    if (argc != 3) {
        Tcl_AppendResult(interp, "wrong # args: should be \"", argv[0], " ID DB",
                         0);
        return TCL_ERROR;
    }
    i = parse_thread_id(interp, argv[1]);
