...
int sqlite3_drop_modules(sqlite3 *db, const char **azNames) {
    ...
    for (pThis = sqliteHashFirst(&db->aModule); pThis; pThis = pNext) {
        ...
        if (azNames) {
            int ii;
            for (ii = 0; azNames[ii] != 0 && strcmp(azNames[ii], pMod->zName) != 0;
                 ii++) {
            }
            if (azNames[ii] != 0)
                continue;
...
** to this procedure.
*/
static int vtabCallConstructor(sqlite3 *db, Table *pTab, Module *pMod,
    ...
    for (pCtx = db->pVtabCtx; pCtx; pCtx = pCtx->pPrior) {
        if (pCtx->pTab == pTab) {
            *pzErr = sqlite3MPrintf(db, "vtable constructor called recursively: %s",
                                    pTab->zName);
...
static int growVTrans(sqlite3 *db) {
    ...
    if ((db->nVTrans % ARRAY_INCR) == 0) {
        VTable **aVTrans;
        sqlite3_int64 nBytes =
            sizeof(sqlite3_vtab *) * ((sqlite3_int64)db->nVTrans + ARRAY_INCR);
...
int sqlite3_declare_vtab(sqlite3 *db, const char *zCreateTable) {
    ...
    ** in case a bug arises. */
    ...
    if (SQLITE_OK == sqlite3RunParser(&sParse, zCreateTable) &&
        ALWAYS(sParse.pNewTable != 0) && ALWAYS(!db->mallocFailed) &&
        ...
        sqlite3ErrorWithMsg(db, SQLITE_ERROR, (sParse.zErrMsg ? "%s" : 0),
                            sParse.zErrMsg);
...
int sqlite3VtabCallDestroy(sqlite3 *db, int iDb, const char *zTab) {
    ...
    pTab = sqlite3FindTable(db, zTab, db->aDb[iDb].zDbSName);
    if (ALWAYS(pTab != 0) && ALWAYS(IsVirtual(pTab)) &&
...
FuncDef *sqlite3VtabOverloadFunction(
    ...
    ** function */
    pNew =
        sqlite3DbMallocZero(db, sizeof(*pNew) + sqlite3Strlen30(pDef->zName) + 1);
    if (pNew == 0) {
        return pDef;
    }
    *pNew = *pDef;
...
int sqlite3_vtab_on_conflict(sqlite3 *db) {
    static const unsigned char aMap[] = {SQLITE_ROLLBACK, SQLITE_ABORT,
                                         SQLITE_FAIL, SQLITE_IGNORE,
                                         SQLITE_REPLACE};
#ifdef SQLITE_ENABLE_API_ARMOR
    if (!sqlite3SafetyCheckOk(db))
        return SQLITE_MISUSE_BKPT;
