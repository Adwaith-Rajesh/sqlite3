...
static int vfslogFullPathname(sqlite3_vfs *, const char *zName, int,
                              char *zOut);
...
#else
static sqlite3_uint64 vfslog_time() { return 0; }
#endif
...
*/
static int vfslogRead(sqlite3_file *pFile, void *zBuf, int iAmt,
                      sqlite_int64 iOfst) {
    int rc;
    sqlite3_uint64 t;
    VfslogFile *p = (VfslogFile *)pFile;
...
*/
static int vfslogWrite(sqlite3_file *pFile, const void *z, int iAmt,
                       sqlite_int64 iOfst) {
    int rc;
    sqlite3_uint64 t;
    VfslogFile *p = (VfslogFile *)pFile;
...
static int vfslogShmMap(sqlite3_file *pFile, int iRegion, int szRegion,
                        int isWrite, volatile void **pp) {
    int rc;
    sqlite3_uint64 t;
    VfslogFile *p = (VfslogFile *)pFile;
    t = vfslog_time();
...
*/
static int vfslogOpen(sqlite3_vfs *pVfs, const char *zName, sqlite3_file *pFile,
...
*/
static int vfslogAccess(sqlite3_vfs *pVfs, const char *zPath, int flags,
                        int *pResOut) {
    int rc;
    sqlite3_uint64 t;
    t = vfslog_time();
...
*/
static int vfslogFullPathname(sqlite3_vfs *pVfs, const char *zPath, int nOut,
                              char *zOut) {
...
static void vfslog_call(sqlite3_vfs *pVfs, int eEvent, int iFileid,
                        sqlite3_int64 nClick, int return_code, int size,
                        int offset) {
...
int sqlite3_vfslog_new(const char *zVfs,       /* New VFS name */
    ...
    nByte = sizeof(VfslogVfs) + pParent->szOsFile + nVfs + 1 +
            pParent->mxPathname + 1;
    ...
    flags =
        SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE | SQLITE_OPEN_SUPER_JOURNAL;
...
*/
static int vlogConnect(sqlite3 *db, void *pAux, int argc,
    ...
    *ppVtab = 0;
    ...
    if (rc == SQLITE_OK) {
        ...
        sqlite3_declare_vtab(
            db, "CREATE TABLE xxx(event, file, click, rc, size, offset)");
        *ppVtab = &p->base;
    } else {
        sqlite3_free(p);
    }
...
static int vlogFilter(sqlite3_vtab_cursor *pCursor, int idxNum,
...
static int vlogColumn(sqlite3_vtab_cursor *pCursor, sqlite3_context *ctx,
                      int i) {
    unsigned int val;
    VfslogCsr *pCsr = (VfslogCsr *)pCursor;
...
static int SQLITE_TCLAPI test_vfslog(void *clientData, Tcl_Interp *interp,
