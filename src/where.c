...
int sqlite3WhereOkOnePass(WhereInfo *pWInfo, int *aiCur) {
    ...
    if (sqlite3WhereTrace && pWInfo->eOnePass != ONEPASS_OFF) {
        sqlite3DebugPrintf("%s cursors: %d %d\n",
                           pWInfo->eOnePass == ONEPASS_SINGLE ? "ONEPASS_SINGLE"
                                                              : "ONEPASS_MULTI",
                           aiCur[0], aiCur[1]);
    }
#endif
...
*/
static int whereOrInsert(WhereOrSet *pSet, /* The WhereOrSet to be updated */
...
static Expr *whereRightSubexprIsColumn(Expr *p) {
    ...
    if (ALWAYS(p != 0) && p->op == TK_COLUMN &&
        !ExprHasProperty(p, EP_FixedCol)) {
        return p;
    }
    return 0;
}
        ...
        do {
            for (pTerm = pWC->a + k; k < pWC->nTerm; k++, pTerm++) {
                ...
                if (pTerm->leftCursor == iCur && pTerm->u.x.leftColumn == iColumn &&
                    (iColumn != XN_EXPR ||
                     sqlite3ExprCompareSkip(pTerm->pExpr->pLeft, pScan->pIdxExpr,
                                            iCur) == 0) &&
                    (pScan->iEquiv <= 1 ||
                    ...
                    if ((pTerm->eOperator & pScan->opMask) != 0) {
                        ...
                        if ((pTerm->eOperator & (WO_EQ | WO_IS)) != 0 &&
                            ...
                            pX->op == TK_COLUMN && pX->iTable == pScan->aiCur[0] &&
                        ...
                        if (sqlite3WhereTrace & 0x20000) {
                            int ii;
                            sqlite3DebugPrintf("SCAN-TERM %p: nEquiv=%d", pTerm,
                                               pScan->nEquiv);
                            for (ii = 0; ii < pScan->nEquiv; ii++) {
                                sqlite3DebugPrintf(" {%d:%d}", pScan->aiCur[ii],
                                                   pScan->aiColumn[ii]);
                            }
                            sqlite3DebugPrintf("\n");
                        }
...
*/
static int findIndexCol(Parse *pParse,   /* Parse context */
    ...
    for (i = 0; i < pList->nExpr; i++) {
        ...
        if (ALWAYS(p != 0) && (p->op == TK_COLUMN || p->op == TK_AGG_COLUMN) &&
            p->iColumn == pIdx->aiColumn[iCol] && p->iTable == iBase) {
...
*/
static LogEst estLog(LogEst N) { return N <= 10 ? 0 : sqlite3LogEst(N) - 33; }
...
static int constraintCompatibleWithOuterJoin(
    ...
    assert((pSrc->fg.jointype & (JT_LEFT | JT_LTORJ | JT_RIGHT)) !=
           0); /* By caller */
...
*/
static int
termCanDriveIndex(const WhereTerm *pTerm, /* WHERE clause term to check */
                  const SrcItem *pSrc,    /* Table we are trying to access */
...
*/
static void
explainAutomaticIndex(Parse *pParse,
    ...
    if (IS_STMT_SCANSTATUS(pParse->db) && pParse->explain != 2) {
        ...
        if (zText == 0) {
            ...
            *pAddrExplain = sqlite3VdbeExplain(pParse, 0, "%s)%s", zText,
                                               (bPartial ? " WHERE <expr>" : ""));
            sqlite3_free(zText);
        }
    }
...
static SQLITE_NOINLINE void constructAutomaticIndex(
    ...
    ** and used to match WHERE clause constraints */
    ...
    for (pTerm = pWC->a; pTerm < pWCEnd; pTerm++) {
        ...
        ** rows of the target table (pSrc) that can be used. */
        ...
        if (termCanDriveIndex(pTerm, pSrc, notReady)) {
            ...
            testcase(iCol == BMS - 1);
            if (!sentWarning) {
                sqlite3_log(SQLITE_WARNING_AUTOINDEX, "automatic index on %s(%s)",
                            pTable->zName, pTable->aCol[iCol].zCnName);
                sentWarning = 1;
            }
            if ((idxCols & cMask) == 0) {
    ...
    pLoop->wsFlags =
        WHERE_COLUMN_EQ | WHERE_IDX_ONLY | WHERE_INDEXED | WHERE_AUTO_INDEX;
    ...
    ** a covering index */
    ...
    regBase = sqlite3GenerateIndexKey(pParse, pIdx, pLevel->iTabCur, regRecord, 0,
                                      0, 0, 0);
    if (pLevel->regFilter) {
        sqlite3VdbeAddOp4Int(v, OP_FilterAdd, pLevel->regFilter, 0, regBase,
                             pLoop->u.btree.nEq);
    }
    ...
    if (pSrc->fg.viaCoroutine) {
        ...
        assert(pLevel->iIdxCur > 0);
        translateColumnToCopy(pParse, addrTop, pLevel->iTabCur, pSrc->regResult,
                              pLevel->iIdxCur);
...
static SQLITE_NOINLINE void sqlite3ConstructBloomFilter(
    ...
    do {
        ...
        */
        ...
        if (OptimizationDisabled(pParse->db, SQLITE_BloomPulldown))
            break;
    ...
    */
    ...
    if (pOrderBy) {
        ...
        for (i = 0; i < n; i++) {
            ...
            if (pOrderBy->a[i].fg.sortFlags & KEYINFO_ORDER_BIGNULL)
                break;
            ...
            ** of the COLLATE operator matches the collation of the column. */
            if (pExpr->op == TK_COLLATE && (pE2 = pExpr->pLeft)->op == TK_COLUMN &&
                ...
                if (pE2->iColumn < 0)
                    continue; /* Collseq does not matter for rowid */
    ...
    for (i = j = 0, pTerm = pWC->a; i < pWC->nTerm; i++, pTerm++) {
        ...
        if (op == WO_AUX) {
            ...
            ** following asserts verify this fact. */
            ...
            assert(pTerm->eOperator &
                   (WO_IN | WO_EQ | WO_LT | WO_LE | WO_GT | WO_GE | WO_AUX));
        ...
        if (iSamp > 0) {
            ...
            ** fields that is greater than the previous effective sample.  */
            for (n = (iTest % nField) + 1; n < nField; n++) {
                if (aSample[iSamp - 1].anLt[n - 1] != aSample[iSamp].anLt[n - 1])
                    break;
            }
        } else {
            n = iTest + 1;
    ...
    if (pParse->db->mallocFailed == 0) {
        if (res == 0) {
            ...
            ** If (i>0), then pRec must also be greater than sample (i-1).  */
            if (iCol > 0) {
                pRec->nField = iCol;
                assert(sqlite3VdbeRecordCompare(aSample[i].n, aSample[i].p, pRec) <=
                           0 ||
                       pParse->db->mallocFailed || CORRUPT_DB);
            }
            if (i > 0) {
                pRec->nField = nField;
                assert(sqlite3VdbeRecordCompare(aSample[i - 1].n, aSample[i - 1].p,
                                                pRec) < 0 ||
                       pParse->db->mallocFailed || CORRUPT_DB);
            }
        }
...
static int whereRangeScanEst(
    ...
    ** match 1/64 of the index. */
    ...
    if (pLoop->nOut > nOut) {
        WHERETRACE(0x20,
                   ("Range scan lowers nOut from %d to %d\n", pLoop->nOut, nOut));
    }
#endif
...
void sqlite3WhereTermPrint(WhereTerm *pTerm, int iTerm) {
    if (pTerm == 0) {
        ...
        if (pTerm->eOperator & WO_SINGLE) {
            assert((pTerm->eOperator & (WO_OR | WO_AND)) == 0);
            sqlite3_snprintf(sizeof(zLeft), zLeft, "left={%d:%d}", pTerm->leftCursor,
        ...
        sqlite3DebugPrintf("TERM-%-3d %p %s %-12s op=%03x wtFlags=%04x", iTerm,
                           pTerm, zType, zLeft, pTerm->eOperator, pTerm->wtFlags);
        /* The 0x10000 .wheretrace flag causes extra information to be
        ** shown about each Term */
        if (sqlite3WhereTrace & 0x10000) {
            sqlite3DebugPrintf(" prob=%-3d prereq=%llx,%llx", pTerm->truthProb,
...
void sqlite3WhereLoopPrint(WhereLoop *p, WhereClause *pWC) {
    ...
    sqlite3DebugPrintf("%c%2d.%0*llx.%0*llx", p->cId, p->iTab, nb, p->maskSelf,
                       nb, p->prereq & mAll);
    sqlite3DebugPrintf(" %12s", pItem->zAlias ? pItem->zAlias : pTab->zName);
    if ((p->wsFlags & WHERE_VIRTUALTABLE) == 0) {
        ...
        char *z;
        if (p->u.vtab.idxStr) {
            z = sqlite3_mprintf("(%d,\"%s\",%#x)", p->u.vtab.idxNum, p->u.vtab.idxStr,
                                p->u.vtab.omitMask);
        } else {
    ...
    for (; p; p = p->pNextLoop) {
        ...
        if (whereLoopCheaperProperSubset(p, pTemplate)) {
            ...
            ** subset p. */
            WHERETRACE(0x80,
                       ("subset cost adjustment %d,%d to %d,%d\n", pTemplate->rRun,
                        pTemplate->nOut, MIN(p->rRun, pTemplate->rRun),
            ...
            ** pTemplate is a proper subset of p */
            WHERETRACE(0x80,
                       ("subset cost adjustment %d,%d to %d,%d\n", pTemplate->rRun,
                        pTemplate->nOut, MAX(p->rRun, pTemplate->rRun),
...
static WhereLoop **whereLoopFindLesser(WhereLoop **ppPrev,
    ...
    for (p = (*ppPrev); p; ppPrev = &p->pNextLoop, p = *ppPrev) {
        ...
        ** than an automatic index. Unless it is a skip-scan. */
        if ((p->wsFlags & WHERE_AUTO_INDEX) != 0 && (pTemplate->nSkip) == 0 &&
...
whereLoopAddBtreeIndex(WhereLoopBuilder *pBuilder, /* The WhereLoop factory */
    ...
    pTerm = whereScanInit(&scan, pBuilder->pWC, pSrc->iCursor, saved_nEq, opMask,
                          pProbe);
    pNew->rSetup = 0;
    ...
    for (; rc == SQLITE_OK && pTerm != 0; pTerm = whereScanNext(&scan)) {
        ...
        ** to mix with a lower range bound from some other source */
        ...
        assert(nInMul == 0 || (pNew->wsFlags & WHERE_COLUMN_NULL) != 0 ||
        ...
        if (eOp & WO_IN) {
            ...
            if (scan.iEquiv > 1)
                pNew->wsFlags |= WHERE_TRANSCONS;
        } else if (eOp & WO_ISNULL) {
            pNew->wsFlags |= WHERE_COLUMN_NULL;
        } else {
            int nVecLen =
                whereRangeVectorLen(pParse, pSrc->iCursor, pProbe, saved_nEq, pTerm);
            if (eOp & (WO_GT | WO_GE)) {
                testcase(eOp & WO_GT);
                testcase(eOp & WO_GE);
        ...
        ** visiting the rows in the main table.  */
        ...
        if (pProbe->idxType == SQLITE_IDXTYPE_IPK) {
            ...
            ** under-estimate the scanning cost. */
            ...
            rCostIdx =
                pNew->nOut + 1 + (15 * pProbe->szIdxRow) / pSrc->pTab->szTabRow;
        }
    ...
    ** more expensive.  */
    ...
    if (saved_nEq == saved_nSkip && saved_nEq + 1 < pProbe->nKeyCol &&
        saved_nEq == pNew->nLTerm && pProbe->noSkipScan == 0 &&
        pProbe->hasStat1 != 0 && OptimizationEnabled(db, SQLITE_SkipScan) &&
        pProbe->aiRowLogEst[saved_nEq + 1] >=
            42 /* TUNING: Minimum for skip-scan */
        ...
        if (pExpr->op == TK_COLUMN && pExpr->iTable == iCursor) {
            ...
            for (jj = 0; jj < pIndex->nKeyCol; jj++) {
                if (pIndex->aiColumn[jj] != XN_EXPR)
                    continue;
                if (sqlite3ExprCompareSkip(pExpr, aColExpr->a[jj].pExpr, iCursor) ==
                    0) {
                    return 1;
                }
            }
...
static int whereUsablePartialIndex(
    ...
    if (pParse->db->flags & SQLITE_EnableQPSG)
        pParse = 0;
...
*/
static int exprIsCoveredByIndex(const Expr *pExpr, const Index *pIdx,
                                int iTabCur) {
    int i;
    for (i = 0; i < pIdx->nColumn; i++) {
        if (pIdx->aiColumn[i] == XN_EXPR &&
            sqlite3ExprCompare(0, pExpr, pIdx->aColExpr->a[i].pExpr, iTabCur) ==
                0) {
            return 1;
        }
    }
...
*/
static void
wherePartIdxExpr(Parse *pParse,  /* Parse context */
    ...
    if ((pPart->op == TK_EQ || pPart->op == TK_IS)) {
        ...
        if (pLeft->op != TK_COLUMN)
            return;
        if (!sqlite3ExprIsConstant(pRight))
            return;
            ...
            */
            ...
            if (pProbe->isCovering) {
                ...
                if (pProbe->pPartIdxWhere) {
                    wherePartIdxExpr(pWInfo->pParse, pProbe, pProbe->pPartIdxWhere, &m, 0,
                                     0);
                }
                ...
                if (m == TOPBIT || (pProbe->bHasExpr && !pProbe->bHasVCol && m != 0)) {
                    ...
                    if (isCov == 0) {
                        WHERETRACE(0x200, ("-> %s is not a covering index"
                        ...
                        if (isCov & WHERE_IDX_ONLY) {
                            WHERETRACE(0x200, ("-> %s is a covering expression index"
                                               " according to whereIsCoveringIndex()\n",
                                               pProbe->zName));
                        } else {
                            assert(isCov == WHERE_EXPRIDX);
                            WHERETRACE(0x200, ("-> %s might be a covering expression index"
                                               " according to whereIsCoveringIndex()\n",
                                               pProbe->zName));
                        }
                    }
                } else if (m == 0) {
                    WHERETRACE(0x200, ("-> %s a covering index according to bitmasks\n",
            ...
            /* Full scan via index */
            if (b || !HasRowid(pTab) || pProbe->pPartIdxWhere != 0 ||
                pSrc->fg.isIndexedBy ||
                (m == 0 && pProbe->bUnordered == 0 &&
    ...
    ** arguments mUsable and mExclude. */
    ...
    for (i = 0; i < nConstraint; i++, pIdxCons++) {
        ...
        if ((iTerm = pUsage[i].argvIndex - 1) >= 0) {
            ...
            if (iTerm >= nConstraint || j < 0 || j >= pWC->nTerm ||
    ...
    pNew->u.vtab.isOrdered =
        (i8)(pIdxInfo->orderByConsumed ? pIdxInfo->nOrderBy : 0);
    pNew->rSetup = 0;
...
int sqlite3_vtab_rhs_value(
    ...
    if (iCons < 0 || iCons >= pIdxInfo->nConstraint) {
        ...
        if (pH->aRhs[iCons] == 0) {
            ...
            rc = sqlite3ValueFromExpr(pH->pParse->db, pTerm->pExpr->pRight,
                                      ENC(pH->pParse->db), SQLITE_AFF_BLOB,
                                      &pH->aRhs[iCons]);
    ...
    if (rc == SQLITE_OK && ((mBest = (pNew->prereq & ~mPrereq)) != 0 || bIn)) {
        ...
        ** xBestIndex again, this time with IN(...) terms disabled. */
        if (bIn) {
            ...
            rc = whereLoopAddVirtualOne(pBuilder, mPrereq, ALLBITS, WO_IN, p, mNoOmit,
                                        &bIn, 0);
            assert(bIn == 0);
        ...
        ** in the set of terms that apply to the current virtual table.  */
        while (rc == SQLITE_OK) {
            ...
            rc = whereLoopAddVirtualOne(pBuilder, mPrereq, mNext | mPrereq, 0, p,
                                        mNoOmit, &bIn, 0);
        ...
        ** usable), make a call here with all source tables disabled */
        if (rc == SQLITE_OK && seenZero == 0) {
            ...
            rc = whereLoopAddVirtualOne(pBuilder, mPrereq, mPrereq, 0, p, mNoOmit,
                                        &bIn, 0);
            if (bIn == 0)
                seenZeroNoIN = 1;
        }
        ...
        ** operator, make a final call to obtain one here.  */
        if (rc == SQLITE_OK && seenZeroNoIN == 0) {
            ...
            rc = whereLoopAddVirtualOne(pBuilder, mPrereq, mPrereq, WO_IN, p, mNoOmit,
                                        &bIn, 0);
        }
    }
...
*/
static int whereLoopAddOr(WhereLoopBuilder *pBuilder, Bitmask mPrereq,
                          Bitmask mUnusable) {
    WhereInfo *pWInfo = pBuilder->pWInfo;
    WhereClause *pWC;
    WhereLoop *pNew;
    ...
    for (iLoop = 0; isOrderDistinct && obSat < obDone && iLoop <= nLoop;
        ...
        if (pLoop->wsFlags & WHERE_VIRTUALTABLE) {
            if (pLoop->u.vtab.isOrdered &&
                ((wctrlFlags & (WHERE_DISTINCTBY | WHERE_SORTBYGROUP)) !=
                 WHERE_DISTINCTBY)) {
                obSat = obDone;
            }
            break;
        ...
        */
        for (i = 0; i < nOrderBy; i++) {
            ...
            if (NEVER(pOBExpr == 0))
                continue;
            if (pOBExpr->op != TK_COLUMN && pOBExpr->op != TK_AGG_COLUMN)
                continue;
            if (pOBExpr->iTable != iCur)
                continue;
            pTerm = sqlite3WhereFindTerm(&pWInfo->sWC, iCur, pOBExpr->iColumn, ~ready,
                                         eqOpMask, 0);
            if (pTerm == 0)
                continue;
        ...
        if ((pLoop->wsFlags & WHERE_ONEROW) == 0) {
            if (pLoop->wsFlags & WHERE_IPK) {
                ...
                ** made at tag-20210426-1 below */
                isOrderDistinct =
                    IsUniqueIndex(pIndex) && (pLoop->wsFlags & WHERE_SKIPSCAN) == 0;
            }
            ...
            */
            ...
            for (j = 0; j < nColumn; j++) {
                ...
                */
                if (isOrderDistinct) {
                    if (iColumn >= 0 && j >= pLoop->u.btree.nEq &&
...
static const char *wherePathName(WherePath *pPath, int nLoop,
                                 WhereLoop *pLast) {
    ...
    for (iLoop = 0; iLoop < nLoop; iLoop++) {
        ...
        for (ii = 0, pFrom = aFrom; ii < nFrom; ii++, pFrom++) {
            for (pWLoop = pWInfo->pLoops; pWLoop; pWLoop = pWLoop->pNextLoop) {
                ...
                ** Compute its cost */
                rUnsorted =
                    sqlite3LogEstAdd(pWLoop->rSetup, pWLoop->rRun + pFrom->nRow);
                ...
                if (isOrdered >= 0 && isOrdered < nOrderBy) {
                    if (aSortCost[isOrdered] == 0) {
                        aSortCost[isOrdered] =
                            whereSortingCost(pWInfo, nRowEst, nOrderBy, isOrdered);
                    }
                ...
                */
                ...
                if (jj >= nTo) {
                    ...
                    if (nTo >= mxChoice &&
                        (rCost > mxCost ||
                         (rCost == mxCost && rUnsorted >= mxUnsorted))) {
...
static int whereShortCut(WhereLoopBuilder *pBuilder) {
    ...
    if (pTerm) {
        ...
        for (pIdx = pTab->pIndex; pIdx; pIdx = pIdx->pNext) {
            ...
            if (!IsUniqueIndex(pIdx) || pIdx->pPartIdxWhere != 0 ||
...
**       LEFT JOIN t3 ON (t1.ipk=t3.ipk)
*/
static SQLITE_NOINLINE Bitmask whereOmitNoopJoin(WhereInfo *pWInfo,
                                                 Bitmask notReady) {
    int i;
    Bitmask tabUsed;
    int hasRightJoin;
    ...
    ** condition (1) of the header comment */
    ...
    for (i = pWInfo->nLevel - 1; i >= 1; i--) {
        ...
        for (pTerm = pWInfo->sWC.a; pTerm < pEnd; pTerm++) {
            ...
            if (hasRightJoin && ExprHasProperty(pTerm->pExpr, EP_InnerON) &&
                pTerm->pExpr->w.iJoin == pItem->iCursor) {
                break; /* restriction (5) */
            }
        }
...
whereCheckIfBloomFilterIsUseful(const WhereInfo *pWInfo) {
    int i;
    LogEst nSearch = 0;
    ...
    for (i = 0; i < pWInfo->nLevel; i++) {
        ...
        Table *pTab = pItem->pTab;
        if ((pTab->tabFlags & TF_HasStat1) == 0)
            break;
        pTab->tabFlags |= TF_StatsUsed;
        if (i >= 1 &&
            ...
            if (nSearch > pTab->nRowLogEst) {
                ...
                WHERETRACE(0xffffffff,
                           ("-> use Bloom-filter on loop %c because there are ~%.1e "
                            ...
                            pLoop->cId, (double)sqlite3LogEstToInt(nSearch),
                            pTab->zName, (double)sqlite3LogEstToInt(pTab->nRowLogEst)));
            }
        }
        nSearch += pLoop->nOut;
    }
...
static SQLITE_NOINLINE void whereAddIndexedExpr(
    ...
    for (i = 0; i < pIdx->nColumn; i++) {
        ...
        if (j == XN_EXPR) {
            ...
            bMaybeNullRow =
                (pTabItem->fg.jointype & (JT_LEFT | JT_LTORJ | JT_RIGHT)) != 0;
...
static SQLITE_NOINLINE void whereReverseScanOrder(WhereInfo *pWInfo) {
    ...
    for (ii = 0; ii < pWInfo->pTabList->nSrc; ii++) {
        ...
        if (!pItem->fg.isCte || pItem->u2.pCteUse->eM10d != M10d_Yes ||
            NEVER(pItem->pSelect == 0) || pItem->pSelect->pOrderBy == 0) {
...
WhereInfo *sqlite3WhereBegin(
    ...
    if (nTabList != 1 || whereShortCut(&sWLB) == 0) {
        ...
        ** loops will be built using the revised truthProb values. */
        if (sWLB.bldFlags2 & SQLITE_BLDF2_2NDPASS) {
            ...
            WHERETRACE(0xffffffff, ("**** Redo all loop computations due to"
    ...
    */
    if (pWInfo->nLevel >= 2 && OptimizationEnabled(db, SQLITE_BloomFilter)) {
    ...
    */
    ...
    if ((wctrlFlags & WHERE_ONEPASS_DESIRED) != 0) {
        ...
        if (bOnerow || (0 != (wctrlFlags & WHERE_ONEPASS_MULTIROW) &&
                        !IsVirtual(pTabList->a[0].pTab) &&
                        (0 == (wsFlags & WHERE_MULTI_OR) ||
    ...
    */
    for (ii = 0, pLevel = pWInfo->a; ii < nTabList; ii++, pLevel++) {
            ...
            if (!HasRowid(pTab) && IsPrimaryKeyIndex(pIx) &&
                ...
                ** WITHOUT ROWID table.  No need for a separate index */
                ...
                if (pIx->pPartIdxWhere && (pTabItem->fg.jointype & JT_RIGHT) == 0) {
                    wherePartIdxExpr(pParse, pIx, pIx->pPartIdxWhere, 0, iIndexCur,
                                     pTabItem);
                }
            }
            ...
            if (op) {
...
#ifdef SQLITE_ENABLE_COLUMN_USED_MASK
                {
                    ...
                    for (ii = 0; ii < pIx->nColumn; ii++) {
                        ...
                        if (jj > 63)
                            jj = 63;
                        if ((pTabItem->colUsed & MASKBIT(jj)) == 0)
                            continue;
        ...
        if ((pTabItem->fg.jointype & JT_RIGHT) != 0 &&
            (pLevel->pRJ = sqlite3WhereMalloc(pWInfo, sizeof(WhereRightJoin))) !=
                0) {
    ...
    */
    for (ii = 0; ii < nTabList; ii++) {
        ...
        notReady =
            sqlite3WhereCodeOneLoopStart(pParse, v, pWInfo, ii, pLevel, notReady);
...
static void sqlite3WhereOpcodeRewriteTrace(sqlite3 *db, int pc, VdbeOp *pOp) {
    if ((db->flags & SQLITE_VdbeAddopTrace) == 0)
        return;
...
static int cursorIsOpen(Vdbe *v, int iCur, int k) {
    while (k >= 0) {
        ...
        if (pOp->p1 != iCur)
            continue;
        if (pOp->opcode == OP_Close)
            return 0;
        if (pOp->opcode == OP_OpenRead)
            return 1;
        if (pOp->opcode == OP_OpenWrite)
            return 1;
        if (pOp->opcode == OP_OpenDup)
            return 1;
        if (pOp->opcode == OP_OpenAutoindex)
            return 1;
        if (pOp->opcode == OP_OpenEphemeral)
            return 1;
    }
    return 0;
}
    ...
    for (i = pWInfo->nLevel - 1; i >= 0; i--) {
        ...
        if (pLevel->op != OP_Noop) {
            ...
            if (pWInfo->eDistinct == WHERE_DISTINCT_ORDERED &&
                ...
                (n = pLoop->u.btree.nDistinctCol) > 0 && pIdx->aiRowLogEst[n] >= 36) {
        ...
        if ((pLoop->wsFlags & WHERE_IN_ABLE) != 0 && pLevel->u.in.nIn > 0) {
            ...
            for (j = pLevel->u.in.nIn, pIn = &pLevel->u.in.aInLoop[j - 1]; j > 0;
                 j--, pIn--) {
                ...
                if (pIn->eEndLoopOp != OP_Noop) {
                    if (pIn->nPrefix) {
                        int bEarlyOut = (pLoop->wsFlags & WHERE_VIRTUALTABLE) == 0 &&
                        ...
                        if (bEarlyOut) {
                            sqlite3VdbeAddOp4Int(v, OP_IfNoHope, pLevel->iIdxCur,
                                                 sqlite3VdbeCurrentAddr(v) + 2, pIn->iBase,
                                                 pIn->nPrefix);
                            VdbeCoverage(v);
...
#ifdef SQLITE_ENABLE_OFFSET_SQL_FUNC
                        assert((pLoop->wsFlags & WHERE_IDX_ONLY) == 0 ||
                               cursorIsOpen(v, pOp->p1, k) || pOp->opcode == OP_Offset);
#else
                        assert((pLoop->wsFlags & WHERE_IDX_ONLY) == 0 ||
