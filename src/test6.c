...
static void crash_free(void *p) { Tcl_Free(p); }
...
static int writeDbFile(CrashFile *p, u8 *z, i64 iAmt, i64 iOff) {
    ...
    int iSkip = 0;
    if ((iAmt - iSkip) > 0) {
        rc = sqlite3OsWrite(p->pRealFile, &z[iSkip], (int)(iAmt - iSkip),
                            iOff + iSkip);
    }
    return rc;
}
...
static int writeListSync(CrashFile *pFile, int isCrash) {
    ...
    */
    ...
    if (!isCrash) {
        ...
        for (pWrite = g.pWriteList; iFinal > 0; pWrite = pWrite->pNext)
            iFinal--;
        pFinal = pWrite;
    }
        ...
        */
        ...
        switch (eAction) {
            case 1: { /* Write out correctly */
                if (pWrite->zBuf) {
                    rc = writeDbFile(pWrite->pFile, pWrite->zBuf, pWrite->nBuf,
                                     pWrite->iOffset);
                } else {
                ...
                *ppPtr = pWrite->pNext;
                ...
                if (isCrash) {
                    printf("Writing %d bytes @ %d (%s)\n", pWrite->nBuf,
                           (int)pWrite->iOffset, pWrite->pFile->zName);
                }
#endif
                crash_free(pWrite);
                break;
            ...
            case 3: { /* Trash sectors */
                ...
                if (zGarbage) {
                    ...
                    for (i = iFirst; rc == SQLITE_OK && i <= iLast; i++) {
                        ...
                        rc = writeDbFile(pWrite->pFile, zGarbage, g.iSectorSize,
                                         i * g.iSectorSize);
                    }
                    crash_free(zGarbage);
                } else {
...
*/
static int writeListAppend(sqlite3_file *pFile, sqlite3_int64 iOffset,
                           const u8 *zBuf, int nBuf) {
    WriteBuffer *pNew;
...
*/
static int cfRead(sqlite3_file *pFile, void *zBuf, int iAmt,
                  sqlite_int64 iOfst) {
    CrashFile *pCrash = (CrashFile *)pFile;
...
*/
static int cfWrite(sqlite3_file *pFile, const void *zBuf, int iAmt,
                   sqlite_int64 iOfst) {
    CrashFile *pCrash = (CrashFile *)pFile;
...
*  interface.
*/
static int cfSectorSize(sqlite3_file *pFile) { return g.iSectorSize; }
...
static int cfShmMap(sqlite3_file *pFile, /* Handle open on database file */
...
*/
static int cfOpen(sqlite3_vfs *pCfVfs, const char *zName, sqlite3_file *pFile,
...
static int cfFullPathname(sqlite3_vfs *pCfVfs, const char *zPath, int nPathOut,
                          char *zPathOut) {
    sqlite3_vfs *pVfs = (sqlite3_vfs *)pCfVfs->pAppData;
...
static int processDevSymArgs(Tcl_Interp *interp, int objc,
                             Tcl_Obj *CONST objv[], int *piDeviceChar,
                             int *piSectorSize) {
    struct DeviceFlag {
        char *zName;
        int iValue;
    } aFlag[] = {{"atomic", SQLITE_IOCAP_ATOMIC},
    ...
    for (i = 0; i < objc; i += 2) {
        ...
        if ((nOpt > 11 || nOpt < 2 || strncmp("-sectorsize", zOpt, nOpt)) &&
            ...
            Tcl_AppendResult(interp, "Bad option: \"", zOpt,
                             "\" - must be \"-characteristics\" or \"-sectorsize\"",
                             0);
            return TCL_ERROR;
        }
        if (i == objc - 1) {
            Tcl_AppendResult(interp, "Option requires an argument: \"", zOpt, "\"",
                             0);
            return TCL_ERROR;
        }
        ...
        if (zOpt[1] == 's') {
            ...
            for (j = 0; j < nObj; j++) {
                ...
                rc = Tcl_GetIndexFromObjStruct(interp, pFlag, aFlag, sizeof(aFlag[0]),
                                               "no such flag", 0, &iChoice);
...
*/
static int SQLITE_TCLAPI crashNowCmd(void *clientData, Tcl_Interp *interp,
...
*/
static int SQLITE_TCLAPI crashEnableCmd(void *clientData, Tcl_Interp *interp,
                                        int objc, Tcl_Obj *CONST objv[]) {
    int isEnable;
    int isDefault = 0;
...
**
*/
static int SQLITE_TCLAPI crashParamsObjCmd(void *clientData, Tcl_Interp *interp,
                                           int objc, Tcl_Obj *CONST objv[]) {
    int iDelay;
...
static int SQLITE_TCLAPI devSymObjCmd(void *clientData, Tcl_Interp *interp,
                                      int objc, Tcl_Obj *CONST objv[]) {
...
*/
static int SQLITE_TCLAPI writeCrashObjCmd(void *clientData, Tcl_Interp *interp,
                                          int objc, Tcl_Obj *CONST objv[]) {
...
*/
static int SQLITE_TCLAPI dsUnregisterObjCmd(void *clientData,
...
*/
static int SQLITE_TCLAPI jtObjCmd(void *clientData, Tcl_Interp *interp,
                                  int objc, Tcl_Obj *CONST objv[]) {
    ...
    if (objc == 3) {
        if (strcmp(zParent, "-default")) {
            Tcl_AppendResult(interp, "bad option \"", zParent, "\": must be -default",
                             0);
            return TCL_ERROR;
        }
...
*/
static int SQLITE_TCLAPI jtUnregisterObjCmd(void *clientData,
                                            Tcl_Interp *interp, int objc,
                                            Tcl_Obj *CONST objv[]) {
...
int Sqlitetest6_Init(Tcl_Interp *interp) {
    ...
    Tcl_CreateObjCommand(interp, "sqlite3_crash_on_write", writeCrashObjCmd, 0,
                         0);
