...
*/
static void
multiplexFilename(const char *zBase, /* Filename for chunk 0 */
...
*/
static sqlite3_file *
multiplexSubOpen(multiplexGroup *pGroup, /* The multiplexor group */
    ...
    *rc = multiplexSubFilename(pGroup, iChunk);
    if ((*rc) == SQLITE_OK && (pSubOpen = pGroup->aReal[iChunk].p) == 0) {
        ...
        *rc = pOrigVfs->xOpen(pOrigVfs, pGroup->aReal[iChunk].z, pSubOpen, flags,
                              pOutFlags);
        if ((*rc) != SQLITE_OK) {
...
multiplexSubSize(multiplexGroup *pGroup, /* The multiplexor group */
                 int iChunk,             /* Which chunk to open.  0==original file */
                 int *rc                 /* Result code in and out */
) {
    sqlite3_file *pSub;
...
*/
static void multiplexControlFunc(sqlite3_context *context, int argc,
                                 sqlite3_value **argv) {
    int rc = SQLITE_OK;
...
*/
static int multiplexFuncInit(sqlite3 *db, char **pzErrMsg,
                             const sqlite3_api_routines *pApi) {
    int rc;
    rc = sqlite3_create_function(db, "multiplex_control", 2, SQLITE_ANY, 0,
                                 multiplexControlFunc, 0, 0);
    return rc;
}
...
*/
static void multiplexSubClose(multiplexGroup *pGroup, int iChunk,
                              sqlite3_vfs *pOrigVfs) {
...
*/
static int multiplexOpen(sqlite3_vfs *pVfs,   /* The multiplex VFS */
    ...
    */
    ...
    if (rc == SQLITE_OK) {
        ...
        pGroup->bTruncate = (unsigned char)sqlite3_uri_boolean(
            zUri, "truncate", (flags & SQLITE_OPEN_MAIN_DB) == 0);
        pGroup->szChunk =
            (int)sqlite3_uri_int64(zUri, "chunksize", SQLITE_MULTIPLEX_CHUNK_SIZE);
...
#endif
            while ((sqlite3PendingByte % pGroup->szChunk) >=
            ...
            if (rc == SQLITE_OK && zName) {
                ...
                if (flags & SQLITE_OPEN_SUPER_JOURNAL) {
                    ...
                    */
                    ...
                    if (rc == SQLITE_OK && bExists && sz64 == (sz64 & 0xffff0000) &&
                        sz64 > 0 && sz64 != pGroup->szChunk) {
...
*/
static int multiplexDelete(sqlite3_vfs *pVfs, /* The multiplex VFS */
...
static int multiplexFullPathname(sqlite3_vfs *a, const char *b, int c,
                                 char *d) {
...
*/
static int multiplexWrite(sqlite3_file *pConn, const void *pBuf, int iAmt,
                          sqlite3_int64 iOfst) {
    multiplexConn *p = (multiplexConn *)pConn;
    multiplexGroup *pGroup = p->pGroup;
    int rc = SQLITE_OK;
                ...
                if (sqlite3_stricmp(aFcntl[1], "multiplex_truncate") == 0) {
                    if (aFcntl[2] && aFcntl[2][0]) {
                        if (sqlite3_stricmp(aFcntl[2], "on") == 0 ||
                            ...
                            pGroup->bTruncate = 1;
                        } else if (sqlite3_stricmp(aFcntl[2], "off") == 0 ||
 ...
 */
static int
multiplexShmMap(sqlite3_file *pConn, /* Handle open on database file */
    ...
    sqlite3_file *pSubOpen = multiplexSubOpen(p->pGroup, 0, &rc, NULL, 0);
    if (pSubOpen) {
        return pSubOpen->pMethods->xShmMap(pSubOpen, iRegion, szRegion, bExtend,
                                           pp);
    }
    return SQLITE_IOERR;
}
...
*/
static int SQLITE_TCLAPI test_multiplex_initialize(void *clientData,
...
*/
static int SQLITE_TCLAPI test_multiplex_shutdown(void *clientData,
                                                 Tcl_Interp *interp, int objc,
                                                 Tcl_Obj *CONST objv[]) {
...
*/
static int SQLITE_TCLAPI test_multiplex_control(ClientData cd,
                                                Tcl_Interp *interp, int objc,
                                                Tcl_Obj *CONST objv[]) {
    ...
    struct SubCommand {
        ...
        int op;
        int argtype;
    } aSub[] = {{"enable", MULTIPLEX_CTRL_ENABLE, 1},
    ...
    rc = Tcl_GetIndexFromObjStruct(interp, objv[3], aSub, sizeof(aSub[0]),
                                   "sub-command", 0, &idx);
    if (rc != TCL_OK)
        return rc;
