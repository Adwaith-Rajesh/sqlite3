...
#if SQLITE_OS_UNIX && defined(SQLITE_ENABLE_UNLOCK_NOTIFY)
    Tcl_CreateObjCommand(interp, "sqlite3_blocking_step", blocking_step_proc, 0,
                         0);
    Tcl_CreateObjCommand(interp, "sqlite3_blocking_prepare_v2",
                         blocking_prepare_v2_proc, (void *)1, 0);
    Tcl_CreateObjCommand(interp, "sqlite3_nonblocking_prepare_v2",
                         blocking_prepare_v2_proc, 0, 0);
#endif
...
*/
static int SQLITE_TCLAPI sqlthread_spawn(ClientData clientData,
...
*/
static int SQLITE_TCLAPI sqlthread_parent(ClientData clientData,
                                          Tcl_Interp *interp, int objc,
                                          Tcl_Obj *CONST objv[]) {
    EvalEvent *pEvent;
    char *zMsg;
...
**     the pointer value.
*/
static int SQLITE_TCLAPI sqlthread_open(ClientData clientData,
                                        Tcl_Interp *interp, int objc,
                                        Tcl_Obj *CONST objv[]) {
    ...
    if (sqlite3TestMakePointerStr(interp, zBuf, db))
        return TCL_ERROR;
...
**     an integer.
*/
static int SQLITE_TCLAPI sqlthread_id(ClientData clientData, Tcl_Interp *interp,
...
*/
static int SQLITE_TCLAPI sqlthread_proc(ClientData clientData,
                                        Tcl_Interp *interp, int objc,
                                        Tcl_Obj *CONST objv[]) {
    struct SubCommand {
        char *zName;
        ...
        int nArg;
        char *zUsage;
    } aSub[] = {{"parent", sqlthread_parent, 1, "SCRIPT"},
    ...
    rc = Tcl_GetIndexFromObjStruct(interp, objv[1], aSub, sizeof(aSub[0]),
                                   "sub-command", 0, &iIndex);
    if (rc != TCL_OK)
        return rc;
    pSub = &aSub[iIndex];
...
** testfixture.
*/
static int SQLITE_TCLAPI clock_seconds_proc(ClientData clientData,
                                            Tcl_Interp *interp, int objc,
                                            Tcl_Obj *CONST objv[]) {
    Tcl_Time now;
    Tcl_GetTime(&now);
...
*/
static int SQLITE_TCLAPI clock_milliseconds_proc(ClientData clientData,
                                                 Tcl_Interp *interp, int objc,
                                                 Tcl_Obj *CONST objv[]) {
    Tcl_Time now;
    Tcl_GetTime(&now);
    Tcl_SetObjResult(interp, Tcl_NewWideIntObj(((Tcl_WideInt)now.sec * 1000) +
                                               (now.usec / 1000)));
    UNUSED_PARAMETER(clientData);
    UNUSED_PARAMETER(objc);
    UNUSED_PARAMETER(objv);
...
*/
static int SQLITE_TCLAPI blocking_step_proc(void *clientData,
                                            Tcl_Interp *interp, int objc,
                                            Tcl_Obj *CONST objv[]) {
    sqlite3_stmt *pStmt;
    int rc;
...
*/
static int SQLITE_TCLAPI blocking_prepare_v2_proc(void *clientData,
                                                  Tcl_Interp *interp, int objc,
                                                  Tcl_Obj *CONST objv[]) {
    sqlite3 *db;
    const char *zSql;
    ...
    if (Tcl_GetIntFromObj(interp, objv[3], &bytes))
        return TCL_ERROR;
...
#else
int SqlitetestThread_Init(Tcl_Interp *interp) { return TCL_OK; }
#endif
