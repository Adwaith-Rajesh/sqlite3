...
static int setDestPgsz(sqlite3_backup *p) {
    int rc;
    rc =
        sqlite3BtreeSetPageSize(p->pDest, sqlite3BtreeGetPageSize(p->pSrc), 0, 0);
    return rc;
}
...
*/
static int backupOnePage(sqlite3_backup *p,  /* Backup handle */
    ...
    */
    for (iOff = iEnd - (i64)nSrcPgsz; rc == SQLITE_OK && iOff < iEnd;
         iOff += nDestPgsz) {
        DbPage *pDestPg = 0;
...
int sqlite3_backup_step(sqlite3_backup *p, int nPage) {
    ...
    if (!isFatalError(rc)) {
        ...
        */
        ...
        assert(nSrcPage >= 0);
        for (ii = 0; (nPage < 0 || ii < nPage) && p->iNext <= (Pgno)nSrcPage && !rc;
             ii++) {
                ...
                */
                ...
                if (pgszSrc < pgszDest) {
                    ...
                    ** journal file.  */
                    ...
                    for (iPg = nDestTruncate; rc == SQLITE_OK && iPg <= (Pgno)nDstPage;
                         iPg++) {
...
** called.
*/
static SQLITE_NOINLINE void backupUpdate(sqlite3_backup *p, Pgno iPage,
                                         const u8 *aData) {
    assert(p != 0);
    do {
...
int sqlite3BtreeCopyFile(Btree *pTo, Btree *pFrom) {
    ...
    if (pFd->pMethods) {
        i64 nByte =
            sqlite3BtreeGetPageSize(pFrom) * (i64)sqlite3BtreeLastPage(pFrom);
