...
*/
static int schemaCreate(sqlite3 *db, void *pAux, int argc,
...
static int schemaNext(sqlite3_vtab_cursor *cur) {
    ...
    while (!pCur->pColumnList || SQLITE_ROW != sqlite3_step(pCur->pColumnList)) {
        if (SQLITE_OK != (rc = finalize(&pCur->pColumnList)))
            goto next_exit;
...
*/
static int schemaFilter(sqlite3_vtab_cursor *pVtabCursor, int idxNum,
    ...
    rc =
        sqlite3_prepare(pVtab->db, "PRAGMA database_list", -1, &pCur->pDbList, 0);
