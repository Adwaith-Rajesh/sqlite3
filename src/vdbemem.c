...
int sqlite3VdbeCheckMemInvariants(Mem *p) {
    ...
    ** That saves a few cycles in inner loops. */
    ...
    if (p->flags & MEM_Null) {
        ...
        assert((p->flags & (MEM_Int | MEM_Real | MEM_Str | MEM_Blob | MEM_Agg)) ==
               0);
    ...
    if (p->flags & MEM_Int) {
        ...
        ** https://gcc.gnu.org/bugzilla/show_bug.cgi?id=96270 */
        ...
        sqlite3_str_appendf(&acc, "%!.15g",
                            (p->flags & MEM_IntReal) != 0 ? (double)p->u.i
                                                          : p->u.r);
...
int sqlite3VdbeMemValidStrRep(Mem *p) {
    ...
    if ((p->flags & (MEM_Int | MEM_Real | MEM_IntReal)) == 0)
        return 1;
...
void sqlite3VdbeMemZeroTerminateIfAble(Mem *pMem) {
    if ((pMem->flags & (MEM_Str | MEM_Term | MEM_Ephem | MEM_Static)) !=
        MEM_Str) {
...
int sqlite3VdbeMemStringify(Mem *pMem, u8 enc, u8 bForce) {
    ...
    if (bForce)
        pMem->flags &= ~(MEM_Int | MEM_Real | MEM_IntReal);
...
int sqlite3RealSameAsInt(double r1, sqlite3_int64 i) {
    double r2 = (double)i;
    return r1 == 0.0 || (memcmp(&r1, &r2, sizeof(r1)) == 0 &&
...
int sqlite3VdbeMemNumerify(Mem *pMem) {
    ...
    if ((pMem->flags & (MEM_Int | MEM_Real | MEM_IntReal | MEM_Null)) == 0) {
        ...
        if (((rc == 0 || rc == 1) &&
             sqlite3Atoi64(pMem->z, &ix, pMem->n, pMem->enc) <= 1) ||
...
int sqlite3VdbeMemSetStr(Mem *pMem,           /* Memory cell to set to string value */
    ...
    if (nByte < 0) {
        ...
        if (enc == SQLITE_UTF8) {
            ...
            for (nByte = 0; nByte <= iLimit && (z[nByte] | z[nByte + 1]);
                 nByte += 2) {
            }
        }
...
int sqlite3ValueIsOfClass(const sqlite3_value *pVal, void (*xFree)(void *)) {
    if (ALWAYS(pVal != 0) && ALWAYS((pVal->flags & (MEM_Str | MEM_Blob)) != 0) &&
        (pVal->flags & MEM_Dyn) != 0 && pVal->xDel == xFree) {
        return 1;
    } else {
        return 0;
    }
...
*/
int sqlite3ValueFromExpr(sqlite3 *db,          /* The database connection */
                         const Expr *pExpr,    /* The expression to evaluate */
                         u8 enc,               /* Encoding to use */
                         u8 affinity,          /* Affinity to use */
                         sqlite3_value **ppVal /* Write the new value here */
...
*/
void sqlite3ValueSetStr(sqlite3_value *v,    /* Value to be set */
...
int sqlite3ValueBytes(sqlite3_value *pVal, u8 enc) {
    ...
    if ((p->flags & MEM_Str) != 0 && enc != SQLITE_UTF8 &&
        pVal->enc != SQLITE_UTF8) {
        return p->n;
    }
