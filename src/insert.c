...
*/
void sqlite3OpenTable(Parse *pParse, /* Generate code into this VDBE */
    ...
    if (!pParse->db->noSharedCache) {
        sqlite3TableLock(pParse, iDb, pTab->tnum, (opcode == OP_OpenWrite) ? 1 : 0,
                         pTab->zName);
    }
    if (HasRowid(pTab)) {
...
*/
static SQLITE_NOINLINE const char *computeIndexAffStr(sqlite3 *db,
                                                      Index *pIdx) {
...
*/
static int autoIncBegin(Parse *pParse, /* Parsing context */
    ...
    if ((pTab->tabFlags & TF_Autoincrement) != 0 &&
        ...
        ** Ticket d8dc2b3a58cd5dc2918a1d4acb 2018-05-23 */
        if (pSeqTab == 0 || !HasRowid(pSeqTab) || NEVER(IsVirtual(pSeqTab)) ||
            pSeqTab->nCol != 2) {
            pParse->nErr++;
...
void sqlite3AutoincrementBegin(Parse *pParse) {
    ...
    ** only called from the top-level */
    ...
    for (p = pParse->pAinc; p; p = p->pNext) {
        ...
        static const VdbeOpList autoInc[] = {/* 0  */ {OP_Null, 0, 0, 0},
                                             /* 1  */ {OP_Rewind, 0, 10, 0},
                                             /* 2  */ {OP_Column, 0, 0, 0},
                                             /* 3  */ {OP_Ne, 0, 9, 0},
                                             /* 4  */ {OP_Rowid, 0, 0, 0},
...
static SQLITE_NOINLINE void autoIncrementEnd(Parse *pParse) {
    ...
    for (p = pParse->pAinc; p; p = p->pNext) {
        static const int iLn = VDBE_OFFSET_LINENO(2);
        static const VdbeOpList autoIncEnd[] = {/* 0 */ {OP_NotNull, 0, 2, 0},
                                                /* 1 */ {OP_NewRowid, 0, 0, 0},
        ...
        assert(sqlite3SchemaMutexHeld(db, 0, pDb->pSchema));
        sqlite3VdbeAddOp3(v, OP_Le, memId + 2, sqlite3VdbeCurrentAddr(v) + 7,
                          memId);
        VdbeCoverage(v);
...
#define isView 0
    ...
    */
    if (pColumn == 0 && pSelect != 0 && pTrigger == 0 &&
    ...
    */
     ...
     */
    if ((db->flags & SQLITE_CountRows) != 0 && !pParse->nested &&
        !pParse->pTriggerTab && !pParse->bReturning) {
    ...
    */
    ...
    for (i = 0; i < pTab->nCol; i++, iRegStore++) {
        ...
        if (pColumn) {
            ...
            sqlite3ExprCodeFactorable(pParse, sqlite3ColumnExpr(pTab, &pTab->aCol[i]),
                                      iRegStore);
            continue;
        } else {
            k = i - nHidden;
        ...
        if (useTempTable) {
            ...
            if (y != iRegStore) {
                sqlite3VdbeAddOp2(v,
                                  ExprHasProperty(pX, EP_Subquery) ? OP_Copy : OP_SCopy,
                                  y, iRegStore);
            }
        }
    }
    ...
    if (tmask & TRIGGER_BEFORE) {
        ...
        */
        ...
        sqlite3CodeRowTrigger(pParse, pTrigger, TK_INSERT, 0, TRIGGER_BEFORE, pTab,
                              regCols - pTab->nCol - 1, onError, endOfLoop);
    ...
    if (!isView) {
        ...
        if (ipkColumn >= 0) {
            ...
            */
            if (!appendFlag) {
                int addr1;
                if (!IsVirtual(pTab)) {
                    addr1 = sqlite3VdbeAddOp1(v, OP_NotNull, regRowid);
                    VdbeCoverage(v);
                    sqlite3VdbeAddOp3(v, OP_NewRowid, iDataCur, regRowid, regAutoinc);
                    sqlite3VdbeJumpHere(v, addr1);
                } else {
                    addr1 = sqlite3VdbeCurrentAddr(v);
                    sqlite3VdbeAddOp2(v, OP_IsNull, regRowid, addr1 + 2);
                    VdbeCoverage(v);
                }
        ...
        */
        ...
        if (IsVirtual(pTab)) {
            ...
            sqlite3VdbeAddOp4(v, OP_VUpdate, 1, pTab->nCol + 2, regIns, pVTab,
                              P4_VTAB);
...
#endif
        {
            ...
            ** functionality.  */
            ...
            sqlite3CompleteInsertion(pParse, pTab, iDataCur, iIdxCur, regIns, aRegIdx,
                                     0, appendFlag, bUseSeek);
        }
    ...
    if (pTrigger) {
        /* Code AFTER triggers */
        sqlite3CodeRowTrigger(pParse, pTrigger, TK_INSERT, 0, TRIGGER_AFTER, pTab,
                              regData - 2 - pTab->nCol, onError, endOfLoop);
    }
...
void sqlite3GenerateConstraintChecks(
    ...
    ** number of fields in the true primary key of the table. */
    ...
    VdbeModuleComment((v, "BEGIN: GenCnstCks(%d,%d,%d,%d,%d)", iDataCur, iIdxCur,
                       regNewData, regOldData, pkChng));
        ...
        while (1) {           /* Make 2 passes over columns. Exit loop via "break" */
            for (i = 0; i < nCol; i++) {
                ...
                switch (onError) {
                    ...
                    case OE_Abort:
                        sqlite3MayAbort(pParse);
                    /* no break */ deliberate_fall_through case OE_Rollback:
                    case OE_Fail: {
    ...
    if (pTab->pCheck && (db->flags & SQLITE_IgnoreChecks) == 0) {
        ...
        for (i = 0; i < pCheck->nExpr; i++) {
            ...
            if (onError == OE_Ignore) {
                ...
                if (onError == OE_Replace)
                    onError = OE_Abort; /* IMP: R-26383-51744 */
                sqlite3HaltConstraint(pParse, SQLITE_CONSTRAINT_CHECK, onError, zName,
            ...
            ** checked first and in order. */
            ...
            for (i = 0, pTerm = pUpsert; pTerm; pTerm = pTerm->pNextUpsert) {
                ...
                if (pTerm->pUpsertIdx == 0)
                    continue; /* Skip ON CONFLICT for the IPK */
                jj = 0;
                pIdx = pTab->pIndex;
    ...
    ** WITHOUT ROWID table.
    */
    for (pIdx = indexIteratorFirst(&sIdxIter, &ix); pIdx;
        ...
        ** is invoked.  */
        ...
        sqlite3VdbeVerifyAbortable(v, onError);
        addrConflictCk = sqlite3VdbeAddOp4Int(v, OP_NoConflict, iThisCur,
                                              addrUniqueOk, regIdx, pIdx->nKeyCol);
        VdbeCoverage(v);
                ...
                if (isUpdate) {
                    ...
                    ** KEY values of this row before the update.  */
                    ...
                    for (i = 0; i < pPk->nKeyCol; i++) {
                        ...
                        sqlite3VdbeAddOp4(v, op, regOldData + 1 + x, addrJump, regCmp + i,
                                          p4, P4_COLLSEQ);
                        sqlite3VdbeChangeP5(v, SQLITE_NOTNULL);
        ...
        assert(onError == OE_Rollback || onError == OE_Abort ||
               onError == OE_Fail || onError == OE_Ignore ||
               onError == OE_Replace || onError == OE_Update);
        switch (onError) {
            case OE_Rollback:
            case OE_Abort:
            case OE_Fail: {
            ...
            default: {
                ...
                sqlite3GenerateRowDelete(
                    pParse, pTab, pTrigger, iDataCur, iIdxCur, regR, nPkField, 0,
                    OE_Replace, (pIdx == pPk ? ONEPASS_SINGLE : ONEPASS_OFF), iThisCur);
                if (pTrigger && isUpdate) {
        ...
        if (pUpsertClause && upsertIpkReturn &&
...
#ifdef SQLITE_ENABLE_PREUPDATE_HOOK
static void
codeWithoutRowidPreupdate(Parse *pParse, /* Parse context */
...
void sqlite3CompleteInsertion(
    ...
    assert(update_flags == 0 || update_flags == OPFLAG_ISUPDATE ||
    ...
    for (i = 0, pIdx = pTab->pIndex; pIdx; pIdx = pIdx->pNext, i++) {
        ...
        assert(pIdx->onError != OE_Replace || pIdx->pNext == 0 ||
...
static int xferOptimization(
    ...
    for (i = 0; i < pDest->nCol; i++) {
        ...
        */
        if ((pDestCol->colFlags & COLFLAG_GENERATED) != 0) {
            if (sqlite3ExprCompare(0, sqlite3ColumnExpr(pSrc, pSrcCol),
        ...
        if ((pDestCol->colFlags & COLFLAG_GENERATED) == 0 && i > 0) {
            ...
            if ((pDestExpr == 0) != (pSrcExpr == 0) ||
                (pDestExpr != 0 &&
                 strcmp(pDestExpr->u.zToken, pSrcExpr->u.zToken) != 0)) {
    ...
    */
    ...
    for (pDestIdx = pDest->pIndex; pDestIdx; pDestIdx = pDestIdx->pNext) {
        ...
        } else if (!HasRowid(pSrc) &&
                   pDestIdx->idxType == SQLITE_IDXTYPE_PRIMARYKEY) {
            idxInsFlags |= OPFLAG_NCHANGE;
        }
        if (idxInsFlags != (OPFLAG_USESEEKRESULT | OPFLAG_PREFORMAT)) {
            sqlite3VdbeAddOp3(v, OP_RowData, iSrc, regData, 1);
            if ((db->mDbFlags & DBFLAG_Vacuum) == 0 && !HasRowid(pDest) &&
