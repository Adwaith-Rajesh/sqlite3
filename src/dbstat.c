...
static int statBestIndex(sqlite3_vtab *tab, sqlite3_index_info *pIdxInfo) {
    ...
    */
    if ((pIdxInfo->nOrderBy == 1 && pIdxInfo->aOrderBy[0].iColumn == 0 &&
        ...
        (pIdxInfo->nOrderBy == 2 && pIdxInfo->aOrderBy[0].iColumn == 0 &&
         pIdxInfo->aOrderBy[0].desc == 0 && pIdxInfo->aOrderBy[1].iColumn == 1 &&
...
*/
static int getLocalPayload(int nUsable, /* Usable bytes per page */
                           u8 flags,    /* Page flags */
...
*/
static int statGetPage(Btree *pBt,   /* Load page from this b-tree */
    ...
    if (pCsr->iPage < 0) {
        ...
        while (p->iCell < p->nCell) {
            ...
            while (pCell->iOvfl < pCell->nOvfl) {
                ...
                sqlite3BtreeEnter(pBt);
                nUsable =
                    sqlite3BtreeGetPageSize(pBt) - sqlite3BtreeGetReserveNoMutex(pBt);
                sqlite3BtreeLeave(pBt);
                pCsr->nPage++;
                ...
                if (!pCsr->isAgg) {
                    ...
                    pCsr->zPath = z =
                        sqlite3_mprintf("%s%.3x+%.6x", p->zPath, p->iCell, iOvfl);
    ...
    */
    if (rc == SQLITE_OK) {
        ...
        if (rc == SQLITE_OK) {
            ...
            if (p->nMxPayload > pCsr->nMxPayload)
...
*/
static int statFilter(sqlite3_vtab_cursor *pCursor, int idxNum,
...
static int statColumn(sqlite3_vtab_cursor *pCursor, sqlite3_context *ctx,
                      int i) {
    StatCursor *pCsr = (StatCursor *)pCursor;
    switch (i) {
        case 0: /* name */
