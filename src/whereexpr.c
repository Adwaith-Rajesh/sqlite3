...
static int whereClauseInsert(WhereClause *pWC, Expr *p, u16 wtFlags) {
    ...
    if (pWC->nTerm >= pWC->nSlot) {
        ...
        pWC->a =
            sqlite3WhereMalloc(pWC->pWInfo, sizeof(pWC->a[0]) * pWC->nSlot * 2);
        if (pWC->a == 0) {
...
static u16 exprCommute(Parse *pParse, Expr *pExpr) {
    if (pExpr->pLeft->op == TK_VECTOR || pExpr->pRight->op == TK_VECTOR ||
            ...
            if (pPrefix) {
                ...
                */
                if (pLeft->op != TK_COLUMN ||
                    ...
                    (ALWAYS(ExprUseYTab(pLeft)) && ALWAYS(pLeft->y.pTab) &&
        ...
        */
        ...
        if (ExprIsVtab(pCol)) {
            ...
            if (pMod->xFindFunction != 0) {
                i = pMod->xFindFunction(pVtab, 2, pExpr->u.zToken, &xNotUsed,
                                        &pNotUsed);
        ...
        assert(pLeft->op != TK_COLUMN ||
               (ExprUseYTab(pLeft) && pLeft->y.pTab != 0));
        if (ExprIsVtab(pLeft)) {
            res++;
        }
...
whereCombineDisjuncts(SrcList *pSrc,    /* the FROM clause */
    ...
    if ((pOne->wtFlags | pTwo->wtFlags) & TERM_VNULL)
        return;
    ...
    if (sqlite3ExprCompare(0, pOne->pExpr->pLeft, pTwo->pExpr->pLeft, -1))
        return;
    if (sqlite3ExprCompare(0, pOne->pExpr->pRight, pTwo->pExpr->pRight, -1))
        return;
            ...
            for (; i >= 0 && okToChngToIN; i--, pOrTerm++) {
                ...
                if (pOrTerm->leftCursor != iCursor) {
                    pOrTerm->wtFlags &= ~TERM_OK;
                           ...
                           (iColumn == XN_EXPR &&
                            sqlite3ExprCompare(pParse, pOrTerm->pExpr->pLeft, pLeft,
                                               -1))) {
                    okToChngToIN = 0;
                } else {
    ...
    do {
        ...
        for (pIdx = pFrom->a[j].pTab->pIndex; pIdx; pIdx = pIdx->pNext) {
            ...
            for (i = 0; i < pIdx->nKeyCol; i++) {
                ...
                if (sqlite3ExprCompareSkip(pExpr, pIdx->aColExpr->a[i].pExpr, iCur) ==
                        0 &&
                    pExpr->op != TK_STRING) {
                    aiCurCol[0] = iCur;
                    aiCurCol[1] = XN_EXPR;
...
static void exprAnalyze(SrcList *pSrc,    /* the FROM clause */
    ...
    if (allowedOp(op)) {
        ...
        if (pRight && exprMightBeIndexed(pSrc, aiCurCol, pRight, op) &&
        ...
        } else if (op == TK_ISNULL && !ExprHasProperty(pExpr, EP_OuterON) &&
    ...
    */
        ...
        for (i = 0; i < 2; i++) {
            ...
            int idxNew;
            pNewExpr =
                sqlite3PExpr(pParse, ops[i], sqlite3ExprDup(db, pExpr->pLeft, 0),
    ...
    */
        ...
        if (pExpr->pLeft->op == TK_COLUMN && pExpr->pLeft->iColumn >= 0 &&
            ...
            pNewExpr = sqlite3PExpr(pParse, TK_GT, sqlite3ExprDup(db, pLeft, 0),
    ...
    ** BLOBs.
    */
    else if (pExpr->op == TK_FUNCTION && pWC->op == TK_AND &&
        ...
        */
        ...
        pNewExpr1 = sqlite3PExpr(
            pParse, TK_GE,
            sqlite3ExprAddCollateString(pParse, pNewExpr1, zCollSeqName), pStr1);
        ...
        pNewExpr2 = sqlite3PExpr(
            pParse, TK_LT,
            sqlite3ExprAddCollateString(pParse, pNewExpr2, zCollSeqName), pStr2);
    ...
    */
    else if (pExpr->op == TK_IN && pTerm->u.x.iField == 0 &&
             pExpr->pLeft->op == TK_VECTOR && ALWAYS(ExprUseXSelect(pExpr)) &&
             (pExpr->x.pSelect->pPrior == 0 ||
              (pExpr->x.pSelect->selFlags & SF_Values))
    ...
    */
        ...
        while (res-- > 0) {
            ...
            if ((prereqExpr & prereqColumn) == 0) {
                Expr *pNewExpr;
                pNewExpr =
                    sqlite3PExpr(pParse, TK_MATCH, 0, sqlite3ExprDup(db, pRight, 0));
...
void SQLITE_NOINLINE sqlite3WhereAddLimit(WhereClause *pWC, Select *p) {
    ...
    if (p->pGroupBy == 0 &&
        ...
        if (pOrderBy) {
            for (ii = 0; ii < pOrderBy->nExpr; ii++) {
                ...
                if (pExpr->iTable != iCsr)
                    return;
                if (pOrderBy->a[ii].fg.sortFlags & KEYINFO_ORDER_BIGNULL)
                    return;
            }
        }
        ...
        whereAddLimitExpr(pWC, p->iLimit, p->pLimit->pLeft, iCsr,
                          SQLITE_INDEX_CONSTRAINT_LIMIT);
        if (p->iOffset > 0) {
            whereAddLimitExpr(pWC, p->iOffset, p->pLimit->pRight, iCsr,
...
*/
static SQLITE_NOINLINE Bitmask sqlite3WhereExprUsageFull(WhereMaskSet *pMaskSet,
                                                         Expr *p) {
    Bitmask mask;
    mask =
        (p->op == TK_IF_NULL_ROW) ? sqlite3WhereGetMask(pMaskSet, p->iTable) : 0;
    if (p->pLeft)
    ...
    if (p->pRight) {
        ...
        if (ExprHasProperty(p, EP_VarSelect))
...
void sqlite3WhereTabFuncArgs(
    ...
    for (j = k = 0; j < pArgs->nExpr; j++) {
        Expr *pRhs;
        u32 joinType;
        while (k < pTab->nCol && (pTab->aCol[k].colFlags & COLFLAG_HIDDEN) == 0) {
            k++;
        }
        if (k >= pTab->nCol) {
