...
*/
static VdbeCursor *
allocateCursor(Vdbe *p,    /* The virtual machine */
    ...
    */
    ...
    int nByte;
    VdbeCursor *pCx = 0;
    nByte = ROUND8P(sizeof(VdbeCursor)) + 2 * sizeof(u32) * nField +
    ...
    ** to hold cursors, it is faster to in-line the logic. */
    ...
    if (eCurType == CURTYPE_BTREE) {
        pCx->uc.pCursor =
            (BtCursor *)&pMem
                ->z[ROUND8P(sizeof(VdbeCursor)) + 2 * sizeof(u32) * nField];
...
static void applyNumericAffinity(Mem *pRec, int bTryForInt) {
    ...
    u8 enc = pRec->enc;
    int rc;
    assert((pRec->flags & (MEM_Str | MEM_Int | MEM_Real | MEM_IntReal)) ==
           MEM_Str);
...
*/
static void applyAffinity(Mem *pRec,     /* The value to apply affinity to */
...
static u16 numericType(Mem *pMem) {
    assert((pMem->flags & MEM_Null) == 0 || pMem->db == 0 ||
...
static int checkSavepointCount(sqlite3 *db) {
    int n = 0;
    Savepoint *p;
    for (p = db->pSavepoint; p; p = p->pNext)
        n++;
...
static SQLITE_NOINLINE int vdbeColumnFromOverflow(
    ...
    if (len > 4000 && pC->pKeyInfo == 0) {
        ...
        */
        ...
        pCache = pC->pCache;
        if (pCache->pCValue == 0 || pCache->iCol != iCol ||
...
static const char *vdbeMemTypeName(Mem *pMem) {
    static const char *azTypes[] = {/* SQLITE_INTEGER */ "INT",
                                    /* SQLITE_FLOAT   */ "REAL",
                                    /* SQLITE_TEXT    */ "TEXT",
                                    /* SQLITE_BLOB    */ "BLOB",
                                    /* SQLITE_NULL    */ "NULL"};
...
int sqlite3VdbeExec(Vdbe *p /* The VDBE */
    ...
    if (p->pc == 0 && (p->db->flags & (SQLITE_VdbeListing | SQLITE_VdbeEQP |
                                       SQLITE_VdbeTrace)) != 0) {
        int i;
        int once = 1;
        ...
        if (p->db->flags & SQLITE_VdbeTrace)
            printf("VDBE Trace:\n");
    }
        ...
        switch (pOp->opcode) {
            ...
            */
            ...
            check_for_interrupt:
                ...
                ** If the progress callback returns non-zero, exit the virtual machine
                *with
            ...
            */
            case OP_Halt: {
                ...
                ** something is wrong with the code generator.  Raise an assertion in
                *order
                ** to bring this to the attention of fuzzers and other testing tools. */
                ...
                if (p->rc) {
                    ...
                    pcx = (int)(pOp - aOp);
                    sqlite3_log(pOp->p1, "abort at %d in [%s]: %s", pcx, p->zSql,
                                p->zErrMsg);
                }
            ...
            */
            case OP_String8: { /* same as TK_STRING, out2 */
                ...
                if (encoding != SQLITE_UTF8) {
                    rc = sqlite3VdbeMemSetStr(pOut, pOp->p4.z, -1, SQLITE_UTF8,
                                              SQLITE_STATIC);
            ...
            */
            case OP_String: { /* out2 */
                ...
                if (pOp->p3 > 0) {
                    ...
                    if (pIn3->u.i == pOp->p5)
                        pOut->flags = MEM_Blob | MEM_Static | MEM_Term;
                }
#endif
                break;
            }
            ...
            */
            case OP_Variable: { /* out2 */
                ...
                assert(pOp->p4.z == 0 ||
                       pOp->p4.z == sqlite3VListNumToName(p->pVList, pOp->p1));
            ...
            *either
            ** true or false and is never NULL.  If both operands are NULL then the
            *result
            ** of comparison is true.  If either operand is NULL then the result is
            *false.
            ...
            case OP_ElseEq: { /* same as TK_ESCAPE, jump */
                ...
                ** OP_Gt with zero or more intervening OP_ReleaseReg opcodes */
                ...
                for (iAddr = (int)(pOp - aOp) - 1; ALWAYS(iAddr >= 0); iAddr--) {
                    if (aOp[iAddr].opcode == OP_ReleaseReg)
                        continue;
                ...
                for (i = 0; i < n; i++) {
                    ...
                    if (iCompare) {
                        if ((pKeyInfo->aSortFlags[i] & KEYINFO_ORDER_BIGNULL) &&
                            ((aMem[p1 + idx].flags & MEM_Null) ||
                             (aMem[p2 + idx].flags & MEM_Null))) {
                            iCompare = -iCompare;
                        }
                        if (bRev)
                            iCompare = -iCompare;
            ...
            */
            case OP_IsTrue: { /* in1, out2 */
                ...
                sqlite3VdbeMemSetInt64(&aMem[pOp->p2],
                                       sqlite3VdbeBooleanValue(&aMem[pOp->p1], pOp->p3) ^
                                           pOp->p4.i);
                break;
            }
            ...
            ** that the datatype is REAL when it should be NULL.  When P1<0 and the
            *value
            ...
            */
            case OP_IsType: { /* jump */
                ...
                assert(pOp->p1 >= 0 ||
                       (pOp->p3 >= 0 && pOp->p3 <= (p->nMem + 1 - p->nCursor)));
                if (pOp->p1 >= 0) {
                    pC = p->apCsr[pOp->p1];
                    assert(pC != 0);
                    assert(pOp->p3 >= 0);
                    if (pOp->p3 < pC->nHdrParsed) {
                        ...
                        if (serialType >= 12) {
                            ...
                            static const unsigned char aMask[] = {0x10, 0x01, 0x01, 0x01,
                                                                  0x01, 0x01, 0x01, 0x2,
                                                                  0x01, 0x01, 0x10, 0x10};
                    ...
                    assert(memIsValid(&aMem[pOp->p3]));
                    typeMask =
                        1 << (sqlite3_value_type((sqlite3_value *)&aMem[pOp->p3]) - 1);
                    testcase(typeMask == 0x01);
                    testcase(typeMask == 0x02);
                    testcase(typeMask == 0x04);
                    testcase(typeMask == 0x08);
            ...
            **
            ** Store in register r[P3] the byte offset into the database file that is
            *the
            ...
            op_column_restart:
                ...
                if (pC->cacheStatus != p->cacheCtr) { /*OPTIMIZATION-IF-FALSE*/
                    ...
                    if (pC->szRow < aOffset[0]) { /*OPTIMIZATION-IF-FALSE*/
                        ...
                        ** need to cover the header of the record.  If pC->aRow does not
                        *contain
                        ...
                        ** accept it for historical reasons.  When aOffset[0]==0, the code
                        *this
                        ** branch jumps to reads past the end of the record, but never more
                        ** than a few bytes.  Even if the record occurs at the end of the page
                        ** content area, the "page header" comes after the page content and so
                        ** this overread is harmless.  Similar overreads can occur for a
                        *corrupt
                        ** database file.
                        */
                        zData = pC->aRow;
                ...
                */
                ...
                if (pC->szRow >= aOffset[p2 + 1]) {
                    /* This is the common case where the desired content fits on the
                     *original
                    ...
                    if (((p5 = (pOp->p5 & OPFLAG_BYTELENARG)) != 0 &&
                        ...
                        */
                        ...
                        rc = vdbeColumnFromOverflow(pC, p2, t, aOffset[p2], p->cacheCtr,
                                                    colCacheCtr, pDest);
                        if (rc) {
                            if (rc == SQLITE_NOMEM)
                                goto no_mem;
            ...
            */
            case OP_TypeCheck: {
                ...
                for (i = 0; i < pTab->nCol; i++) {
                    if (aCol[i].colFlags & COLFLAG_GENERATED) {
                        if (aCol[i].colFlags & COLFLAG_VIRTUAL)
                            continue;
                        if (pOp->p3) {
                            pIn1++;
                            continue;
            ...
            vdbe_type_error:
                sqlite3VdbeError(
                    p, "cannot store %s value in %s column %s.%s", vdbeMemTypeName(pIn1),
            ...
            */
            case OP_MakeRecord: {
                    ...
                    if (pRec->flags & MEM_Null) {
                        ...
                        if (uu <= 127) {
                            ...
                            if (pRec->flags & MEM_IntReal) {
                                /* If the value is IntReal and is going to take up 8 bytes to
                                 *store
                ...
                ** sqlite3VdbeMemClearAndResize() could clobber the value before it is
                *used).
                */
                ...
                */
                ...
                if (p1 == SAVEPOINT_BEGIN) {
                    if (db->nVdbeWrite > 0) {
                        ...
                        */
                        sqlite3VdbeError(
                            p, "cannot open savepoint - SQL statements in progress");
                        rc = SQLITE_BUSY;
                    } else {
                    ...
                    ** an error is returned to the user.  */
                    ...
                    if (!pSavepoint) {
                        ...
                        */
                        int isTransaction =
                            pSavepoint->pNext == 0 && db->isTransactionSavepoint;
            ...
            ** read-transaction is already active, it is upgraded to a
            *write-transaction.
            ...
            ** active statements. A statement transaction allows the changes made by
            *this
            ...
            */
            case OP_Transaction: {
                ...
                assert(rc == SQLITE_OK);
                if (pOp->p2 &&
                    (db->flags & (SQLITE_QueryOnly | SQLITE_CorruptRdOnly)) != 0) {
                ...
                if (rc == SQLITE_OK && pOp->p5 &&
                    (iMeta != pOp->p3 || pDb->pSchema->iGeneration != pOp->p4.i)) {
                    /*
                    ** IMPLEMENTATION-OF: R-03189-51135 As each SQL statement runs, the
                    *schema
                    ** version is checked to ensure that the schema has not changed since
                    *the
                    ...
                    */
                    ...
                    /* Set changeCntOn to 0 to prevent the value returned by
                     *sqlite3_changes()
            ...
            **       equality lookups (implemented as a pair of opcodes
            *OP_SeekGE/OP_IdxGT
            ...
            ** cursor P2.  The P2 cursor must have been opened by a prior
            *OP_OpenEphemeral
            ** opcode.  Only ephemeral cursors may be duplicated.
            **
            ** Duplicate ephemeral cursors are used for self-joins of materialized
            *views.
            */
                    ...
                    ** empty again, rather than creating a new table. */
                    ...
                    if (rc == SQLITE_OK) {
                        ...
                        if (rc == SQLITE_OK) {
                            ...
                            ** automatically created table with root-page 1 (an BLOB_INTKEY
                            *table).
                            */
                            if ((pCx->pKeyInfo = pKeyInfo = pOp->p4.pKeyInfo) != 0) {
                                ...
                                if (rc == SQLITE_OK) {
                                    ...
                                    assert(pKeyInfo->db == db);
                                    assert(pKeyInfo->enc == ENC(db));
                                    rc = sqlite3BtreeCursor(pCx->ub.pBtx, pCx->pgnoRoot,
                                                            BTREE_WRCSR, pKeyInfo, pCx->uc.pCursor);
                                }
                                pCx->isTable = 0;
                            } else {
            ...
            case OP_SeekGT: {     /* jump, in3, group, ncycle */
                ...
                if (pC->isTable) {
                    ...
                    ** the seek, so convert it. */
                    pIn3 = &aMem[pOp->p3];
                    flags3 = pIn3->flags;
                    if ((flags3 & (MEM_Int | MEM_Real | MEM_IntReal | MEM_Str)) ==
                        MEM_Str) {
                        applyNumericAffinity(pIn3, 0);
                    }
                    iKey = sqlite3VdbeIntValue(pIn3); /* Get the integer key value */
                    newType =
                        pIn3->flags;      /* Record the type after applying numeric affinity */
                    ...
                    */
...
#ifdef SQLITE_DEBUG
                    {
                        ...
                        for (i = 0; i < r.nField; i++) {
                            ...
                            if (i > 0)
                                REGISTER_TRACE(pOp->p3 + i, &r.aMem[i]);
                        }
                    }
#endif
                    r.eqSeen = 0;
            ...
            ** then this opcode is a no-op and control passes through into the
            *OP_SeekGE.
            **
            ...
            */
            case OP_SeekScan: { /* ncycle */
                ...
                /* If pOp->p5 is clear, then pOp->p2 points to the first instruction past
                 *the
            ...
            case OP_Found: {    /* jump, in3, ncycle */
                ...
                if (r.nField > 0) {
                    ...
                    for (ii = 0; ii < r.nField; ii++) {
                        ...
                        if (ii)
                            REGISTER_TRACE(pOp->p3 + ii, &r.aMem[ii]);
                    }
#endif
            ...
            ** instruction.
            **
            ** The OP_NotExists opcode performs the same operation, but with
            *OP_NotExists
            ** the P3 register must be guaranteed to contain an integer value.  With
            *this
            ...
            */
            ...
            /* no break */ deliberate_fall_through case OP_NotExists: /* jump, in3,
                                                                         ncycle */
                pIn3 = &aMem[pOp->p3];
...
#define MAX_ROWID (i64)((((u64)0x7fffffff) << 32) | (u64)0xffffffff)
...
#endif
                    if (pC->useRandomRowid) {
                        /* IMPLEMENTATION-OF: R-07677-41881 If the largest ROWID is equal to
                         *the
                        ...
                        do {
                            ...
                            v &= (MAX_ROWID >> 1);
                            v++; /* Ensure that v is greater than zero */
                        } while (((rc = sqlite3BtreeTableMoveto(pC->uc.pCursor, (u64)v, 0,
                                                                &res)) == SQLITE_OK) &&
                                 (res == 0) && (++cnt < 100));
                        if (rc)
                            goto abort_due_to_error;
            ...
            ** the OPFLAG_USESEEKRESULT flag must only be set if there have been no
            *prior
            ...
            case OP_Insert: {
                ...
                if (pTab) {
                    if (db->xPreUpdateCallback && !(pOp->p5 & OPFLAG_ISUPDATE)) {
                        sqlite3VdbePreUpdateHook(p, pC, SQLITE_INSERT, zDb, pTab, x.nKey,
                                                 pOp->p2, -1);
                    }
                ...
                if (pOp->p5 & OPFLAG_ISNOOP)
                    break;
#endif

                assert((pOp->p5 & OPFLAG_LASTROWID) == 0 ||
                       (pOp->p5 & OPFLAG_NCHANGE) != 0);
                ...
                if (pTab) {
                    ...
                    db->xUpdateCallback(db->pUpdateArg,
                                        (pOp->p5 & OPFLAG_ISUPDATE) ? SQLITE_UPDATE
                                                                    : SQLITE_INSERT,
            ...
            */
            case OP_Delete: {
                ...
                if (pOp->p4type == P4_TABLE && HasRowid(pOp->p4.pTab) && pOp->p5 == 0 &&
                    ...
                    ** OP_Delete will have also set the pC->movetoTarget field to the rowid
                    *of
                ...
                ** VdbeCursor.movetoTarget to the current rowid.  */
                ...
                if (db->xPreUpdateCallback && pTab) {
                    assert(!(opflags & OPFLAG_ISUPDATE) || HasRowid(pTab) == 0 ||
                           (aMem[pOp->p3].flags & MEM_Int));
                    sqlite3VdbePreUpdateHook(
                        p, pC, (opflags & OPFLAG_ISUPDATE) ? SQLITE_UPDATE : SQLITE_DELETE,
                        zDb, pTab, pC->movetoTarget, pOp->p3, -1);
                }
                if (opflags & OPFLAG_ISNOOP)
                    break;
#endif
...
#ifdef SQLITE_DEBUG
                if (p->pFrame == 0) {
                    if (pC->isEphemeral == 0 && (pOp->p5 & OPFLAG_AUXDELETE) == 0 &&
            ...
            ** us from having to issue a separate NullRow instruction to clear that
            *cache.
            */
            ...
            */
            case OP_IfSmaller: { /* jump */
                ...
                if (res == 0) {
                    ...
                    if (ALWAYS(sz >= 0) && sqlite3LogEst((u64)sz) < pOp->p3)
                        res = 1;
                }
                VdbeBranchTaken(res != 0, 2);
                if (res)
            ...
            */
            case OP_SorterNext: { /* jump */
                ...
                case OP_Prev: /* jump, ncycle */
                    assert(pOp->p1 >= 0 && pOp->p1 < p->nCursor);
                    assert(pOp->p5 == 0 || pOp->p5 == SQLITE_STMTSTATUS_FULLSCAN_STEP ||
            ...
            */
            case OP_IdxDelete: {
                ...
                assert(pOp->p3 > 0);
                assert(pOp->p2 > 0 &&
                       pOp->p2 + pOp->p3 <= (p->nMem + 1 - p->nCursor) + 1);
                assert(pOp->p1 >= 0 && pOp->p1 < p->nCursor);
                pC = p->apCsr[pOp->p1];
                assert(pC != 0);
                assert(pC->eCurType == CURTYPE_BTREE);
                ...
                if (res == 0) {
                    ...
                    rc = sqlite3ReportError(SQLITE_CORRUPT_INDEX, __LINE__,
                                            "index corruption");
                    goto abort_due_to_error;
                }
                assert(pC->deferredMoveto == 0);
            ...
            case OP_ParseSchema: {
                ...
                */
...
#endif
                {
                    ...
                    zSql =
                        sqlite3MPrintf(db, "SELECT*FROM\"%w\".%s WHERE %s ORDER BY rowid",
                    ...
                    if (zSql == 0) {
                        ...
                        if (rc == SQLITE_OK && initData.nInitRow == 0) {
                            /* The OP_ParseSchema opcode with a non-NULL P4 argument should
                             *parse
            ...
            **
            ** The register P3 contains one less than the maximum number of allowed
            *errors.
            ...
            */
            case OP_Program: {        /* jump */
                ...
                ** variable.  */
                if (pOp->p5) {
                    t = pProgram->token;
                    for (pFrame = p->pFrame; pFrame && pFrame->token != t;
                ...
                ** is already allocated. Otherwise, it must be initialized.  */
                if ((pRt->flags & MEM_Blob) == 0) {
                    ...
                    */
                    ...
                    if (pProgram->nCsr == 0)
                        nMem++;
                    nByte = ROUND8(sizeof(VdbeFrame)) + nMem * sizeof(Mem) +
                            pProgram->nCsr * sizeof(VdbeCursor *) + (pProgram->nOp + 7) / 8;
                ...
                ** try to reuse register values from the first use. */
                {
                    ...
                    for (i = 0; i < p->nMem; i++) {
                        ...
                        MemSetTypeFlag(&aMem[i],
                                       MEM_Undefined); /* Fault if this reg is reused */
                    }
                }
#endif
                pOp = &aOp[-1];
            ...
            ** instruction.
            **
            ** If P1 is non-zero, then the jump is taken if the database
            *constraint-counter
            ...
            ** RESTART, or TRUNCATE.  Write 1 or 0 into mem[P3] if the checkpoint
            *returns
            ...
            */
            case OP_JournalMode: { /* out2 */
                ...
                assert(eNew == PAGER_JOURNALMODE_DELETE ||
                       ...
                       eNew == PAGER_JOURNALMODE_WAL || eNew == PAGER_JOURNALMODE_QUERY);
                assert(pOp->p1 >= 0 && pOp->p1 < db->nDb);
                assert(p->readOnly == 0);
                ...
                */
                ...
                if ((eNew != eOld) &&
                    ...
                    if (!db->autoCommit || db->nVdbeRead > 1) {
                        rc = SQLITE_ERROR;
                        sqlite3VdbeError(
                            p, "cannot change %s wal mode from within a transaction",
                        ...
                        */
                        ...
                        if (rc == SQLITE_OK) {
                            rc = sqlite3BtreeSetVersion(
                                pBt, (eNew == PAGER_JOURNALMODE_WAL ? 2 : 1));
                        }
                    }
                }
            ...
            ** The P2==1 case occurs when a CREATE INDEX or similar schema change
            *happens
            ...
            */
            case OP_VRename: {
                ...
                if (isLegacy == 0)
                    db->flags &= ~(u64)SQLITE_LegacyAlter;
                sqlite3VtabImportErrmsg(p, pVtab);
                p->expired = 0;
                if (rc)
                    goto abort_due_to_error;
            ...
            */
            ...
            case OP_Function: { /* group */
                ...
                ** reinitializes the relevant parts of the sqlite3_context object */
                ...
                assert((pOut->flags & MEM_Str) == 0 || pOut->enc == encoding ||
                       db->mallocFailed);
            ...
            */
            ...
            case OP_Init: { /* jump */
                ...
                */
                assert(pOp->p4.z == 0 || strncmp(pOp->p4.z,
                                                 "-"
                                                 "- ",
                                                 3) == 0);
                ...
                if (zTrace) {
                    int j;
                    for (j = 0; j < db->nDb; j++) {
                        if (DbMaskTest(p->btreeMask, j) == 0)
                            continue;
                        sqlite3_file_control(db, db->aDb[j].zDbSName, SQLITE_FCNTL_TRACE,
                                             zTrace);
                    }
                }
            ...
            */
            case OP_CursorHint: {
                ...
                if (pC) {
                    assert(pC->eCurType == CURTYPE_BTREE);
                    sqlite3BtreeCursorHint(pC->uc.pCursor, BTREE_HINT_RANGE, pOp->p4.pExpr,
                                           aMem);
                }
                break;
            }
            ...
            ** a change to the value of the source register for the OP_SCopy will no
            *longer
            ...
            ** validate the generated bytecode.  This opcode does not actually
            *contribute
            ...
            case OP_ReleaseReg: {
                ...
                for (i = 0; i < pOp->p2; i++, pMem++) {
                    if (i >= 32 || (constMask & MASKBIT32(i)) == 0) {
                        pMem->pScopyFrom = 0;
                        if (i < 32 && pOp->p5)
                            MemSetTypeFlag(pMem, MEM_Undefined);
                    }
                }
                break;
            }
            ...
            */
            default: { /* This is really OP_Noop, OP_Explain */
                ...
                /*****************************************************************************
                ** The cases of the switch statement above this line should all be
                *indented
                ** by 6 spaces.  But the left-most 6 spaces have been removed to improve
                *the
...
abort_due_to_error:
    ...
    sqlite3_log(rc, "statement aborts at %d: [%s] %s", (int)(pOp - aOp), p->zSql,
                p->zErrMsg);
