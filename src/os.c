...
int sqlite3OsShmMap(sqlite3_file *id, /* Database file handle */
                    int iPage, int pgsz,
...
int sqlite3OsFullPathname(sqlite3_vfs *pVfs, const char *zPath, int nPathOut,
                          char *zPathOut) {
    DO_OS_MALLOC_TEST(0);
    zPathOut[0] = 0;
...
int sqlite3OsRandomness(sqlite3_vfs *pVfs, int nByte, char *zBufOut) {
    if (sqlite3Config.iPrngSeed) {
        ...
        if (ALWAYS(nByte > (signed)sizeof(unsigned)))
...
int sqlite3OsOpenMalloc(sqlite3_vfs *pVfs, const char *zFile,
