...
*/
int sqlite3HeapNearlyFull(void) { return AtomicLoad(&mem0.nearlyFull); }
...
static SQLITE_NOINLINE void *dbMallocRawFinish(sqlite3 *db, u64 n) {
    ...
    if (!p)
        sqlite3OomFault(db);
    sqlite3MemdebugSetType(p, (db->lookaside.bDisable == 0) ? MEMTYPE_LOOKASIDE
                                                            : MEMTYPE_HEAP);
    return p;
}
...
static SQLITE_NOINLINE void *dbReallocFinish(sqlite3 *db, void *p, u64 n) {
    ...
    if (db->mallocFailed == 0) {
        if (isLookaside(db, p)) {
            ...
            assert(sqlite3MemdebugHasType(p, (MEMTYPE_LOOKASIDE | MEMTYPE_HEAP)));
            assert(
                sqlite3MemdebugNoType(p, (u8) ~(MEMTYPE_LOOKASIDE | MEMTYPE_HEAP)));
            sqlite3MemdebugSetType(p, MEMTYPE_HEAP);
...
char *sqlite3DbSpanDup(sqlite3 *db, const char *zStart, const char *zEnd) {
    ...
    ** at least one non-space character */
    for (n = 0; sqlite3Isspace(zStart[n]); n++) {
        assert(&zStart[n] < zEnd);
    }
#endif
    while (sqlite3Isspace(zStart[0]))
        zStart++;
    n = (int)(zEnd - zStart);
    while (sqlite3Isspace(zStart[n - 1]))
        n--;
...
void *sqlite3OomFault(sqlite3 *db) {
    if (db->mallocFailed == 0 && db->bBenignMalloc == 0) {
        ...
        if (db->pParse) {
            ...
            for (pParse = db->pParse->pOuterParse; pParse;
