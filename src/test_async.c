...
*/
static int SQLITE_TCLAPI testAsyncInit(void *clientData, Tcl_Interp *interp,
...
*/
static int SQLITE_TCLAPI testAsyncShutdown(void *clientData, Tcl_Interp *interp,
                                           int objc, Tcl_Obj *CONST objv[]) {
    sqlite3async_shutdown();
    return TCL_OK;
}
...
*/
static int SQLITE_TCLAPI testAsyncStart(void *clientData, Tcl_Interp *interp,
                                        int objc, Tcl_Obj *CONST objv[]) {
...
*/
static int SQLITE_TCLAPI testAsyncWait(void *clientData, Tcl_Interp *interp,
                                       int objc, Tcl_Obj *CONST objv[]) {
    int eCond;
    if (objc != 1) {
...
*/
static int SQLITE_TCLAPI testAsyncControl(void *clientData, Tcl_Interp *interp,
                                          int objc, Tcl_Obj *CONST objv[]) {
    int rc = SQLITE_OK;
    ...
    if (rc == SQLITE_OK) {
        rc = sqlite3async_control(eOpt == SQLITEASYNC_HALT ? SQLITEASYNC_GET_HALT
                                  : eOpt == SQLITEASYNC_DELAY
...
int Sqlitetestasync_Init(Tcl_Interp *interp) {
    ...
    Tcl_CreateObjCommand(interp, "sqlite3async_shutdown", testAsyncShutdown, 0,
                         0);
#endif /* SQLITE_ENABLE_ASYNCIO */
    return TCL_OK;
}
