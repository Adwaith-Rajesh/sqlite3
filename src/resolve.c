...
*/
int sqlite3MatchEName(const struct ExprList_item *pItem, const char *zCol,
                      const char *zTab, const char *zDb, int *pbRowid) {
    int n;
    const char *zSpan;
    ...
    zSpan += n + 1;
    if (zCol) {
        if (eEName == ENAME_TAB && sqlite3StrICmp(zSpan, zCol) != 0)
            return 0;
        if (eEName == ENAME_ROWID && sqlite3IsRowid(zCol) == 0)
            return 0;
    }
...
*/
static void
extendFJMatch(Parse *pParse,     /* Parsing context */
...
static SQLITE_NOINLINE int isValidSchemaTableName(
    ...
    assert(pTab != 0);
    assert(pTab->tnum == 1);
    if (sqlite3StrNICmp(zTab, "sqlite_", 7) != 0)
        return 0;
    zLegacy = pTab->zName;
    if (strcmp(zLegacy + 7, &LEGACY_TEMP_SCHEMA_TABLE[7]) == 0) {
        ...
        if (pSchema == 0)
            return 0;
        if (sqlite3StrICmp(zTab + 7, &LEGACY_SCHEMA_TABLE[7]) == 0)
            return 1;
        if (sqlite3StrICmp(zTab + 7, &PREFERRED_SCHEMA_TABLE[7]) == 0)
            return 1;
    } else {
        if (sqlite3StrICmp(zTab + 7, &PREFERRED_SCHEMA_TABLE[7]) == 0)
            return 1;
...
lookupName(Parse *pParse,    /* The parsing context */
    ...
    */
    ...
    do {
            ...
            for (i = 0, pItem = pSrcList->a; i < pSrcList->nSrc; i++, pItem++) {
                            ...
                            if (cnt > 0) {
                                if (pItem->fg.isUsing == 0 ||
                                    ...
                                    ** by clearing pFJMatch and letting cnt go above 1. */
                                    ...
                                    pFJMatch = 0;
                                } else if ((pItem->fg.jointype & JT_RIGHT) == 0) {
                                    /* An INNER or LEFT JOIN.  Use the left-most table */
                                    continue;
                                } else if ((pItem->fg.jointype & JT_LEFT) == 0) {
                ...
                if (zTab) {
                    ...
                    if (pItem->zAlias != 0) {
                        ...
                        if (pTab->tnum != 1)
                            continue;
                        if (!isValidSchemaTableName(zTab, pTab, pSchema))
                            continue;
                    }
                    assert(ExprUseYTab(pExpr));
                ...
                if (0 == cnt && VisibleRowid(pTab)) {
                    ...
                    */
                    if (cntTab == 0 || (cntTab == 1 && ALWAYS(pMatch != 0) &&
        ...
        */
        if (cnt == 0 && zDb == 0) {
            ...
            if (pParse->pTriggerTab != 0) {
                ...
                } else if (op != TK_DELETE && zTab &&
                           sqlite3StrICmp("new", zTab) == 0) {
                    pExpr->iTable = 1;
                    pTab = pParse->pTriggerTab;
                } else if (op != TK_INSERT && zTab &&
        ...
        */
        if (cnt == 0 && (pNC->ncFlags & NC_UEList) != 0 && zTab == 0) {
...
*/
static void notValidImpl(Parse *pParse,    /* Leave error message here */
                         NameContext *pNC, /* The name context */
                         const char *zMsg, /* Type of error */
                         Expr *pExpr,      /* Invalidate this expression on error */
        ...
        case TK_FUNCTION: {
                ...
                if ((pDef->funcFlags & SQLITE_FUNC_INTERNAL) != 0 &&
                    ...
                    ** used to activate internal functions for testing purposes */
                    ...
                    pDef = 0;
                            ...
                            (SQLITE_FUNC_DIRECT | SQLITE_FUNC_UNSAFE)) != 0 &&
                           !IN_RENAME_OBJECT) {
            ...
            if (0 == IN_RENAME_OBJECT) {
#ifndef SQLITE_OMIT_WINDOWFUNC
                assert(
                    is_agg == 0 || (pDef->funcFlags & SQLITE_FUNC_MINMAX) ||
                    (pDef->xValue == 0 && pDef->xInverse == 0) ||
                    (pDef->xValue && pDef->xInverse && pDef->xSFunc && pDef->xFinalize));
                if (pDef && pDef->xValue == 0 && pWin) {
                    sqlite3ErrorMsg(pParse, "%#T() may not be used as a window function",
                                    pExpr);
                    pNC->nNcErr++;
                } else if ((is_agg && (pNC->ncFlags & NC_AllowAgg) == 0) ||
                ...
                if ((is_agg && (pNC->ncFlags & NC_AllowAgg) == 0)) {
                    ...
                    sqlite3ErrorMsg(
                        pParse, "FILTER may not be used with non-aggregate %#T()", pExpr);
                    pNC->nNcErr++;
                }
#endif
            ...
            if (is_agg) {
...
#endif /* SQLITE_OMIT_WINDOWFUNC */
                {
                    ...
                    if (pNC2 && pDef) {
                        ...
                        pNC2->ncFlags |=
                            NC_HasAgg | ((pDef->funcFlags ^ SQLITE_FUNC_ANYORDER) &
...
*/
static int resolveAsName(Parse *pParse,    /* Parsing context for error messages */
