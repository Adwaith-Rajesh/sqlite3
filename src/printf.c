...
*/
void sqlite3_str_vappendf(sqlite3_str *pAccum, /* Accumulate results here */
    ...
    ** optimization. */
    assert(pAccum->nChar > 0 ||
           (pAccum->printfFlags & SQLITE_PRINTF_MALLOCED) == 0);
    ...
    for (; (c = (*fmt)) != 0; ++fmt) {
        ...
        flag_leftjustify = flag_prefix = cThousand = flag_alternateform =
        ...
        switch (xtype) {
            case etPOINTER:
                flag_long = sizeof(char *) == sizeof(i64)        ? 2
                            : sizeof(char *) == sizeof(long int) ? 1
                                                                 : 0;
            /* no break */ deliberate_fall_through case etORDINAL:
            case etRADIX:
                cThousand = 0;
            /* no break */ deliberate_fall_through case etDECIMAL:
            ...
            case etCHARX:
                ...
                if (precision > 1) {
                    ...
                    while (precision > 1) {
                        ...
                        if (pAccum->accError)
                            break;
                        sqlite3_str_append(pAccum, &pAccum->zText[pAccum->nChar - nCopyBytes],
                                           nCopyBytes);
                        precision -= nPrior;
                        nPrior *= 2;
                    }
            ...
            case etDYNSTRING:
                ...
                if (bufpt == 0) {
                    ...
                    if (pAccum->nChar == 0 && pAccum->mxAlloc && width == 0 &&
                        precision < 0 && pAccum->accError == 0) {
            ...
            case etTOKEN: {
                if ((pAccum->printfFlags & SQLITE_PRINTF_INTERNAL) == 0)
                    return;
...
void sqlite3RecordErrorOffsetOfExpr(sqlite3 *db, const Expr *pExpr) {
    while (pExpr && (ExprHasProperty(pExpr, EP_OuterON | EP_InnerON) ||
                     pExpr->w.iOfst <= 0)) {
        pExpr = pExpr->pLeft;
    }
    if (pExpr == 0)
...
*/
static void SQLITE_NOINLINE enlargeAndAppend(StrAccum *p, const char *z,
                                             int N) {
...
*/
static sqlite3_str sqlite3OomStr = {0, 0, 0, 0, 0, SQLITE_NOMEM, 0};
...
sqlite3_str *sqlite3_str_new(sqlite3 *db) {
    ...
    if (p) {
        sqlite3StrAccumInit(
            p, 0, 0, 0, db ? db->aLimit[SQLITE_LIMIT_LENGTH] : SQLITE_MAX_LENGTH);
    } else {
        p = &sqlite3OomStr;
    }
    return p;
