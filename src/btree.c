...
int corruptPageError(int lineno, MemPage *p) {
    ...
    zMsg = sqlite3_mprintf("database corruption page %u of %s", p->pgno,
                           sqlite3PagerFilename(p->pBt->pPager, 0));
...
static int hasReadConflicts(Btree *pBtree, Pgno iRoot) {
    ...
    for (p = pBtree->pBt->pCursor; p; p = p->pNext) {
        if (p->pgnoRoot == iRoot && p->pBtree != pBtree &&
...
static int querySharedCacheTableLock(Btree *p, Pgno iTab, u8 eLock) {
    ...
    assert(p->db != 0);
    assert(!(p->db->flags & SQLITE_ReadUncommit) || eLock == WRITE_LOCK ||
           iTab == 1);
...
int sqlite3BtreeCursorHasMoved(BtCursor *pCur) {
    assert(EIGHT_BYTE_ALIGNMENT(pCur) || pCur == sqlite3BtreeFakeValidCursor());
...
** into *pRC.
*/
static void ptrmapPut(BtShared *pBt, Pgno key, u8 eType, Pgno parent,
                      int *pRC) {
...
static SQLITE_NOINLINE void btreeParseCellAdjustSizeForOverflow(
    ...
    */
    ...
    surplus =
        minLocal + (pInfo->nPayload - minLocal) % (pPage->pBt->usableSize - 4);
...
*/
static void ptrmapPutOvflPtr(MemPage *pPage, MemPage *pSrc, u8 *pCell,
                             int *pRC) {
    CellInfo info;
    if (*pRC)
        return;
...
static int btreeInitPage(MemPage *pPage) {
    ...
    ** bytes of reserved space. */
    assert(pPage->nCell > 0 ||
           get2byteNotZero(&data[5]) == (int)pBt->usableSize || CORRUPT_DB);
...
*/
static Pgno btreePagecount(BtShared *pBt) { return pBt->nPage; }
    ...
    */
    ...
    if (pBt == 0) {
        ...
        */
        ...
        rc = sqlite3PagerOpen(pVfs, &pBt->pPager, zFilename, sizeof(MemPage), flags,
                              vfsFlags, pageReinit);
        if (rc == SQLITE_OK) {
...
static void setDefaultSyncFlag(BtShared *pBt, u8 safety_level) {
    ...
    if ((db = pBt->db) != 0 && (pDb = db->aDb) != 0) {
        while (pDb->pBt == 0 || pDb->pBt->pBt != pBt) {
            pDb++;
        }
        if (pDb->bSyncSet == 0 && pDb->safety_level != safety_level &&
            pDb != &db->aDb[1]) {
...
int sqlite3BtreeBeginTrans(Btree *p, int wrflag, int *pSchemaVersion) {
    BtShared *pBt;
    if (p->sharable || p->inTrans == TRANS_NONE ||
...
static int autoVacuumCommit(Btree *p) {
    ...
    if (!pBt->incrVacuum) {
        ...
        if (db->xAutovacPages) {
            ...
            nVac = db->xAutovacPages(db->pAutovacPagesArg, db->aDb[iDb].zDbSName,
                                     nOrig, nFree, pBt->pageSize);
            if (nVac > nFree) {
                nVac = nFree;
            }
            if (nVac == 0) {
...
btreeCursor(Btree *p,                 /* The btree */
    ...
    assert(sqlite3BtreeHoldsMutex(p));
    assert(wrFlag == 0 || wrFlag == BTREE_WRCSR ||
...
** this routine.
*/
int sqlite3BtreeCursorSize(void) { return ROUND8(sizeof(BtCursor)); }
...
static int cellInfoEqual(CellInfo *a, CellInfo *b) {
    if (a->nKey != b->nKey)
        return 0;
    if (a->pPayload != b->pPayload)
        return 0;
    if (a->nPayload != b->nPayload)
        return 0;
...
getOverflowPage(BtShared *pBt,    /* The database file */
    ...
    */
    ...
    if (rc == SQLITE_OK) {
        rc =
            btreeGetPage(pBt, ovfl, &pPage, (ppPage == 0) ? PAGER_GET_READONLY : 0);
        ...
        */
        if ((pCur->curFlags & BTCF_ValidOvfl) == 0) {
            ...
            if (pCur->aOverflow == 0 ||
                ...
                Pgno *aNew =
                    (Pgno *)sqlite3Realloc(pCur->aOverflow, nOvfl * 2 * sizeof(Pgno));
                if (aNew == 0) {
                    return SQLITE_NOMEM_BKPT;
                } else {
                    pCur->aOverflow = aNew;
                ...
                */
                if (eOp == 0                                           /* (1) */
                    ...
                    rc = sqlite3OsRead(fd, aWrite, a + 4,
                                       (i64)pBt->pageSize * (nextPage - 1));
                    if (rc && nextPage > pBt->nPage)
                        rc = SQLITE_CORRUPT_BKPT;
...
int sqlite3BtreePayloadChecked(BtCursor *pCur, u32 offset, u32 amt,
                               void *pBuf) {
    if (pCur->eState == CURSOR_VALID) {
        assert(cursorOwnsBtShared(pCur));
...
static void moveToParent(BtCursor *pCur) {
    ...
    assert(pCur->iPage > 0);
    assert(pCur->pPage);
    assertParentIndex(pCur->apPage[pCur->iPage - 1], pCur->aiIdx[pCur->iPage - 1],
                      pCur->pPage->pgno);
    ...
    if (rc == SQLITE_OK) {
        ...
        *pRes = 0;
        ...
        assert(pCur->pgnoRoot == 0 ||
               (pCur->pPage != 0 && pCur->pPage->nCell == 0));
        *pRes = 1;
        rc = SQLITE_OK;
    }
    return rc;
...
*/
static int indexCellCompare(BtCursor *pCur, int idx, UnpackedRecord *pIdxKey,
...
int sqlite3BtreeIndexMoveto(BtCursor *pCur,          /* The cursor to be moved */
    ...
    pIdxKey->errCode = 0;
    assert(pIdxKey->default_rc == 1 || pIdxKey->default_rc == 0 ||
    ...
    */
    if (pCur->eState == CURSOR_VALID && pCur->pPage->leaf &&
            ...
            */
            ...
            if (nCell <= pPage->max1bytePayload) {
                ...
                ** b-tree page.  */
                testcase(pCell + nCell + 1 == pPage->aDataEnd);
                c = xRecordCompare(nCell, (void *)&pCell[1], pIdxKey);
            } else if (!(pCell[1] & 0x80) && (nCell = ((nCell & 0x7f) << 7) +
...
int sqlite3BtreePrevious(BtCursor *pCur, int flags) {
    ...
    pCur->info.nSize = 0;
    if (pCur->eState != CURSOR_VALID || pCur->ix == 0 || pCur->pPage->leaf == 0) {
            ...
            if (k == 0 && !searchList) {
                ...
                *ppPage = pTrunk;
                ...
                if (!searchList ||
                    ...
                    *pPgno = iPage;
                    TRACE((
                        "ALLOCATE: %u was leaf %u of %u on trunk %u"
                        ": %u more free pages\n",
                    ...
                    noContent =
                        !btreeGetHasContent(pBt, *pPgno) ? PAGER_GET_NOCONTENT : 0;
    ...
    if (nFree != 0) {
        ...
        if (nLeaf < (u32)pBt->usableSize / 4 - 8) {
            ...
            */
            ...
            TRACE(
                ("FREE-PAGE: %u leaf on trunk page %u\n", pPage->pgno, pTrunk->pgno));
            goto freepage_out;
        }
    }
...
clearCellOverflow(MemPage *pPage,       /* The page that contains the Cell */
...
static void dropCell(MemPage *pPage, int idx, int sz, int *pRC) {
    ...
    if (pPage->nCell == 0) {
        ...
        pPage->nFree =
            pPage->pBt->usableSize - pPage->hdrOffset - pPage->childPtrSize - 8;
    } else {
...
insertCell(MemPage *pPage, /* Page into which we are copying */
    ...
    if (pPage->nOverflow || sz + 2 > pPage->nFree) {
        ...
        */
        assert(j == 0 ||
               pPage->aiOvfl[j - 1] < (u16)i); /* Overflows in sorted order */
        assert(j == 0 ||
               i == pPage->aiOvfl[j - 1] + 1); /* Overflows are sequential */
    } else {
...
*/
static int
insertCellFast(MemPage *pPage, /* Page into which we are copying */
               int i,          /* New cell becomes the i-th cell of the page */
               u8 *pCell,      /* Content of the new cell */
               int sz          /* Bytes of content in pCell */
) {
    ...
    while (1 /*Exit by break*/) {
        ...
        ** since memcpy() sends SIGABORT with overlapping buffers on OpenBSD */
        assert((pSlot + sz) <= pCArray->apCell[i] ||
               pSlot >= (pCArray->apCell[i] + sz) || CORRUPT_DB);
...
static int editPage(MemPage *pPg,      /* Edit this page */
    ...
    if (pageInsertArray(pPg, pBegin, &pData, pCellptr, iNew + nCell, nNew - nCell,
                        pCArray))
        goto editpage_fail;
    ...
    */
    ...
    if (rc == SQLITE_OK) {
        ...
        */
        ...
        if (rc == SQLITE_OK) {
            rc = insertCell(pParent, pParent->nCell, pSpace, (int)(pOut - pSpace), 0,
                            pPage->pgno);
        }
...
balance_nonroot(MemPage *pParent, /* Parent page of siblings being balanced */
        ...
        */
        ...
        if (pOld->nOverflow > 0) {
            ...
            for (k = 0; k < pOld->nOverflow; k++) {
                assert(k == 0 ||
                       pOld->aiOvfl[k - 1] + 1 == pOld->aiOvfl[k]); /* NOTE 1 */
    ...
    */
    for (i = k - 1; i > 0; i--) {
        ...
        do {
            ...
            if (szRight != 0 &&
                (bBulk ||
                 szRight + szD + 2 > szLeft - (szR + (i == k - 1 ? 0 : 2)))) {
                break;
            }
            szRight += szD + 2;
            szLeft -= szR + 2;
    ...
    */
    ...
    TRACE((
        "BALANCE: new: %u(%u nc=%u) %u(%u nc=%u) %u(%u nc=%u) "
        "%u(%u nc=%u) %u(%u nc=%u)\n",
        apNew[0]->pgno, szNew[0], cntNew[0], nNew >= 2 ? apNew[1]->pgno : 0,
        nNew >= 2 ? szNew[1] : 0,
    ...
    */
    ...
    if (isRoot && pParent->nCell == 0 && pParent->hdrOffset <= apNew[0]->nFree) {
        ...
        */
        ...
        assert(apNew[0]->nFree == (get2byteNotZero(&apNew[0]->aData[5]) -
                                   apNew[0]->cellOffset - apNew[0]->nCell * 2) ||
               rc != SQLITE_OK);
            ...
            ** of its own ancestor pages. Corruption. */
            ...
            if (rc == SQLITE_OK) {
                ...
                if (pPage->intKeyLeaf && pPage->nOverflow == 1 &&
                    pPage->aiOvfl[0] == pPage->nCell && pParent->pgno != 1 &&
                    pParent->nCell == iIdx) {
...
*/
static int
btreeOverwriteContent(MemPage *pPage,         /* MemPage on which writing will occur */
...
static int btreeOverwriteCell(BtCursor *pCur, const BtreePayload *pX) {
    ...
    if (pCur->info.nLocal == nTotal) {
        ...
        return btreeOverwriteContent(pPage, pCur->info.pPayload, pX, 0,
                                     pCur->info.nLocal);
    } else {
...
int sqlite3BtreeInsert(
    ...
    assert((flags & (BTREE_SAVEPOSITION | BTREE_APPEND | BTREE_PREFORMAT)) ==
           flags);
        ...
        */
        if (loc == 0 && (flags & BTREE_SAVEPOSITION) == 0) {
            if (pX->nMem) {
                ...
                rc = btreeMoveto(pCur, pX->pKey, pX->nKey, (flags & BTREE_APPEND) != 0,
                                 &loc);
            }
            if (rc)
                return rc;
...
static void checkAppendMsg(IntegrityCk *pCheck, const char *zFormat, ...) {
    ...
    if (pCheck->zPfx) {
        sqlite3_str_appendf(&pCheck->errMsg, pCheck->zPfx, pCheck->v0, pCheck->v1,
                            pCheck->v2);
    }
    ...
    ** integer offsets to the cell contents. */
    for (i = nCell - 1; i >= 0 && pCheck->mxErr; i--) {
        ...
        if (pc < contentOffset || pc > usableSize - 4) {
            checkAppendMsg(pCheck, "Offset %u out of range %u..%u", pc, contentOffset,
                           usableSize - 4);
            doCoverageCheck = 0;
            continue;
        }
    ...
    if (doCoverageCheck && pCheck->mxErr > 0) {
        ...
        */
        ...
        while (i > 0) {
            int size, j;
            assert((u32)i <=
                   usableSize - 4); /* Enforced by btreeComputeFreeSpace() */
            size = get2byte(&data[i + 2]);
            assert((u32)(i + size) <=
                   usableSize); /* due to btreeComputeFreeSpace() */
        ...
        */
        ...
        while (btreeHeapPull(heap, &x)) {
            if ((prev & 0xffff) >= (x >> 16)) {
                checkAppendMsg(pCheck, "Multiple uses for byte %u of page %u", x >> 16,
                               iPage);
                break;
            } else {
...
int sqlite3BtreeIntegrityCheck(
    ...
    if (!bPartial) {
        if (pBt->autoVacuum) {
            ...
            if (mx != mxInHdr) {
                checkAppendMsg(&sCheck, "max rootpage (%u) disagrees with header (%u)",
                               mx, mxInHdr);
            }
