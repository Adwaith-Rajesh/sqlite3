...
*/
static sqlite3_pcache *testpcacheCreate(int szPage, int szExtra,
                                        int bPurgeable) {
    int nMem;
    char *x;
    testpcache *p;
...
** Fetch a page.
*/
static sqlite3_pcache_page *testpcacheFetch(sqlite3_pcache *pCache,
...
** Unpin a page.
*/
static void testpcacheUnpin(sqlite3_pcache *pCache,
                            sqlite3_pcache_page *pOldPage, int discard) {
    testpcache *p = (testpcache *)pCache;
    int i;
    assert(p->iMagic == TESTPCACHE_VALID);
    assert(testpcacheGlobal.pDummy != 0);
...
** Rekey a single page.
*/
static void testpcacheRekey(sqlite3_pcache *pCache,
                            sqlite3_pcache_page *pOldPage, unsigned oldKey,
                            unsigned newKey) {
    testpcache *p = (testpcache *)pCache;
    int i;
    assert(p->iMagic == TESTPCACHE_VALID);
