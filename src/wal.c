...
#define WALINDEX_LOCK_OFFSET \
    (sizeof(WalIndexHdr) * 2 + offsetof(WalCkptInfo, aLock))
...
walIndexPageRealloc(Wal *pWal,            /* The WAL context */
    ...
    if (pWal->exclusiveMode == WAL_HEAPMEMORY_MODE) {
        ...
        rc = sqlite3OsShmMap(pWal->pDbFd, iPage, WALINDEX_PGSZ, pWal->writeLock,
                             (void volatile **)&pWal->apWiData[iPage]);
        assert(pWal->apWiData[iPage] != 0 || rc != SQLITE_OK ||
...
static int walIndexPage(Wal *pWal,            /* The WAL context */
                        int iPage,            /* The page we seek */
                        volatile u32 **ppPage /* Write the page pointer here */
) {
    SEH_INJECT_FAULT;
...
*/
static void
walEncodeFrame(Wal *pWal,     /* The write-ahead log */
...
*/
static int
walDecodeFrame(Wal *pWal,       /* The write-ahead log */
...
static int walLockShared(Wal *pWal, int lockIdx) {
    ...
    VVA_ONLY(pWal->lockError =
                 (u8)(rc != SQLITE_OK && (rc & 0xFF) != SQLITE_BUSY);)
#ifdef SQLITE_USE_SEH
    if (rc == SQLITE_OK)
...
*/
static int walHashGet(Wal *pWal,       /* WAL handle */
...
static int walFramePage(u32 iFrame) {
    int iHash =
        (iFrame + HASHTABLE_NPAGE - HASHTABLE_NPAGE_ONE - 1) / HASHTABLE_NPAGE;
...
static u32 walFramePgno(Wal *pWal, u32 iFrame) {
    ...
    return pWal
        ->apWiData[iHash][(iFrame - 1 - HASHTABLE_NPAGE_ONE) % HASHTABLE_NPAGE];
}
...
static void walCleanupHash(Wal *pWal) {
    ...
    */
    if (iLimit) {
        ...
        int iKey; /* Hash key */
        for (j = 0; j < iLimit; j++) {
            for (iKey = walHash(sLoc.aPgno[j]); sLoc.aHash[iKey];
                 iKey = walNextHash(iKey)) {
                if (sLoc.aHash[iKey] == j + 1)
                    break;
            }
    ...
    if (rc == SQLITE_OK) {
        ...
        */
        if ((idx & 0x3ff) == 0) {
            int i; /* Loop counter */
            for (i = 0; i < idx; i++) {
                for (iKey = walHash(sLoc.aPgno[i]); sLoc.aHash[iKey];
                     iKey = walNextHash(iKey)) {
                    if (sLoc.aHash[iKey] == i + 1)
                        break;
                }
    ...
    */
    ...
    if (nSize > WAL_HDRSIZE) {
        ...
        */
        ...
        if ((magic & 0xFFFFFFFE) != WAL_MAGIC || szPage & (szPage - 1) ||
            szPage > SQLITE_MAX_PAGE_SIZE || szPage < 512) {
            goto finished;
        }
        ...
        walChecksumBytes(pWal->hdr.bigEndCksum == SQLITE_BIGENDIAN, aBuf,
                         WAL_HDRSIZE - 2 * 4, 0, pWal->hdr.aFrameCksum);
        ...
        ** are able to understand */
        ...
        for (iPg = 0; iPg <= (u32)walFramePage(iLastFrame); iPg++) {
            ...
            u32 iFirst =
                1 +
                (iPg == 0 ? 0 : HASHTABLE_NPAGE_ONE + (iPg - 1) * HASHTABLE_NPAGE);
            u32 nHdr, nHdr32;
        ...
        */
        if (pWal->hdr.nPage) {
            sqlite3_log(SQLITE_NOTICE_RECOVER_WAL,
                        "recovered %d frames from WAL file %s", pWal->hdr.mxFrame,
                        pWal->zWalName);
        }
    }
...
*/
int sqlite3WalOpen(sqlite3_vfs *pVfs,    /* vfs module to open wal and wal-index */
...
*/
static int
walIteratorNext(WalIterator *p, /* Iterator */
...
*/
static void walMerge(const u32 *aContent, /* Pages in wal - keys for the sort */
    ...
    while (iRight < nRight || iLeft < nLeft) {
        ...
        if ((iLeft < nLeft) && (iRight >= nRight || aContent[aLeft[iLeft]] <
                                                        aContent[aRight[iRight]])) {
            logpage = aLeft[iLeft++];
        } else {
...
*/
static void
walMergesort(const u32 *aContent, /* Pages in wal */
...
*/
static void walIteratorFree(WalIterator *p) { sqlite3_free(p); }
...
static int walIteratorInit(Wal *pWal, u32 nBackfill, WalIterator **pp) {
    ...
    */
    ...
    nByte = sizeof(WalIterator) + (nSegment - 1) * sizeof(struct WalSegment) +
            iLast * sizeof(ht_slot);
    p = (WalIterator *)sqlite3_malloc64(
        nByte +
...
static int walEnableBlocking(Wal *pWal) {
    ...
    if (pWal->db) {
        ...
        if (tmout) {
            int rc;
            rc = sqlite3OsFileControl(pWal->pDbFd, SQLITE_FCNTL_LOCK_TIMEOUT,
                                      (void *)&tmout);
            res = (rc == SQLITE_OK);
        }
    }
...
*/
void sqlite3WalDb(Wal *pWal, sqlite3 *db) { pWal->db = db; }
...
*/
static int walBusyLock(Wal *pWal,            /* WAL connection */
...
** time.
*/
static int walCheckpoint(Wal *pWal,            /* Wal connection */
    ...
    if (pInfo->nBackfill < pWal->hdr.mxFrame) {
        ...
        */
        ...
        if (pIter && (rc = walBusyLock(pWal, xBusy, pBusyArg, WAL_READ_LOCK(0),
                                       1)) == SQLITE_OK) {
                ...
                if (rc == SQLITE_OK && nSize < nReq) {
                    if ((nSize + 65536 + (i64)pWal->hdr.mxFrame * szPage) < nReq) {
                        ...
                        ** must be corruption somewhere.  */
                        ...
                        sqlite3OsFileControlHint(pWal->pDbFd, SQLITE_FCNTL_SIZE_HINT,
                                                 &nReq);
                    }
                }
            }
    ...
    */
    if (rc == SQLITE_OK && eMode != SQLITE_CHECKPOINT_PASSIVE) {
        ...
        if (pInfo->nBackfill < pWal->hdr.mxFrame) {
            ...
            rc =
                walBusyLock(pWal, xBusy, pBusyArg, WAL_READ_LOCK(1), WAL_NREADER - 1);
            if (rc == SQLITE_OK) {
...
static int walHandleException(Wal *pWal) {
    if (pWal->exclusiveMode == 0) {
        ...
        for (ii = 0; ii < SQLITE_SHM_NLOCK; ii++) {
            if ((S << ii) & mUnlock)
                walUnlockShared(pWal, ii);
            if ((E << ii) & mUnlock)
                walUnlockExclusive(pWal, ii, 1);
        }
    }
...
*/
int sqlite3WalClose(Wal *pWal,         /* Wal to close */
                    sqlite3 *db,       /* For interrupt flag */
                    int sync_flags,    /* Flags to pass to OsSync() (or 0) */
                    int nBuf, u8 *zBuf /* Buffer of at least nBuf bytes */
) {
    int rc = SQLITE_OK;
    if (pWal) {
        ...
        */
        if (zBuf != 0 &&
            ...
            rc = sqlite3WalCheckpoint(pWal, db, SQLITE_CHECKPOINT_PASSIVE, 0, 0,
                                      sync_flags, nBuf, zBuf, 0, 0);
            if (rc == SQLITE_OK) {
                int bPersist = -1;
...
static int walBeginShmUnreliable(Wal *pWal, int *pChanged) {
    ...
    ** the caller.  */
    ...
    for (iOffset = walFrameOffset(pWal->hdr.mxFrame + 1, pWal->szPage);
         iOffset + szFrame <= szWal; iOffset += szFrame) {
        u32 pgno;      /* Database page number for frame */
        u32 nTruncate; /* dbsize field from frame header */
...
*/
static int
walSnapshotRecover(Wal *pWal,   /* WAL handle */
    ...
    if (rc == SQLITE_OK) {
        ...
        for (i = pInfo->nBackfillAttempted; i > AtomicLoad(&pInfo->nBackfill);
             i--) {
            WalHashLoc sLoc; /* Hash table location */
...
int sqlite3WalSnapshotRecover(Wal *pWal) {
    ...
    if (rc == SQLITE_OK) {
        ...
        if (pBuf1 == 0 || pBuf2 == 0) {
            ...
            pWal->ckptLock = 1;
            SEH_TRY { rc = walSnapshotRecover(pWal, pBuf1, pBuf2); }
...
int sqlite3WalBeginReadTransaction(Wal *pWal, int *pChanged) {
    int rc;
    SEH_TRY { rc = walBeginReadTransaction(pWal, pChanged); }
    SEH_EXCEPT(rc = walHandleException(pWal);)
    return rc;
}
...
*/
static int walFindFrame(Wal *pWal,  /* WAL handle */
...
*/
int sqlite3WalFindFrame(Wal *pWal,  /* WAL handle */
                        Pgno pgno,  /* Database page number to read data for */
                        u32 *piRead /* OUT: Frame number (or zero) */
) {
    int rc;
    SEH_TRY { rc = walFindFrame(pWal, pgno, piRead); }
    SEH_EXCEPT(rc = SQLITE_IOERR_IN_PAGE;)
    return rc;
}
...
*/
int sqlite3WalReadFrame(Wal *pWal, /* WAL handle */
...
int sqlite3WalBeginWriteTransaction(Wal *pWal) {
    ...
    */
    SEH_TRY {
        if (memcmp(&pWal->hdr, (void *)walIndexHdr(pWal), sizeof(WalIndexHdr)) !=
            0) {
        ...
        SEH_TRY {
            ...
            */
            ...
            for (iFrame = pWal->hdr.mxFrame + 1;
                 ALWAYS(rc == SQLITE_OK) && iFrame <= iMax; iFrame++) {
...
*/
static int
walWriteToLog(WalWriter *p,         /* WAL to write to */
...
*/
static int walFrames(Wal *pWal,      /* Wal handle to write to */
    ...
    ** nTruncate==0 then this frame set does not complete the transaction. */
...
#if defined(SQLITE_TEST) && defined(SQLITE_DEBUG)
    {
        ...
        WALTRACE(("WAL%p: frame write begin. %d frames. mxFrame=%d. %s\n", pWal,
                  cnt, pWal->hdr.mxFrame, isCommit ? "Commit" : "Spill"));
    }
#endif
...
*/
int sqlite3WalFrames(Wal *pWal,      /* Wal handle to write to */
                     int szPage,     /* Database page-size in bytes */
                     PgHdr *pList,   /* List of dirty pages to write */
                     Pgno nTruncate, /* Database size after this commit */
                     int isCommit,   /* True if this is a commit */
    ...
    SEH_TRY {
        ...
        if (rc == SQLITE_OK) {
            if (pWal->hdr.mxFrame && walPagesize(pWal) != nBuf) {
                ...
                rc =
                    walCheckpoint(pWal, db, eMode2, xBusy2, pBusyArg, sync_flags, zBuf);
            }
...
int sqlite3_snapshot_cmp(sqlite3_snapshot *p1, sqlite3_snapshot *p2) {
    ...
    ** is incremented each time the wal file is restarted.  */
    if (pHdr1->aSalt[0] < pHdr2->aSalt[0])
        return -1;
    if (pHdr1->aSalt[0] > pHdr2->aSalt[0])
        return +1;
    if (pHdr1->mxFrame < pHdr2->mxFrame)
        return -1;
    if (pHdr1->mxFrame > pHdr2->mxFrame)
        return +1;
    return 0;
