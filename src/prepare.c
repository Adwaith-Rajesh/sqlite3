...
corruptSchema(InitData *pData,   /* Initialization context */
    ...
    if (db->mallocFailed) {
        ...
        static const char *azAlterType[] = {"rename", "drop column", "add column"};
        *pData->pzErrMsg = sqlite3MPrintf(
            db, "error in %s %s after %s: %s", azObj[0], azObj[1],
            azAlterType[(pData->mInitFlags & INITFLAG_AlterMask) - 1], zExtra);
...
int sqlite3InitCallback(void *pInit, int argc, char **argv, char **NotUsed) {
    ...
    if (argv[3] == 0) {
        ...
        */
        ...
        } else if (sqlite3GetUInt32(argv[3], &pIndex->tnum) == 0 ||
                   pIndex->tnum < 2 || pIndex->tnum > pData->mxPage ||
...
int sqlite3InitOne(sqlite3 *db, int iDb, char **pzErrMsg, u32 mFlags) {
    ...
    */
    ...
    {
        char *zSql;
        zSql = sqlite3MPrintf(db, "SELECT*FROM\"%w\".%s ORDER BY rowid",
    ...
    if (db->mallocFailed) {
        ...
        pDb = &db->aDb[iDb];
               ...
               ((db->flags & SQLITE_NoSchemaError) && rc != SQLITE_NOMEM)) {
...
int sqlite3_prepare16_v2(
    ...
    int rc;
    rc = sqlite3Prepare16(db, zSql, nBytes, SQLITE_PREPARE_SAVESQL, ppStmt,
                          pzTail);
    assert(rc == SQLITE_OK || ppStmt == 0 || *ppStmt == 0); /* VERIFY: F13021 */
    return rc;
}
