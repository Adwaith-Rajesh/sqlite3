...
*/
static int blobHandleFromObj(Tcl_Interp *interp, Tcl_Obj *pObj,
                             sqlite3_blob **ppBlob) {
    char *z;
    int n;
...
static int SQLITE_TCLAPI test_blob_open(
    ...
    if (Tcl_GetWideIntFromObj(interp, objv[5], &iRowid))
        return TCL_ERROR;
    if (Tcl_GetIntFromObj(interp, objv[6], &flags))
        return TCL_ERROR;
...
static int SQLITE_TCLAPI test_blob_close(
    ...
    if (blobHandleFromObj(interp, objv[1], &pBlob))
        return TCL_ERROR;
