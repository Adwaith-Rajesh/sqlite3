...
*/
static void pager_test_reiniter(DbPage *pNotUsed) { return; }
...
static int SQLITE_TCLAPI pager_open(
    ...
    if (Tcl_GetInt(interp, argv[2], &nPage))
        return TCL_ERROR;
...
static int SQLITE_TCLAPI pager_close(
    ...
    int rc;
    if (argc != 2) {
        Tcl_AppendResult(interp, "wrong # args: should be \"", argv[0], " ID\"", 0);
        return TCL_ERROR;
    }
...
static int SQLITE_TCLAPI page_get(
    ...
    pPager = sqlite3TestTextToPtr(argv[1]);
    if (Tcl_GetInt(interp, argv[2], &pgno))
        return TCL_ERROR;
...
static int SQLITE_TCLAPI page_unref(
    ...
    DbPage *pPage;
    if (argc != 2) {
        Tcl_AppendResult(interp, "wrong # args: should be \"", argv[0], " PAGE\"",
                         0);
        return TCL_ERROR;
    }
...
static int SQLITE_TCLAPI testPendingByte(
    ...
    if (Tcl_GetInt(interp, argv[1], &pbyte))
        return TCL_ERROR;
...
static int faultSimCallback(int x) {
    ...
    if (x == 0) {
        ...
        if (isNeg)
            zInt[i--] = '-';
        memcpy(faultSimScript + faultSimScriptSize, zInt + i + 1,
               sizeof(zInt) - i - 1);
    }
...
static int SQLITE_TCLAPI faultInstallCmd(
    ...
    int rc;
    if (argc != 1 && argc != 2) {
        Tcl_AppendResult(interp, "wrong # args: should be \"", argv[0], " SCRIPT\"",
                         (void *)0);
    }
...
static int SQLITE_TCLAPI testBitvecBuiltinTest(
    ...
    if (Tcl_GetInt(interp, argv[1], &sz))
        return TCL_ERROR;
    z = argv[2];
    while (nProg < 99 && *z) {
...
int Sqlitetest2_Init(Tcl_Interp *interp) {
    ...
    Tcl_LinkVar(interp, "sqlite_io_error_hit", (char *)&sqlite3_io_error_hit,
                TCL_LINK_INT);
    Tcl_LinkVar(interp, "sqlite_io_error_hardhit",
                (char *)&sqlite3_io_error_hardhit, TCL_LINK_INT);
    Tcl_LinkVar(interp, "sqlite_diskfull_pending",
                (char *)&sqlite3_diskfull_pending, TCL_LINK_INT);
    Tcl_LinkVar(interp, "sqlite_diskfull", (char *)&sqlite3_diskfull,
                TCL_LINK_INT);
#ifndef SQLITE_OMIT_WSD
    Tcl_LinkVar(interp, "sqlite_pending_byte", (char *)&sqlite3PendingByte,
