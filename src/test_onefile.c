...
static int tmpWrite(sqlite3_file *, const void *, int iAmt,
                    sqlite3_int64 iOfst);
...
static int fsFullPathname(sqlite3_vfs *, const char *zName, int nOut,
                          char *zOut);
...
*/
static int tmpRead(sqlite3_file *pFile, void *zBuf, int iAmt,
                   sqlite_int64 iOfst) {
    tmp_file *pTmp = (tmp_file *)pFile;
...
*/
static int tmpWrite(sqlite3_file *pFile, const void *zBuf, int iAmt,
                    sqlite_int64 iOfst) {
    tmp_file *pTmp = (tmp_file *)pFile;
...
** Sync a tmp-file.
*/
static int tmpSync(sqlite3_file *pFile, int flags) { return SQLITE_OK; }
...
** Lock a tmp-file.
*/
static int tmpLock(sqlite3_file *pFile, int eLock) { return SQLITE_OK; }

/*
** Unlock a tmp-file.
*/
static int tmpUnlock(sqlite3_file *pFile, int eLock) { return SQLITE_OK; }
...
*/
static int fsRead(sqlite3_file *pFile, void *zBuf, int iAmt,
                  sqlite_int64 iOfst) {
    int rc = SQLITE_OK;
    fs_file *p = (fs_file *)pFile;
    fs_real_file *pReal = p->pReal;
    ...
    if ((p->eType == DATABASE_FILE && (iAmt + iOfst) > pReal->nDatabase) ||
        ...
        while (iRem > 0 && rc == SQLITE_OK) {
            int iRealOff =
                pReal->nBlob - BLOCKSIZE * ((ii / BLOCKSIZE) + 1) + ii % BLOCKSIZE;
...
*/
static int fsWrite(sqlite3_file *pFile, const void *zBuf, int iAmt,
                   sqlite_int64 iOfst) {
    int rc = SQLITE_OK;
    fs_file *p = (fs_file *)pFile;
    fs_real_file *pReal = p->pReal;
    ...
    if (p->eType == DATABASE_FILE) {
        ...
        while (iRem > 0 && rc == SQLITE_OK) {
            ...
            if (iRealOff < (pReal->nDatabase + BLOCKSIZE)) {
                ...
                rc =
                    pF->pMethods->xWrite(pF, &((char *)zBuf)[iBuf], iRealAmt, iRealOff);
                ii += iRealAmt;
                iBuf += iRealAmt;
                iRem -= iRealAmt;
            }
...
** Lock an fs-file.
*/
static int fsLock(sqlite3_file *pFile, int eLock) { return SQLITE_OK; }

/*
** Unlock an fs-file.
*/
static int fsUnlock(sqlite3_file *pFile, int eLock) { return SQLITE_OK; }
...
*/
static int fsSectorSize(sqlite3_file *pFile) { return BLOCKSIZE; }
...
*/
static int fsOpen(sqlite3_vfs *pVfs, const char *zName, sqlite3_file *pFile,
...
*/
static int fsFullPathname(sqlite3_vfs *pVfs, /* Pointer to vfs object */
