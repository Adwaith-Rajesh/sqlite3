...
char sqlite3TableColumnAffinity(const Table *pTab, int iCol) {
    if (iCol < 0 || NEVER(iCol >= pTab->nCol))
...
char sqlite3ExprAffinity(const Expr *pExpr) {
    ...
    while (1 /* exit-by-break */) {
        ...
        if (op != TK_REGISTER || (op = pExpr->op2) == TK_REGISTER)
            break;
    }
...
CollSeq *sqlite3ExprCollSeq(Parse *pParse, const Expr *pExpr) {
    ...
    while (p) {
        ...
        if (op == TK_REGISTER)
            op = p->op2;
        if ((op == TK_AGG_COLUMN && p->y.pTab != 0) || op == TK_COLUMN ||
            op == TK_TRIGGER) {
            int j;
...
*/
static u8 binaryCompareP5(const Expr *pExpr1, /* Left operand */
...
*/
CollSeq *sqlite3BinaryCompareCollSeq(Parse *pParse, const Expr *pLeft,
                                     const Expr *pRight) {
    CollSeq *pColl;
    assert(pLeft);
...
codeCompare(Parse *pParse,    /* The parsing (and code generating) context */
    ...
    addr = sqlite3VdbeAddOp4(pParse->pVdbe, opcode, in2, dest, in1, (void *)p4,
                             P4_COLLSEQ);
...
*/
static void codeVectorCompare(Parse *pParse, /* Code generator context */
    ...
    assert(pExpr->op == TK_EQ || pExpr->op == TK_NE || pExpr->op == TK_IS ||
           pExpr->op == TK_ISNOT || pExpr->op == TK_LT || pExpr->op == TK_GT ||
    ...
    for (i = 0; 1 /*Loop exits by "break"*/; i++) {
        ...
        if ((opx == TK_LT || opx == TK_GT) && i < nLeft - 1) {
            ...
            testcase(opx == TK_LT);
            VdbeCoverageIf(v, opx == TK_LT);
            testcase(opx == TK_GT);
            VdbeCoverageIf(v, opx == TK_GT);
        }
        if (p5 == SQLITE_NULLEQ) {
...
*/
Expr *sqlite3ExprAlloc(sqlite3 *db,         /* Handle for sqlite3DbMallocRawNN() */
...
*/
Expr *sqlite3PExpr(Parse *pParse, /* Parsing context */
                   int op,        /* Expression opcode */
...
Select *sqlite3ExprListToValues(Parse *pParse, int nElem, ExprList *pEList) {
    ...
    for (ii = 0; ii < pEList->nExpr; ii++) {
        ...
        assert(ExprUseXList(pExpr));
        pSel =
            sqlite3SelectNew(pParse, pExpr->x.pList, 0, 0, 0, 0, 0, SF_Values, 0);
        pExpr->x.pList = 0;
        if (pSel) {
            if (pRet) {
                pSel->op = TK_ALL;
...
** arguments.
*/
Expr *sqlite3ExprFunction(Parse *pParse,       /* Parsing context */
    ...
    if (eDistinct == SF_Distinct)
        ExprSetProperty(pNew, EP_Distinct);
    return pNew;
}
...
void sqlite3ExprOrderByAggregateError(Parse *pParse, Expr *p) {
    sqlite3ErrorMsg(pParse, "ORDER BY may not be used with non-aggregate %#T()",
                    p);
}
...
void sqlite3ExprAddFunctionOrderBy(
    ...
    if (pExpr->x.pList == 0 || NEVER(pExpr->x.pList->nExpr == 0)) {
        ...
        sqlite3ParserAddCleanup(
            pParse, (void (*)(sqlite3 *, void *))sqlite3ExprListDelete, pOrderBy);
        return;
    }
...
void sqlite3ExprDeferredDelete(Parse *pParse, Expr *pExpr) {
    sqlite3ParserAddCleanup(
        pParse, (void (*)(sqlite3 *, void *))sqlite3ExprDelete, pExpr);
}
...
static int dupedExprStructSize(const Expr *p, int flags) {
    int nSize;
    assert(flags == EXPRDUP_REDUCE ||
           flags == 0); /* Only one flag value allowed */
...
*/
static Expr *
exprDup(sqlite3 *db,      /* Database connection (for memory allocation) */
        ...
        if (((p->flags | pNew->flags) & (EP_TokenOnly | EP_Leaf)) == 0) {
            ...
            if (dupFlags) {
                if (p->op == TK_SELECT_COLUMN) {
                    ...
                    pNew->pLeft =
                        p->pLeft ? exprDup(db, p->pLeft, EXPRDUP_REDUCE, &sEdupBuf) : 0;
                }
                pNew->pRight =
                    p->pRight ? exprDup(db, p->pRight, EXPRDUP_REDUCE, &sEdupBuf) : 0;
            } else {
                if (p->op == TK_SELECT_COLUMN) {
                    pNew->pLeft = p->pLeft;
                    assert(p->pRight == 0 || p->pRight == p->pLeft ||
...
ExprList *sqlite3ExprListDup(sqlite3 *db, const ExprList *p, int flags) {
    ...
    for (i = 0; i < p->nExpr; i++, pItem++, pOldItem++) {
        ...
        if (pOldExpr && pOldExpr->op == TK_SELECT_COLUMN &&
...
IdList *sqlite3IdListDup(sqlite3 *db, const IdList *p) {
    ...
    pNew =
        sqlite3DbMallocRawNN(db, sizeof(*pNew) + (p->nId - 1) * sizeof(p->a[0]));
    if (pNew == 0)
        return 0;
    pNew->nId = p->nId;
    pNew->eU4 = p->eU4;
...
SQLITE_NOINLINE ExprList *sqlite3ExprListAppendGrow(
    ...
    pNew = sqlite3DbRealloc(
        db, pList, sizeof(*pList) + (pList->nAlloc - 1) * sizeof(pList->a[0]));
    if (pNew == 0) {
...
void sqlite3ExprListSetSortOrder(ExprList *p, int iSortOrder, int eNulls) {
    ...
    assert(iSortOrder == SQLITE_SO_UNDEFINED || iSortOrder == SQLITE_SO_ASC ||
           iSortOrder == SQLITE_SO_DESC);
    assert(eNulls == SQLITE_SO_UNDEFINED || eNulls == SQLITE_SO_ASC ||
...
*/
void sqlite3ExprListCheckLength(Parse *pParse, ExprList *pEList,
                                const char *zObject) {
    ...
    ** from being considered constant. */
    ...
    switch (pExpr->op) {
        ...
        ** SQLITE_FUNC_CONST flag. */
        ...
        /* no break */ deliberate_fall_through case TK_COLUMN:
        case TK_AGG_FUNCTION:
        case TK_AGG_COLUMN:
        ...
        /* no break */ deliberate_fall_through case TK_IF_NULL_ROW:
        case TK_REGISTER:
        case TK_DOT:
        ...
        case TK_VARIABLE:
            ...
            /* no break */ deliberate_fall_through default
                : testcase(pExpr->op ==
                           TK_SELECT);        /* sqlite3SelectWalkFail() disallows */
...
** a constant.
*/
int sqlite3ExprIsConstant(Expr *p) { return exprIsConst(p, 1, 0); }
...
*/
int sqlite3ExprIsConstantNotJoin(Expr *p) { return exprIsConst(p, 2, 0); }
...
int sqlite3ExprIsInteger(const Expr *p, int *pValue) {
    int rc = 0;
    if (NEVER(p == 0))
        return 0; /* Used to only happen following on OOM */
...
const char *sqlite3RowidAlias(Table *pTab) {
    ...
    for (ii = 0; ii < ArraySize(azOpt); ii++) {
        ...
        for (iCol = 0; iCol < pTab->nCol; iCol++) {
            if (sqlite3_stricmp(azOpt[ii], pTab->aCol[iCol].zCnName) == 0)
                break;
        }
        if (iCol == pTab->nCol) {
            return azOpt[ii];
...
int sqlite3FindInIndex(
                ...
                for (pIdx = pTab->pIndex; pIdx && eType == 0; pIdx = pIdx->pNext) {
                    ...
                    ** BITMASK(nExpr) without overflowing */
                    ...
                    if (colUsed == (MASKBIT(nExpr) - 1)) {
                        ...
                        VdbeCoverage(v);
                        ExplainQueryPlan(
                            (pParse, 0, "USING INDEX %s FOR IN-OPERATOR", pIdx->zName));
                        ...
                        if (prRhsHasNull) {
                            ...
                            i64 mask = (1 << nExpr) - 1;
                            sqlite3VdbeAddOp4Dup8(v, OP_ColumnsUsed, iTab, 0, 0, (u8 *)&mask,
                                                  P4_INT64);
#endif
    ...
    */
    if (eType == 0 && (inFlags & IN_INDEX_NOOP_OK) && ExprUseXList(pX) &&
...
** is used.
*/
void sqlite3CodeRhsOfIN(Parse *pParse, /* Parsing context */
    ...
    if (!ExprHasProperty(pExpr, EP_VarSelect) && pParse->iSelfTab == 0) {
        ...
        */
        if (ExprHasProperty(pExpr, EP_Subrtn)) {
            ...
            if (ExprUseXSelect(pExpr)) {
                ExplainQueryPlan(
                    (pParse, 0, "REUSE LIST SUBQUERY %d", pExpr->x.pSelect->selId));
            }
    ...
    */
        ...
        ** error will have been caught long before we reach this point. */
        if (ALWAYS(pEList->nExpr == nVal)) {
            ...
            for (i = 0; i < nVal; i++) {
                ...
                pKeyInfo->aColl[i] =
                    sqlite3BinaryCompareCollSeq(pParse, p, pEList->a[i].pExpr);
            }
        }
    ...
    if (addrOnce) {
        ...
        /* Subroutine return */
        assert(ExprUseYSub(pExpr));
        assert(sqlite3VdbeGetOp(v, pExpr->y.sub.iAddr - 1)->opcode ==
                   OP_BeginSubrtn ||
               pParse->nErr);
        sqlite3VdbeAddOp3(v, OP_Return, pExpr->y.sub.regReturn, pExpr->y.sub.iAddr,
                          1);
        VdbeCoverage(v);
...
static void sqlite3ExprCodeIN(
    ...
    ** the RHS has not yet been coded.  */
    ...
    eType = sqlite3FindInIndex(
        pParse, pExpr, IN_INDEX_MEMBERSHIP | IN_INDEX_NOOP_OK,
        destIfFalse == destIfNull ? 0 : &rRhsHasNull, aiMap, &iTab);
    ...
    if (eType == IN_INDEX_NOOP) {
        ...
        for (ii = 0; ii < pList->nExpr; ii++) {
            ...
            if (ii < pList->nExpr - 1 || destIfNull != destIfFalse) {
                ...
                sqlite3VdbeAddOp4(v, op, rLhs, labelOk, r2, (void *)pColl, P4_COLLSEQ);
                ...
                sqlite3VdbeAddOp4(v, op, rLhs, destIfFalse, r2, (void *)pColl,
                                  P4_COLLSEQ);
                VdbeCoverageIf(v, op == OP_Ne);
    ...
    */
    if (eType == IN_INDEX_ROWID) {
        ...
        ** into a single opcode. */
        ...
        addrTruthOp = sqlite3VdbeAddOp4Int(v, OP_Found, iTab, 0, rLhs, nVector);
        VdbeCoverage(v);
    }
    ...
    */
    ...
    for (i = 0; i < nVector; i++) {
        ...
        sqlite3VdbeAddOp4(v, OP_Ne, rLhs + i, destNotNull, r3, (void *)pColl,
                          P4_COLLSEQ);
        VdbeCoverage(v);
...
static void codeInteger(Parse *pParse, Expr *pExpr, int negFlag, int iMem) {
    ...
    if (pExpr->flags & EP_IntValue) {
        ...
        if ((c == 3 && !negFlag) || (c == 2) ||
            ...
            sqlite3ErrorMsg(pParse, "oversized integer: %s%#T", negFlag ? "-" : "",
                            pExpr);
#else
            ...
            if (negFlag) {
                value = c == 3 ? SMALLEST_INT64 : -value;
            }
...
int sqlite3ExprCodeGetColumn(
    ...
    if (p5) {
        ...
        if (pOp->opcode == OP_Column)
            pOp->p5 = p5;
        if (pOp->opcode == OP_VColumn)
            pOp->p5 = (p5 & OPFLAG_NOCHNG);
    }
    return iReg;
}
        ...
        */
        ...
        case INLINEFUNC_expr_implies_expr: {
            ...
            sqlite3VdbeAddOp2(v, OP_Integer,
                              sqlite3ExprImpliesExpr(pParse, pFarg->a[0].pExpr,
                                                     pFarg->a[1].pExpr, -1),
                              target);
            break;
        }
...
expr_code_doover:
    if (pExpr == 0) {
        op = TK_NULL;
    } else if (pParse->pIdxEpr != 0 && !ExprHasProperty(pExpr, EP_Leaf) &&
    ...
    switch (op) {
        case TK_AGG_COLUMN: {
            ...
            if (!pAggInfo->directMode) {
                ...
                if (pTab == 0) {
                    ...
                    VdbeComment(
                        (v, "%s.%s", pTab->zName, pTab->aCol[pCol->iColumn].zCnName));
        ...
        case TK_COLUMN: {
            ...
            if (ExprHasProperty(pExpr, EP_FixedCol)) {
                ...
                */
                ...
                if (aff > SQLITE_AFF_BLOB) {
                    ...
                    sqlite3VdbeAddOp4(v, OP_Affinity, iReg, 1, 0, &zAff[(aff - 'B') * 2],
                                      P4_STATIC);
                }
                return iReg;
            }
            ...
            assert(ExprUseYTab(pExpr));
            assert(pExpr->y.pTab != 0);
            iReg = sqlite3ExprCodeGetColumn(pParse, pExpr->y.pTab, pExpr->iColumn, iTab,
                                            target, pExpr->op2);
            return iReg;
        }
        case TK_INTEGER: {
        ...
        case TK_FUNCTION: {
            ...
            */
            ...
            sqlite3VdbeAddFunctionCall(pParse, constMask, r1, target, nFarg, pDef,
                                       pExpr->op2);
            if (nFarg) {
                if (constMask == 0) {
        ...
        case TK_SELECT: {
            ...
            if (pParse->db->mallocFailed) {
                return 0;
            } else if (op == TK_SELECT && ALWAYS(ExprUseXSelect(pExpr)) &&
        ...
        */
        ...
        case TK_TRIGGER: {
            ...
            */
            ...
            VdbeComment(
                (v, "r[%d]=%s.%s", target, (pExpr->iTable ? "new" : "old"),
        ...
        case TK_IF_NULL_ROW: {
            ...
            if (pAggInfo) {
                ...
                if (pExpr->pAggInfo->useSortingIdx) {
                    sqlite3VdbeAddOp3(v, OP_Column, pAggInfo->sortingIdxPTab,
                                      pAggInfo->aCol[pExpr->iAgg].iSorterColumn, target);
                    inReg = target;
                    break;
                }
            }
        ...
        */
        ...
        case TK_RAISE: {
            assert(pExpr->affExpr == OE_Rollback || pExpr->affExpr == OE_Abort ||
                   pExpr->affExpr == OE_Fail || pExpr->affExpr == OE_Ignore);
            ...
            if (pExpr->affExpr == OE_Ignore) {
                sqlite3VdbeAddOp4(v, OP_Halt, SQLITE_OK, OE_Ignore, 0, pExpr->u.zToken,
                                  0);
                VdbeCoverage(v);
            } else {
                sqlite3HaltConstraint(pParse,
                                      pParse->pTriggerTab ? SQLITE_CONSTRAINT_TRIGGER
                                                          : SQLITE_ERROR,
...
int sqlite3ExprCodeTemp(Parse *pParse, Expr *pExpr, int *pReg) {
    ...
    if (ConstFactorOk(pParse) && ALWAYS(pExpr != 0) && pExpr->op != TK_REGISTER &&
...
void sqlite3ExprIfTrue(Parse *pParse, Expr *pExpr, int dest, int jumpIfNull) {
    ...
    switch (op) {
        ...
        /* no break */ deliberate_fall_through case TK_LT:
        case TK_LE:
        case TK_GT:
        case TK_GE:
        case TK_NE:
        case TK_EQ: {
            ...
            r1 = sqlite3ExprCodeTemp(pParse, pExpr->pLeft, &regFree1);
            r2 = sqlite3ExprCodeTemp(pParse, pExpr->pRight, &regFree2);
            codeCompare(pParse, pExpr->pLeft, pExpr->pRight, op, r1, r2, dest,
                        jumpIfNull, ExprHasProperty(pExpr, EP_Commuted));
            assert(TK_LT == OP_Lt);
            testcase(op == OP_Lt);
            VdbeCoverageIf(v, op == OP_Lt);
            assert(TK_LE == OP_Le);
...
*/
void sqlite3ExprIfFalseDup(Parse *pParse, Expr *pExpr, int dest,
                           int jumpIfNull) {
    sqlite3 *db = pParse->db;
...
*/
static int exprCompareVariable(const Parse *pParse, const Expr *pVar,
                               const Expr *pExpr) {
    int res = 0;
    int iVar;
...
*/
int sqlite3ExprCompare(const Parse *pParse, const Expr *pA, const Expr *pB,
                       int iTab) {
    u32 combinedFlags;
    if (pA == 0 || pB == 0) {
        return pB == pA ? 0 : 2;
    ...
    if (pA->op != pB->op || pA->op == TK_RAISE) {
        ...
        if (pA->op == TK_AGG_COLUMN && pB->op == TK_COLUMN && pB->iTable < 0 &&
            pA->iTable == iTab) {
            /* fall through */
        } else {
            return 2;
    ...
    if (pA->u.zToken) {
        if (pA->op == TK_FUNCTION || pA->op == TK_AGG_FUNCTION) {
            ...
            if (sqlite3_stricmp(pA->u.zToken, pB->u.zToken) != 0)
                return 2;
        } else if (pB->u.zToken != 0 && pA->op != TK_COLUMN &&
                   pA->op != TK_AGG_COLUMN &&
    ...
    if (ALWAYS((combinedFlags & EP_TokenOnly) == 0)) {
        ...
        if ((combinedFlags & EP_FixedCol) == 0 &&
            sqlite3ExprCompare(pParse, pA->pLeft, pB->pLeft, iTab))
            return 2;
        ...
        if (pA->op != TK_STRING && pA->op != TK_TRUEFALSE &&
            ...
            if (pA->iColumn != pB->iColumn)
                return 2;
            if (pA->op2 != pB->op2 && pA->op == TK_TRUTH)
                return 2;
...
int sqlite3ExprCompareSkip(Expr *pA, Expr *pB, int iTab) {
    return sqlite3ExprCompare(0, sqlite3ExprSkipCollate(pA),
                              sqlite3ExprSkipCollate(pB), iTab);
}
...
static int exprImpliesNotNull(
    ...
    switch (p->op) {
        case TK_IN: {
            if (seenNot && ExprHasProperty(p, EP_xIsSelect))
                return 0;
        ...
        case TK_CONCAT:
            seenNot = 1;
        /* no break */ deliberate_fall_through case TK_STAR:
        case TK_REM:
        case TK_BITAND:
        case TK_SLASH: {
...
*/
int sqlite3ExprImpliesExpr(const Parse *pParse, const Expr *pE1,
                           const Expr *pE2, int iTab) {
...
static int exprIdxCover(Walker *pWalker, Expr *pExpr) {
    if (pExpr->op == TK_COLUMN && pExpr->iTable == pWalker->u.pIdxCover->iCur &&
        sqlite3TableColumnToIndex(pWalker->u.pIdxCover->pIdx, pExpr->iColumn) <
            0) {
        pWalker->eCode = 1;
        return WRC_Abort;
    }
...
static int addAggInfoColumn(sqlite3 *db, AggInfo *pInfo) {
    int i;
    pInfo->aCol = sqlite3ArrayAllocate(db, pInfo->aCol, sizeof(pInfo->aCol[0]),
                                       &pInfo->nColumn, &i);
    return i;
}
...
static int addAggInfoFunc(sqlite3 *db, AggInfo *pInfo) {
    int i;
    pInfo->aFunc = sqlite3ArrayAllocate(db, pInfo->aFunc, sizeof(pInfo->aFunc[0]),
                                        &pInfo->nFunc, &i);
    return i;
}
...
static int analyzeAggregate(Walker *pWalker, Expr *pExpr) {
    ...
    switch (pExpr->op) {
        default: {
            ...
            if (pIEpr == 0)
                break;
            if (NEVER(!ExprUseYTab(pExpr)))
                break;
            for (i = 0; i < pSrcList->nSrc; i++) {
                if (pSrcList->a[0].iCursor == pIEpr->iDataCur)
                    break;
            }
            if (i >= pSrcList->nSrc)
                break;
            if (NEVER(pExpr->pAggInfo != 0))
                break; /* Resolved by outer context */
            if (pParse->nErr) {
                return WRC_Abort;
            }
                ...
                */
                ...
                if (i >= pAggInfo->nFunc) {
                    ...
                    if (i >= 0) {
                        ...
                        pItem->pFunc =
                            sqlite3FindFunction(pParse->db, pExpr->u.zToken, nArg, enc, 0);
...
int sqlite3NoTempsInRange(Parse *pParse, int iFirst, int iLast) {
    int i;
    if (pParse->nRangeReg > 0 && pParse->iRangeReg + pParse->nRangeReg > iFirst &&
