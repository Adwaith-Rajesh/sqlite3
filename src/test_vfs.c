...
static int tvfsWrite(sqlite3_file *, const void *, int iAmt,
                     sqlite3_int64 iOfst);
...
static void tvfsExecTcl(Testvfs *p, const char *zMethod, Tcl_Obj *arg1,
                        Tcl_Obj *arg2, Tcl_Obj *arg3, Tcl_Obj *arg4) {
...
static int tvfsClose(sqlite3_file *pFile) {
    ...
    if (p->pScript && p->mask & TESTVFS_CLOSE_MASK) {
        tvfsExecTcl(p, "xClose", Tcl_NewStringObj(pFd->zFilename, -1), pFd->pShmId,
                    0, 0);
    }
...
*/
static int tvfsRead(sqlite3_file *pFile, void *zBuf, int iAmt,
                    sqlite_int64 iOfst) {
    int rc = SQLITE_OK;
    TestvfsFd *pFd = tvfsGetFd(pFile);
    Testvfs *p = (Testvfs *)pFd->pVfs->pAppData;
    if (p->pScript && p->mask & TESTVFS_READ_MASK) {
        tvfsExecTcl(p, "xRead", Tcl_NewStringObj(pFd->zFilename, -1), pFd->pShmId,
                    0, 0);
        tvfsResultCode(p, &rc);
    }
...
*/
static int tvfsWrite(sqlite3_file *pFile, const void *zBuf, int iAmt,
                     sqlite_int64 iOfst) {
    int rc = SQLITE_OK;
    TestvfsFd *pFd = tvfsGetFd(pFile);
    Testvfs *p = (Testvfs *)pFd->pVfs->pAppData;
    ...
    if (p->pScript && p->mask & TESTVFS_WRITE_MASK) {
        tvfsExecTcl(p, "xWrite", Tcl_NewStringObj(pFd->zFilename, -1), pFd->pShmId,
...
static int tvfsTruncate(sqlite3_file *pFile, sqlite_int64 size) {
    ...
    if (p->pScript && p->mask & TESTVFS_TRUNCATE_MASK) {
        tvfsExecTcl(p, "xTruncate", Tcl_NewStringObj(pFd->zFilename, -1),
                    pFd->pShmId, 0, 0);
        tvfsResultCode(p, &rc);
    }
...
static int tvfsSync(sqlite3_file *pFile, int flags) {
    ...
    if (p->pScript && p->mask & TESTVFS_SYNC_MASK) {
        ...
        tvfsExecTcl(p, "xSync", Tcl_NewStringObj(pFd->zFilename, -1), pFd->pShmId,
...
static int tvfsFileControl(sqlite3_file *pFile, int op, void *pArg) {
    ...
    if (p->pScript && (p->mask & TESTVFS_FCNTL_MASK)) {
        ...
        if (i < sizeof(aF) / sizeof(aF[0])) {
            int rc = 0;
            tvfsExecTcl(p, "xFileControl", Tcl_NewStringObj(pFd->zFilename, -1),
                        Tcl_NewStringObj(aF[i].zFnctl, -1), 0, 0);
            tvfsResultCode(p, &rc);
            if (rc)
                return (rc < 0 ? SQLITE_OK : rc);
        }
    }
...
*/
static int tvfsOpen(sqlite3_vfs *pVfs, const char *zName, sqlite3_file *pFile,
    ...
    */
    ...
    if ((p->mask & TESTVFS_OPEN_MASK) && tvfsInjectIoerr(p))
        return SQLITE_IOERR;
    if (tvfsInjectCantopenerr(p))
        return SQLITE_CANTOPEN;
    if (tvfsInjectFullerr(p))
        return SQLITE_FULL;
...
static int tvfsDelete(sqlite3_vfs *pVfs, const char *zPath, int dirSync) {
    ...
    if (p->pScript && p->mask & TESTVFS_DELETE_MASK) {
        tvfsExecTcl(p, "xDelete", Tcl_NewStringObj(zPath, -1),
                    Tcl_NewIntObj(dirSync), 0, 0);
        tvfsResultCode(p, &rc);
    }
    if (rc == SQLITE_OK) {
...
*/
static int tvfsAccess(sqlite3_vfs *pVfs, const char *zPath, int flags,
                      int *pResOut) {
    Testvfs *p = (Testvfs *)pVfs->pAppData;
    if (p->pScript && p->mask & TESTVFS_ACCESS_MASK) {
        ...
        if (flags == SQLITE_ACCESS_EXISTS)
            zArg = "SQLITE_ACCESS_EXISTS";
        if (flags == SQLITE_ACCESS_READWRITE)
            zArg = "SQLITE_ACCESS_READWRITE";
        if (flags == SQLITE_ACCESS_READ)
            zArg = "SQLITE_ACCESS_READ";
        tvfsExecTcl(p, "xAccess", Tcl_NewStringObj(zPath, -1),
                    Tcl_NewStringObj(zArg, -1), 0, 0);
        if (tvfsResultCode(p, &rc)) {
            if (rc != SQLITE_OK)
                return rc;
        } else {
            ...
            if (TCL_OK ==
                Tcl_GetBooleanFromObj(0, Tcl_GetObjResult(interp), pResOut)) {
                return SQLITE_OK;
            }
        }
    }
...
*/
static int tvfsFullPathname(sqlite3_vfs *pVfs, const char *zPath, int nOut,
                            char *zOut) {
    Testvfs *p = (Testvfs *)pVfs->pAppData;
...
static int tvfsShmOpen(sqlite3_file *pFile) {
    ...
    */
    ...
    for (pBuffer = p->pBuffer; pBuffer; pBuffer = pBuffer->pNext) {
        if (0 == strcmp(pFd->zFilename, pBuffer->zFile))
            break;
    }
    if (!pBuffer) {
...
static int tvfsShmMap(sqlite3_file *pFile, /* Handle open on database file */
    ...
    if (p->pScript && p->mask & TESTVFS_SHMMAP_MASK) {
        ...
        tvfsExecTcl(p, "xShmMap", Tcl_NewStringObj(pFd->pShm->zFile, -1),
                    pFd->pShmId, pArg, 0);
        tvfsResultCode(p, &rc);
        Tcl_DecrRefCount(pArg);
    }
...
static int tvfsShmLock(sqlite3_file *pFile, int ofst, int n, int flags) {
    int rc = SQLITE_OK;
    TestvfsFd *pFd = tvfsGetFd(pFile);
    Testvfs *p = (Testvfs *)(pFd->pVfs->pAppData);
    int nLock;
    ...
    if (p->pScript && p->mask & TESTVFS_SHMLOCK_MASK) {
        ...
        tvfsExecTcl(p, "xShmLock", Tcl_NewStringObj(pFd->pShm->zFile, -1),
                    pFd->pShmId, Tcl_NewStringObj(zLock, -1), 0);
        tvfsResultCode(p, &rc);
    }
...
static int tvfsShmUnmap(sqlite3_file *pFile, int deleteFlag) {
    ...
    if (p->pScript && p->mask & TESTVFS_SHMCLOSE_MASK) {
        tvfsExecTcl(p, "xShmUnmap", Tcl_NewStringObj(pFd->pShm->zFile, -1),
                    pFd->pShmId, 0, 0);
        tvfsResultCode(p, &rc);
    }
...
static int tvfsFetch(sqlite3_file *pFile, sqlite3_int64 iOfst, int iAmt,
                     void **pp) {
    TestvfsFd *pFd = tvfsGetFd(pFile);
...
static int SQLITE_TCLAPI testvfs_obj_cmd(ClientData cd, Tcl_Interp *interp,
    ...
    if (Tcl_GetIndexFromObjStruct(interp, objv[1], aSubcmd, sizeof(aSubcmd[0]),
                                  "subcommand", 0, &i)) {
        return TCL_ERROR;
    }
    ...
    switch (aSubcmd[i].eCmd) {
        case CMD_SHM: {
            ...
            rc = p->pParent->xFullPathname(p->pParent, Tcl_GetString(objv[2]),
                                           p->pParent->mxPathname, zName);
            if (rc != SQLITE_OK) {
                Tcl_AppendResult(interp,
                                 "failed to get full path: ", Tcl_GetString(objv[2]), 0);
                ckfree(zName);
                return TCL_ERROR;
            }
            for (pBuffer = p->pBuffer; pBuffer; pBuffer = pBuffer->pNext) {
                if (0 == strcmp(pBuffer->zFile, zName))
                    break;
            }
            ckfree(zName);
            if (!pBuffer) {
        ...
        */
        ...
        case CMD_FULLERR: {
            ...
            if (objc == 4) {
                ...
                pTest->eFault =
                    iPersist ? FAULT_INJECT_PERSISTENT : FAULT_INJECT_TRANSIENT;
                pTest->iCnt = iCnt;
            } else if (objc != 2) {
        ...
        case CMD_DEVCHAR: {
            ...
            if (objc == 3) {
                ...
                for (j = 0; j < nFlags; j++) {
                    int idx = 0;
                    if (Tcl_GetIndexFromObjStruct(interp, flags[j], aFlag, sizeof(aFlag[0]),
                                                  "flag", 0, &idx)) {
                        return TCL_ERROR;
                    }
            ...
            for (iFlag = 0; iFlag < sizeof(aFlag) / sizeof(aFlag[0]); iFlag++) {
                if (p->iDevchar & aFlag[iFlag].iValue) {
                    Tcl_ListObjAppendElement(interp, pRet,
                                             Tcl_NewStringObj(aFlag[iFlag].zName, -1));
                }
            }
...
*/
static int SQLITE_TCLAPI testvfs_cmd(ClientData cd, Tcl_Interp *interp,
                                     int objc, Tcl_Obj *CONST objv[]) {
...
bad_args:
    Tcl_WrongNumArgs(interp, 1, objv,
                     "VFSNAME ?-noshm BOOL? ?-fullshm BOOL? ?-default BOOL? "
                     "?-mxpathname INT? ?-szosfile INT? ?-iversion INT?");
    return TCL_ERROR;
}
...
*/
static int SQLITE_TCLAPI test_vfs_shmlock(void *clientData, Tcl_Interp *interp,
                                          int objc, Tcl_Obj *CONST objv[]) {
...
static int SQLITE_TCLAPI test_vfs_set_readmark(void *clientData,
