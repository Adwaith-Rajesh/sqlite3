...
void sqlite3DequoteToken(Token *p) {
    ...
    if (p->n < 2)
        return;
    if (!sqlite3Isquote(p->z[0]))
        return;
    for (i = 1; i < p->n - 1; i++) {
        if (sqlite3Isquote(p->z[i]))
            return;
    }
    p->n -= 2;
    p->z++;
...
void sqlite3FpDecode(FpDecode *p, double r, int iRound, int mxRound) {
    ...
    */
    ...
    while (ALWAYS(p->n > 0) && p->z[p->n - 1] == '0') {
        p->n--;
    }
}
...
static void logBadConnection(const char *zType) {
    sqlite3_log(SQLITE_MISUSE, "API call with %s database connection pointer",
                zType);
}
...
int sqlite3SafetyCheckSickOrOk(sqlite3 *db) {
    u8 eOpenState;
    eOpenState = db->eOpenState;
    if (eOpenState != SQLITE_STATE_SICK && eOpenState != SQLITE_STATE_OPEN &&
