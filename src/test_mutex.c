...
static const char *aName[MAX_MUTEXES + 1] = {
    "fast", "recursive", "static_main", "static_mem", "static_open",
    "static_prng", "static_lru", "static_pmem", "static_app1", "static_app2",
    "static_app3", "static_vfs1", "static_vfs2", "static_vfs3", 0};
...
** sqlite3_shutdown
*/
static int SQLITE_TCLAPI test_shutdown(void *clientData, Tcl_Interp *interp,
...
** sqlite3_initialize
*/
static int SQLITE_TCLAPI test_initialize(void *clientData, Tcl_Interp *interp,
                                         int objc, Tcl_Obj *CONST objv[]) {
    int rc;
...
*/
static int SQLITE_TCLAPI test_install_mutex_counters(void *clientData,
                                                     Tcl_Interp *interp,
                                                     int objc,
                                                     Tcl_Obj *CONST objv[]) {
    int rc = SQLITE_OK;
    ...
    sqlite3_mutex_methods counter_methods = {
        ...
        counterMutexFree, counterMutexEnter, counterMutexTry,
        counterMutexLeave, counterMutexHeld, counterMutexNotheld};
    ...
    if (isInstall == g.isInstalled) {
        ...
        Tcl_AppendResult(interp, isInstall ? "already installed" : "not installed",
                         0);
        return TCL_ERROR;
    }
...
** read_mutex_counters
*/
static int SQLITE_TCLAPI test_read_mutex_counters(void *clientData,
...
*/
static int SQLITE_TCLAPI test_clear_mutex_counters(void *clientData,
                                                   Tcl_Interp *interp, int objc,
                                                   Tcl_Obj *CONST objv[]) {
    int ii;
...
*/
static int SQLITE_TCLAPI test_alloc_mutex(void *clientData, Tcl_Interp *interp,
                                          int objc, Tcl_Obj *CONST objv[]) {
...
*/
static int SQLITE_TCLAPI test_config(void *clientData, Tcl_Interp *interp,
                                     int objc, Tcl_Obj *CONST objv[]) {
    struct ConfigOption {
        const char *zName;
        int iValue;
    } aOpt[] = {{"singlethread", SQLITE_CONFIG_SINGLETHREAD},
...
static sqlite3_mutex *getStaticMutexPointer(Tcl_Interp *pInterp,
                                            Tcl_Obj *pObj) {
    int iMutex;
...
static int SQLITE_TCLAPI test_enter_static_mutex(void *clientData,
                                                 Tcl_Interp *interp, int objc,
                                                 Tcl_Obj *CONST objv[]) {
...
static int SQLITE_TCLAPI test_leave_static_mutex(void *clientData,
                                                 Tcl_Interp *interp, int objc,
                                                 Tcl_Obj *CONST objv[]) {
    sqlite3_mutex *pMutex;
    if (objc != 2) {
...
static int SQLITE_TCLAPI test_enter_db_mutex(void *clientData,
                                             Tcl_Interp *interp, int objc,
                                             Tcl_Obj *CONST objv[]) {
    sqlite3 *db;
    if (objc != 2) {
...
static int SQLITE_TCLAPI test_leave_db_mutex(void *clientData,
                                             Tcl_Interp *interp, int objc,
                                             Tcl_Obj *CONST objv[]) {
    sqlite3 *db;
    if (objc != 2) {
...
int Sqlitetest_mutex_Init(Tcl_Interp *interp) {
    ...
    Tcl_LinkVar(interp, "disable_mutex_init", (char *)&g.disableInit,
                TCL_LINK_INT);
    Tcl_LinkVar(interp, "disable_mutex_try", (char *)&g.disableTry, TCL_LINK_INT);
    return SQLITE_OK;
}
