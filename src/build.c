...
lockTable(Parse *pParse,    /* Parsing context */
    ...
    if (v) {
        ...
        */
        ...
        do {
            ...
            VdbeComment(
                (v, "usesStmtJournal=%d", pParse->mayAbort && pParse->isMultiWrite));
        } while (++iDb < db->nDb);
    ...
    ** exists */
    ...
    if (zDatabase) {
        ...
        if (p == 0 && sqlite3StrNICmp(zName, "sqlite_", 7) == 0) {
            ...
            } else if (sqlite3StrICmp(zName + 7, &PREFERRED_TEMP_SCHEMA_TABLE[7]) ==
                       0) {
                p = sqlite3HashFind(&db->aDb[1].pSchema->tblHash,
                                    LEGACY_TEMP_SCHEMA_TABLE);
            }
    ...
    ** and code in pParse and return NULL. */
    ...
    if (p == 0) {
        ...
        ** can be an eponymous virtual table. */
        if ((pParse->prepFlags & SQLITE_PREPARE_NO_VTAB) == 0 &&
            db->init.busy == 0) {
...
Expr *sqlite3ColumnExpr(Table *pTab, Column *pCol) {
    if (pCol->iDflt == 0)
        return 0;
    if (NEVER(!IsOrdinaryTable(pTab)))
        return 0;
    if (NEVER(pTab->u.tab.pDfltList == 0))
        return 0;
    if (NEVER(pTab->u.tab.pDfltList->nExpr < pCol->iDflt))
        return 0;
...
const char *sqlite3ColumnColl(Column *pCol) {
    const char *z;
    if ((pCol->colFlags & COLFLAG_HASCOLL) == 0)
        return 0;
    z = pCol->zCnName;
    while (*z) {
        z++;
...
int sqlite3WritableSchema(sqlite3 *db) {
    ...
    return (db->flags & (SQLITE_WriteSchema | SQLITE_Defensive)) ==
           SQLITE_WriteSchema;
}
...
int sqlite3CheckObjectName(
    ...
    if (db->init.busy) {
        ...
        if ((pParse->nested == 0 && 0 == sqlite3StrNICmp(zName, "sqlite_", 7)) ||
            (sqlite3ReadOnlyShadowTables(db) &&
             sqlite3ShadowTableName(db, zName))) {
...
void sqlite3StartTable(
    ...
    {
        static const u8 aCode[] = {SQLITE_CREATE_TABLE, SQLITE_CREATE_TEMP_TABLE,
    ...
    */
    if (!db->init.busy && (v = sqlite3GetVdbe(pParse)) != 0) {
        ...
        */
        ...
        VdbeCoverage(v);
        fileFormat =
            (db->flags & SQLITE_LegacyFileFmt) != 0 ? 1 : SQLITE_MAX_FILE_FORMAT;
...
void sqlite3AddReturning(Parse *pParse, ExprList *pList) {
    ...
    sqlite3ParserAddCleanup(
        pParse, (void (*)(sqlite3 *, void *))sqlite3DeleteReturning, pRet);
    testcase(pParse->earlyCleanup);
    if (db->mallocFailed)
        return;
    sqlite3_snprintf(sizeof(pRet->zName), pRet->zName, "sqlite_returning_%p",
                     pParse);
    ...
    pHash = &(db->aDb[1].pSchema->trigHash);
    assert(sqlite3HashFind(pHash, pRet->zName) == 0 || pParse->nErr ||
           pParse->ifNotExists);
...
void sqlite3AddColumn(Parse *pParse, Token sName, Token sType) {
    ...
    ** text. */
    if (sType.n >= 16 &&
        ...
        sType.n -= 6;
        while (ALWAYS(sType.n > 0) && sqlite3Isspace(sType.z[sType.n - 1]))
            sType.n--;
        if (sType.n >= 9 &&
            sqlite3_strnicmp(sType.z + (sType.n - 9), "generated", 9) == 0) {
            sType.n -= 9;
            while (sType.n > 0 && sqlite3Isspace(sType.z[sType.n - 1]))
                sType.n--;
        }
    }
    ...
    ** the column name, in order to save space. */
    ...
    for (i = 0; i < p->nCol; i++) {
        if (p->aCol[i].hName == hName &&
            sqlite3StrICmp(z, p->aCol[i].zCnName) == 0) {
...
void sqlite3AddPrimaryKey(
    ...
    if (nTerm == 1 && pCol && pCol->eCType == COLTYPE_INTEGER &&
...
static void identPut(char *z, int *pIdx, char *zSignedIdent) {
    ...
    needQuote = sqlite3Isdigit(zIdent[0]) ||
                sqlite3KeywordCode(zIdent, j) != TK_ID || zIdent[j] != 0 ||
                j == 0;
...
void sqlite3MarkAllShadowTablesOf(sqlite3 *db, Table *pTab) {
    ...
    for (k = sqliteHashFirst(&pTab->pSchema->tblHash); k; k = sqliteHashNext(k)) {
        ...
        if (!IsOrdinaryTable(pOther))
            continue;
        if (pOther->tabFlags & TF_Shadow)
            continue;
...
void sqlite3EndTable(
    ...
    */
        ...
        for (ii = 0; ii < p->nCol; ii++) {
            ...
            if (pCol->eCType == COLTYPE_CUSTOM) {
                if (pCol->colFlags & COLFLAG_HASTYPE) {
                    ...
                    sqlite3ErrorMsg(pParse, "missing datatype for %s.%s", p->zName,
                                    pCol->zCnName);
                }
                return;
            ...
            if ((pCol->colFlags & COLFLAG_PRIMKEY) != 0 && p->iPKey != ii &&
    ...
    assert((p->tabFlags & TF_HasPrimaryKey) == 0 || p->iPKey >= 0 ||
           sqlite3PrimaryKeyIndex(p) != 0);
    ...
    if (!db->init.busy) {
        ...
        */
        ...
        if (pSelect) {
            ...
            if (pEnd2->z[0] != ';')
                n += pEnd2->n;
            zStmt =
                sqlite3MPrintf(db, "CREATE %s %.*s", zType2, n, pParse->sNameToken.z);
        }
        ...
        */
        sqlite3NestedParse(
            ...
            " WHERE rowid=#%d",
            db->aDb[iDb].zDbSName, zType, p->zName, p->zName, pParse->regRoot,
    ...
    */
    ...
    if (pSel) {
        ...
        if (pSelTab == 0) {
            ...
            */
            sqlite3ColumnsFromExprList(pParse, pTable->pCheck, &pTable->nCol,
                                       &pTable->aCol);
...
int sqlite3ViewGetColumnNames(Parse *pParse, Table *pTable) {
    assert(pTable != 0);
    if (!IsVirtual(pTable) && pTable->nCol > 0)
        return 0;
...
static void sqliteViewResetAll(sqlite3 *db, int idx) {
    ...
    if (!DbHasProperty(db, idx, DB_UnresetViews))
        return;
    for (i = sqliteHashFirst(&db->aDb[idx].pSchema->tblHash); i;
         i = sqliteHashNext(i)) {
...
sqlite3ClearStatTables(Parse *pParse,     /* The parsing context */
    ...
    for (i = 1; i <= 4; i++) {
        ...
        if (sqlite3FindTable(pParse->db, zTab, zDbName)) {
            sqlite3NestedParse(pParse, "DELETE FROM %Q.%s WHERE %s=%Q", zDbName, zTab,
                               zType, zName);
        }
    }
}
...
#ifndef SQLITE_OMIT_VIRTUALTABLE
    if ((db->flags & SQLITE_Defensive) != 0 && db->pVtabCtx == 0 &&
        db->nVdbeExec == 0 && !sqlite3VtabInSync(db)) {
        return 1;
    }
#endif
    return 0;
...
void sqlite3CreateForeignKey(
    ...
    pNextTo = (FKey *)sqlite3HashInsert(&p->pSchema->fkeyHash, pFKey->zTo,
                                        (void *)pFKey);
    if (pNextTo == pFKey) {
        sqlite3OomFault(db);
        goto fk_end;
...
static void sqlite3RefillIndex(Parse *pParse, Index *pIndex, int memRootPage) {
    ...
    sqlite3VdbeAddOp4(v, OP_SorterOpen, iSorter, 0, pIndex->nKeyCol,
                      (char *)sqlite3KeyInfoRef(pKey), P4_KEYINFO);
    ...
    ** records into the sorter. */
    ...
    sqlite3GenerateIndexKey(pParse, pIndex, iTab, regRecord, 0, &iPartIdxLabel, 0,
                            0);
    ...
    sqlite3VdbeAddOp4(v, OP_OpenWrite, iIdx, (int)tnum, iDb, (char *)pKey,
                      P4_KEYINFO);
    sqlite3VdbeChangeP5(v, OPFLAG_BULKCSR |
                               ((memRootPage >= 0) ? OPFLAG_P2ISREG : 0));
...
void sqlite3CreateIndex(
    ...
    */
    ...
    assert(pTab != 0);
    if (sqlite3StrNICmp(pTab->zName, "sqlite_", 7) == 0 && db->init.busy == 0 &&
        pTblName != 0
#if SQLITE_USER_AUTHENTICATION
    ...
    */
    if (pName) {
        ...
        assert(pName->z != 0);
        if (SQLITE_OK !=
            sqlite3CheckObjectName(pParse, zName, "index", pTab->zName)) {
            goto exit_create_index;
        }
        if (!IN_RENAME_OBJECT) {
            if (!db->init.busy) {
    ...
    ** it as a covering index */
    assert(HasRowid(pTab) || pTab->iPKey < 0 ||
           sqlite3TableColumnToIndex(pIndex, pTab->iPKey) >= 0);
        ...
        */
            ...
            */
            ...
            sqlite3NestedParse(pParse,
                               ...
                               db->aDb[iDb].zDbSName, pIndex->zName, pTab->zName,
                               iMem, zStmt);
            sqlite3DbFree(db, zStmt);
...
exit_create_index:
    ...
    if (pTab) {
        ...
        ** order.  So we have to reorder at most one index. */
        Index **ppFrom;
        Index *pThis;
        for (ppFrom = &pTab->pIndex; (pThis = *ppFrom) != 0;
             ppFrom = &pThis->pNext) {
            Index *pNext;
        ...
        ** comes after a REPLACE index on the list. */
        for (pThis = pTab->pIndex; pThis; pThis = pThis->pNext) {
            assert(pThis->onError != OE_Replace || pThis->pNext == 0 ||
    ...
    if (p && p->nSrc > 1) {
        ...
        ** JT_LTORJ flags */
        if (allFlags & JT_RIGHT) {
            for (i = p->nSrc - 1;
                 ALWAYS(i > 0) && (p->a[i].fg.jointype & JT_RIGHT) == 0; i--) {
            }
            i--;
            assert(i >= 0);
            do {
...
*/
void sqlite3HaltConstraint(Parse *pParse, /* Parsing context */
...
KeyInfo *sqlite3KeyInfoOfIndex(Parse *pParse, Index *pIdx) {
    ...
    if (pKey) {
        ...
        for (i = 0; i < nCol; i++) {
            ...
            pKey->aColl[i] =
                zColl == sqlite3StrBINARY ? 0 : sqlite3LocateCollSeq(pParse, zColl);
...
*/
With *sqlite3WithAdd(Parse *pParse, /* Parsing context */
