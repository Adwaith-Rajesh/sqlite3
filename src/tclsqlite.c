...
*/
static int SQLITE_TCLAPI incrblobInput(ClientData instanceData, char *buf,
...
*/
static int SQLITE_TCLAPI incrblobSeek(ClientData instanceData, long offset,
...
static int SQLITE_TCLAPI incrblobHandle(ClientData instanceData, int dir,
                                        ClientData *hPtr) {
    return TCL_ERROR;
}
...
*/
static int createIncrblobChannel(Tcl_Interp *interp, SqliteDb *pDb,
                                 const char *zDb, const char *zTable,
                                 const char *zColumn, sqlite_int64 iRow,
                                 int isReadonly) {
    IncrblobChannel *p;
    sqlite3 *db = pDb->db;
    sqlite3_blob *pBlob;
...
*/
static void addDatabaseRef(SqliteDb *pDb) { pDb->nRef++; }
...
*/
static int
DbTraceV2Handler(unsigned type, /* One of the SQLITE_TRACE_* event types. */
    ...
    switch (type) {
        case SQLITE_TRACE_STMT: {
            ...
            Tcl_ListObjAppendElement(pDb->interp, pCmd, Tcl_NewStringObj(zSql, -1));
...
*/
static int DbWalHandler(void *clientData, sqlite3 *db, const char *zDb,
                        int nEntry) {
    int ret = SQLITE_OK;
    Tcl_Obj *p;
    SqliteDb *pDb = (SqliteDb *)clientData;
...
*/
static void DbPreUpdateHandler(void *p, sqlite3 *db, int op, const char *zDb,
                               const char *zTbl, sqlite_int64 iKey1,
                               sqlite_int64 iKey2) {
...
static void DbUpdateHandler(void *p, int op, const char *zDb, const char *zTbl,
                            sqlite_int64 rowid) {
    SqliteDb *pDb = (SqliteDb *)p;
    Tcl_Obj *pCmd;
    static const char *azStr[] = {"DELETE", "INSERT", "UPDATE"};
...
static void tclCollateNeeded(void *pCtx, sqlite3 *db, int enc,
                             const char *zName) {
...
** using TCL script.
*/
static int tclSqlCollate(void *pCtx, int nA, const void *zA, int nB,
                         const void *zB) {
...
** using TCL script.
*/
static void tclSqlFunc(sqlite3_context *context, int argc,
                       sqlite3_value **argv) {
...
*/
static int auth_callback(void *pArg, int code, const char *zArg1,
...
*/
static int dbPrepare(SqliteDb *pDb,         /* Database object */
...
static int dbPrepareAndBind(
    ...
    *ppPreStmt = 0;
    ...
    for (pPreStmt = pDb->stmtList; pPreStmt; pPreStmt = pPreStmt->pNext) {
        int n = pPreStmt->nSql;
        if (nSql >= n && memcmp(pPreStmt->zSql, zSql, n) == 0 &&
            (zSql[n] == 0 || zSql[n - 1] == ';')) {
...
*/
static void
dbEvalInit(DbEvalContext *p, /* Pointer to structure to initialize */
           SqliteDb *pDb,    /* Database handle */
...
*/
static void dbEvalRowInfo(DbEvalContext *p,     /* Evaluation context */
    ...
    while ((rc == TCL_OK || rc == TCL_CONTINUE) &&
        ...
        for (i = 0; i < nCol; i++) {
            if (pArray == 0) {
                ...
                Tcl_ObjSetVar2(interp, pArray, apColName[i], dbEvalColumnValue(p, i),
                               0);
            }
        }
        ...
        ** next iteration of this while(...) loop.  */
        if (DbUseNre()) {
            Tcl_NRAddCallback(interp, DbEvalNextCmd, (void *)p, (void *)pScript, 0,
                              0);
...
*/
static void DbHookCmd(Tcl_Interp *interp, /* Tcl interpreter */
                      SqliteDb *pDb,      /* Database handle */
...
#ifdef SQLITE_ENABLE_PREUPDATE_HOOK
    sqlite3_preupdate_hook(db, (pDb->pPreUpdateHook ? DbPreUpdateHandler : 0),
                           pDb);
#endif
...
*/
static int SQLITE_TCLAPI DbObjCmd(void *cd, Tcl_Interp *interp, int objc,
                                  Tcl_Obj *const *objv) {
    SqliteDb *pDb = (SqliteDb *)cd;
    int choice;
    int rc = TCL_OK;
    static const char *DB_strs[] = {"authorizer",
                                    "backup",
                                    "bind_fallback",
                                    "busy",
                                    "cache",
    ...
    switch ((enum DB_enum)choice) {
        ...
        */
        case DB_AUTHORIZER: {
            ...
            if (objc > 3) {
                ...
                if (pDb->zAuth) {
                    typedef int (*sqlite3_auth_cb)(void *, int, const char *, const char *,
        ...
        case DB_BACKUP: {
            ...
            if (rc != SQLITE_OK) {
                Tcl_AppendResult(interp,
                                 "cannot open target database: ", sqlite3_errmsg(pDest),
                                 (char *)0);
                sqlite3_close(pDest);
                return TCL_ERROR;
            }
            ...
            if (pBackup == 0) {
                Tcl_AppendResult(interp, "backup failed: ", sqlite3_errmsg(pDest),
                                 (char *)0);
                sqlite3_close(pDest);
                return TCL_ERROR;
            }
                ...
                if (objc != 4) {
                    ...
                    if (TCL_ERROR == Tcl_GetIntFromObj(interp, objv[3], &n)) {
                        Tcl_AppendResult(interp, "cannot convert \"",
                                         Tcl_GetStringFromObj(objv[3], 0), "\" to integer",
                                         (char *)0);
                        return TCL_ERROR;
                    } else {
                        if (n < 0) {
        ...
        */
        case DB_COLLATE: {
            ...
            if (sqlite3_create_collation(pDb->db, zName, SQLITE_UTF8, pCollate,
                                         tclSqlCollate)) {
                Tcl_SetResult(interp, (char *)sqlite3_errmsg(pDb->db), TCL_VOLATILE);
                return TCL_ERROR;
            }
        ...
        */
        case DB_COPY: {
            ...
            if (strcmp(zConflict, "rollback") != 0 && strcmp(zConflict, "abort") != 0 &&
                strcmp(zConflict, "fail") != 0 && strcmp(zConflict, "ignore") != 0 &&
                strcmp(zConflict, "replace") != 0) {
                Tcl_AppendResult(interp, "Error: \"", zConflict,
                                 "\", conflict-algorithm must be one of: rollback, "
                                 "abort, fail, ignore, or replace",
                                 (char *)0);
                return TCL_ERROR;
            }
        ...
        */
        case DB_DESERIALIZE: {
            ...
            Tcl_AppendResult(interp, "MEMDB not available in this build", (char *)0);
            rc = TCL_ERROR;
#else
            ...
            if (pData == 0 && len > 0) {
                ...
                if (mxSize > 0) {
                    sqlite3_file_control(pDb->db, zSchema, SQLITE_FCNTL_SIZE_LIMIT,
                                         &mxSize);
                }
            }
        deserialize_error:
        ...
        */
        case DB_EVAL: {
            ...
            if (objc < 3 || objc > 5) {
                Tcl_WrongNumArgs(interp, 2, objv, "?OPTIONS? SQL ?ARRAY-NAME? ?SCRIPT?");
                return TCL_ERROR;
            }
        ...
        case DB_FUNCTION: {
            ...
            for (i = 3; i < (objc - 1); i++) {
                ...
                if (n > 1 && strncmp(z, "-argcount", n) == 0) {
                    if (i == (objc - 2)) {
                        Tcl_AppendResult(interp, "option requires an argument: ", z,
                                         (char *)0);
                        return TCL_ERROR;
                    }
                    if (Tcl_GetIntFromObj(interp, objv[i + 1], &nArg))
                        return TCL_ERROR;
                    if (nArg < 0) {
            ...
            pFunc->eType = eType;
            rc = sqlite3_create_function(pDb->db, zName, nArg, flags, pFunc, tclSqlFunc,
                                         0, 0);
            if (rc != SQLITE_OK) {
                rc = TCL_ERROR;
                Tcl_SetResult(interp, (char *)sqlite3_errmsg(pDb->db), TCL_VOLATILE);
    ...
    !defined(SQLITE_OMIT_DEPRECATED)
        ...
        */
        case DB_RESTORE: {
            ...
            rc = sqlite3_open_v2(zSrcFile, &pSrc, SQLITE_OPEN_READONLY | pDb->openFlags,
                                 0);
            if (rc != SQLITE_OK) {
                Tcl_AppendResult(interp,
                                 "cannot open source database: ", sqlite3_errmsg(pSrc),
                                 (char *)0);
                sqlite3_close(pSrc);
                return TCL_ERROR;
            }
            ...
            if (pBackup == 0) {
                Tcl_AppendResult(interp, "restore failed: ", sqlite3_errmsg(pDb->db),
                                 (char *)0);
                sqlite3_close(pSrc);
                return TCL_ERROR;
            }
        ...
        */
        case DB_STATUS: {
            ...
            if (strcmp(zOp, "step") == 0) {
                ...
                Tcl_AppendResult(
                    interp, "bad argument: should be autoindex, step, sort or vmstep",
                    (char *)0);
                return TCL_ERROR;
            }
    ...
    !defined(SQLITE_OMIT_DEPRECATED)
        ...
        */
        case DB_TRACE_V2: {
            if (objc > 4) {
                ...
                Tcl_WideInt wMask = 0;
                if (objc == 4) {
                    static const char *TTYPE_strs[] = {"statement", "profile", "row",
                                                       "close", 0};
                    enum TTYPE_enum { TTYPE_STMT,
                                      TTYPE_PROFILE,
                                      TTYPE_ROW,
                                      TTYPE_CLOSE };
                    int i;
                    ...
                    for (i = 0; i < len; i++) {
                        ...
                        if (Tcl_GetIndexFromObj(interp, pObj, TTYPE_strs, "trace type", 0,
                                                &ttype) != TCL_OK) {
                            Tcl_WideInt wType;
                            ...
                            switch ((enum TTYPE_enum)ttype) {
                                case TTYPE_STMT:
                                    ...
                                    break;
                                case TTYPE_PROFILE:
                                    wMask |= SQLITE_TRACE_PROFILE;
                                    break;
                                case TTYPE_ROW:
                                    wMask |= SQLITE_TRACE_ROW;
                                    break;
                                case TTYPE_CLOSE:
                                    wMask |= SQLITE_TRACE_CLOSE;
                                    break;
                            }
                        }
                    }
        ...
        */
        case DB_TRANSACTION: {
            ...
            if (pDb->nTransaction == 0 && objc == 4) {
                static const char *TTYPE_strs[] = {"deferred", "exclusive", "immediate",
                                                   0};
                enum TTYPE_enum { TTYPE_DEFERRED,
                                  TTYPE_EXCLUSIVE,
                                  TTYPE_IMMEDIATE };
                int ttype;
        ...
        */
        case DB_PREUPDATE: {
            ...
            enum DbPreupdateSubCmd { PRE_COUNT,
                                     PRE_DEPTH,
                                     PRE_HOOK,
                                     PRE_NEW,
                                     PRE_OLD };
...
*/
static int SQLITE_TCLAPI DbObjCmdAdaptor(void *cd, Tcl_Interp *interp, int objc,
                                         Tcl_Obj *const *objv) {
...
** incorrect.
*/
static int sqliteCmdUsage(Tcl_Interp *interp, Tcl_Obj *const *objv) {
    Tcl_WrongNumArgs(
        interp, 1, objv,
...
**
*/
static int SQLITE_TCLAPI DbMain(void *cd, Tcl_Interp *interp, int objc,
                                Tcl_Obj *const *objv) {
    SqliteDb *p;
    const char *zArg;
    char *zErrMsg;
    ...
    */
    ...
    for (i = 2; i < objc; i++) {
        ...
        if (strcmp(zArg, "-key") == 0) {
            ...
            int b;
            if (Tcl_GetBooleanFromObj(interp, objv[i], &b))
                return TCL_ERROR;
            if (b) {
    ...
    zArg = Tcl_GetStringFromObj(objv[1], 0);
    if (DbUseNre()) {
        Tcl_NRCreateCommand(interp, zArg, DbObjCmdAdaptor, DbObjCmd, (char *)p,
                            DbDeleteCmd);
    } else {
