...
*/
static int faultsimFailures(void) { return memfault.nFail; }

/*
** Return the number of benign faults that have occurred since the
** injector was last configured.
*/
static int faultsimBenignFailures(void) { return memfault.nBenign; }
...
static void faultsimBeginBenign(void) { memfault.isBenignMode++; }
static void faultsimEndBenign(void) { memfault.isBenignMode--; }
...
static int faultsimInstall(int install) {
    ...
    if (install) {
        ...
        ** a zeroed allocator then calling GETMALLOC. */
        ...
        sqlite3_test_control(SQLITE_TESTCTRL_BENIGN_MALLOC_HOOKS, (void *)0,
                             (void *)0);
    }
...
*/
static int SQLITE_TCLAPI test_malloc(void *clientData, Tcl_Interp *interp,
    ...
    if (Tcl_GetIntFromObj(interp, objv[1], &nByte))
        return TCL_ERROR;
...
*/
static int SQLITE_TCLAPI test_realloc(void *clientData, Tcl_Interp *interp,
                                      int objc, Tcl_Obj *CONST objv[]) {
    int nByte;
    void *pPrior, *p;
    char zOut[100];
    ...
    if (Tcl_GetIntFromObj(interp, objv[2], &nByte))
        return TCL_ERROR;
    if (textToPointer(Tcl_GetString(objv[1]), &pPrior)) {
        Tcl_AppendResult(interp, "bad pointer: ", Tcl_GetString(objv[1]),
                         (char *)0);
        return TCL_ERROR;
    }
...
*/
static int SQLITE_TCLAPI test_free(void *clientData, Tcl_Interp *interp,
                                   int objc, Tcl_Obj *CONST objv[]) {
    void *pPrior;
    if (objc != 2) {
...
*/
static int SQLITE_TCLAPI test_memset(void *clientData, Tcl_Interp *interp,
                                     int objc, Tcl_Obj *CONST objv[]) {
    void *p;
    int size, n, i;
    char *zHex;
...
*/
static int SQLITE_TCLAPI test_memget(void *clientData, Tcl_Interp *interp,
                                     int objc, Tcl_Obj *CONST objv[]) {
    void *p;
    int size, n;
    char *zBin;
...
*/
static int SQLITE_TCLAPI test_memory_used(void *clientData, Tcl_Interp *interp,
                                          int objc, Tcl_Obj *CONST objv[]) {
...
*/
static int SQLITE_TCLAPI test_memory_highwater(void *clientData,
    ...
    if (objc == 2) {
        if (Tcl_GetBooleanFromObj(interp, objv[1], &resetFlag))
            return TCL_ERROR;
    }
...
*/
static int SQLITE_TCLAPI test_memdebug_backtrace(void *clientData,
                                                 Tcl_Interp *interp, int objc,
                                                 Tcl_Obj *CONST objv[]) {
    int depth;
    if (objc != 2) {
    ...
    if (Tcl_GetIntFromObj(interp, objv[1], &depth))
        return TCL_ERROR;
...
*/
static int SQLITE_TCLAPI test_memdebug_dump(void *clientData,
                                            Tcl_Interp *interp, int objc,
                                            Tcl_Obj *CONST objv[]) {
    if (objc != 2) {
...
*/
static int SQLITE_TCLAPI test_memdebug_malloc_count(void *clientData,
                                                    Tcl_Interp *interp,
                                                    int objc,
                                                    Tcl_Obj *CONST objv[]) {
    int nMalloc = -1;
...
*/
static int SQLITE_TCLAPI test_memdebug_fail(void *clientData,
                                            Tcl_Interp *interp, int objc,
                                            Tcl_Obj *CONST objv[]) {
    int ii;
    int iFail;
    ...
    if (Tcl_GetIntFromObj(interp, objv[1], &iFail))
        return TCL_ERROR;
...
*/
static int SQLITE_TCLAPI test_memdebug_pending(void *clientData,
                                               Tcl_Interp *interp, int objc,
                                               Tcl_Obj *CONST objv[]) {
    int nPending;
    if (objc != 1) {
...
*/
static int SQLITE_TCLAPI test_memdebug_settitle(void *clientData,
                                                Tcl_Interp *interp, int objc,
                                                Tcl_Obj *CONST objv[]) {
...
static void test_memdebug_log_clear(void) {
    ...
    Tcl_HashEntry *pEntry;
    for (pEntry = Tcl_FirstHashEntry(&aMallocLog, &search); pEntry;
...
static int SQLITE_TCLAPI test_memdebug_log(void *clientData, Tcl_Interp *interp,
                                           int objc, Tcl_Obj *CONST objv[]) {
    static int isInit = 0;
    int iSub;
...
*/
static int SQLITE_TCLAPI test_config_pagecache(void *clientData,
                                               Tcl_Interp *interp, int objc,
                                               Tcl_Obj *CONST objv[]) {
    int sz, N;
    Tcl_Obj *pRes;
    ...
    if (Tcl_GetIntFromObj(interp, objv[1], &sz))
        return TCL_ERROR;
...
** seed.
*/
static int SQLITE_TCLAPI test_alt_pcache(void *clientData, Tcl_Interp *interp,
                                         int objc, Tcl_Obj *CONST objv[]) {
    int installFlag;
    int discardChance = 0;
    int prngSeed = 0;
    ...
    if (Tcl_GetIntFromObj(interp, objv[1], &installFlag))
        return TCL_ERROR;
...
*/
static int SQLITE_TCLAPI test_config_memstatus(void *clientData,
                                               Tcl_Interp *interp, int objc,
                                               Tcl_Obj *CONST objv[]) {
    int enable, rc;
    if (objc != 2) {
...
**
*/
static int SQLITE_TCLAPI test_config_lookaside(void *clientData,
                                               Tcl_Interp *interp, int objc,
                                               Tcl_Obj *CONST objv[]) {
    int sz, cnt;
    Tcl_Obj *pRet;
    ...
    if (Tcl_GetIntFromObj(interp, objv[1], &sz))
        return TCL_ERROR;
    if (Tcl_GetIntFromObj(interp, objv[2], &cnt))
        return TCL_ERROR;
    pRet = Tcl_NewObj();
    Tcl_ListObjAppendElement(interp, pRet,
...
*/
static int SQLITE_TCLAPI test_db_config_lookaside(void *clientData,
                                                  Tcl_Interp *interp, int objc,
                                                  Tcl_Obj *CONST objv[]) {
    int rc;
    int sz, cnt;
    ...
    if (getDbPointer(interp, Tcl_GetString(objv[1]), &db))
        return TCL_ERROR;
    if (Tcl_GetIntFromObj(interp, objv[2], &bufid))
        return TCL_ERROR;
    if (Tcl_GetIntFromObj(interp, objv[3], &sz))
        return TCL_ERROR;
    if (Tcl_GetIntFromObj(interp, objv[4], &cnt))
        return TCL_ERROR;
    if (bufid == 0) {
        rc = sqlite3_db_config(db, SQLITE_DBCONFIG_LOOKASIDE, (void *)0, sz, cnt);
    } else if (bufid >= 1 && bufid <= 2 && sz * cnt <= sizeof(azBuf[0])) {
        rc =
            sqlite3_db_config(db, SQLITE_DBCONFIG_LOOKASIDE, azBuf[bufid], sz, cnt);
    } else {
        Tcl_AppendResult(interp, "illegal arguments - see documentation",
                         (char *)0);
        return TCL_ERROR;
    }
    Tcl_SetObjResult(interp, Tcl_NewIntObj(rc));
...
*/
static int SQLITE_TCLAPI test_config_heap(void *clientData, Tcl_Interp *interp,
                                          int objc, Tcl_Obj *CONST objv[]) {
    ...
    if (Tcl_GetIntFromObj(interp, aArg[0], &nByte))
        return TCL_ERROR;
    if (Tcl_GetIntFromObj(interp, aArg[1], &nMinAlloc))
        return TCL_ERROR;
...
*/
static int SQLITE_TCLAPI test_config_heap_size(void *clientData,
                                               Tcl_Interp *interp, int objc,
                                               Tcl_Obj *CONST objv[]) {
...
*/
static int SQLITE_TCLAPI test_config_error(void *clientData, Tcl_Interp *interp,
                                           int objc, Tcl_Obj *CONST objv[]) {
    sqlite3 *db;
    extern int getDbPointer(Tcl_Interp *, const char *, sqlite3 **);
    ...
    if (objc == 2) {
        ...
        if (sqlite3_db_config(db, 99999) != SQLITE_ERROR) {
            Tcl_AppendResult(
                interp, "sqlite3_db_config(db, 99999) does not return SQLITE_ERROR",
                (char *)0);
            return TCL_ERROR;
        }
    } else {
...
** SQLITE_CONFIG_URI.
*/
static int SQLITE_TCLAPI test_config_uri(void *clientData, Tcl_Interp *interp,
                                         int objc, Tcl_Obj *CONST objv[]) {
    int rc;
    int bOpenUri;
...
*/
static int SQLITE_TCLAPI test_config_cis(void *clientData, Tcl_Interp *interp,
                                         int objc, Tcl_Obj *CONST objv[]) {
    int rc;
    int bUseCis;
...
*/
static int SQLITE_TCLAPI test_config_pmasz(void *clientData, Tcl_Interp *interp,
                                           int objc, Tcl_Obj *CONST objv[]) {
    int rc;
    int iPmaSz;
...
*/
static int SQLITE_TCLAPI test_dump_memsys3(void *clientData, Tcl_Interp *interp,
                                           int objc, Tcl_Obj *CONST objv[]) {
    if (objc != 2) {
        Tcl_WrongNumArgs(interp, 1, objv, "FILENAME");
        return TCL_ERROR;
...
*/
static int SQLITE_TCLAPI test_status(void *clientData, Tcl_Interp *interp,
                                     int objc, Tcl_Obj *CONST objv[]) {
    ...
    if (i >= ArraySize(aOp)) {
        if (Tcl_GetIntFromObj(interp, objv[1], &op))
            return TCL_ERROR;
    }
    if (Tcl_GetBooleanFromObj(interp, objv[2], &resetFlag))
        return TCL_ERROR;
    iValue = 0;
    mxValue = 0;
...
** code, the current value, and the high-water mark value.
*/
static int SQLITE_TCLAPI test_db_status(void *clientData, Tcl_Interp *interp,
                                        int objc, Tcl_Obj *CONST objv[]) {
    int rc, iValue, mxValue;
    int i, op = 0, resetFlag;
    const char *zOpName;
    ...
    if (i >= ArraySize(aOp)) {
        if (Tcl_GetIntFromObj(interp, objv[2], &op))
            return TCL_ERROR;
    }
    if (Tcl_GetBooleanFromObj(interp, objv[3], &resetFlag))
        return TCL_ERROR;
    iValue = 0;
    mxValue = 0;
...
*/
static int SQLITE_TCLAPI test_install_malloc_faultsim(void *clientData,
                                                      Tcl_Interp *interp,
                                                      int objc,
                                                      Tcl_Obj *CONST objv[]) {
    int rc;
...
*/
static int SQLITE_TCLAPI test_install_memsys3(void *clientData,
                                              Tcl_Interp *interp, int objc,
                                              Tcl_Obj *CONST objv[]) {
...
static int SQLITE_TCLAPI test_vfs_oom_test(void *clientData, Tcl_Interp *interp,
                                           int objc, Tcl_Obj *CONST objv[]) {
