...
static int simulateVtabError(echo_vtab *p, const char *zMethod) {
    ...
    char zVarname[128];
    zVarname[127] = '\0';
    sqlite3_snprintf(127, zVarname, "echo_module_fail(%s,%s)", zMethod,
                     p->zTableName);
...
** on *paCol.
*/
static int getColumnNames(sqlite3 *db, const char *zTab, char ***paCol,
                          int *pnCol) {
    char **aCol = 0;
    char *zSql;
...
*/
static int getIndexArray(sqlite3 *db,      /* Database connection */
                         const char *zTab, /* Name of table in database db */
                         int nCol, int **paIndex) {
    sqlite3_stmt *pStmt = 0;
    int *aIndex = 0;
    int rc;
    char *zSql;
...
*/
static int echoDeclareVtab(echo_vtab *pVtab, sqlite3 *db) {
    int rc = SQLITE_OK;

    if (pVtab->zTableName) {
        sqlite3_stmt *pStmt = 0;
        rc = sqlite3_prepare(
            db, "SELECT sql FROM sqlite_schema WHERE type = 'table' AND name = ?",
            -1, &pStmt, 0);
        if (rc == SQLITE_OK) {
...
** virtual table.
*/
static int echoConstructor(sqlite3 *db, void *pAux, int argc,
...
*/
static int echoCreate(sqlite3 *db, void *pAux, int argc,
                      const char *const *argv, sqlite3_vtab **ppVtab,
                      char **pzErr) {
    int rc = SQLITE_OK;
...
*/
static int echoConnect(sqlite3 *db, void *pAux, int argc,
                       const char *const *argv, sqlite3_vtab **ppVtab,
                       char **pzErr) {
...
*/
static int echoFilter(sqlite3_vtab_cursor *pVtabCursor, int idxNum,
    ...
    for (i = 0; i < argc; i++) {
        appendToEchoModule(pVtab->interp,
...
static int echoBestIndex(sqlite3_vtab *tab, sqlite3_index_info *pIdxInfo) {
    ...
    */
    ...
    for (ii = 0; ii < pIdxInfo->nConstraint; ii++) {
        ...
        if (!isIgnoreUsable && !pConstraint->usable)
            continue;
        ...
        if (iCol < 0 || pVtab->aIndex[iCol]) {
            ...
            if (zOp) {
                if (zOp[0] == 'L') {
                    zNew = sqlite3_mprintf(" %s %s LIKE (SELECT '%%'||?||'%%')", zSep,
                                           zNewCol);
                } else {
    ...
    ** the ORDER BY clause.
    */
    if (pIdxInfo->nOrderBy == 1 && (pIdxInfo->aOrderBy->iColumn < 0 ||
    ...
    if (nData > 1 && sqlite3_value_type(apData[0]) == SQLITE_INTEGER) {
        ...
        for (i = 2; i < nData; i++) {
            if (apData[i] == 0)
                continue;
            string_concat(&z,
                          sqlite3_mprintf("%s %Q=?%d", zSep, pVtab->aCol[i - 2], i),
                          1, &rc);
            zSep = ",";
        }
    ...
    /* If the first argument is NULL and there are more than two args, INSERT */
        ...
        for (ii = 2; ii < nData; ii++) {
            string_concat(
                ...
                &rc);
            string_concat(&zValues, sqlite3_mprintf("%s?%d", zValues ? ", " : "", ii),
                          1, &rc);
        }
...
*/
static void overloadedGlobFunction(sqlite3_context *pContext, int nArg,
                                   sqlite3_value **apArg) {
...
*/
static int echoFindFunction(sqlite3_vtab *vtab, int nArg, const char *zFuncName,
...
static int echoRename(sqlite3_vtab *vtab, const char *zNewName) {
    ...
    if (p->isPattern) {
        ...
        char *zSql = sqlite3_mprintf("ALTER TABLE %s RENAME TO %s%s", p->zTableName,
                                     zNewName, &p->zTableName[nThis]);
        rc = sqlite3_exec(p->db, zSql, 0, 0, 0);
        sqlite3_free(zSql);
    }
...
static int SQLITE_TCLAPI register_echo_module(
    ...
    pMod->interp = interp;
    pMod->db = db;
    rc = sqlite3_create_module_v2(db, "echo", &echoModule, (void *)pMod,
                                  moduleDestroy);
    ...
    if (rc == SQLITE_OK) {
        ...
        pMod->interp = interp;
        pMod->db = db;
        rc = sqlite3_create_module_v2(db, "echo_v2", &echoModuleV2, (void *)pMod,
                                      moduleDestroy);
    }
