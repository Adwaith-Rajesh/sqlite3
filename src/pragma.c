...
static void setAllPagerFlags(sqlite3 *db) {
    if (db->autoCommit) {
        ...
        while ((n--) > 0) {
            if (pDb->pBt) {
                sqlite3BtreeSetPagerFlags(pDb->pBt, pDb->safety_level |
...
*/
static void
pragmaFunclistLine(Vdbe *v,            /* The prepared statement being created */
                   FuncDef *p,         /* A particular function definition */
                   int isBuiltin,      /* True if this is a built-in function */
                   int showInternFuncs /* True if showing internal functions */
) {
    u32 mask = SQLITE_DETERMINISTIC | SQLITE_DIRECTONLY | SQLITE_SUBTYPE |
    ...
    for (; p; p = p->pNext) {
        ...
        if (p->xSFunc == 0)
            continue;
        if ((p->funcFlags & SQLITE_FUNC_INTERNAL) != 0 && showInternFuncs == 0) {
            continue;
        }
        if (p->xValue != 0) {
            zType = "w";
        ...
        sqlite3VdbeMultiLoad(v, 1, "sissii", p->zName, isBuiltin, zType,
                             azEnc[p->funcFlags & SQLITE_FUNC_ENCMASK], p->nArg,
    ...
    switch (pPragma->ePragTyp) {
        ...
        */
        case PragTyp_CACHE_SPILL: {
            assert(sqlite3SchemaMutexHeld(db, iDb, 0));
            if (!zRight) {
                returnSingleInt(v, (db->flags & SQLITE_CacheSpill) == 0
                                       ? 0
        ...
        */
        case PragTyp_LOCK_PROXY_FILE: {
            if (!zRight) {
                ...
                int res;
                if (zRight[0]) {
                    res = sqlite3OsFileControl(pFile, SQLITE_SET_LOCKPROXYFILE, zRight);
                } else {
                    res = sqlite3OsFileControl(pFile, SQLITE_SET_LOCKPROXYFILE, NULL);
                }
                if (res != SQLITE_OK) {
        ...
        case PragTyp_SYNCHRONOUS: {
            if (!zRight) {
                ...
                if (!db->autoCommit) {
                    ...
                    int iLevel =
                        (getSafetyLevel(zRight, 0, 1) + 1) & PAGER_SYNCHRONOUS_MASK;
                    if (iLevel == 0)
                        iLevel = 1;
                    ...
                    for (i = 0, pCol = pTab->aCol; i < pTab->nCol; i++, pCol++) {
                        ...
                        if (pCol->colFlags & COLFLAG_NOINSERT) {
                            ...
                            if (pCol->colFlags & COLFLAG_VIRTUAL) {
                                ...
                                assert(pCol->colFlags & COLFLAG_HIDDEN);
                                isHidden = 1; /* HIDDEN */
                            }
                        }
                        ...
                        sqlite3VdbeMultiLoad(
                            v, 1, pPragma->iArg ? "issisii" : "issisi", i - nHidden,
                            pCol->zCnName, sqlite3ColumnType(pCol, ""), pCol->notNull ? 1 : 0,
                            (isHidden >= 2 || pColExpr == 0) ? 0 : pColExpr->u.zToken, k,
                            isHidden);
                    }
                }
            }
        ...
        */
            ...
            for (ii = 0; ii < db->nDb; ii++) {
                ...
                */
                ...
                for (k = sqliteHashFirst(pHash); k; k = sqliteHashNext(k)) {
                    ...
                    sqlite3VdbeMultiLoad(v, 1, "sssiii", db->aDb[ii].zDbSName,
                                         sqlite3PreferredTableName(pTab->zName), zType,
                                         pTab->nCol,
        ...
        case PragTyp_STATS: {
            ...
            for (i = sqliteHashFirst(&pDb->pSchema->tblHash); i;
                ...
                sqlite3VdbeMultiLoad(v, 1, "ssiii",
                                     sqlite3PreferredTableName(pTab->zName), 0,
                                     pTab->szTabRow, pTab->nRowLogEst, pTab->tabFlags);
                for (pIdx = pTab->pIndex; pIdx; pIdx = pIdx->pNext) {
                    sqlite3VdbeMultiLoad(v, 2, "siiiX", pIdx->zName, pIdx->szIdxRow,
                                         pIdx->aiRowLogEst[0], pIdx->hasStat1);
            ...
            if (zRight) {
                ...
                if (pIdx) {
                    ...
                    for (i = 0; i < mx; i++) {
                        ...
                        if (pPragma->iArg) {
                            sqlite3VdbeMultiLoad(v, 4, "isiX", pIdx->aSortOrder[i],
                                                 pIdx->azColl[i], i < pIdx->nKeyCol);
                        }
            ...
            if (zRight) {
                ...
                if (pTab) {
                    ...
                    for (pIdx = pTab->pIndex, i = 0; pIdx; pIdx = pIdx->pNext, i++) {
                        ...
                        sqlite3VdbeMultiLoad(v, 1, "isisi", i, pIdx->zName,
                                             IsUniqueIndex(pIdx), azOrigin[pIdx->idxType],
        ...
        case PragTyp_DATABASE_LIST: {
            ...
            for (i = 0; i < db->nDb; i++) {
                ...
                sqlite3VdbeMultiLoad(v, 1, "iss", i, db->aDb[i].zDbSName,
                    ...
                    if (pFK) {
                        ...
                        while (pFK) {
                            ...
                            for (j = 0; j < pFK->nCol; j++) {
                                sqlite3VdbeMultiLoad(v, 1, "iissssss", i, j, pFK->zTo,
        ...
        case PragTyp_FOREIGN_KEY_CHECK: {
            ...
            while (k) {
                ...
                if (pTab == 0 || !IsOrdinaryTable(pTab) || pTab->u.tab.pFKey == 0)
                    continue;
                ...
                for (i = 1, pFK = pTab->u.tab.pFKey; pFK; i++, pFK = pFK->pNextFrom) {
                    ...
                    ** key. If a match is found, jump to addrOk. */
                    if (pIdx) {
                        ...
                        sqlite3VdbeAddOp3(v, OP_SeekRowid, i, jmp, regRow);
                        VdbeCoverage(v);
...
#define SQLITE_INTEGRITY_CHECK_ERROR_MAX 100
            ...
            ** of all attached databases.  */
            ...
            for (i = 0; i < db->nDb; i++) {
                ...
                */
                ...
                sqlite3VdbeAddOp4(v, OP_IntegrityCk, 2, cnt, 1, (char *)aRoot,
                                  P4_INTARRAY);
                ...
                for (x = sqliteHashFirst(pTbls); x; x = sqliteHashNext(x)) {
                    ...
                    sqlite3OpenTableAndIndices(pParse, pTab, OP_OpenRead, 0, 1, 0,
                                               &iDataCur, &iIdxCur);
                    ...
                    */
                    ...
                    if (HasRowid(pTab)) {
                        ...
                        for (j = 0; j < pTab->nCol; j++) {
                            if ((pTab->aCol[j].colFlags & COLFLAG_VIRTUAL) == 0)
                                mxCol++;
                        }
                        if (mxCol == pTab->iPKey)
                            mxCol--;
                    ...
                    if (!isQuick) {
                        if (pPk) {
                            ...
                            int a1;
                            char *zErr;
                            a1 = sqlite3VdbeAddOp4Int(v, OP_IdxGT, iDataCur, 0, r2,
                                                      pPk->nKeyCol);
                            VdbeCoverage(v);
                            sqlite3VdbeAddOp1(v, OP_IsNull, r2);
                            VdbeCoverage(v);
                            zErr = sqlite3MPrintf(db, "row not in PRIMARY KEY order for %s",
                                                  pTab->zName);
                    ...
                    */
                    ...
                    for (j = 0; j < pTab->nCol; j++) {
                        ...
                        if (bStrict && doTypeCheck) {
                            ...
                            zErr = sqlite3MPrintf(db, "non-%s value in %s.%s",
                                                  sqlite3StdType[pCol->eCType - 1], pTab->zName,
                                                  pTab->aCol[j].zCnName);
                            sqlite3VdbeAddOp4(v, OP_String8, 0, 3, 0, zErr, P4_DYNAMIC);
                            ...
                            **     NULL, TEXT, or BLOB. */
                            ...
                            VdbeCoverage(v);
                            zErr = sqlite3MPrintf(db, "NUMERIC value in %s.%s", pTab->zName,
                                                  pTab->aCol[j].zCnName);
                            sqlite3VdbeAddOp4(v, OP_String8, 0, 3, 0, zErr, P4_DYNAMIC);
                            ...
                            **     be a TEXT value that can be converted to numeric. */
                            ...
                            sqlite3VdbeChangeP5(v, 0x1c); /* NULL, TEXT, or BLOB */
                            VdbeCoverage(v);
                            zErr = sqlite3MPrintf(db, "TEXT value in %s.%s", pTab->zName,
                                                  pTab->aCol[j].zCnName);
                            sqlite3VdbeAddOp4(v, OP_String8, 0, 3, 0, zErr, P4_DYNAMIC);
                        }
                    ...
                    if (!isQuick) { /* Omit the remaining tests for quick_check */
                        ...
                        for (j = 0, pIdx = pTab->pIndex; pIdx; pIdx = pIdx->pNext, j++) {
                            ...
                            r1 = sqlite3GenerateIndexKey(pParse, pIdx, iDataCur, 0, 0, &jmp3,
                                                         pPrior, r1);
                            pPrior = pIdx;
                            sqlite3VdbeAddOp2(v, OP_AddImm, 8 + j,
                            ...
                            ** the exact same text value as the table. */
                            ...
                            for (kk = 0; kk < pIdx->nKeyCol; kk++) {
                                ...
                                sqlite3VdbeAddOp3(v, OP_Ne, 3, label6, r1 + kk);
                                VdbeCoverage(v);
                            }
                            if (label6) {
                    ...
                    if (!isQuick) {
                        ...
                        for (j = 0, pIdx = pTab->pIndex; pIdx; pIdx = pIdx->pNext, j++) {
                            ...
                            addr = sqlite3VdbeAddOp3(v, OP_Eq, 8 + j, 0, 3);
                            VdbeCoverage(v);
        ...
        */
        case PragTyp_HEADER_VALUE: {
            ...
            if (zRight && (pPragma->mPragFlg & PragFlg_ReadOnly) == 0) {
                ...
                static const VdbeOpList readCookie[] = {{OP_Transaction, 0, 0, 0}, /* 0 */
        ...
        */
        case PragTyp_WAL_AUTOCHECKPOINT: {
            ...
            returnSingleInt(v, db->xWalCallback == sqlite3WalDefaultHook
                                   ? SQLITE_PTR_TO_INT(db->pWalArg)
                                   : 0);
        } break;
#endif
            ...
            for (iDbLast = zDb ? iDb : db->nDb - 1; iDb <= iDbLast; iDb++) {
                ...
                for (k = sqliteHashFirst(&pSchema->tblHash); k; k = sqliteHashNext(k)) {
                    ...
                    ** This also has the effect of skipping virtual tables and views */
                    if ((pTab->tabFlags & TF_StatsUsed) == 0)
                        continue;
        ...
        */
        case PragTyp_LOCK_STATUS: {
            static const char *const azLockName[] = {"unlocked", "shared", "reserved",
                                                     "pending", "exclusive"};
            int i;
            pParse->nMem = 2;
            for (i = 0; i < db->nDb; i++) {
...
*/
static int pragmaVtabConnect(sqlite3 *db, void *pAux, int argc,
...
*/
static int pragmaVtabFilter(sqlite3_vtab_cursor *pVtabCursor, int idxNum,
