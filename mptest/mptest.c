...
static void fatalError(const char *zFormat, ...) {
    ...
    if (g.db) {
        int nTry = 0;
        g.iTimeout = 0;
        while (trySql("UPDATE client SET wantHalt=1;") == SQLITE_BUSY && (nTry++) < 100) {
            sqlite3_sleep(10);
        }
    }
    sqlite3_close(g.db);
...
static int extractToken(const char *zIn, int nIn, char *zOut, int nOut) {
    ...
    for (i = 0; i < nIn && i < nOut - 1 && !ISSPACE(zIn[i]); i++) {
        zOut[i] = zIn[i];
    }
    zOut[i] = 0;
    return i;
...
static int findEndif(const char *z, int stopAtElse, int *pnLine) {
    ...
    while (z[n]) {
        ...
        if ((strncmp(z + n, "--endif", 7) == 0 && ISSPACE(z[n + 7])) || (stopAtElse && strncmp(z + n, "--else", 6) == 0 && ISSPACE(z[n + 6]))) {
            return n + len;
        }
...
static void waitForClient(int iClient, int iTimeout, char *zErrPrefix) {
    ...
    g.iTimeout = 0;
    while (((rc = sqlite3_step(pStmt)) == SQLITE_BUSY || rc == SQLITE_ROW) && iTimeout > 0) {
        sqlite3_reset(pStmt);
        sqlite3_sleep(50);
        iTimeout -= 50;
    }
...
static void runScript(
    ...
    while ((c = zScript[ii]) != 0) {
        ...
        for (nArg = 0; n < len - 2 && nArg < MX_ARG; nArg++) {
            while (n < len - 2 && ISSPACE(zScript[ii + 2 + n])) {
                n++;
            }
            if (n >= len - 2) break;
                                                                    ...
                                                                    */
                                                                    if (strcmp(zCmd, "task") == 0 && iClient == 0) {
                                                                        ...
                                                                        if (iTarget < 0) {
                                                                            ...
                                                                            startClient(iTarget);
                                                                            runSql(
                                                                                "INSERT INTO task(client,script,name)"
                                                                                " VALUES(%d,'%q',%Q)",
                                                                                iTarget, zTask, zTName);
                                                                            sqlite3_free(zTask);
                                                                            sqlite3_free(zTName);
                                                                        }
...
int SQLITE_CDECL main(int argc, char **argv) {
    ...
    if (zJMode) {
#if defined(_WIN32)
        if (sqlite3_stricmp(zJMode, "persist") == 0 || sqlite3_stricmp(zJMode, "truncate") == 0) {
    ...
    if (iClient > 0) {
        ...
        g.iTimeout = 0;
        iTimeout = 1000;
        while (((rc = trySql("SELECT 1 FROM client")) == SQLITE_BUSY || rc == SQLITE_ROW) && iTimeout > 0) {
            sqlite3_sleep(10);
            iTimeout -= 10;
        }
        sqlite3_sleep(100);
