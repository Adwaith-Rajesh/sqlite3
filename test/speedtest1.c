...
static void printSql(const char *zSql) {
    ...
    if (g.bExplain
        ...
        && (sqlite3_strglob("CREATE *", zSql) == 0 || sqlite3_strglob("DROP *", zSql) == 0 || sqlite3_strglob("ALTER *", zSql) == 0)
#endif
    ) {
        printf("%.*s;\n", n, zSql);
    }
...
void testset_rtree(int p1, int p2) {
    ...
    speedtest1_prepare(
        "INSERT INTO rt1(id,x0,x1,y0,y1,z0,z1)"
    ...
    speedtest1_prepare(
        "SELECT count(*) FROM rt1 WHERE x1>=?1 AND x0<=?2"
    ...
    speedtest1_prepare(
        "UPDATE rt1 SET x0=x0-100, x1=x1-100"
    ...
    for (i = 1; i < argc; i++) {
        ...
        if (z[0] == '-') {
            ...
            if (strcmp(z, "autovacuum") == 0) {
                ...
                fatal_error(
                    "The --output option is not supported with"
...
#else
            fatal_error(
                "compile with -DSQLITE_ENABLE_RTREE to enable "
                "the R-Tree tests\n");
#endif
        } else {
            fatal_error(
