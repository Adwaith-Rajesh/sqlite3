...
int fuzz_invariant(
    ...
    if (rc == SQLITE_DONE) {
        ...
        ** can happen with a corrupt database file */
        ...
        rc = sqlite3_step(pCk);
        if (rc != SQLITE_ROW || sqlite3_column_text(pCk, 0) == 0 || strcmp((const char *)sqlite3_column_text(pCk, 0), "ok") != 0) {
            *pbCorrupt = 1;
            sqlite3_finalize(pCk);
            sqlite3_finalize(pTestStmt);
...
static char *fuzz_invariant_sql(sqlite3_stmt *pStmt, int iCnt) {
    ...
    for (i = 0; i < sqlite3_column_count(pStmt); i++) {
        ...
        if (zSuffix && isdigit(zSuffix[1]) && (zSuffix[1] > '3' || isdigit(zSuffix[2]))) {
            /* This is a randomized column name and so cannot be used in the
            ** WHERE clause. */
            continue;
        }
#ifdef SQLITE_ALLOW_ROWID_IN_VIEW
        if (sqlite3_strlike("%rowid%", zColName, 0) == 0 || sqlite3_strlike("%oid%", zColName, 0) == 0) {
...
static int sameValue(
    ...
    if (t1 != t2) {
        if ((t1 == SQLITE_INTEGER && t2 == SQLITE_FLOAT) || (t1 == SQLITE_FLOAT && t2 == SQLITE_INTEGER)) {
    ...
    switch (sqlite3_column_type(pS1, i1)) {
        ...
        case SQLITE_TEXT: {
            ...
            if (pTestCompare) {
                ...
                x = sqlite3_step(pTestCompare) == SQLITE_ROW && sqlite3_column_int(pTestCompare, 0) != 0;
