...
static int vt02Filter(
    ...
    if (idxNum == 0) {
        ...
        for (m = 1000, i = 0; i <= e; i++, m /= 10) {
            sqlite3_int64 v;
            pVal = 0;
            if (sqlite3_vtab_in_first(0, &pVal) != SQLITE_MISUSE || sqlite3_vtab_in_first(argv[iArg], &pVal) != SQLITE_ERROR) {
...
static int vt02BestIndex(sqlite3_vtab *pVTab, sqlite3_index_info *pInfo) {
    ...
    ** usable RHS values */
    for (i = 0; i < pInfo->nConstraint; i++) {
        ...
        switch (pInfo->aConstraint[i].iColumn) {
            case VT02_COL_FLAGS:
                if (sqlite3_vtab_rhs_value(pInfo, i, &pVal) == SQLITE_OK && sqlite3_value_type(pVal) == SQLITE_INTEGER) {
                    flags = sqlite3_value_int(pVal);
                }
                iFlagTerm = i;
                break;
            case VT02_COL_LOGTAB:
                if (sqlite3_vtab_rhs_value(pInfo, i, &pVal) == SQLITE_OK && sqlite3_value_type(pVal) == SQLITE_TEXT) {
    ...
    for (i = 0; i < pInfo->nConstraint; i++) {
        ...
        if (pInfo->aConstraint[i].usable == 0 && (flags & VT02_IGNORE_USABLE) == 0) continue;
        if (j < 0) j = VT02_COL_X;
    ...
    */
    if (pInfo->nOrderBy > 0 && (flags & VT02_NO_SORT_OPT) == 0) {
        ...
        } else if (pInfo->aOrderBy[0].iColumn <= 0 && pInfo->aOrderBy[0].desc == 0) {
            /* First column of order by is X ascending */
            pInfo->orderByConsumed = 1;
        } else if (sqlite3_vtab_distinct(pInfo) >= 1) {
            unsigned int x = 0;
            for (i = 0; i < pInfo->nOrderBy; i++) {
                int iCol = pInfo->aOrderBy[i].iColumn;
                if (iCol < 0) iCol = 0;
    ...
    if (iOffset >= 0) {
        ...
        if ((flags & VT02_NO_OFFSET) == 0 && (pInfo->nOrderBy == 0 || pInfo->orderByConsumed)) {
    ...
    */
    ...
    if (flags & 0x40) {
        for (i = 0; i < pInfo->nConstraint; i++) {
            if (pInfo->aConstraint[i].usable && pInfo->aConstraintUsage[i].argvIndex == 0) {
