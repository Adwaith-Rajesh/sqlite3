...
static unsigned int sessionChangeHash(
    ...
    for (i = 0; i < pTab->nCol; i++) {
        ...
        ** primary key columns. */
        assert(eType == SQLITE_INTEGER || eType == SQLITE_FLOAT || eType == SQLITE_TEXT || eType == SQLITE_BLOB || eType == SQLITE_NULL || eType == 0);
...
static int sessionMergeUpdate(
    ...
    for (i = 0; i < pTab->nCol; i++) {
        ...
        aOld = sessionMergeValue(&aOld1, &aOld2, &nOld);
        aNew = sessionMergeValue(&aNew1, &aNew2, &nNew);
        if (bPatchset == 0 && (pTab->abPK[i] || (nOld == nNew && 0 == memcmp(aOld, aNew, nNew)))) {
            *(aOut++) = '\0';
        } else {
    ...
    for (iCol = 0; iCol < pTab->nCol; iCol++) {
        if (!pTab->abPK[iCol]) {
            ...
            ** this (that the method has already been called). */
            ...
            assert(eType == SQLITE_INTEGER || eType == SQLITE_FLOAT || eType == SQLITE_BLOB || eType == SQLITE_TEXT);
    ...
    if (pTab->nCol == 0) {
        ...
        if (rc == SQLITE_OK) {
            ...
            if (pSession && pSession->bEnableSize) {
                pSession->nMaxChangesetSize += (1 + sessionVarintLen(pTab->nCol) + pTab->nCol + strlen(pTab->zName) + 1);
            }
        }
    }
        ...
        for (ii = pTab->bRowid; ii < pTab->nCol; ii++) {
            ...
            switch (eType) {
                ...
                default: {
                    ...
                    nOld += nByte;
                    assert(eType == SQLITE_TEXT || eType == SQLITE_BLOB);
                    if (eType == sqlite3_value_type(p) && nByte == sqlite3_value_bytes(p) && (nByte == 0 || 0 == memcmp(pCsr, sqlite3_value_blob(p), nByte))) {
                        bChanged = 0;
                    }
                    pCsr += nByte;
                    break;
    ...
    if (bNull == 0) {
        ...
        if (pC == 0) {
            ...
            ** change is "direct", mark the change object as direct. */
            if (pSession->hook.xDepth(pSession->hook.pCtx) == 0 && pSession->bIndirect == 0) {
                pC->bIndirect = 0;
            }
        }
...
static int sessionFindTable(
    ...
    if (pRet == 0 && pSession->bAutoAttach) {
        ...
        ** do not automatically add the new table. */
        if (pSession->xTableFilter == 0 || pSession->xTableFilter(pSession->pFilterCtx, zName)) {
    ...
    for (i = 0; i < sqlite3_column_count(pStmt); i++) {
        ...
        switch (eType) {
            ...
            default: {
                ...
                assert(eType == SQLITE_TEXT || eType == SQLITE_BLOB);
                nAdvance = nHdr + n;
                if (eType == sqlite3_column_type(pStmt, i) && n == sqlite3_column_bytes(pStmt, i) && (n == 0 || 0 == memcmp(&pCsr[nHdr], sqlite3_column_blob(pStmt, i), n))) {
                    break;
                }
                bChanged = 1;
            }
            ...
            for (i = 0; i < pTab->nChange && rc == SQLITE_OK; i++) {
                ...
                for (p = pTab->apChange[i]; rc == SQLITE_OK && p; p = p->pNext) {
                    ...
                    ** its contents to the xOutput() callback. */
                    if (xOutput && rc == SQLITE_OK && buf.nBuf > nNoop && buf.nBuf > sessions_strm_chunk_size) {
...
int sqlite3session_changeset(
    ...
    assert(rc || pnChangeset == 0 || pSession->bEnableSize == 0 || *pnChangeset <= pSession->nMaxChangesetSize);
    return rc;
}
...
static int sessionConflictHandler(
    ...
    if (rc == SQLITE_ROW) {
        ...
        if (p->bIgnoreNoop && sqlite3_column_int(p->pSelect, sqlite3_column_count(p->pSelect) - 1)) {
            ...
            if (schemaMismatch) {
                ...
                if (sApply.nCol == 0) {
                    ...
                    if (0 == sqlite3_stricmp(zTab, "sqlite_stat1")) {
                        ...
                        if ((rc = sessionSelectRow(db, zTab, &sApply)) || (rc = sessionDeleteRow(db, zTab, &sApply)) || (rc = sessionInsertRow(db, zTab, &sApply))) {
                            break;
                        }
                        sApply.bStat1 = 0;
                    }
...
static int sessionChangeMerge(
    ...
    if (!pExist) {
        ...
        */
        if ((op1 == SQLITE_INSERT && op2 == SQLITE_INSERT) || (op1 == SQLITE_UPDATE && op2 == SQLITE_INSERT) || (op1 == SQLITE_DELETE && op2 == SQLITE_UPDATE) || (op1 == SQLITE_DELETE && op2 == SQLITE_DELETE)) {
            pNew = pExist;
