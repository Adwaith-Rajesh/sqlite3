...
#define CMDLINE_INT32(x, y, z) \
    { x, y, CMDLINE_INTEGER, z }
#define CMDLINE_BOOL(x, y, z) \
    { x, y, CMDLINE_BOOLEAN, z }
#define CMDLINE_TEXT(x, y, z) \
    { x, y, CMDLINE_STRING, z }
#define CMDLINE_NONE(x, y, z) \
    { x, y, CMDLINE_BARE, z }

static void option_requires_argument_error(CmdLineOption *pOpt) {
    fprintf(stderr, "Option requires a%s argument: %s\n",
            pOpt->eType == CMDLINE_INTEGER ? "n integer" : pOpt->eType == CMDLINE_STRING ? " string"
                                                                                         : " boolean",
            pOpt->zText);
    exit(1);
}
...
static void unknown_option_error(
    ...
    for (i = 0; aOpt[i].zText; i++) {
        ...
        char *zOpt = sqlite3_mprintf("%s %s", aOpt[i].zText,
                                     eType == CMDLINE_BARE ? "" : eType == CMDLINE_INTEGER ? "N"
                                                              : eType == CMDLINE_BOOLEAN   ? "BOOLEAN"
                                                                                           : "TEXT");
