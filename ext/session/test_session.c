...
static int test_filter_handler(
    ...
    if (TCL_OK != Tcl_ListObjAppendElement(0, pEval, Tcl_NewStringObj(zTab, -1)) || TCL_OK != Tcl_EvalObjEx(interp, pEval, TCL_EVAL_GLOBAL) || TCL_OK != Tcl_GetIntFromObj(interp, Tcl_GetObjResult(interp), &res)) {
...
static int test_conflict_handler(
    ...
    if (eConf == SQLITE_CHANGESET_FOREIGN_KEY) {
        ...
        Tcl_ListObjAppendElement(0, pEval, Tcl_NewStringObj(op == SQLITE_INSERT ? "INSERT" : op == SQLITE_UPDATE ? "UPDATE"
                                                                                                                 : "DELETE",
                                                            -1));
...
static int SQLITE_TCLAPI testSqlite3changesetApply(
    ...
    if (objc != 4 && objc != 5) {
        ...
        if (bV2) {
            zMsg =
                "?-nosavepoint? ?-inverse? ?-ignorenoop? "
...
static int SQLITE_TCLAPI test_sqlite3session_foreach(
    ...
    while (objc > 1) {
        ...
        if (nOpt <= 7 && 0 == sqlite3_strnicmp(zOpt, "-invert", nOpt)) {
            isInvert = 1;
        } else if (nOpt <= 5 && 0 == sqlite3_strnicmp(zOpt, "-next", nOpt)) {
            isCheckNext = 1;
        } else {
            break;
        }
    ...
    while (SQLITE_ROW == sqlite3changeset_next(pIter)) {
        ...
        ** iterator. */
        ...
        pVar = Tcl_NewObj();
        Tcl_ListObjAppendElement(0, pVar, Tcl_NewStringObj(op == SQLITE_INSERT ? "INSERT" : op == SQLITE_UPDATE ? "UPDATE"
                                                                                                                : "DELETE",
                                                           -1));
