...
static i64 fuzzGetI64(u8 *aRec) {
    return (i64)((((u64)aRec[0]) << 56) + (((u64)aRec[1]) << 48) + (((u64)aRec[2]) << 40) + (((u64)aRec[3]) << 32) + (((u64)aRec[4]) << 24) + (((u64)aRec[5]) << 16) + (((u64)aRec[6]) << 8) + (((u64)aRec[7]) << 0));
}
...
static void fuzzPrintGroup(FuzzChangeset *pParse, FuzzChangesetGroup *pGrp) {
    ...
    for (i = 0; i < pGrp->nChange; i++) {
        ...
        u8 bIndirect = p[1];
        printf("%s (ind=%d):",
               (eType == SQLITE_INSERT) ? "INSERT" : (eType == SQLITE_DELETE ? "DELETE" : "UPDATE"),
               bIndirect);
        p += 2;
...
static int fuzzSelectChange(FuzzChangeset *pParse, FuzzChange *pChange) {
    ...
    if (pChange->eType == FUZZ_VALUE_SUB || pChange->eType == FUZZ_VALUE_MOD || pChange->eType == FUZZ_VALUE_RND) {
...
static int fuzzCopyChange(
    ...
    if (pFuzz->iCurrent == pFuzz->iChange && pFuzz->eType == FUZZ_CHANGE_FIELD && eType == SQLITE_UPDATE) {
        int sz;
        int i;
        int nDef = 0;
        u8 *pCsr = p + 1;
    ...
    for (iRec = 0; iRec < nRec; iRec++) {
        ...
        for (i = 0; i < pGrp->nCol; i++) {
            ...
            ** write a randomly selected value to the output.  */
            ...
            if ((pFuzz->iGroup != iGrp || i != pFuzz->iDelete) && (eNew == eType || eType != SQLITE_UPDATE || iRec == 0) && (eNew == eType || eNew != SQLITE_DELETE || !bPS || pGrp->aPK[i])) {
                fuzzChangeSize(pCopy, &sz);
                memcpy(pOut, pCopy, sz);
                pOut += sz;
    ...
    ** change, remove it altogether. */
    if (pFuzz->eType == FUZZ_COLUMN_DEL && pFuzz->iGroup == iGrp && eType == SQLITE_UPDATE && nUpdate == 0) {
        pOut = *ppOut;
    }
            ...
            ** copies of the group. */
            ...
            for (j = 0; j < nRep; j++) {
                ...
                if (iGrp == change.iGroup) {
                    if (change.eType == FUZZ_COLUMN_ADD || change.eType == FUZZ_COLUMN_ADDPK) {
                        nCol++;
