...
static int fts5VocabTableType(const char *zType, char **pzErr, int *peType) {
    ...
    if (rc == SQLITE_OK) {
        ...
        } else if (sqlite3_stricmp(zCopy, "instance") == 0) {
...
static int fts5VocabInstanceNext(Fts5VocabCursor *pCsr) {
    ...
    assert(pCsr->bEof == 0);
    while (eDetail == FTS5_DETAIL_NONE || sqlite3Fts5PoslistNext64(pIter->pData, pIter->nData, po, pp)) {
        pCsr->iInstPos = 0;
        pCsr->iInstOff = 0;
    ...
    if (pTab->eType != FTS5_VOCAB_COL || pCsr->iCol >= nCol) {
        if (sqlite3Fts5IterEof(pCsr->pIter)) {
            ...
            while (rc == SQLITE_OK) {
                ...
                if (rc == SQLITE_OK) {
                    zTerm = sqlite3Fts5IterTerm(pCsr->pIter, &nTerm);
                    if (nTerm != pCsr->term.n || (nTerm > 0 && memcmp(zTerm, pCsr->term.p, nTerm))) {
                        break;
                    }
...
static int fts5VocabFilterMethod(
    ...
    if (rc == SQLITE_OK && !pCsr->bEof && (eType != FTS5_VOCAB_INSTANCE || pCsr->pFts5->pConfig->eDetail != FTS5_DETAIL_NONE)) {
