...
static int fts5UsePatternMatch(
    ...
    if (pConfig->ePattern == FTS5_PATTERN_LIKE && (p->op == FTS5_PATTERN_LIKE || p->op == FTS5_PATTERN_GLOB)) {
        return 1;
    }
    return 0;
}
...
static int fts5BestIndexMethod(sqlite3_vtab *pVTab, sqlite3_index_info *pInfo) {
    ...
    for (i = 0; i < pInfo->nConstraint; i++) {
        ...
        int iCol = p->iColumn;
        if (p->op == SQLITE_INDEX_CONSTRAINT_MATCH || (p->op == SQLITE_INDEX_CONSTRAINT_EQ && iCol >= nCol)) {
...
static void fts5CsrNewrow(Fts5Cursor *pCsr) {
    CsrFlagSet(pCsr,
               FTS5CSR_REQUIRE_CONTENT | FTS5CSR_REQUIRE_DOCSIZE | FTS5CSR_REQUIRE_INST | FTS5CSR_REQUIRE_POSLIST);
}
...
static void fts5TripCursors(Fts5FullTable *pTab) {
    Fts5Cursor *pCsr;
    for (pCsr = pTab->pGlobal->pCsr; pCsr; pCsr = pCsr->pNext) {
        if (pCsr->ePlan == FTS5_PLAN_MATCH && pCsr->base.pVtab == (sqlite3_vtab *)pTab) {
...
static i64 fts5CursorRowid(Fts5Cursor *pCsr) {
    assert(pCsr->ePlan == FTS5_PLAN_MATCH || pCsr->ePlan == FTS5_PLAN_SORTED_MATCH || pCsr->ePlan == FTS5_PLAN_SOURCE);
    if (pCsr->pSorter) {
...
static int fts5UpdateMethod(
    ...
    assert(sqlite3_value_type(apVal[0]) == SQLITE_INTEGER || sqlite3_value_type(apVal[0]) == SQLITE_NULL);
    ...
    if (eType0 == SQLITE_NULL && sqlite3_value_type(apVal[2 + pConfig->nCol]) != SQLITE_NULL) {
        ...
        if (pConfig->eContent != FTS5_CONTENT_NORMAL && 0 == sqlite3_stricmp("delete", z)) {
        ...
        ** VIRTUAL TABLE statement contained "contentless_delete=1". */
        if (eType0 == SQLITE_INTEGER && pConfig->eContent == FTS5_CONTENT_NONE && pConfig->bContentlessDelete == 0) {
            pTab->p.base.zErrMsg = sqlite3_mprintf(
    ...
    if (rc == SQLITE_OK && bUpdateOrDelete && pConfig->bSecureDelete && pConfig->iVersion == FTS5_CURRENT_VERSION) {
...
static int fts5ApiColumnText(
    ...
    int rc = SQLITE_OK;
    Fts5Cursor *pCsr = (Fts5Cursor *)pCtx;
    if (fts5IsContentless((Fts5FullTable *)(pCsr->base.pVtab)) || pCsr->ePlan == FTS5_PLAN_SPECIAL) {
        *pz = 0;
        *pn = 0;
    } else {
    ...
    if (aIter) {
        ...
        if (rc == SQLITE_OK) {
            while (1) {
                ...
                int iBest = -1;
                for (i = 0; i < nIter; i++) {
                    if ((aIter[i].bEof == 0) && (iBest < 0 || aIter[i].iPos < aIter[iBest].iPos)) {
                        iBest = i;
                    }
                }
                if (iBest < 0) break;
...
static int fts5ApiInstCount(Fts5Context *pCtx, int *pnInst) {
    Fts5Cursor *pCsr = (Fts5Cursor *)pCtx;
    int rc = SQLITE_OK;
    if (CsrFlagTest(pCsr, FTS5CSR_REQUIRE_INST) == 0 || SQLITE_OK == (rc = fts5CacheInstArray(pCsr))) {
    ...
    if (pCsr->ePlan == FTS5_PLAN_SPECIAL) {
        ...
        ** auxiliary function.  */
        ...
        if (pCsr->ePlan == FTS5_PLAN_SOURCE) {
            ...
            pCsr->ePlan == FTS5_PLAN_MATCH || pCsr->ePlan == FTS5_PLAN_SORTED_MATCH) {
        ...
        char *zErr = sqlite3_mprintf(
            "cannot UPDATE a subset of "
            "columns on fts5 contentless-delete table: %s",
            pConfig->zName);
...
static int fts5IntegrityMethod(
    ...
    if ((rc & 0xff) == SQLITE_CORRUPT) {
        ...
        *pzErr = sqlite3_mprintf(
            "unable to validate the inverted index for"
            " FTS5 table %s.%s: %s",
