...
int sqlite3Fts5ExprPattern(
    ...
    if (zExpr == 0) {
        ...
        while (i <= nText) {
            if (i == nText || zText[i] == aSpec[0] || zText[i] == aSpec[1] || zText[i] == aSpec[2]) {
    ...
    if (pExpr->pConfig->eDetail != FTS5_DETAIL_FULL) {
        ...
        ** phrase is not a match, break out of the loop early.  */
        for (i = 0; rc == SQLITE_OK && i < pNear->nPhrase; i++) {
            ...
            if (pPhrase->nTerm > 1 || pPhrase->aTerm[0].pSynonym || pNear->pColset || pPhrase->aTerm[0].bFirst) {
                int bMatch = 0;
...
static int fts5ExprNodeTest_STRING(
    ...
    assert(pNear->nPhrase > 1 || pNear->apPhrase[0]->nTerm > 1 || pNear->apPhrase[0]->aTerm[0].pSynonym || pNear->apPhrase[0]->aTerm[0].bFirst);
        ...
        ** is valid - each iterator that points to a rowid before iFrom.  */
        for (p = pTerm; p; p = p->pSynonym) {
            if (sqlite3Fts5IterEof(p->pIter) == 0) {
                ...
                if (ii == iRowid || (bFromValid && ii != iFrom && (ii > iFrom) == pExpr->bDesc)) {
                    if (bFromValid) {
...
static int fts5ExprNodeNext_OR(
    ...
    for (i = 0; i < pNode->nChild; i++) {
        ...
        if (p1->bEof == 0) {
            if ((p1->iRowid == iLast) || (bFromValid && fts5RowidCmp(pExpr, p1->iRowid, iFrom) < 0)) {
...
int sqlite3Fts5ExprFirst(Fts5Expr *p, Fts5Index *pIdx, i64 iFirst, int bDesc) {
    ...
    ** the iteration order, move to document iFirst or later. */
    if (rc == SQLITE_OK && 0 == pRoot->bEof && fts5RowidCmp(p, pRoot->iRowid, iFirst) < 0) {
...
int sqlite3Fts5ExprClonePhrase(
    ...
    if (rc == SQLITE_OK && ALWAYS(sCtx.pPhrase)) {
        ...
        if (pOrig->nTerm == 1 && pOrig->aTerm[0].pSynonym == 0 && pOrig->aTerm[0].bFirst == 0) {
...
static void fts5ParseSetColset(
    ...
    Fts5Colset **ppFree) {
    if (pParse->rc == SQLITE_OK) {
        assert(pNode->eType == FTS5_TERM || pNode->eType == FTS5_STRING || pNode->eType == FTS5_AND || pNode->eType == FTS5_OR || pNode->eType == FTS5_NOT || pNode->eType == FTS5_EOF);
        if (pNode->eType == FTS5_STRING || pNode->eType == FTS5_TERM) {
            Fts5ExprNearset *pNear = pNode->pNear;
            if (pNear->pColset) {
...
static void fts5ExprAssignXNext(Fts5ExprNode *pNode) {
    switch (pNode->eType) {
        case FTS5_STRING: {
            Fts5ExprNearset *pNear = pNode->pNear;
            if (pNear->nPhrase == 1 && pNear->apPhrase[0]->nTerm == 1 && pNear->apPhrase[0]->aTerm[0].pSynonym == 0 && pNear->apPhrase[0]->aTerm[0].bFirst == 0) {
...
Fts5ExprNode *sqlite3Fts5ParseNode(
    ...
    if (pParse->rc == SQLITE_OK) {
        ...
        assert((eType != FTS5_STRING && !pNear) || (eType == FTS5_STRING && !pLeft && !pRight));
        if (eType == FTS5_STRING && pNear == 0) return 0;
        if (eType != FTS5_STRING && pLeft == 0) return pRight;
        if (eType != FTS5_STRING && pRight == 0) return pLeft;

        if (eType == FTS5_STRING && pParse->bPhraseToAnd && pNear->apPhrase[0]->nTerm > 1) {
            ...
            if (pRet) {
                ...
                if (eType == FTS5_STRING) {
                    ...
                    if (pParse->pConfig->eDetail != FTS5_DETAIL_FULL) {
                        Fts5ExprPhrase *pPhrase = pNear->apPhrase[0];
                        if (pNear->nPhrase != 1 || pPhrase->nTerm > 1 || (pPhrase->nTerm > 0 && pPhrase->aTerm[0].bFirst)) {
                            sqlite3Fts5ParseError(pParse,
...
Fts5ExprNode *sqlite3Fts5ParseImplicitAnd(
    ...
    if (pParse->rc) {
        ...
        assert(pLeft->eType == FTS5_STRING || pLeft->eType == FTS5_TERM || pLeft->eType == FTS5_EOF || pLeft->eType == FTS5_AND);
        assert(pRight->eType == FTS5_STRING || pRight->eType == FTS5_TERM || pRight->eType == FTS5_EOF);
        ...
        assert(pPrev->eType == FTS5_STRING || pPrev->eType == FTS5_TERM || pPrev->eType == FTS5_EOF);
...
static char *fts5ExprPrint(Fts5Config *pConfig, Fts5ExprNode *pExpr) {
    ...
    } else if (pExpr->eType == FTS5_STRING || pExpr->eType == FTS5_TERM) {
        Fts5ExprNearset *pNear = pExpr->pNear;
        int i;
        int iTerm;
...
static int fts5ExprPopulatePoslistsCb(
    ...
    for (i = 0; i < pExpr->nPhrase; i++) {
        ...
        for (pTerm = &pExpr->apExprPhrase[i]->aTerm[0]; pTerm; pTerm = pTerm->pSynonym) {
            ...
            if ((nTerm == nToken || (nTerm < nToken && pTerm->bPrefix)) && memcmp(pTerm->zTerm, pToken, nTerm) == 0) {
...
int sqlite3Fts5ExprPopulatePoslists(
    ...
    for (i = 0; i < pExpr->nPhrase; i++) {
        ...
        if ((pColset && 0 == fts5ExprColsetTest(pColset, iCol)) || aPopulator[i].bMiss) {
            aPopulator[i].bOk = 0;
        } else {
            aPopulator[i].bOk = 1;
        }
...
static int fts5ExprCheckPoslists(Fts5ExprNode *pNode, i64 iRowid) {
    ...
    switch (pNode->eType) {
        ...
        default: {
            assert(pNode->eType == FTS5_NOT);
            if (0 == fts5ExprCheckPoslists(pNode->apChild[0], iRowid) || 0 != fts5ExprCheckPoslists(pNode->apChild[1], iRowid)) {
                fts5ExprClearPoslists(pNode);
                return 0;
            }
            break;
...
int sqlite3Fts5ExprPhraseCollist(
    ...
    if (pNode->bEof == 0 && pNode->iRowid == pExpr->pRoot->iRowid && pPhrase->poslist.n > 0) {
