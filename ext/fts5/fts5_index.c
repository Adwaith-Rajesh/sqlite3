...
static u64 fts5GetU64(u8 *a) {
    return ((u64)a[0] << 56) + ((u64)a[1] << 48) + ((u64)a[2] << 40) + ((u64)a[3] << 32) + ((u64)a[4] << 24) + ((u64)a[5] << 16) + ((u64)a[6] << 8) + ((u64)a[7] << 0);
}
...
static void fts5DataWrite(Fts5Index *p, i64 iRowid, const u8 *pData, int nData) {
    ...
    if (p->pWriter == 0) {
        Fts5Config *pConfig = p->pConfig;
        fts5IndexPrepareStmt(p, &p->pWriter, sqlite3_mprintf("REPLACE INTO '%q'.'%q_data'(id, block) VALUES(?,?)", pConfig->zDb, pConfig->zName));
        if (p->rc) return;
    }
...
static int fts5StructureDecode(
    ...
    ** structure record.  */
    ...
    if (nLevel > FTS5_MAX_SEGMENT || nLevel < 0 || nSegment > FTS5_MAX_SEGMENT || nSegment < 0) {
        return FTS5_CORRUPT;
    }
    nByte = (sizeof(Fts5Structure) +                   /* Main structure */
...
static void fts5StructureAddLevel(int *pRc, Fts5Structure **ppStruct) {
    ...
    if (*pRc == SQLITE_OK) {
        ...
        sqlite3_int64 nByte = (sizeof(Fts5Structure) +                   /* Main structure */
                               sizeof(Fts5StructureLevel) * (nLevel + 1) /* aLevel[] array */
        );
...
static void fts5SegIterLoadDlidx(Fts5Index *p, Fts5SegIter *pIter) {
    ...
    ** term. */
    if (pIter->iTermLeafPgno == pIter->iLeafPgno && pIter->iEndofDoclist < pLeaf->szLeaf) {
        return;
    }
...
static sqlite3_stmt *fts5IdxSelectStmt(Fts5Index *p) {
    if (p->pIdxSelect == 0) {
        Fts5Config *pConfig = p->pConfig;
        fts5IndexPrepareStmt(p, &p->pIdxSelect, sqlite3_mprintf("SELECT pgno FROM '%q'.'%q_idx' WHERE "
...
static void fts5AssertMultiIterSetup(Fts5Index *p, Fts5Iter *pIter) {
    if (p->rc == SQLITE_OK) {
        ...
        for (i = 0; i < pIter->nSeg; i++) {
            ...
            assert(p1 == pFirst || p1->pLeaf == 0 || fts5BufferCompare(&pFirst->term, &p1->term) || p1->iRowid == pIter->iSwitchRowid || (p1->iRowid < pIter->iSwitchRowid) == pIter->bRev);
        }
...
static int fts5MultiIterAdvanceRowid(
    ...
    if (pNew->iRowid == pIter->iSwitchRowid || (pNew->iRowid < pIter->iSwitchRowid) == pIter->bRev) {
        int i;
...
static void fts5MultiIterNext(
    ...
    while (p->rc == SQLITE_OK) {
        ...
        if (pSeg->pLeaf == 0 || bNewTerm || fts5MultiIterAdvanceRowid(pIter, iFirst, &pSeg)) {
        ...
        if ((pIter->bSkipEmpty == 0 || pSeg->nPos) && 0 == fts5MultiIterIsDeleted(pIter)) {
...
static void fts5MultiIterNext2(
    ...
    if (p->rc == SQLITE_OK) {
        *pbNewTerm = 0;
        ...
        } while ((fts5MultiIterIsEmpty(p, pIter) || fts5MultiIterIsDeleted(pIter)) && (p->rc == SQLITE_OK));
    }
}
...
static void fts5MultiIterNew(
    ...
    ** object and set the output variable to NULL.  */
    if (p->rc == SQLITE_OK) {
        ...
        if ((pNew->bSkipEmpty && fts5MultiIterIsEmpty(p, pNew)) || fts5MultiIterIsDeleted(pNew)) {
...
static int fts5MultiIterEof(Fts5Index *p, Fts5Iter *pIter) {
    ...
    assert(p->rc != SQLITE_OK || (pIter->aSeg[pIter->aFirst[1].iFirst].pLeaf == 0) == pIter->base.bEof);
...
static void fts5WriteAppendPoslistData(
    ...
    while (p->rc == SQLITE_OK && (pPage->buf.n + pPage->pgidx.n + n) >= p->pConfig->pgsz) {
...
static void fts5WriteInit(
    ...
    if (p->pIdxWriter == 0) {
        Fts5Config *pConfig = p->pConfig;
        fts5IndexPrepareStmt(p, &p->pIdxWriter, sqlite3_mprintf("INSERT INTO '%q'.'%q_idx'(segid,term,pgno) VALUES(?,?,?)", pConfig->zDb, pConfig->zName));
    }
            ...
            if (pData) {
                if (iOff > pData->szLeaf) {
                    ...
                    ** segment currently being trimmed.  */
                    ...
                    if (pSeg->iLeafPgno == pSeg->iTermLeafPgno && pSeg->iEndofDoclist < pData->szLeaf && pSeg->iPgidxOff <= pData->nn) {
...
static void fts5SecureDeleteIdxEntry(
    ...
    if (iPgno != 1) {
        ...
        if (p->pDeleteFromIdx == 0) {
            fts5IndexPrepareStmt(p, &p->pDeleteFromIdx, sqlite3_mprintf("DELETE FROM '%q'.'%q_idx' WHERE (segid, (pgno/2)) = (?1, ?2)", p->pConfig->zDb, p->pConfig->zName));
        }
        if (p->rc == SQLITE_OK) {
...
static void fts5DoSecureDelete(
    ...
    ** to be the last rowid in the doclist for its term.  */
    ...
    if (pSeg->bDel) {
        ...
        pSeg->iLeafPgno == pSeg->iTermLeafPgno && iStart == pSeg->iTermLeafOffset) {
...
static void fts5FlushSecureDelete(
    ...
    if (fts5MultiIterEof(p, pIter) == 0) {
        ...
        if (p->rc == SQLITE_OK && fts5MultiIterEof(p, pIter) == 0 && iRowid == fts5MultiIterRowid(pIter)) {
            Fts5SegIter *pSeg = &pIter->aSeg[pIter->aFirst[1].iFirst];
...
static void fts5SetupPrefixIter(
    ...
    if (aBuf && pStruct) {
        const int flags = FTS5INDEX_QUERY_SCAN | FTS5INDEX_QUERY_SKIPEMPTY | FTS5INDEX_QUERY_NOOUTPUT;
        int i;
        i64 iLastRowid = 0;
...
int sqlite3Fts5IndexBeginWrite(Fts5Index *p, int bDelete, i64 iRowid) {
    ...
    if (iRowid < p->iWriteRowid || (iRowid == p->iWriteRowid && p->bDelete == 0) || (p->nPendingData > p->pConfig->nHashSize)) {
        fts5IndexFlush(p);
    }
...
static void fts5TestTerm(
    ...
    } else if (rc == SQLITE_OK && (pPrev->n != n || memcmp(pPrev->p, z, n))) {
        u64 cksum3 = *pCksum;
...
static void fts5IndexIntegrityCheckSegment(
    ...
    fts5IndexPrepareStmt(p, &pStmt, sqlite3_mprintf("SELECT segid, term, (pgno>>1), (pgno&1) FROM %Q.'%q_idx' WHERE segid=%d "
                                                    "ORDER BY 1, 2",
    ...
    while (p->rc == SQLITE_OK && SQLITE_ROW == sqlite3_step(pStmt)) {
        ...
        ** location before the term.  */
        if (pLeaf->nn <= pLeaf->szLeaf) {
            if (nIdxTerm == 0 && pConfig->iVersion == FTS5_CURRENT_VERSION_SECUREDELETE && pLeaf->nn == pLeaf->szLeaf && pLeaf->nn == 4) {
