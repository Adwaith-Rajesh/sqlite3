...
#define CASE(i, str) \
    case i:          \
        assert(strcmp(aSub[i].zName, str) == 0);
    switch (iSub) {
        CASE(0, "xColumnCount") {
            int nCol;
...
static int f5tTokenizerTokenize(
    ...
    assert(
        flags == FTS5_TOKENIZE_DOCUMENT || flags == FTS5_TOKENIZE_AUX || flags == FTS5_TOKENIZE_QUERY || flags == (FTS5_TOKENIZE_QUERY | FTS5_TOKENIZE_PREFIX));
...
static int SQLITE_TCLAPI f5tTokenizerReturn(
    ...
    if (Tcl_GetIntFromObj(interp, objv[objc - 2], &iStart) || Tcl_GetIntFromObj(interp, objv[objc - 1], &iEnd)) {
        return TCL_ERROR;
    }
