...
#define ASYNC_TRACE(X) \
    if (sqlite3async_trace) asyncTrace X
    ...
    */
    ...
    if (rc == SQLITE_OK) {
        ...
        for (pWrite = async.pQueueFirst; pWrite; pWrite = pWrite->pNext) {
            if (pWrite->op == ASYNC_WRITE && ((pWrite->pFileData == p) ||
...
int asyncFileSize(sqlite3_file *pFile, sqlite3_int64 *piSize) {
    ...
    */
    ...
    if (rc == SQLITE_OK) {
        AsyncWrite *pWrite;
        for (pWrite = async.pQueueFirst; pWrite; pWrite = pWrite->pNext) {
            if (pWrite->op == ASYNC_DELETE && p->zName && strcmp(p->zName, pWrite->zBuf) == 0) {
                s = 0;
            } else if (pWrite->pFileData && ((pWrite->pFileData == p) || (p->zName && pWrite->pFileData->zName == p->zName))) {
                switch (pWrite->op) {
                    case ASYNC_WRITE:
...
static int asyncLock(sqlite3_file *pFile, int eLock) {
    ...
    if (p->zName) {
        ...
        if (p->lock.eLock < eLock) {
            ...
            for (pIter = pLock->pList; pIter; pIter = pIter->pNext) {
                if (pIter != &p->lock && ((eLock == SQLITE_LOCK_EXCLUSIVE && pIter->eLock >= SQLITE_LOCK_SHARED) ||
...
static int doAsynchronousOpen(int flags) {
    return (flags & SQLITE_OPEN_CREATE) && ((flags & SQLITE_OPEN_MAIN_JOURNAL) ||
...
static int asyncOpen(
    ...
    */
    ...
    nByte = (sizeof(AsyncFileData) + /* AsyncFileData structure */
    ...
    if (!isAsyncOpen) {
        ...
        if (rc == SQLITE_OK && (flagsout & SQLITE_OPEN_READWRITE) && (flags & SQLITE_OPEN_EXCLUSIVE) == 0) {
    ...
    if (rc == SQLITE_OK && flags == SQLITE_ACCESS_EXISTS) {
        for (p = async.pQueueFirst; p; p = p->pNext) {
            if (p->op == ASYNC_DELETE && 0 == strcmp(p->zBuf, zName)) {
                ret = 0;
            } else if (p->op == ASYNC_OPENEXCLUSIVE && p->pFileData->zName && 0 == strcmp(p->pFileData->zName, zName)) {
                ret = 1;
            }
        }
    }
    ASYNC_TRACE(("ACCESS(%s): %s = %d\n",
                 flags == SQLITE_ACCESS_READWRITE ? "read-write" : flags == SQLITE_ACCESS_READ ? "read"
                                                                                               : "exists",
                 zName, ret));
    async_mutex_leave(ASYNC_MUTEX_QUEUE);
    *pResOut = ret;
...
int sqlite3async_control(int op, ...) {
    ...
    switch (op) {
        case SQLITEASYNC_HALT: {
            ...
            if (eWhen != SQLITEASYNC_HALT_NEVER && eWhen != SQLITEASYNC_HALT_NOW && eWhen != SQLITEASYNC_HALT_IDLE) {
                rc = SQLITE_MISUSE;
                break;
            }
            async.eHalt = eWhen;
