...
#if CIO_WIN_WC_XLATE
#define PST_INITIALIZER                              \
    { INVALID_HANDLE_VALUE, SHELL_INVALID_CONS_MODE, \
      {0, 0, 0, 0}, SHELL_INVALID_FILE_PTR }
#else
#define PST_INITIALIZER \
    { 0, SHELL_INVALID_FILE_PTR }
#endif
...
#define SHELL_CONI_MODE \
    (ENABLE_ECHO_INPUT | ENABLE_INSERT_MODE | ENABLE_LINE_INPUT | 0x80 | ENABLE_QUICK_EDIT_MODE | ENABLE_EXTENDED_FLAGS | ENABLE_PROCESSED_INPUT)
#define SHELL_CONO_MODE (ENABLE_PROCESSED_OUTPUT | ENABLE_WRAP_AT_EOL_OUTPUT | ENABLE_VIRTUAL_TERMINAL_PROCESSING)
#endif
...
SQLITE_INTERNAL_LINKAGE void consoleRenewSetup(void) {
    ...
    int ix = 0;
    while (ix < 6) {
        PerStreamTags *ppst = (ix < 3) ? &consoleInfo.pstSetup[ix] : &consoleInfo.pstDesignated[ix - 3];
...
static FILE *designateEmitStream(FILE *pf, unsigned chix) {
    ...
    if (pf == invalidFileStream)
        ...
        if (ppst != 0) {
            ...
            streamOfConsole(pf, &consoleInfo.pstDesignated[chix]);
    }
    return rv;
}
...
#define setBinaryMode(f, bFlush) \
    do {                         \
        if ((bFlush)) fflush(f); \
    } while (0)
#define setTextMode(f, bFlush)   \
    do {                         \
        if ((bFlush)) fflush(f); \
    } while (0)
...
#if CIO_WIN_WC_XLATE
    if (pstReachesConsole(ppst))
        return conZstrEmit(ppst, z, (int)strlen(z));
    else {
#endif
...
#if CIO_WIN_WC_XLATE
    if (pfIn == consoleInfo.pstSetup[0].pf && (consoleInfo.sacSetup & SAC_InConsole) != 0) {
