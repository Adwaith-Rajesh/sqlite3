...
static void showSegmentStats(sqlite3 *db, const char *zTab) {
    ...
    for (i = 0; i <= mxLevel; i++) {
        pStmt = prepare(db,
                        ...
                        "           AND (b.level%%1024)==%d)",
                        pgsz - 45, zTab, zTab, i);
        if (sqlite3_step(pStmt) == SQLITE_ROW && (nLeaf = sqlite3_column_int(pStmt, 0)) > 0) {
            sqlite3_int64 sz;
        ...
        while (sqlite3_step(pStmt) == SQLITE_ROW) {
            ...
            if (iLEnd > iStart) {
                ...
                if (iLEnd + 1 <= iEnd) {
                    ...
                    if (iLower >= 0) {
                        if (iLower == iPrev && iLower == iEnd && isNullSegment(db, zTab, iLower)) {
                            printf("                 null   %9lld\n", iLower);
                        } else {
                            printTreeLine(iLower, iPrev);
                        }
