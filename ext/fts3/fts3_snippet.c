...
static MatchinfoBuffer *fts3MIBufferNew(size_t nElem, const char *zMatchinfo) {
    ...
    sqlite3_int64 nByte = sizeof(u32) * (2 * (sqlite3_int64)nElem + 1) + sizeof(MatchinfoBuffer);
    sqlite3_int64 nStr = strlen(zMatchinfo);

    pRet = sqlite3Fts3MallocZero(nByte + nStr + 1);
    if (pRet) {
        pRet->aMatchinfo[0] = (u8 *)(&pRet->aMatchinfo[1]) - (u8 *)pRet;
        pRet->aMatchinfo[1 + nElem] = pRet->aMatchinfo[0] + sizeof(u32) * ((int)nElem + 1);
...
static void fts3MIBufferFree(void *p) {
    ...
    assert((u32 *)p == &pBuf->aMatchinfo[1] || (u32 *)p == &pBuf->aMatchinfo[pBuf->nElem + 2]);
...
static int fts3SnippetFindPositions(Fts3Expr *pExpr, int iPhrase, void *ctx) {
    ...
    if (pCsr) {
        ...
        assert(rc != SQLITE_OK || (pPhrase->pList == 0 && pPhrase->pHead == 0 && pPhrase->pTail == 0));
    }
...
static int fts3MatchinfoCheck(
    ...
    char cArg,
    char **pzErr) {
    if ((cArg == FTS3_MATCHINFO_NPHRASE) || (cArg == FTS3_MATCHINFO_NCOL) || (cArg == FTS3_MATCHINFO_NDOC && pTab->bFts4) || (cArg == FTS3_MATCHINFO_AVGLENGTH && pTab->bFts4) || (cArg == FTS3_MATCHINFO_LENGTH && pTab->bHasDocsize) || (cArg == FTS3_MATCHINFO_LCS) || (cArg == FTS3_MATCHINFO_HITS) || (cArg == FTS3_MATCHINFO_LHITS) || (cArg == FTS3_MATCHINFO_LHITS_BM)) {
        return SQLITE_OK;
    }
