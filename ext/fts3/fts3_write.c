...
int sqlite3Fts3SelectDoctotal(
    ...
    if (rc == SQLITE_OK) {
        ...
        if (sqlite3_step(pStmt) != SQLITE_ROW || sqlite3_column_type(pStmt, 0) != SQLITE_BLOB) {
            rc = sqlite3_reset(pStmt);
            if (rc == SQLITE_OK) rc = FTS_CORRUPT_VTAB;
            pStmt = 0;
        }
...
static int fts3PendingListAppend(
    ...
    if (iCol > 0 && p->iLastCol != iCol) {
        if (SQLITE_OK != (rc = fts3PendingListAppendVarint(&p, 1)) || SQLITE_OK != (rc = fts3PendingListAppendVarint(&p, iCol))) {
            goto pendinglistappend_out;
        }
        p->iLastCol = iCol;
        p->iLastPos = 0;
...
static int fts3PendingTermsAdd(
    ...
    ** return early. */
    ...
    xNext = pModule->xNext;
    while (SQLITE_OK == rc && SQLITE_OK == (rc = xNext(pCsr, &zToken, &nToken, &iStart, &iEnd, &iPos))) {
        int i;
...
static int fts3PendingTermsDocid(
    ...
    */
    if (iDocid < p->iPrevDocid || (iDocid == p->iPrevDocid && p->bPrevDelete == 0) || p->iPrevLangid != iLangid || p->nPendingData > p->nMaxPendingData) {
...
static int fts3InsertData(
    ...
    */
    if (SQLITE_NULL != sqlite3_value_type(apVal[3 + p->nColumn])) {
        if (SQLITE_NULL == sqlite3_value_type(apVal[0]) && SQLITE_NULL != sqlite3_value_type(apVal[1])) {
...
static int fts3SegReaderRequire(Fts3SegReader *pReader, char *pFrom, int nByte) {
    int rc = SQLITE_OK;
    assert(!pReader->pBlob || (pFrom >= pReader->aNode && pFrom < &pReader->aNode[pReader->nNode]));
    while (pReader->pBlob && rc == SQLITE_OK && (pFrom - pReader->aNode + nByte) > pReader->nPopulate) {
...
static int fts3SegReaderNext(
    ...
    ** safe (no risk of overread) even if the node data is corrupted. */
    ...
    if (nSuffix <= 0 || (&pReader->aNode[pReader->nNode] - pNext) < nSuffix || nPrefix > pReader->nTerm) {
    ...
    */
    if (pReader->nDoclist > pReader->nNode - (pReader->aDoclist - pReader->aNode) || (pReader->nPopulate == 0 && pReader->aDoclist[pReader->nDoclist - 1]) || pReader->nDoclist == 0) {
        return FTS_CORRUPT_VTAB;
    }
    return SQLITE_OK;
}
...
int sqlite3Fts3MsrOvfl(
    ...
    for (ii = 0; rc == SQLITE_OK && ii < pMsr->nSegment; ii++) {
        ...
        if (!fts3SegReaderIsPending(pReader) && !fts3SegReaderIsRootOnly(pReader)) {
            sqlite3_int64 jj;
...
int sqlite3Fts3MsrIncrNext(
    ...
    int (*xCmp)(Fts3SegReader *, Fts3SegReader *) = (p->bDescIdx ? fts3SegReaderDoclistCmpRev : fts3SegReaderDoclistCmp);
    ...
    while (1) {
        ...
        if (pSeg->pOffsetList == 0) {
            *paPoslist = 0;
            ...
            j = 1;
            while (rc == SQLITE_OK && j < nMerge && apSegment[j]->pOffsetList && apSegment[j]->iDocid == iDocid) {
...
int sqlite3Fts3SegReaderStep(
    ...
    do {
        ...
        */
        ...
        if (nMerge == 1 && !isIgnoreEmpty && !isFirst && (p->bDescIdx == 0 || fts3SegReaderIsPending(apSegment[0]) == 0)) {
...
static int fts3SegmentMerge(
    ...
    assert(iLevel == FTS3_SEGCURSOR_ALL || iLevel == FTS3_SEGCURSOR_PENDING || iLevel >= 0);
    assert(iLevel < FTS3_SEGDIR_MAXLEVEL);
    assert(iIndex >= 0 && iIndex < p->nIndex);
...
int sqlite3Fts3PendingTermsFlush(Fts3Table *p) {
    ...
    */
    if (rc == SQLITE_OK && p->bHasStat && p->nAutoincrmerge == 0xff && p->nLeafAdd > 0) {
        sqlite3_stmt *pStmt = 0;
...
static int fts3IncrmergeAppend(
    ...
    ** the database. */
    if (pLeaf->block.n > 0 && (pLeaf->block.n + nSpace) > p->nNodeSize && pLeaf->iBlock < (pWriter->iStart + pWriter->nLeafEst)) {
...
int sqlite3Fts3Incrmerge(Fts3Table *p, int nMerge, int nMin) {
    ...
    while (rc == SQLITE_OK && nRem > 0) {
        ...
        ** to start work on some other level.  */
        ...
        if (SQLITE_OK == rc && pCsr->nSegment == nSeg && SQLITE_OK == (rc = sqlite3Fts3SegReaderStart(p, pCsr, pFilter))) {
            int bEmpty = 0;
...
int sqlite3Fts3UpdateMethod(
    ...
    */
    if (nArg > 1 && sqlite3_value_type(apVal[0]) == SQLITE_NULL && sqlite3_value_type(apVal[p->nColumn + 2]) != SQLITE_NULL) {
    ...
    */
    if (nArg > 1 && p->zContentTbl == 0) {
        ...
        if (sqlite3_value_type(pNewRowid) != SQLITE_NULL && (sqlite3_value_type(apVal[0]) == SQLITE_NULL || sqlite3_value_int64(apVal[0]) != sqlite3_value_int64(pNewRowid))) {
