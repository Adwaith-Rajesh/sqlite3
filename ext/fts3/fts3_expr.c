    ...
    while (rc == SQLITE_OK) {
        ...
        if (rc == SQLITE_OK) {
            if (p) {
                ...
                if (!sqlite3_fts3_enable_parentheses && p->eType == FTSQUERY_PHRASE && pParse->isNot) {
                    ...
                    */
                     ...
                     */
                    if (pPrev && ((eType == FTSQUERY_NEAR && !isPhrase && pPrev->eType != FTSQUERY_PHRASE) || (eType != FTSQUERY_PHRASE && isPhrase && pPrev->eType == FTSQUERY_NEAR))) {
                        sqlite3Fts3ExprFree(p);
                        rc = SQLITE_ERROR;
                        goto exprparse_out;
                    }
