...
static int fts3InitVtab(
    ...
    assert((sqlite3_strnicmp(argv[0], "fts4", 4) == 0 && isFts4) || (sqlite3_strnicmp(argv[0], "fts3", 4) == 0 && !isFts4));
    ...
    */
    ...
    for (iCol = 0; iCol < nCol; iCol++) {
        ...
        for (i = 0; i < nNotindexed; i++) {
            ...
            if (zNot && n == (int)strlen(zNot) && 0 == sqlite3_strnicmp(p->azColumn[iCol], zNot, n)) {
...
static int fts3BestIndexMethod(sqlite3_vtab *pVTab, sqlite3_index_info *pInfo) {
    ...
    */
    ...
    for (i = 0; i < pInfo->nConstraint; i++) {
        ...
        ** context" error.
        */
        if (pCons->op == SQLITE_INDEX_CONSTRAINT_MATCH && pCons->iColumn >= 0 && pCons->iColumn <= p->nColumn) {
        ...
        if (pCons->op == SQLITE_INDEX_CONSTRAINT_EQ && pCons->iColumn == p->nColumn + 2) {
            iLangidCons = i;
        }
...
static int fts3TermSelect(
    ...
    filter.flags = FTS3_SEGMENT_IGNORE_EMPTY | FTS3_SEGMENT_REQUIRE_POS | (pTok->isPrefix ? FTS3_SEGMENT_PREFIX : 0) | (pTok->bFirst ? FTS3_SEGMENT_FIRST : 0) | (iColumn < p->nColumn ? FTS3_SEGMENT_COLUMN_FILTER : 0);
    filter.iCol = iColumn;
    filter.zTerm = pTok->z;
    filter.nTerm = pTok->n;

    rc = sqlite3Fts3SegReaderStart(p, pSegcsr, &filter);
    while (SQLITE_OK == rc && SQLITE_ROW == (rc = sqlite3Fts3SegReaderStep(p, pSegcsr))) {
...
static int fts3SyncMethod(sqlite3_vtab *pVtab) {
    ...
    */
    ...
    if (rc == SQLITE_OK && p->nLeafAdd > (nMinMerge / 16) && p->nAutoincrmerge && p->nAutoincrmerge != 0xff) {
...
static void fts3SnippetFunc(
    ...
    */
    ...
    switch (nVal) {
        ...
        /* no break */ deliberate_fall_through case 5:
            iCol = sqlite3_value_int(apVal[4]);
        /* no break */ deliberate_fall_through case 4:
            zEllipsis = (const char *)sqlite3_value_text(apVal[3]);
        /* no break */ deliberate_fall_through case 3:
            zEnd = (const char *)sqlite3_value_text(apVal[2]);
        /* no break */ deliberate_fall_through case 2:
...
int sqlite3Fts3Init(sqlite3 *db) {
    ...
    if (rc == SQLITE_OK) {
        if (sqlite3Fts3HashInsert(&pHash->hash, "simple", 7, (void *)pSimple) || sqlite3Fts3HashInsert(&pHash->hash, "porter", 7, (void *)pPorter)
#ifndef SQLITE_DISABLE_FTS3_UNICODE
    ...
    ** module with sqlite.
    */
    if (SQLITE_OK == rc && SQLITE_OK == (rc = sqlite3Fts3InitHashTable(db, &pHash->hash, "fts3_tokenizer")) && SQLITE_OK == (rc = sqlite3_overload_function(db, "snippet", -1)) && SQLITE_OK == (rc = sqlite3_overload_function(db, "offsets", 1)) && SQLITE_OK == (rc = sqlite3_overload_function(db, "matchinfo", 1)) && SQLITE_OK == (rc = sqlite3_overload_function(db, "matchinfo", 2)) && SQLITE_OK == (rc = sqlite3_overload_function(db, "optimize", 1))) {
        pHash->nRef++;
...
static int fts3EvalPhraseStart(Fts3Cursor *pCsr, int bOptOk, Fts3Phrase *p) {
    ...
    ** tokens or prefix tokens that cannot use a prefix-index.  */
    int bHaveIncr = 0;
    int bIncrOk = (bOptOk && pCsr->bDesc == pTab->bDescIdx && p->nToken <= MAX_INCR_PHRASE_TOKENS && p->nToken > 0
#if defined(SQLITE_DEBUG) || defined(SQLITE_TEST)
    ...
    if (p->nToken == 1) {
        ...
        while (bEof == 0) {
            ...
            for (i = 0; i < p->nToken; i++) {
                while (rc == SQLITE_OK && bEof == 0 && a[i].bIgnore == 0 && DOCID_CMP(a[i].iDocid, iMax) < 0) {
                    rc = incrPhraseTokenNext(pTab, p, i, &a[i], &bEof);
...
static void fts3EvalTokenCosts(
    ...
    if (*pRc == SQLITE_OK) {
        if (pExpr->eType == FTSQUERY_PHRASE) {
            ...
            assert(pExpr->eType == FTSQUERY_OR || pExpr->eType == FTSQUERY_AND || pExpr->eType == FTSQUERY_NEAR);
...
static int fts3EvalStart(Fts3Cursor *pCsr) {
    ...
    if (rc == SQLITE_OK && nToken > 1 && pTab->bFts4) {
        ...
        aTC = (Fts3TokenAndCost *)sqlite3_malloc64(
            sizeof(Fts3TokenAndCost) * nToken + sizeof(Fts3Expr *) * nOr * 2);
    ...
    if (*pRc == SQLITE_OK && pExpr->bEof == 0) {
        ...
        switch (pExpr->eType) {
            ...
            case FTSQUERY_NOT: {
                ...
                fts3EvalNextRow(pCsr, pLeft, pRc);
                if (pLeft->bEof == 0) {
                    while (!*pRc && !pRight->bEof && DOCID_CMP(pLeft->iDocid, pRight->iDocid) > 0) {
                        fts3EvalNextRow(pCsr, pRight, pRc);
                    }
                }
                pExpr->iDocid = pLeft->iDocid;
...
static int fts3EvalNearTest(Fts3Expr *pExpr, int *pRc) {
    ...
    */
    if (*pRc == SQLITE_OK && pExpr->eType == FTSQUERY_NEAR && (pExpr->pParent == 0 || pExpr->pParent->eType != FTSQUERY_NEAR)) {
        Fts3Expr *p;
        ...
        if (!aTmp) {
            *pRc = SQLITE_NOMEM;
            ...
            for (p = pExpr->pLeft; p && res; p = p->pLeft) {
                ...
                pPhrase = (p->eType == FTSQUERY_NEAR ? p->pRight->pPhrase : p->pPhrase);
                res = fts3EvalNearTrim(nNear, aTmp, &aPoslist, &nToken, pPhrase);
            }
        }
    ...
    if (*pRc == SQLITE_OK) {
        switch (pExpr->eType) {
            ...
            case FTSQUERY_AND:
                ...
                */
                if (bHit == 0 && pExpr->eType == FTSQUERY_NEAR && (pExpr->pParent == 0 || pExpr->pParent->eType != FTSQUERY_NEAR)) {
                    Fts3Expr *p;
            ...
            default: {
#ifndef SQLITE_DISABLE_FTS4_DEFERRED
                if (pCsr->pDeferred && (pExpr->bDeferred || (pExpr->iDocid == pCsr->iPrevId && pExpr->pPhrase->doclist.pList))) {
                    Fts3Phrase *pPhrase = pExpr->pPhrase;
                    if (pExpr->bDeferred) {
                        fts3EvalInvalidatePoslist(pPhrase);
                    }
...
#endif
                {
                    bHit = (pExpr->bEof == 0 && pExpr->iDocid == pCsr->iPrevId && pExpr->pPhrase->doclist.nList > 0);
                }
                break;
            }
        }
...
static int fts3EvalNext(Fts3Cursor *pCsr) {
    ...
    ** by Fts3Cursor.iMinDocid/iMaxDocid. If so, set the EOF flag.  */
    if (rc == SQLITE_OK && ((pCsr->bDesc == 0 && pCsr->iPrevId > pCsr->iMaxDocid) || (pCsr->bDesc != 0 && pCsr->iPrevId < pCsr->iMinDocid))) {
        pCsr->isEof = 1;
    }
...
static int fts3EvalGatherStats(
    ...
    if (pExpr->aMI == 0) {
        ...
        pRoot = pExpr;
        while (pRoot->pParent && (pRoot->pParent->eType == FTSQUERY_NEAR || pRoot->bDeferred)) {
            pRoot = pRoot->pParent;
        }
        iDocid = pRoot->iDocid;
        bEof = pRoot->bEof;
        ...
        while (pCsr->isEof == 0 && rc == SQLITE_OK) {
            ...
            } while (pCsr->isEof == 0 && pRoot->eType == FTSQUERY_NEAR && sqlite3Fts3EvalTestDeferred(pCsr, &rc));
