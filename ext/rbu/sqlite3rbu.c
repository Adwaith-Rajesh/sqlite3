...
static int rbuObjIterCacheTableInfo(sqlite3rbu *p, RbuObjIter *pIter) {
    if (pIter->azTblCol == 0) {
        ...
        assert(pIter->eType == RBU_PK_NONE || pIter->eType == RBU_PK_IPK || pIter->eType == RBU_PK_EXTERNAL || pIter->eType == RBU_PK_WITHOUT_ROWID || pIter->eType == RBU_PK_VTAB);
        ...
        ** "rbu_".  */
        ...
        if (p->rc == SQLITE_OK && rbuIsVacuum(p) == 0 && bRbuRowid != (pIter->eType == RBU_PK_VTAB || pIter->eType == RBU_PK_NONE)) {
            p->rc = SQLITE_ERROR;
...
static void rbuObjIterPrepareTmpInsert(
    ...
    if (zBind) {
        ...
        p->rc = prepareFreeAndCollectError(
            p->dbRbu, &pIter->pTmpInsert, &p->zErrmsg, sqlite3_mprintf("INSERT INTO %s.'rbu_tmp_%q'(rbu_control,%s%s) VALUES(%z)", p->zStateDb, pIter->zDataTbl, zCollist, zRbuRowid, zBind));
    }
}
...
static void rbuTmpInsertFunc(
    ...
    assert(sqlite3_value_int(apVal[0]) != 0 || p->objiter.eType == RBU_PK_EXTERNAL || p->objiter.eType == RBU_PK_NONE);
...
static int rbuObjIterPrepareAll(
    ...
    if (pIter->pSelect == 0 && rbuObjIterCacheTableInfo(p, pIter) == SQLITE_OK) {
        ...
        if (zIdx) {
            ...
            int bRbuRowid = (pIter->eType == RBU_PK_VTAB) || (pIter->eType == RBU_PK_NONE) || (pIter->eType == RBU_PK_EXTERNAL && rbuIsVacuum(p));
            ...
            ** an rbu vacuum handle.  */
            ...
            if (rbuIsVacuum(p) == 0 && pIter->abIndexed) {
                ...
                rbuMPrintfExec(p, p->dbRbu,
                               ...
                               p->zStateDb, pIter->zDataTbl, (pIter->eType == RBU_PK_EXTERNAL ? ", 0 AS rbu_rowid" : ""), pIter->zDataTbl);
    ...
    while (rc == SQLITE_OK && SQLITE_ROW == sqlite3_step(pStmt)) {
        switch (sqlite3_column_int(pStmt, 0)) {
            case RBU_STATE_STAGE:
                ...
                if (pRet->eStage != RBU_STAGE_OAL && pRet->eStage != RBU_STAGE_MOVE && pRet->eStage != RBU_STAGE_CKPT) {
                    p->rc = SQLITE_CORRUPT;
                }
                break;
...
static void rbuMoveOalFile(sqlite3rbu *p) {
    ...
    if (zWal == 0 || zOal == 0) {
        ...
        */
        ...
        if (p->rc != SQLITE_OK || rbuIsVacuum(p) || rbuExclusiveCheckpoint(dbMain) == 0) {
...
static void rbuStepOneOp(sqlite3rbu *p, int eType) {
    ...
    assert(eType == RBU_DELETE || eType == RBU_IDX_DELETE || eType == RBU_INSERT || eType == RBU_IDX_INSERT);
    ...
    ** is invoked by the trigger.  */
    ...
    for (i = 0; i < pIter->nCol; i++) {
        ...
        ** to write a NULL into the IPK column. That is not permitted.  */
        if (eType == RBU_INSERT && pIter->zIdx == 0 && pIter->eType == RBU_PK_IPK && pIter->abTblPk[i] && sqlite3_column_type(pIter->pSelect, i) == SQLITE_NULL) {
    ...
    if (pIter->zIdx == 0) {
        if (pIter->eType == RBU_PK_VTAB || pIter->eType == RBU_PK_NONE || (pIter->eType == RBU_PK_EXTERNAL && rbuIsVacuum(p))) {
...
static int rbuStep(sqlite3rbu *p) {
    ...
    if (eType) {
        assert(eType == RBU_INSERT || eType == RBU_DELETE || eType == RBU_REPLACE || eType == RBU_IDX_DELETE || eType == RBU_IDX_INSERT || eType == RBU_UPDATE);
        ...
        if (pIter->zIdx == 0 && (eType == RBU_IDX_DELETE || eType == RBU_IDX_INSERT)) {
            ...
            if (pUpdate) {
                ...
                if (p->rc == SQLITE_OK && (pIter->eType == RBU_PK_VTAB || pIter->eType == RBU_PK_NONE)) {
                    ...
                    if (p->nStep >= p->nFrame) {
                        ...
                        ** lies on a different disk sector to the current one. */
                        ...
                        do {
                            ...
                            p->nStep++;
                        } while (p->nStep < p->nFrame && iSector == ((p->aFrame[p->nStep].iDbPage - 1) / p->nPagePerSector) && p->rc == SQLITE_OK);
                    }
                    p->nProgress++;
                }
                break;
...
static void rbuSetupOal(sqlite3rbu *p, RbuState *pState) {
    ...
    if (pState->zTbl) {
        ...
        while (rc == SQLITE_OK && pIter->zTbl && (pIter->bCleanup || rbuStrCompare(pIter->zIdx, pState->zIdx) || (pState->zDataTbl == 0 && rbuStrCompare(pIter->zTbl, pState->zTbl)) || (pState->zDataTbl && rbuStrCompare(pIter->zDataTbl, pState->zDataTbl)))) {
...
static sqlite3rbu *openRbuHandle(
    ...
    if (p) {
        ...
        if (p->rc == SQLITE_OK && (p->eStage == RBU_STAGE_OAL || p->eStage == RBU_STAGE_MOVE) && pState->eStage != 0) {
            rbu_file *pFd = (rbuIsVacuum(p) ? p->pRbuFd : p->pTargetFd);
...
int sqlite3rbu_state(sqlite3rbu *p) {
    ...
    if (p->rc != SQLITE_OK && p->rc != SQLITE_DONE) {
        ...
        assert(p->eStage == RBU_STAGE_OAL || p->eStage == RBU_STAGE_MOVE || p->eStage == RBU_STAGE_CKPT || p->eStage == RBU_STAGE_DONE);
...
static u32 rbuGetU32(u8 *aBuf) {
    return ((u32)aBuf[0] << 24) + ((u32)aBuf[1] << 16) + ((u32)aBuf[2] << 8) + ((u32)aBuf[3]);
}
...
static int rbuVfsRead(
    ...
    if (pRbu && pRbu->eStage == RBU_STAGE_CAPTURE) {
        ...
        if (pRbu && pRbu->eStage == RBU_STAGE_OAL && (p->openFlags & SQLITE_OPEN_WAL) && iOfst >= pRbu->iOalSz) {
            rc = SQLITE_OK;
            memset(zBuf, 0, iAmt);
        } else {
            ...
            ** SQLite will not check for a *-wal file.  */
            if (pRbu && rbuIsVacuum(pRbu) && rc == SQLITE_IOERR_SHORT_READ && iOfst == 0 && (p->openFlags & SQLITE_OPEN_MAIN_DB) && pRbu->rc == SQLITE_OK) {
...
static int rbuVfsWrite(
    ...
    if (pRbu && pRbu->eStage == RBU_STAGE_CAPTURE) {
        ...
        if (pRbu) {
            if (pRbu->eStage == RBU_STAGE_OAL && (p->openFlags & SQLITE_OPEN_WAL) && iOfst >= pRbu->iOalSz) {
...
static int rbuVfsFileSize(sqlite3_file *pFile, sqlite_int64 *pSize) {
    ...
    ** similar logic.  */
    if (rc == SQLITE_OK && *pSize == 0 && p->pRbu && rbuIsVacuum(p->pRbu) && (p->openFlags & SQLITE_OPEN_MAIN_DB)) {
        *pSize = 1024;
    }
    return rc;
}
...
static int rbuVfsLock(sqlite3_file *pFile, int eLock) {
    ...
    if (eLock == SQLITE_LOCK_EXCLUSIVE && (p->bNolock || (pRbu && pRbu->eStage != RBU_STAGE_DONE))) {
...
static int rbuVfsFileControl(sqlite3_file *pFile, int op, void *pArg) {
    ...
    assert(p->openFlags & (SQLITE_OPEN_MAIN_DB | SQLITE_OPEN_TEMP_DB) || p->openFlags & (SQLITE_OPEN_TRANSIENT_DB | SQLITE_OPEN_TEMP_JOURNAL));
...
static int rbuVfsShmLock(sqlite3_file *pFile, int ofst, int n, int flags) {
    ...
    assert(p->openFlags & (SQLITE_OPEN_MAIN_DB | SQLITE_OPEN_TEMP_DB));
    if (pRbu && (pRbu->eStage == RBU_STAGE_OAL || pRbu->eStage == RBU_STAGE_MOVE || pRbu->eStage == RBU_STAGE_DONE)) {
...
static int rbuVfsOpen(
    ...
    if (oflags & SQLITE_OPEN_MAIN_DB && sqlite3_uri_boolean(zName, "rbu_memory", 0)) {
