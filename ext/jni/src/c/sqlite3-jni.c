...
#define MARKER(pfexp)                                                  \
    do {                                                               \
        printf("MARKER: %s:%d:%s():\t", __FILE__, __LINE__, __func__); \
        printf pfexp;                                                  \
    } while (0)
...
#define S3JniExceptionIsFatal(MSG)    \
    S3JniIfThrew {                    \
        S3JniExceptionReport;         \
        S3JniExceptionClear;          \
...
#define s3jni_oom_fatal(EXPR) \
    if (!(EXPR)) s3jni_oom(env)
...
#define s3jni_mutf8_release(ARG, VAR) \
    if (VAR) (*env)->ReleaseStringUTFChars(env, ARG, VAR)
...
static S3JniNphOp *s3jni__nphop(JNIEnv *const env, S3JniNphOp const *pRef) {
    ...
    assert((void *)pRef >= (void *)&S3JniNphOps && (void *)pRef < (void *)(&S3JniNphOps + 1) && "pRef is out of range");
    assert(pRef->index >= 0 && (pRef->index < (sizeof(S3JniNphOps) / sizeof(S3JniNphOp))) && "pRef->index is out of range");
    if (!pNC->klazz) {
...
static jfieldID s3jni_nphop_field(JNIEnv *const env, S3JniNphOp const *pRef) {
    ...
    if (!pNC->fidValue) {
        ...
        if (!pNC->fidValue) {
            ...
            S3JniExceptionIsFatal(
                "Code maintenance required: missing "
...
static void OutputPointer_set_Bool(JNIEnv *const env, jobject const jOut,
                                   int v) {
    (*env)->SetBooleanField(env, jOut, s3jni_nphop_field(env, S3JniNph(OutputPointer_Bool)), v ? JNI_TRUE : JNI_FALSE);
...
static void OutputPointer_set_Int32(JNIEnv *const env, jobject const jOut,
                                    int v) {
    (*env)->SetIntField(env, jOut, s3jni_nphop_field(env, S3JniNph(OutputPointer_Int32)), (jint)v);
...
static void OutputPointer_set_Int64(JNIEnv *const env, jobject const jOut,
                                    jlong v) {
    (*env)->SetLongField(env, jOut, s3jni_nphop_field(env, S3JniNph(OutputPointer_Int64)), v);
...
static int s3jni_run_java_auto_extensions(sqlite3 *pDb, const char **pzErr,
    ...
    if (!ps) {
        *pzErr = sqlite3_mprintf(
            "Unexpected arrival of null S3JniDb in "
...
static jobject s3jni_commit_rollback_hook(int isCommit, JNIEnv *const env,
    ...
    if (pOld && jHook &&
        ...
        *pHook = S3JniHook_empty;
        if (isCommit)
            sqlite3_commit_hook(ps->pDb, 0, 0);
        else
            sqlite3_rollback_hook(ps->pDb, 0, 0);
    } else {
        jclass const klazz = (*env)->GetObjectClass(env, jHook);
        ...
        S3JniIfThrew {
            ...
            pHook->jObj = S3JniRefGlobal(jHook);
            if (isCommit)
                sqlite3_commit_hook(ps->pDb, s3jni_commit_hook_impl, ps);
            else
                sqlite3_rollback_hook(ps->pDb, s3jni_rollback_hook_impl, ps);
            if (pOld) {
                jobject tmp = S3JniRefLocal(pOld);
                S3JniUnrefGlobal(pOld);
                pOld = tmp;
...
S3JniApi(sqlite3_complete(), int, 1complete)(
    ...
    assert((nBA > 0 ? 0 == pBuf[nBA - 1] : (pBuf ? 0 == *pBuf : 1)) && "Byte array is not NUL-terminated.");
...
S3JniApi(sqlite3_config() /* for SQLITE_CONFIG_LOG */,
         jint, 1config__Lorg_sqlite_jni_ConfigLogCallback_2)(JniArgsEnvClass, jobject jLog) {
...
static int CollationCallback_xCompare(void *pArg, int nLhs, const void *lhs,
    ...
    if (pCC->jObj) {
        ...
        jbyteArray jbaRhs = jbaLhs
                                ? s3jni_new_jbyteArray(rhs, (jint)nRhs)
                                : 0;
        if (!jbaRhs) {
...
S3JniApi(sqlite3_create_collation() sqlite3_create_collation_v2(),
         jint, 1create_1collation)(JniArgsEnvClass, jobject jDb, jstring name, jint eTextRep,
                                   jobject oCollation) {
    int rc;
    S3JniDb *ps;
...
S3JniApi(sqlite3_create_function() sqlite3_create_function_v2()
         ...
         jint, 1create_1function)(JniArgsEnvClass, jobject jDb, jstring jFuncName, jint nArg,
...
S3JniApi(sqlite3_db_config() /*for MAINDBNAME*/,
         jint, 1db_1config__Lorg_sqlite_jni_capi_sqlite3_2ILjava_lang_String_2)(JniArgsEnvClass, jobject jDb, jint op, jstring jStr) {
...
static const char *sqlite3_normalized_sql(sqlite3_stmt *s) {
    S3JniDeclLocal_env;
    (*env)->FatalError(env,
                       "dummy sqlite3_normalized_sql() was "
...
jint sqlite3_jni_prepare_v123(int prepVersion, JNIEnv *const env, jclass self,
    ...
    switch (prepVersion) {
        ...
        case 2:
            ...
            break;
        case 3:
            rc = sqlite3_prepare_v3(pDb, (const char *)pBuf,
...
#ifdef SQLITE_ENABLE_PREUPDATE_HOOK
        if (isPre)
            sqlite3_preupdate_hook(ps->pDb, 0, 0);
        else
#endif
...
#ifdef SQLITE_ENABLE_PREUPDATE_HOOK
        if (isPre)
            sqlite3_preupdate_hook(ps->pDb, s3jni_preupdate_hook_impl, ps);
        else
#endif
...
S3JniApi(sqlite3_table_column_metadata(), int, 1table_1column_1metadata)(
    ...
    zColumnName = (zTableName && jColumnName)
                      ? s3jni_jstring_to_utf8(jColumnName, 0)
                      : 0;
    rc = zTableName
    ...
    if (0 == rc) {
        ...
        jstring jdtype = jDataType
                             ...
                             : 0;
        if ((jCollSeq && pzCollSeq && !jseq) || (jDataType && pzDataType && !jdtype)) {
            rc = SQLITE_NOMEM;
        } else {
