...
#define CmppLevel_empty_m \
    { 0U, 0U, CmppToken_empty_m, TS_Start }
...
#define FINI(M) \
    if (g.stmt.M) sqlite3_finalize(g.stmt.M)
    FINI(defIns);
    FINI(defDel);
    FINI(defHas);
    FINI(inclIns);
...
static unsigned short cmpp_is_delim(unsigned char const *zBegin,
    ...
    assert(zPos >= zBegin);
    if (zPos > zBegin &&
        ('\n' != *(zPos - 1) || ((unsigned)(zEnd - zPos) <= g.nDelim))) {
        return 0;
...
static int cmpp_next_keyword_line(CmppTokenizer *const t) {
    ...
    zStart = z = t->zPos;
    *tok = CmppToken_empty;
    while (z < t->zEnd && 0 == (isDelim = cmpp_is_delim(t->zBegin, t->zEnd, z))) {
        ++z;
    }
    if (z > zStart) {
        ...
        case TT_ElifNot:
            switch (CT_pstate(t)) {
                case TS_If:
                    break;
                case TS_IfPassed:
                    CT_level(t).flags |= CmppLevel_F_ELIDE;
                    return;
                default:
                    goto misuse;
            }
...
#define arg(F, D) fprintf(fOut, "  %s\n      %s\n", F, D)
    ...
    arg("-d|--delimiter VALUE",
        "Set keyword delimiter to VALUE "
