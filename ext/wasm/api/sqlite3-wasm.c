...
/* Core output macros... */
#define lenCheck                                                                  \
    assert(zPos < zEnd - 128 && "sqlite3_wasm_enum_json() buffer is too small."); \
...
#define IndexSzCheck(T, M) \
    (sizeof(T) == sizeof(*siiCheck.M))
            if (!IndexSzCheck(sqlite3_index_constraint, aConstraint) || !IndexSzCheck(sqlite3_index_orderby, aOrderBy) || !IndexSzCheck(sqlite3_index_constraint_usage, aConstraintUsage)) {
