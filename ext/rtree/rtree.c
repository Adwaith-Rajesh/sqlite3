...
#else
#define DCOORD(coord) ( \
    (pRtree->eCoordType == RTREE_COORD_REAL32) ? ((double)coord.f) : ((double)coord.i))
#endif
...
#define RTREE_DECODE_COORD(eInt, a, r)                                         \
    {                                                                          \
        RtreeCoord c; /* Coordinate decoded */                                 \
        c.u = ((u32)a[0] << 24) + ((u32)a[1] << 16) + ((u32)a[2] << 8) + a[3]; \
        r = eInt ? (sqlite3_rtree_dbl)c.i : (sqlite3_rtree_dbl)c.f;            \
    }
#endif
...
static int rtreeCallbackConstraint(
    ...
    if (eInt == 0) {
        switch (nCoord) {
            case 10:
                readCoord(pCellData + 36, &c);
                aCoord[9] = c.f;
                readCoord(pCellData + 32, &c);
                aCoord[8] = c.f;
            case 8:
                readCoord(pCellData + 28, &c);
                aCoord[7] = c.f;
                readCoord(pCellData + 24, &c);
                aCoord[6] = c.f;
            case 6:
                readCoord(pCellData + 20, &c);
                aCoord[5] = c.f;
                readCoord(pCellData + 16, &c);
                aCoord[4] = c.f;
            case 4:
                readCoord(pCellData + 12, &c);
                aCoord[3] = c.f;
                readCoord(pCellData + 8, &c);
                aCoord[2] = c.f;
            default:
                readCoord(pCellData + 4, &c);
                aCoord[1] = c.f;
                readCoord(pCellData, &c);
                aCoord[0] = c.f;
        }
    } else
#endif
...
static void rtreeNonleafConstraint(
    ...
    */
    ...
    assert(p->op == RTREE_LE || p->op == RTREE_LT || p->op == RTREE_GE || p->op == RTREE_GT || p->op == RTREE_EQ || p->op == RTREE_TRUE || p->op == RTREE_FALSE);
...
static RtreeSearchPoint *rtreeSearchPointFirst(RtreeCursor *pCur) {
    return pCur->bPoint ? &pCur->sPoint : pCur->nPoint ? pCur->aPoint
                                                       : 0;
}
...
static RtreeSearchPoint *rtreeSearchPointNew(
    ...
    pCur->anQueue[iLevel]++;
    if (pFirst == 0 || pFirst->rScore > rScore || (pFirst->rScore == rScore && pFirst->iLevel > iLevel)) {
        if (pCur->bPoint) {
            int ii;
...
static int rtreeFilter(
    ...
    if (idxNum == 1) {
        ...
        if (eType == SQLITE_INTEGER || (eType == SQLITE_FLOAT && sqlite3_value_double(argv[0]) == iRowid)) {
                ...
                for (ii = 0; ii < argc; ii++) {
                    ...
                    if (p->op >= RTREE_MATCH) {
                        ...
                        */
                        ...
                        if (iVal >= ((sqlite3_int64)1) << 48 || iVal <= -(((sqlite3_int64)1) << 48)) {
...
static int rtreeBestIndex(sqlite3_vtab *tab, sqlite3_index_info *pIdxInfo) {
    ...
    ** possible. */
    ...
    for (ii = 0; ii < pIdxInfo->nConstraint && iIdx < (int)(sizeof(zIdxStr) - 1); ii++) {
        ...
        if (bMatch == 0 && p->usable && p->iColumn <= 0 && p->op == SQLITE_INDEX_CONSTRAINT_EQ) {
        ...
        if (p->usable && ((p->iColumn > 0 && p->iColumn <= pRtree->nDim2) || p->op == SQLITE_INDEX_CONSTRAINT_MATCH)) {
            u8 op;
            u8 doOmit = 1;
            switch (p->op) {
...
static RtreeDValue cellArea(Rtree *pRtree, RtreeCell *p) {
    ...
    if (pRtree->eCoordType == RTREE_COORD_REAL32) {
        switch (pRtree->nDim) {
            case 5:
                area = p->aCoord[9].f - p->aCoord[8].f;
            case 4:
                area *= p->aCoord[7].f - p->aCoord[6].f;
            case 3:
                area *= p->aCoord[5].f - p->aCoord[4].f;
            case 2:
                area *= p->aCoord[3].f - p->aCoord[2].f;
            default:
                area *= p->aCoord[1].f - p->aCoord[0].f;
        }
    } else
#endif
    {
        switch (pRtree->nDim) {
            case 5:
                area = (i64)p->aCoord[9].i - (i64)p->aCoord[8].i;
            case 4:
                area *= (i64)p->aCoord[7].i - (i64)p->aCoord[6].i;
            case 3:
                area *= (i64)p->aCoord[5].i - (i64)p->aCoord[4].i;
            case 2:
                area *= (i64)p->aCoord[3].i - (i64)p->aCoord[2].i;
            default:
                area *= (i64)p->aCoord[1].i - (i64)p->aCoord[0].i;
        }
    }
    return area;
}
...
static int splitNodeStartree(
    ...
    for (ii = 0; ii < pRtree->nDim; ii++) {
        ...
        for (
            ...
            if ((nLeft == RTREE_MINCELLS(pRtree)) || (overlap < fBestOverlap) || (overlap == fBestOverlap && area < fBestArea)) {
                iBestLeft = nLeft;
...
static int SplitNode(
    ...
    */
    if (SQLITE_OK != (rc = nodeWrite(pRtree, pRight)) || (0 == pLeft->iNode && SQLITE_OK != (rc = nodeWrite(pRtree, pLeft)))) {
        goto splitnode_out;
    }
    ...
    if (nData > 1) {
        ...
        ** the table. If so, the constraint has failed. */
        if (sqlite3_value_type(aData[2]) != SQLITE_NULL) {
            ...
            if (sqlite3_value_type(aData[0]) == SQLITE_NULL || sqlite3_value_int64(aData[0]) != cell.iRowid) {
                int steprc;
...
static int rtreeRename(sqlite3_vtab *pVtab, const char *zNewName) {
    ...
    char *zSql = sqlite3_mprintf(
        ...
        pRtree->zDb, pRtree->zName, zNewName, pRtree->zDb, pRtree->zName, zNewName, pRtree->zDb, pRtree->zName, zNewName);
    if (zSql) {
        nodeBlobReset(pRtree);
...
static void rtreedepth(sqlite3_context *ctx, int nArg, sqlite3_value **apArg) {
    UNUSED_PARAMETER(nArg);
    if (sqlite3_value_type(apArg[0]) != SQLITE_BLOB || sqlite3_value_bytes(apArg[0]) < 2
...
static void rtreeCheckCellCoord(
    ...
    for (i = 0; i < pCheck->nDim; i++) {
        ...
        if (pParent) {
            ...
            if ((pCheck->bInt ? c1.i < p1.i : c1.f < p1.f) || (pCheck->bInt ? c2.i > p2.i : c2.f > p2.f)) {
                rtreeCheckAppendMsg(pCheck,
                                    "Dimension %d of cell %d on node %lld is corrupt relative to parent", i, iCell, iNode);
            }
        }
    }
}
...
static void geomCallback(sqlite3_context *ctx, int nArg, sqlite3_value **aArg) {
    ...
    nBlob = sizeof(RtreeMatchArg) + (nArg - 1) * sizeof(RtreeDValue) + nArg * sizeof(sqlite3_value *);
