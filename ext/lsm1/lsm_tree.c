...
static int treeInsert(
    ...
    ** insert the new key directly into pNode.  */
    ...
    if (pNode->aiKeyPtr[0] && pNode->aiKeyPtr[2]) {
        u32 iLeft;
        TreeNode *pLeft; /* New left-hand sibling node */
        u32 iRight;
        TreeNode *pRight; /* New right-hand sibling node */
    ...
    if (pNew) {
        if (pLeaf->aiKeyPtr[0] && pLeaf->aiKeyPtr[2]) {
            ...
            if (pRight) {
                ...
                switch (iSlot) {
                    case 0:
                        pNew->aiKeyPtr[0] = iTreeKey;
                        break;
                    case 1:
                        pNew->aiKeyPtr[2] = iTreeKey;
                        break;
                    case 2:
                        pRight->aiKeyPtr[0] = iTreeKey;
                        break;
...
void lsmTreeDiscardOld(lsm_db *pDb) {
    assert(lsmShmAssertLock(pDb, LSM_LOCK_WRITER, LSM_LOCK_EXCL) || lsmShmAssertLock(pDb, LSM_LOCK_DMS2, LSM_LOCK_EXCL));
...
static int treeInsertEntry(
    ...
    assert(flags == LSM_INSERT || flags == LSM_POINT_DELETE || flags == LSM_START_DELETE || flags == LSM_END_DELETE);
    ...
    if (p->iRoot) {
        ...
        if (flags == LSM_START_DELETE) {
            ...
            ** then the new entry is not required.  */
            if ((res <= 0 && (pRes->flags & LSM_START_DELETE)) || (res > 0 && treePrevIsStartDelete(pDb, &csr))) {
            ...
            ** then the new entry is not required.  */
            if ((res < 0 && treeNextIsEndDelete(pDb, &csr)) || (res >= 0 && (pRes->flags & LSM_END_DELETE))) {
                goto insert_entry_out;
            }
        }
        ...
        if (flags & (LSM_INSERT | LSM_POINT_DELETE)) {
            if ((res < 0 && (pRes->flags & LSM_START_DELETE)) || (res > 0 && (pRes->flags & LSM_END_DELETE))) {
        ...
        ** two new nodes.  */
        ...
        if (pPeer->aiKeyPtr[0] && pPeer->aiKeyPtr[2]) {
            ...
            ** and a new parent node are required. */
            ...
            TreeNode *pNew2;
            u32 iNew2; /* Second new leaf node */
            TreeNode *pNewP;
            u32 iNewP; /* New parent node */
  ...
  dump_tree_contents(db, "before delete");
    ...
    ** range being deleted. Or until an error occurs. */
    while (bDone == 0 && rc == LSM_OK) {
        ...
        void *pDel;
        int nDel; /* Key to (possibly) delete this iteration */
#ifndef NDEBUG
