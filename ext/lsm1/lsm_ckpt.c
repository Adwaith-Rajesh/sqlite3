...
static void ckptExportLevel(
    ...
    if (pMerge) {
        ...
        assert(pMerge->nInput == pLevel->nRight || pMerge->nInput == pLevel->nRight + 1);
...
static int ckptExportSnapshot(
    ...
    assert(iId >= 0);
    assert(pSnap->iCmpId == pDb->compress.iId || pSnap->iCmpId == LSM_COMPRESSION_EMPTY);
...
int lsmCheckpointClientCacheOk(lsm_db *pDb) {
    return (pDb->pClient && pDb->pClient->iId == lsmCheckpointId(pDb->aSnapshot, 0) && pDb->pClient->iId == lsmCheckpointId(pDb->pShmhdr->aSnap1, 0) && pDb->pClient->iId == lsmCheckpointId(pDb->pShmhdr->aSnap2, 0));
}
...
int lsmCheckpointLoadWorker(lsm_db *pDb) {
    ...
    assert(
        lsmShmAssertLock(pDb, LSM_LOCK_WORKER, LSM_LOCK_EXCL) || lsmShmAssertLock(pDb, LSM_LOCK_DMS1, LSM_LOCK_EXCL));
