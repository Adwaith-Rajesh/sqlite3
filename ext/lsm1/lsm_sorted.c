...
struct SegmentPtr {
    ...
    void *pKey;
    int nKey; /* Key associated with current record */
    void *pVal;
    int nVal; /* Current record value (eType==WRITE only) */
...
u32 lsmGetU32(u8 *aOut) {
    return ((u32)aOut[0] << 24) + ((u32)aOut[1] << 16) + ((u32)aOut[2] << 8) + ((u32)aOut[3]);
}

u64 lsmGetU64(u8 *aOut) {
    return ((u64)aOut[0] << 56) + ((u64)aOut[1] << 48) + ((u64)aOut[2] << 40) + ((u64)aOut[3] << 32) + ((u64)aOut[4] << 24) + ((u32)aOut[5] << 16) + ((u32)aOut[6] << 8) + ((u32)aOut[7]);
}
        ...
        if (rc == LSM_OK && nDepth > 1) {
            ...
            do {
                ...
                if (rc == LSM_OK) {
                    ...
                    iPg++;
                    assert(iPg != nDepth - 1 || lsmFsRedirectPage(pCsr->pFS, pSeg->pRedirect, iLoad) == iLeaf);
                }
...
static int segmentPtrIgnoreSeparators(MultiCursor *pCsr, SegmentPtr *pPtr) {
    return (pCsr->flags & CURSOR_READ_SEPARATORS) == 0 || (pPtr != &pCsr->aPtr[pCsr->nPtr - 1]);
}
...
static int segmentPtrAdvance(
    ...
    do {
        ...
        if (iCell >= pPtr->nCell || iCell < 0) {
            ...
            } while (rc == LSM_OK && pPtr->pPg && (pPtr->nCell == 0 || (pPtr->flags & SEGMENT_BTREE_FLAG)));
            if (rc != LSM_OK) return rc;
    ...
    } while (pCsr && pPtr->pPg && segmentPtrIgnoreSeparators(pCsr, pPtr) && rtIsSeparator(pPtr->eType));
    ...
    while (rc == LSM_OK && (pPtr->flags & PGFTR_SKIP_NEXT_FLAG)) {
        ...
        ** page.  */
        ...
        while (1) {
            ...
            if ((pageGetFlags(aData, nData) & SEGMENT_BTREE_FLAG) == 0 && pageGetNRec(aData, nData) > 0) {
                break;
            }
        }
        if (pNext == 0) break;
...
static int segmentPtrFwdPointer(
    ...
    if (pPtr->pSeg == &pLvl->lhs || pPtr->pSeg == &pLvl->aRhs[pLvl->nRight - 1]) {
        if (pNext == 0 || (pNext->nRight == 0 && pNext->lhs.iRoot) || (pNext->nRight != 0 && pNext->aRhs[0].iRoot)) {
...
static int segmentPtrSeek(
  ...
  assert( assertKeyLocation(pCsr, pPtr, pKey, nKey) );
    ...
    assert(pPtr->nCell > 0 || pPtr->pSeg->nSize == 1 || lsmFsDbPageIsLast(pPtr->pSeg, pPtr->pPg));
    if (pPtr->nCell == 0) {
            ...
            if (rc == LSM_OK) {
                switch (eSeek) {
                    case LSM_SEEK_EQ: {
                        ...
                        if ((res < 0 && (eType & LSM_START_DELETE)) || (res > 0 && (eType & LSM_END_DELETE)) || (res == 0 && (eType & LSM_POINT_DELETE))) {
                            *pbStop = 1;
                    ...
                    case LSM_SEEK_GE: {
                        ...
                        if ((res <= 0 && (pPtr->eType & LSM_START_DELETE)) || (res > 0 && (pPtr->eType & LSM_END_DELETE))) {
        ...
        ** supposed to ignore separators keys, advance to the next entry.  */
        if (rc == LSM_OK && pPtr->pPg && segmentPtrIgnoreSeparators(pCsr, pPtr) && rtIsSeparator(pPtr->eType)) {
...
static int multiCursorAddTree(MultiCursor *pCsr, Snapshot *pSnap, int eTree) {
    ...
    if (eTree != TREE_NONE && lsmTreeHasOld(db) && db->treehdr.iOldLog != pSnap->iLogOff) {
...
static int multiCursorGetVal(
    ...
    *pnVal = 0;
    ...
    switch (iVal) {
        ...
        case CURSOR_DATA_SYSTEM: {
            Snapshot *pWorker = pCsr->pDb->pWorker;
            if (pWorker && (pCsr->iFree % 2) == 0 && pCsr->iFree < (pWorker->freelist.nEntry * 2)) {
...
int lsmSortedLoadFreelist(
    ...
    if (rc == LSM_OK) {
        rc = lsmMCursorLast(pCsr);
        if (rc == LSM_OK && rtIsWrite(pCsr->eType) && rtIsSystem(pCsr->eType) && pCsr->key.nData == 8 && 0 == memcmp(pCsr->key.pData, "FREELIST", 8)) {
            void *pVal;
            int nVal; /* Value read from database */
...
static void assertCursorTree(MultiCursor *pCsr) {
    ...
    if (rc == LSM_OK) {
        ...
        assert(nSave == pCsr->nTree && 0 == memcmp(aSave, pCsr->aTree, sizeof(int) * nSave));
    ...
    */
    ...
    for (i = 0; i < iKey; i++) {
        ...
        if ((rdmask & csrflags)) {
            ...
            if ((csrflags & SD_ED) == SD_ED || (pCsr->flags & CURSOR_IGNORE_DELETE) == 0) {
                void *pKey;
                int nKey;
            ...
            case LSM_SEEK_EQ: {
                ...
                if ((res < 0 && (eType & LSM_START_DELETE)) || (res > 0 && (eType & LSM_END_DELETE)) || (res == 0 && (eType & LSM_POINT_DELETE))) {
                    *pbStop = 1;
                    ...
                    lsm_env *pEnv = pCsr->pDb->pEnv;
                    void *p;
                    int n; /* Key/value from tree-cursor */
                    *pbStop = 1;
                    pCsr->flags |= CURSOR_SEEK_EQ;
        ...
        do {
            ...
            ** from, record the current absolute pointer value.  */
            if (pCsr->pPrevMergePtr) {
                ...
                } else if (pCsr->pBtCsr == 0 && pCsr->nPtr > 0 && iKey == (CURSOR_DATA_SEGMENT + pCsr->nPtr - 1)) {
...
static int mergeWorkerPageOffset(u8 *aData, int nData) {
    ...
    eType = aData[iOff++];
    assert(eType == 0 || eType == (LSM_SYSTEMKEY | LSM_SEPARATOR) || eType == (LSM_SEPARATOR));
...
static void mergeRangeDeletes(MultiCursor *pCsr, int *piVal, int *piFlags) {
    ...
    if (pCsr->flags & CURSOR_IGNORE_DELETE) {
        ...
        ** retaining any range-delete flags.  */
        ...
        if ((f & LSM_START_DELETE) && (f & LSM_END_DELETE) && (f & LSM_POINT_DELETE)) {
            f = 0;
        }
    }
...
static int mergeWorkerStep(MergeWorker *pMW) {
    ...
    ** value written with the previous key.  */
    ...
    if (pCsr->pBtCsr) {
        ...
        if (pPtr->pPg && 0 == pDb->xCmp(pPtr->pKey, pPtr->nKey, pKey, nKey)) {
...
static int sortedMergeSetup(
    ...
    if (rc == LSM_OK) {
        ...
        *pp = pNew;
        ...
        if (pNext && pNext->pMerge == 0 && pNext->lhs.iRoot && pNext && (bFreeOnly == 0 || (pNext->flags & LEVEL_FREELIST_ONLY))) {
            bUseNext = 1;
        }
    }
    ...
    ** with the largest number of right-hand segments. Work on it. */
    for (pLevel = pTopLevel; pLevel; pLevel = pLevel->pNext) {
        if (pLevel->nRight == 0 && pThis && pLevel->iAge == pThis->iAge) {
            ...
            if (nThis > nBest) {
                if ((pLevel->iAge != pThis->iAge + 1) || (pLevel->nRight == 0 && sortedCountLevels(pLevel) <= pDb->nMerge)) {
                    pBest = pThis;
                    nBest = nThis;
                }
            }
...
static int sortedDbIsFull(lsm_db *pDb) {
    ...
    if (pTop && pTop->iAge == 0 && (pTop->nRight || sortedCountLevels(pTop) >= pDb->nMerge)) {
        return 1;
    }
    return 0;
}
...
static int sortedWork(
    ...
    while (nRemaining > 0) {
        ...
        if (pLevel == 0) {
            ...
            while (rc == LSM_OK && 0 == mergeWorkerDone(&mergeworker) && (mergeworker.nWork < nRemaining || pDb->bUseFreelist)) {
                ...
                ** free-list-only blocks.  */
                if (rc == LSM_OK && nMerge == 1 && eType == 0 && (rtTopic(mergeworker.pCsr->eType) || mergeWorkerDone(&mergeworker))) {
...
static int sortedTreeHasOld(lsm_db *pDb, int *pRc) {
    ...
    assert(pDb->pWorker);
    if (*pRc == LSM_OK) {
        if (rc == LSM_OK && pDb->treehdr.iOldShmid && pDb->treehdr.iOldLog != pDb->pWorker->iLogOff) {
            bRet = 1;
        } else {
            bRet = 0;
        }
...
static int doLsmSingleWork(
    ...
    ** top-level containing just the in-memory free-list entries to disk. */
    ...
    if (rc == LSM_OK) {
        *pnWrite = (nMax - nRem);
        *pbCkpt = (bCkpt && nRem <= 0);
        if (nMerge == 1 && pDb->nAutockpt > 0 && *pnWrite > 0 && pWorker->pLevel && pWorker->pLevel->nRight == 0 && pWorker->pLevel->pNext == 0) {
            *pbCkpt = 1;
        }
    }
...
static int infoPageDump(
    ...
    ** to lsmFsDbPageGet() here.  */
    ...
    if (rc == LSM_OK) {
        ...
        int iCell;
        u8 *aData;
        int nData; /* Page data and size thereof */
            ...
            for (i = 0; i < nRec; i++) {
                ...
                if (i == 0 && blob2.nData) {
                    assert(sortedKeyCompare(
                               ...
                               iTopic1, blob1.pData, blob1.nData) < 0);
                }
...
static int assertBtreeOk(
    ...
    if (pSeg->iRoot) {
        ...
        while (rc == LSM_OK) {
            ...
            aData = fsPageData(pPg, &nData);
            flags = pageGetFlags(aData, nData);
            if (rc == LSM_OK && 0 == ((SEGMENT_BTREE_FLAG | PGFTR_SKIP_THIS_FLAG) & flags) && 0 != pageGetNRec(aData, nData)) {
                u8 *pKey;
                int nKey;
                int iTopic;
