...
static int walkFreelistCb(void *pCtx, int iBlk, i64 iSnapshot) {
    ...
    if (pFree) {
        while ((p->iFree < pFree->nEntry) && p->iFree >= 0) {
            ...
            if ((p->bReverse == 0 && pEntry->iBlk > (u32)iBlk) || (p->bReverse != 0 && pEntry->iBlk < (u32)iBlk)) {
                break;
            } else {
                p->iFree += iDir;
    ...
    while (rc == LSM_OK && pDb->iReader < 0 && (iAttempt++) < nMaxAttempt) {
        ...
        ** (starting with loading the in-memory tree header).  */
        if (rc == LSM_OK) {
            ...
            if (rc == LSM_OK) {
                if (lsmTreeLoadHeaderOk(pDb, iTreehdr) && lsmCheckpointLoadOk(pDb, iSnap)) {
...
int lsmBeginRoTrans(lsm_db *db) {
    ...
    if (db->bRoTrans == 0) {
        ...
        ** are guaranteed not to change during the lifetime of this connection. */
        if (LSM_OK == rc && 0 == lsmCheckpointClientCacheOk(db) && LSM_OK == (rc = lsmCheckpointLoad(db, 0))) {
...
static int slotIsUsable(ShmReader *p, i64 iLsm, u32 iShmMin, u32 iShmMax) {
    return (
        p->iLsmId && p->iLsmId <= iLsm && shm_sequence_ge(iShmMax, p->iTreeId) && shm_sequence_ge(p->iTreeId, iShmMin));
}
...
static int isInUse(lsm_db *db, i64 iLsmId, u32 iShmid, int *pbInUse) {
    ...
    for (i = 0; rc == LSM_OK && i < LSM_LOCK_NREADER; i++) {
        ShmReader *p = &pShm->aReader[i];
        if (p->iLsmId) {
            if ((iLsmId != 0 && p->iLsmId != 0 && iLsmId >= p->iLsmId) || (iLsmId == 0 && shm_sequence_ge(p->iTreeId, iShmid))) {
                rc = lsmShmLock(db, LSM_LOCK_READER(i), LSM_LOCK_EXCL, 0);
                if (rc == LSM_OK) {
                    p->iLsmId = 0;
                    lsmShmLock(db, LSM_LOCK_READER(i), LSM_LOCK_UNLOCK, 0);
...
int lsmShmLock(
    ...
    if ((eOp == LSM_LOCK_UNLOCK && (db->mLock & (me | ms)) != 0) || (eOp == LSM_LOCK_SHARED && (db->mLock & (me | ms)) != ms) || (eOp == LSM_LOCK_EXCL && (db->mLock & me) == 0)) {
