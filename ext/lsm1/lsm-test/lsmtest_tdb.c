...
int tdb_open(const char *zLib, const char *zDb, int bClear, TestDb **ppDb) {
    ...
    for (i = 0; i < ArraySize(aLib); i++) {
        if ((int)strlen(aLib[i].zName) == nLib && 0 == memcmp(zLib, aLib[i].zName, nLib)) {
