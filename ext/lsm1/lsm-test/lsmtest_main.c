...
void testFetchCompare(
    ...
    if (*pRc == 0 && (nDbVal1 != nDbVal2 || (nDbVal1 > 0 && memcmp(pDbVal1, pDbVal2, nDbVal1)))) {
        testSetError(1);
    }
}
...
static void scanCompareCb(
  ...
  assert( rc==0 );
    ...
    if (p->pKey1 && ((memcmp(p->pKey1, pKey, MIN(p->nKey1, nKey)) > 0) || (memcmp(p->pKey1, pKey, MIN(p->nKey1, nKey)) == 0 && p->nKey1 > nKey))) {
        testPrintError("Returned key too small at %s:%d\n", __FILE__, __LINE__);
    }
    if (p->pKey2 && ((memcmp(p->pKey2, pKey, MIN(p->nKey2, nKey)) < 0) || (memcmp(p->pKey2, pKey, MIN(p->nKey2, nKey)) == 0 && p->nKey2 < nKey))) {
...
#else
                void *pKey;
                int nKey; /* Database key to query for */
                void *pVal;
                int nVal; /* Result of query */
...
static void do_insert_write_hook(
    ...
    } else if (pHook->nData && bLog == pHook->bLog && iOff == (pHook->iOff + pHook->nData)) {
        pHook->nData += nData;
    } else {
        flushHook(pHook);
        pHook->bLog = bLog;
...
static int do_insert(int nArg, char **azArg) {
    ...
    if (rc != 0) {
        ...
        if (rc == 0) {
            for (i = 0; i < nRow; i++) {
                void *pKey;
                int nKey; /* Database key to insert */
                void *pVal;
                int nVal; /* Database value to insert */
