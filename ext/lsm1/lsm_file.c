...
static int fsIsFirst(FileSystem *pFS, LsmPgno iPg) {
    const int nPagePerBlock = (pFS->nBlocksize / pFS->nPagesize);
    assert(!pFS->pCompress);
    return ((iPg % nPagePerBlock) == 1 || (iPg < nPagePerBlock && iPg == fsFirstPageOnBlock(pFS, 1)));
}
...
LsmPgno lsmFsRedirectPage(FileSystem *pFS, Redirect *pRedir, LsmPgno iPg) {
    ...
    if (pRedir) {
        const int nPagePerBlock = (pFS->pCompress ? pFS->nBlocksize : (pFS->nBlocksize / pFS->nPagesize));
...
static int fsPageGet(
    ...
    *ppPg = 0;
    ...
    if (p) {
        ...
        assert((rc == LSM_OK && (p || (pnSpace && *pnSpace))) || (rc != LSM_OK && p == 0));
    }
...
static int fsRunEndsBetween(
    ...
    LsmPgno iFirst,
    LsmPgno iLast) {
    return (pRun != pIgnore && ((pRun->iFirst >= iFirst && pRun->iFirst <= iLast) || (pRun->iLastPg >= iFirst && pRun->iLastPg <= iLast)));
}
...
static int fsSegmentRedirects(FileSystem *pFS, Segment *p) {
    return (p && (fsPageRedirects(pFS, p, p->iFirst) || fsPageRedirects(pFS, p, p->iRoot) || fsPageRedirects(pFS, p, p->iLastPg)));
}
#endif
...
void lsmFsGobble(
    ...
    while (rc == LSM_OK) {
        ...
        pRun->nSize -= (1 + fsLastPageOnBlock(pFS, iBlk) - fsFirstPageOnBlock(pFS, iBlk));
        iBlk = iNext;
    }
...
int lsmFsSortedPadding(
    ...
    if (pFS->pCompress && pSeg->iFirst) {
        ...
        assert(rc != LSM_OK || pSeg->iLastPg == fsLastPageOnPagesBlock(pFS, pSeg->iLastPg) || ((pSeg->iLastPg + 1) % pFS->szSector) == 0);
    }
