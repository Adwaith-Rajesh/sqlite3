...
static int fuzzerSkipRule(
    ...
    return pRule && ((pRule->iRuleset != iRuleset) || (pStem->nBasis + pRule->nTo - pRule->nFrom) > FUZZER_MX_OUTPUT_LENGTH);
}
...
static int fuzzerAdvance(fuzzer_cursor *pCur, fuzzer_stem *pStem) {
    ...
    while ((pRule = pStem->pRule) != 0) {
        ...
        while (pStem->n < pStem->nBasis - pRule->nFrom) {
            pStem->n++;
            if (pRule->nFrom == 0 || memcmp(&pStem->zBasis[pStem->n], pRule->zFrom, pRule->nFrom) == 0) {
...
static int fuzzerBestIndex(sqlite3_vtab *tab, sqlite3_index_info *pIdxInfo) {
    ...
    for (i = 0; i < pIdxInfo->nConstraint; i++, pConstraint++) {
        if (pConstraint->iColumn == 0 && pConstraint->op == SQLITE_INDEX_CONSTRAINT_MATCH) {
            seenMatch = 1;
        }
