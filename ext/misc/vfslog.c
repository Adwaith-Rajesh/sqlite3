...
static VLogLog *vlogLogOpen(const char *zFilename) {
    ...
    } else if (nName > 8 && strcmp(zFilename + nName - 8, "-journal") == 0) {
        nName -= 8;
        isJournal = 1;
    } else if (nName > 12 && sqlite3_strglob("-mj??????9??", zFilename + nName - 12) == 0) {
...
static int vlogRead(
    ...
    if (rc == SQLITE_OK && p->pLog && p->pLog->zFilename && iOfst <= 24 && iOfst + iAmt >= 28) {
