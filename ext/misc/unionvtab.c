...
static int unionIsIntkeyTable(
    ...
    if (rc == SQLITE_ERROR || (rc == SQLITE_OK && (!bPk || sqlite3_stricmp("integer", zType)))) {
        rc = SQLITE_ERROR;
    ...
    if (sqlite3_stricmp("temp", argv[1])) {
        ...
        ** check that the first table is a rowid table only.  */
        ...
        if (rc == SQLITE_OK) {
            ...
            pStmt = unionPreparePrintf(&rc, pzErr, tdb,
                                       "SELECT "
                                       "'CREATE TABLE xyz('"
...
static int unionFilter(
    ...
    assert(idxNum == 0 || idxNum == SQLITE_INDEX_CONSTRAINT_EQ || idxNum == SQLITE_INDEX_CONSTRAINT_LE || idxNum == SQLITE_INDEX_CONSTRAINT_GE || idxNum == SQLITE_INDEX_CONSTRAINT_LT || idxNum == SQLITE_INDEX_CONSTRAINT_GT || idxNum == (SQLITE_INDEX_CONSTRAINT_GE | SQLITE_INDEX_CONSTRAINT_LE));
    ...
    for (i = 0; i < pTab->nSrc; i++) {
        ...
        zSql = sqlite3_mprintf("%z%sSELECT rowid, * FROM %s%q%s%Q", zSql, (zSql ? " UNION ALL " : ""), (pSrc->zDb ? "'" : ""), (pSrc->zDb ? pSrc->zDb : ""), (pSrc->zDb ? "'." : ""), pSrc->zTab);
        if (zSql == 0) {
            rc = SQLITE_NOMEM;
            break;
        }
