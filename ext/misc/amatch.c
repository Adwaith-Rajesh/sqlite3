...
static int amatchNext(sqlite3_vtab_cursor *cur) {
    ...
    do {
        ...
        while (1) {
            ...
            if ((zNextIn[0] == '*' && zNextIn[1] == 0) || (zNextIn[0] == 0 && zW[nWord] == 0)) {
                isMatch = 1;
                zNextIn[0] = 0;
                nNextIn = 0;
                break;
...
static int amatchBestIndex(
    ...
    for (i = 0; i < pIdxInfo->nConstraint; i++, pConstraint++) {
        ...
        if ((iPlan & 1) == 0 && pConstraint->iColumn == 0 && pConstraint->op == SQLITE_INDEX_CONSTRAINT_MATCH) {
            iPlan |= 1;
            pIdxInfo->aConstraintUsage[i].argvIndex = 1;
            pIdxInfo->aConstraintUsage[i].omit = 1;
        }
        if ((iPlan & 2) == 0 && pConstraint->iColumn == 1 && (pConstraint->op == SQLITE_INDEX_CONSTRAINT_LT || pConstraint->op == SQLITE_INDEX_CONSTRAINT_LE)) {
            iPlan |= 2;
            iDistTerm = i;
        }
        if ((iPlan & 4) == 0 && pConstraint->iColumn == 2 && pConstraint->op == SQLITE_INDEX_CONSTRAINT_EQ) {
            iPlan |= 4;
            pIdxInfo->aConstraintUsage[i].omit = 1;
            iLangTerm = i;
        }
...
static int amatchUpdate(
    ...
    if (sqlite3_value_type(argv[2 + AMATCH_COL_WORD]) != SQLITE_NULL || sqlite3_value_type(argv[2 + AMATCH_COL_DISTANCE]) != SQLITE_NULL || sqlite3_value_type(argv[2 + AMATCH_COL_LANGUAGE]) != SQLITE_NULL) {
