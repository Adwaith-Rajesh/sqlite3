...
static int cksmRead(
    ...
    if (rc == SQLITE_OK) {
        if (iOfst == 0 && iAmt >= 100 && (memcmp(zBuf, "SQLite format 3", 16) == 0 || memcmp(zBuf, "ZV-", 3) == 0)) {
            u8 *d = (u8 *)zBuf;
...
static int cksmWrite(
    ...
    */
    if (iAmt >= 512 && p->computeCksm && !p->inCkpt) {
...
static int cksmFileControl(sqlite3_file *pFile, int op, void *pArg) {
    ...
    if (op == SQLITE_FCNTL_PRAGMA) {
        ...
        if (sqlite3_stricmp(azArg[1], "checksum_verification") == 0) {
            char *zArg = azArg[2];
            if (zArg != 0) {
                if ((zArg[0] >= '1' && zArg[0] <= '9') || sqlite3_strlike("enable%", zArg, 0) == 0 || sqlite3_stricmp("yes", zArg) == 0 || sqlite3_stricmp("on", zArg) == 0) {
            ...
            return SQLITE_OK;
        } else if (p->computeCksm && azArg[2] != 0 && sqlite3_stricmp(azArg[1], "page_size") == 0) {
