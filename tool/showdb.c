...
static void ptrmap_coverage_report(const char *zDbName) {
    ...
    for (pgno = 2; pgno <= g.mxPage; pgno += perPage + 1) {
        ...
        for (i = 0; i + 5 <= usable && pgno + 1 + i / 5 <= g.mxPage; i += 5) {
            ...
            switch (a[i]) {
                case 1:
                    zType = "b-tree root page";
                    break;
                case 2:
                    zType = "freelist page";
                    break;
                case 3:
                    zType = "first page of overflow";
                    break;
                case 4:
                    zType = "later page of overflow";
                    break;
                case 5:
                    zType = "b-tree non-root page";
                    break;
            }
...
int main(int argc, char **argv) {
    ...
    if (nArg > 1) {
        if (sqlite3_stricmp("-raw", azArg[1]) == 0 || sqlite3_stricmp("--raw", azArg[1]) == 0) {
            g.bRaw = 1;
            azArg++;
            nArg--;
        }
