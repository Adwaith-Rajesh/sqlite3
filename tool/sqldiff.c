...
static char **columnNames(
    ...
    if (g.bSchemaCompare) {
        assert(sqlite3_stricmp(zTab, "sqlite_schema") == 0 || sqlite3_stricmp(zTab, "sqlite_master") == 0);
    ...
    *pnPKey = nPK;
    ...
    while (SQLITE_ROW == sqlite3_step(pStmt)) {
        ...
        if (truePk && (iPKey = sqlite3_column_int(pStmt, 5)) > 0) {
            ...
            if (!g.bSchemaCompare || !(strcmp(sid, "rootpage") == 0 || strcmp(sid, "name") == 0 || strcmp(sid, "type") == 0)) {
    ...
    while (sqlite3_step(pStmt) == SQLITE_ROW) {
        ...
        ** printed again.  */
        ...
        if (sqlite3_column_type(pStmt, nCol) == SQLITE_INTEGER) {
            ...
            for (i = 0; i < nCol; i++) {
                int bDone = 0;
                if (i >= nPK && sqlite3_column_type(pStmt, i) == SQLITE_BLOB && sqlite3_column_type(pStmt, nCol + 1 + i) == SQLITE_BLOB) {
    ...
    for (i = 1; i < argc; i++) {
        ...
        if (z[0] == '-') {
            ...
            if (strcmp(z, "changeset") == 0) {
                ...
                zTab = argv[++i];
                g.bSchemaCompare =
                    sqlite3_stricmp(zTab, "sqlite_schema") == 0 || sqlite3_stricmp(zTab, "sqlite_master") == 0;
            } else if (strcmp(z, "transaction") == 0) {
                useTransaction = 1;
            } else if (strcmp(z, "vtab") == 0) {
                g.bHandleVtab = 1;
            } else {
