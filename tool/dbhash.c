...
static void hash_finish(const char *zName) {
    ...
    for (i = 0; i < 8; i++) {
        finalcount[i] = (unsigned char)((g.cx.count[(i >= 4 ? 0 : 1)] >> ((3 - (i & 3)) * 8)) & 255); /* Endian independent */
    }
    ...
    for (i = 1; i < argc; i++) {
        ...
        if (z[0] == '-') {
            ...
            if (strcmp(z, "debug") == 0) {
                ...
                showHelp();
                return 0;
            } else if (strcmp(z, "like") == 0) {
                if (i == argc - 1) cmdlineError("missing argument to %s", argv[i]);
                if (zLike != 0) cmdlineError("only one --like allowed");
                zLike = argv[++i];
            } else if (strcmp(z, "schema-only") == 0) {
                omitContent = 1;
            } else if (strcmp(z, "without-schema") == 0) {
                omitSchema = 1;
            } else {
